...
class goods_classModel extends Model
{
    ...
    public function getGoodsClass($store_id, $pid = 0, $deep = 1, $group_id = null, $gc_limits = 1, $is_all_class = null) {
        // 读取商品分类 by sh opwt .com 批量添加分类修改
    ...
    public function getGoodsClassNav($id = 0, $sign = 1) {
        ...
        // 首页导航
        $nav_link[] = array('title' => Language::get('homepage'), 'link' => BASE_SITE_URL);
     ...
     * @return array $nav_link 返回数组形式类别导航连接
     */
    public function getGoodsClassNavsearch_FenXiao($id = 0, $sign = 1) {
        if (intval ( $id ) > 0) {
            $data = $this->getGoodsClassIndexedListAll();
            ...
            if ($sign == 2) {
                ...
                $nav_link [] = array(
                        'title' => $data[$id]['gc_name'],
                        'id' => $data[$id]['gc_id'],
                        'link' => FENXIAO_SITE_URL.'/index.php?w=search&cate_id='.$data[$id]['gc_id']
                );
            }
     ...
     * 获取全部子集分类
     */
    public function get_child_all_list() {
		$gc_list = array();
		if (!($gc_list = rkcache('get_child_all_list'))) {
			$categoryList = Model('goods_class')->getGoodsClassListByParentId(0);
			if(!empty($categoryList) && is_array($categoryList)){
				foreach($categoryList as $v){
					$ginfo = array();
					$ginfo['gc_id'] = $v['gc_id'];
					$ginfo['gc_name'] = $v['gc_name'];
					$ginfo['gc_parent_id'] = $v['gc_parent_id'];
					$ginfo['gc_sort'] = $v['gc_sort'];
					$ginfo['deep'] = 1;
					$ginfo['appImage'] = '';
					$ginfo['appImageUrl'] = '';
					$ginfo['is_show'] = $v['is_show'];
					$child2 = Model('goods_class')->getGoodsClassListByParentId($v['gc_id']);
					if(!empty($child2) && is_array($child2)){
						foreach($child2 as $v2){
							$ginfo2 = array();
							$ginfo2['gc_id'] = $v2['gc_id'];
							$ginfo2['gc_name'] = $v2['gc_name'];
							$ginfo2['gc_parent_id'] = $v2['gc_parent_id'];
							$ginfo2['gc_sort'] = $v2['gc_sort'];
							$ginfo2['deep'] = 2;
							$ginfo2['appImage'] = '';
							$ginfo2['appImageUrl'] = '';
							$ginfo2['is_show'] = $v2['is_show'];
							$child3 = Model('goods_class')->getGoodsClassListByParentId($v2['gc_id']);
							if(!empty($child3) && is_array($child3)){
								foreach($child3 as $v3){
									$ginfo3 = array();
									$ginfo3['gc_id'] = $v3['gc_id'];
									$ginfo3['gc_name'] = $v3['gc_name'];
									$ginfo3['gc_parent_id'] = $v3['gc_parent_id'];
									$ginfo3['gc_sort'] = $v3['gc_sort'];
									$ginfo3['deep'] = 3;
									$ginfo3['appImage'] = '';
									$ginfo3['appImageUrl'] = '';
									$ginfo3['is_show'] = $v3['is_show'];
									$ginfo2['child'][] = $ginfo3;
								}
							}
							$ginfo['gc_list'][] = $ginfo2;
						}
					}
			 ...
			 wkcache('get_child_all_list', $gc_list);
			//echo(json_encode($gc_list)); exit();
