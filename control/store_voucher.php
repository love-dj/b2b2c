...
class store_voucherControl extends BaseSellerControl{
    ...
    public function __construct() {
        ...
        if (C('voucher_allow') != 1){
            showMessage(Language::get('voucher_unavailable'),'index.php?w=store','html','error');
        }
        //申请记录状态
     ...
     * 导出
     */
    public function voucher_exportWt(){
        $t_id = intval($_GET['tid']);
        if($t_id <= 0){
            showDialog('参数错误');
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?w=store_voucher&t=voucher_export');
    ...
    public function quotaaddWt(){
        if (chksubmit()){
            ...
            //获取当前价格
            $current_price = intval(C('sale_voucher_price'));
            ...
            $this->recordSellerLog('购买'.$quota_quantity.'份代金券套餐，单价'.$current_price.L('wt_yuan'));

            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?w=store_voucher&t=quotalist','succ');
            } else {
                showDialog(Language::get('wt_common_op_fail'),'index.php?w=store_voucher&t=quotalist');
            }
        }else {
            //输出导航
    ...
    public function templateaddWt(){
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?w=store_voucher&t=quotaadd','html','error');
            }
            ...
            if ($count >= C('sale_voucher_storetimes_limit')){
                $message = sprintf(Language::get('voucher_template_noresidual'),C('sale_voucher_storetimes_limit'));
                showMessage($message,'index.php?w=store_voucher&t=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),'index.php?w=store_voucher&t=templatelist','html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('wt_common_save_succ'),'index.php?w=store_voucher&t=templatelist','succ');
                }else{
                    showDialog(Language::get('wt_common_save_fail'),'index.php?w=store_voucher&t=templatelist','error');
                }
            }
        }else{
            //店铺分类
    ...
    public function templateeditWt(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?w=store_voucher&t=templatelist','html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('wt_common_op_succ'),'index.php?w=store_voucher&t=templatelist','succ');
                }else{
                    showDialog(Language::get('wt_common_op_fail'),'index.php?w=store_voucher&t=templatelist','error');
                }
            }
        }else{
    ...
    public function templatedelWt(){
        ...
        if ($rs){
            ...
            showDialog(Language::get('wt_common_del_succ'),'reload','succ');
        }else {
            showDialog(Language::get('wt_common_del_fail'));
        }
    }
    /**
     * 查看代金券详细
     */
    public function templateinfoWt(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?w=store_voucher&t=templatelist','html','error');
        }
     ...
     * 生成卡密代金券
     */
    public function bulidvoucherWt(){
        $t_id = intval($_GET['tid']);
    	if ($t_id <= 0){
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('wt_member_path_store_voucher'), 'menu_url'=>'index.php?w=store_voucher&t=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('wt_member_path_store_voucher'), 'menu_url'=>'index.php?w=store_voucher&t=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>'index.php?w=store_voucher&t=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('wt_member_path_store_voucher'), 'menu_url'=>'index.php?w=store_voucher&t=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>'index.php?w=store_voucher&t=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
