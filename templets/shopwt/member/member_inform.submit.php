...
<div class="wtm-flow-box">
  <div id="wtmInformFlow" class="wtm-flow-container">
    ...
    <div class="wtm-default-form">
      <div id="warning"></div>
      <form id="add_form" enctype="multipart/form-data" method="post" action="index.php?w=member_inform&t=inform_save">
        <input name="inform_goods_id" type="hidden" value=<?php echo $output['goods_content']['goods_id']; ?> />
        <dl>
          <dt>被举报商家：</dt>
          <dd><a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['goods_content']['store_id']));?>" target="_blank";><?php echo $output['goods_content']['store_name'];?></a></dd>
        </dl>
        <dl>
        ...
        <div class="bottom">
          <label class="submit-border">
            <input id="btn_inform_submit" type="submit" class="submit" value="<?php echo $lang['wt_ok'];?>" />
          </label>
...
<script type="text/javascript" src="<?php echo STATIC_SITE_URL;?>/js/jquery.selectboxes.pack.js" charset="utf-8" ></script> 
...
function bindInformSubject(key) {
    ...
    $.ajax({
        type:'POST',
        url:'index.php?w=member_inform&t=get_subject_by_typeid',
        cache:false,
        data:'typeid='+type_id,
        dataType:'json',
        success:function(type_list){
            ...
            if(type_list.length >= 1) {
                $("#inform_subject").addOption('','<?php echo $lang['wt_please_choose'];?>');
