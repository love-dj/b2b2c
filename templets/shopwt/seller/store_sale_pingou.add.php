...
<div class="wtsc-form-default">
    <?php if(empty($output['pingou_info'])) { ?>
    <form id="add_form" action="index.php?w=store_sale_pingou&t=pingou_save" method="post">
    <?php } else { ?>
    <form id="add_form" action="index.php?w=store_sale_pingou&t=pingou_edit_save" method="post">
        <input type="hidden" name="pingou_id" value="<?php echo $output['pingou_info']['pingou_id'];?>">
    <?php } ?>
    <dl>
      <dt><i class="required">*</i>活动名称<?php echo $lang['wt_colon'];?></dt>
      <dd>
          <input id="pingou_name" name="pingou_name" type="text"  maxlength="25" class="text w400" value="<?php echo empty($output['pingou_info'])?'':$output['pingou_info']['pingou_name'];?>"/>
          <span></span>
        <p class="hint"><?php echo $lang['pingou_name_explain'];?></p>
      </dd>
    </dl>
    <?php if(empty($output['pingou_info'])) { ?>
    <dl>
      <dt><i class="required">*</i>开始时间<?php echo $lang['wt_colon'];?></dt>
      <dd>
          <input id="start_time" name="start_time" type="text" class="text w130" /><em class="add-on"><i class="icon-calendar"></i></em><span></span>
        <p class="hint">
<?php if (!$output['isOwnShop'] && $output['current_pingou_quota']['start_time'] > 1) { ?>
        开始时间不能为空且不能早于<?php echo date('Y-m-d H:i',$output['current_pingou_quota']['start_time']);?>
<?php } ?>
        </p>
      </dd>
    </dl>
    <dl>
      <dt><i class="required">*</i>结束时间<?php echo $lang['wt_colon'];?></dt>
      <dd>
...
<?php if (!$output['isOwnShop']) { ?>
        结束时间不能为空且不能晚于<?php echo date('Y-m-d H:i',$output['current_pingou_quota']['end_time']);?>
<?php } ?>
        </p>
      </dd>
    </dl>
    <?php } ?>
    <dl>
      <dt><i class="required">*</i>参团人数<?php echo $lang['wt_colon'];?></dt>
      <dd>
        <input id="min_num" name="min_num" type="text" class="text w130" value="<?php echo empty($output['pingou_info'])?'3':$output['pingou_info']['min_num'];?>"/><span></span>
    ...
    $("#add_form").validate({
        ...
        rules : {
            ...
            end_time : {
                required : true,
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<?php echo date('Y-m-d H:i',$output['current_pingou_quota']['end_time']);?>',
<?php } ?>
...
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<i class="icon-exclamation-sign"></i>结束时间不能为空且不能晚于<?php echo date('Y-m-d H:i',$output['current_pingou_quota']['end_time']);?>',
<?php } ?>
