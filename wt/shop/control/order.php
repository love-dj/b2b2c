...
class orderControl extends SystemControl{
    ...
    public function get_xmlWt(){
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            if ($order_info['if_system_receive_pay']) {
                ...
                $operation_detail .= "<li><a href=\"index.php?w=order&t=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">{$op_name}</a></li>";
            }
            if ($operation_detail) {
    ...
    private function _order_cancel($order_info) {
        ...
        if ($order_info['order_type'] == 2) {
            //预定订单
            $result = Handle('order_book')->changeOrderStateCancel($order_info, 'admin', $this->admin_info['name']);
        } else {
            $cancel_condition = array();
            if ($order_info['payment_code'] != 'offline') {
                $cancel_condition['order_state'] = ORDER_STATE_NEW;
            }
            $result =  $handle_order->changeOrderStateCancel($order_info,'admin', $this->admin_info['name'],'',true,$c);
        }
        if ($result['state']) {
    ...
    private function _order_receive_pay($order_info, $post) {
        ...
        //取订单其它扩展信息
        $result = Handle('payment')->getOrderExtendList($order_list,'admin');
        if (!$result['state']) {
            return $result;
        }
        $result = $handle_order->changeOrderReceivePay($order_list,'admin',$this->admin_info['name'],$post);
        if ($result['state']) {
    ...
    public function export_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=order&t=index');
		Tpl::setDirquna('shop');
