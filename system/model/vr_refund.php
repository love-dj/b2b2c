...
class vr_refundModel extends Model{
    ...
    public function addRefund($refund_array, $order = array()) {
        ...
        try {
            ...
            $model_order_vr->editOrderCode(array('refund_lock'=> 1),array('vr_code'=> array('in',$code_array)));//退款锁定
            $this->commit();
            return $refund_id;
        } catch (Exception $e) {
            $this->rollback();
        ...
        try {
            ...
            if ($state && $refund['admin_state'] == '2') {//审核状态:1为待审核,2为同意,3为不同意
                ...
                if ($state) {
                    ...
                    $state = $model_order_vr->editOrder($order_array,array('order_id'=> $order_id));//更新订单退款
                }
            }
            if ($state) {
                $code_array = explode(',', $refund['code_sn']);
                $state = $model_order_vr->editOrderCode(array('refund_lock'=> $refund_lock),array('vr_code'=> array('in',$code_array)));//更新退款的兑换码
                if ($state && $refund['admin_state'] == '2') {
                    Handle('order_vr')->changeOrderStateSuccess($order_id);//更新订单状态
                }
            }
            $this->commit();
            return $state;
    ...
    public function getRightOrderList($order_condition){
        ...
        $model_order_vr = Model('order_vr');
        $order_info = $model_order_vr->getOrderInfo($order_condition);
