  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        ...
        <li><a href="index.php?w=predeposit&t=pd_cash_list"><?php echo $lang['admin_predeposit_cashmanage']; ?></a></li>
...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=predeposit&t=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=predeposit&t=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?w=predeposit&t=get_xml',
        colModel : [
...
function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?w=predeposit&t=recharge_del', {id:id}, function(data){
            if (data.state) {
