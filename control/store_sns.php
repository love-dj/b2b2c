...
class store_snsControl extends BaseSellerControl{
     ...
     * 保存动态
     */
    public function store_sns_saveWt(){
        /**
         * 验证表单
         */
        ...
        switch ($_POST['type']){
            case '2':
                ...
                if($sns_image != '') $content   = '<div class="fd-media">
                                    <div class="thumb-image"><a href="javascript:void(0);" wt_type="thumb-image"><img src="'.$sns_image.'" /><i></i></a></div>
                                    <div class="origin-image"><a href="javascript:void(0);" wt_type="origin-image"></a></div>
                                </div>';
                break;
            case '9':
        ...
        if($rs){
            showDialog(Language::get('wt_common_op_succ'), 'index.php?w=store_sns', 'succ');
        }else{
    ...
    public function settingWt(){
        ...
        if(chksubmit()){
            ...
            $update['sauto_robbuy']       = isset($_POST['robbuy'])?1:0;
            $updata['sauto_robbuytitle']  = trim($_POST['robbuy_title']);
    ...
    private function profile_menu($menu_key) {
        $menu_array = array(
                1=>array('menu_key'=>'store_sns_add', 'menu_name'=>Language::get('store_sns_add'), 'menu_url'=>'index.php?w=store_sns&t=add'),
                2=>array('menu_key'=>'store_sns_setting', 'menu_name'=>Language::get('store_sns_setting'), 'menu_url'=>'index.php?w=store_sns&t=setting'),
    ...
    private function getGoodsByUrl($url){
        ...
        if(isset($array['query'])){
            ...
            $item = end($item);
            $id = preg_replace('/goods-(\d+)\.html/i', '$1', $item);
        }
        if(intval($id) > 0){
            // 查询商品信息
            $field = 'goods_id,store_id,goods_name,goods_image,goods_price,goods_freight';
