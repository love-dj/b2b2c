...
$(function(){
	...
	$("#ap_id_screen").append(screen_show_append);
	...
	$("#ap_id_focus").append(focus_show_append);
...
function select_ap_screen() {//选择广告位
    ...
    if (ap_id > 0 && typeof screen_show_list[ap_id] !== "undefined") {
        var show = screen_show_list[ap_id];
	    var color = $("#ap_color").val();
	    var pic_name = show['ap_name'];
	    var pic_img = show['ap_img'];
	    var obj = screen_obj.find("li.selected");
	    var pic_id = obj.attr("screen_id");
	    obj.find("img").attr("src",UPLOAD_SITE_URL+'/'+ATTACH_SHOW+'/'+pic_img);
	    obj.find("img").attr("title",pic_name);
        obj.find("input[name='screen_list["+pic_id+"][pic_name]']").val(pic_name);
        obj.find("input[name='screen_list["+pic_id+"][ap_id]']").val(ap_id);
        obj.find("input[name='screen_list["+pic_id+"][color]']").val(color);
        obj.find("input[name='screen_list["+pic_id+"][pic_img]']").val(ATTACH_SHOW+'/'+pic_img);
	    obj.find(".focus-thumb").css("background-color",color);
    }
}
...
function select_ap_focus() {//选择广告位
    ...
    if (ap_id > 0 && typeof focus_show_list[ap_id] !== "undefined") {
        var show = focus_show_list[ap_id];
	    var pic_name = show['ap_name'];
	    var pic_img = show['ap_img'];
        ...
        obj.find("input[name*='[pic_img]']").val(ATTACH_SHOW+'/'+pic_img);
    }
}
