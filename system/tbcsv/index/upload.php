...
if (isset($_FILES["Filedata"]) || !is_uploaded_file($_FILES["Filedata"]["tmp_name"]) || $_FILES["Filedata"]["error"] != 0) {
	...
	$getalbum_class_sql = "select aclass_id from $tablealbum_class where store_id='".intval(trim($_POST['storeid']))."' and is_default=1 ";
	$qres=mysql_query($getalbum_class_sql,$conn1);
	$row=mysql_fetch_row($qres);
	$album_class_id = isset($row[0])?$row[0]:0;
	...
	//插入album_pic表
	$insertpic = "INSERT INTO $tablealbum_pic (apic_name,aclass_id,apic_cover,store_id,upload_time) VALUES('".$sourimgname."','".$album_class_id."','".$rukuimgname."','".trim($_POST['storeid'])."','".time()."')";
