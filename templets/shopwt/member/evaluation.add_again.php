...
<div class="wtm-flow-box">
  <div class="wtm-flow-container">
    ...
    <form id="evalform" method="post" action="index.php?w=member_evaluate&t=add_again&order_id=<?php echo $_GET['order_id'];?>">
      ...
      <table class="wtm-default-table mb30">       
        <tbody>
          ...
          <tr class="bd-line">
          <td class="w20"></td>
            <td class="pic-mode w200"><div class="pic-thumb"><a href="index.php?w=goods&goods_id=<?php echo $goods['geval_goodsid']; ?>" target="_blank"><img src="<?php echo $goods['geval_goodsimage']; ?>"/></a></div><dl class="goods-name">
                <dt><a href="index.php?w=goods&goods_id=<?php echo $goods['goods_id'];?>" target="_blank" title="<?php echo $goods['geval_goodsname'];?>"><?php echo $goods['geval_goodsname'];?></a></dt>
              </dl></td>
            ...
            <td valign="top" class="tl">
              ...
              <div class="show-pic">
                ...
                <div class="evaluation-image"><ul wttype="ul_evaluate_image<?php echo $goods['geval_id'];?>" data-count='0'></ul></div>
               </div>
            </td>
          </tr>
          <?php }?>
...
$(function(){
    ...
    $('#file<?php echo $goods['geval_id'];?>').fileupload({
        ...
        formData: '',
        add: function (e,data) {
            var $count = parseInt($('ul[wttype="ul_evaluate_image<?php echo $goods['geval_id'];?>"]').attr('data-count'));
            if ($count >= 5) {
                return false;
            }
            $('ul[wttype="ul_evaluate_image<?php echo $goods['geval_id'];?>"]').attr('data-count', $count +1);
            ...
            if(data.result.state == 'true') {
                $('<li>' +
                        ...
                        '<img src="' + data.result.file_url + '"><input type="hidden" wttype="input_image" name="goods[<?php echo $goods['geval_id'];?>][evaluate_image][]" value=" ' + data.result.file_name + ' " >' +
                        '<a href="javascript:;" wttype="del" data-file-id="' + data.result.file_id + '" class="del" title="移除">X</a>' +
                        '</div></li>').appendTo('ul[wttype="ul_evaluate_image<?php echo $goods['geval_id'];?>"]');
            } else {
