...
class store_videoControl extends BaseSellerControl {
     ...
     * 视频列表
     */
    public function album_video_listWt() {
        /**
         * 验证是否存在默认视频
         */
     ...
     * 视频删除
     */
    public function album_video_delWt() {
        if (empty($_POST))
            $_POST = $_GET;
        if (empty($_POST['id'])) {
            showDialog('参数错误');
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch ($menu_type) {
            case 'album_video':
                $menu_array = array(
                    3 => array('menu_key' => 'video_list', 'menu_name' => '视频列表', 'menu_url' => 'index.php?w=store_video&t=album_video_list&id=' . intval($_GET['id']))
                );
                break;
        }
        if (C('oss.open')) {
