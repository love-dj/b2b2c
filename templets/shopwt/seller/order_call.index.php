...
<div class="alert alert-block mt10">
  <ul class="mt5">
    <li>1、自动获取最新订单，并自动弹出打印，请允许浏览器弹出。</li>
    <li>2、只自动打印未打印的订单，已打印的打印状态会自动显示为已打印。</li>
  </ul>
</div>
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="w" value="order_call" />
...
<table class="wtsc-default-table order" id="new_order" style="border-top:solid 1px #E6E6E6">
  <thead>
    <tr>
      <th class="w10"></th>
...
<span class="fr mr5"> <a href="index.php?w=store_order&t=order_print&order_id=<?php echo $order_id;?>" class="wtsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>A5打印</a></span>
<span class="fr mr5"> <a href="index.php?w=store_order&t=order_print_small&order_id=<?php echo $order_id;?>" class="wtsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>小票打印</a></span>
 </th>
    </tr>
    <?php $i = 0;?>
...
function refresh()
{
	$.ajax({
		type: "GET",
		url: BASE_SITE_URL+'/index.php?w=order_call&t=getajaxorder',
		dataType:"json",
		async: true,
		success: function(data){
		    ...
		    {	
				for(var i = 0; i < order_list.length; i++)
				{
					...
					html_str += '<tr><th colspan="20"><span class="ml10">订单编号：<em>'+order_list[i].order_sn+'</em></span> <span>下单时间：<em class="goods-time">'+order_list[i].add_time+'</em></span> <span class="fr mr5"> <a href="index.php?w=store_order_print&amp;order_id='+order_list[i].order_id+'" class="wtsc-btn-mini" target="_blank" title="打印发货单"><i class="icon-print"></i>打印发货单</a></span></th></tr>';
					...
					for(var k = 0; k < order_list[i].goods_list.length; k++)
					{
						...
						html_str += '<td class="w70"><div class="wtsc-goods-thumb"><a href="'+order_list[i].goods_list[k].goods_url+'" target="_blank"><img src="'+order_list[i].goods_list[k].image_60_url+'" onMouseOver="toolTip(\'<img src='+order_list[i].goods_list[k].image_240_url+'>\')" onMouseOut="toolTip()"/></a></div></td>';
						...
						if ((order_list[i].goods_count > 0 && k==0) || (order_list[i].goods_count) == 1 )
						{
							...
							{
								html_str +='<a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid='+order_list[i].extend_member.member_ww+'&site=cntaobao&s=2&charset='+charset+'"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid='+order_list[i].extend_member.member_ww+'&site=cntaobao&s=2&charset='+charset+'" alt="Wang Wang" style=" vertical-align: middle;" /></a>';
							}
							...
							html_str += '<td class="bdl" rowspan="'+order_list[i].goods_count+'"><p class="wtsc-order-amount">'+order_list[i].order_amount+'</p>';
							...
							//订单查看
							html_str += '	<p><a href="index.php?w=store_order&t=show_order&order_id='+order_list[i].order_id+'" target="_blank">订单详情</a></p>';
								
							//物流跟踪
							html_str += '		<p>';
							if(order_list[i].if_deliver)
							{
								html_str += '<a href=\'index.php?w=store_deliver&t=search_deliver&order_sn='+order_list[i].order_sn+'\'>查看物流</a>';
							}
							...
							{
								html_str += '<p><a href="javascript:void(0)" class="wtsc-btn wtsc-btn-red mt5" wt_type="dialog" uri="index.php?w=store_order&t=change_state&state_type=order_cancel&order_sn='+order_list[i].order_sn+'&order_id='+order_list[i].order_id+'" dialog_title="取消订单" dialog_id="seller_order_cancel_order" dialog_width="400" id="order'+order_list[i].order_id+'_action_cancel" /><i class="icon-remove-bbs"></i>取消订单</a></p>';
							}
							
							//修改运费
							if(order_list[i].if_modify_price)
							{
								html_str += '<p><a href="javascript:void(0)" class="wtsc-btn-mini wtsc-btn-orange mt10" uri="index.php?w=store_order&t=change_state&state_type=modify_price&order_sn='+order_list[i].order_sn+'&order_id='+order_list[i].order_id+'" dialog_width="480" dialog_title="调整费用" wt_type="dialog"  dialog_id="seller_order_adjust_fee" id="order'+order_list[i].order_id+'_action_adjust_fee" /><i class="icon-pencil"></i>修改运费</a></p>';
							}
							
							//修改价格
							if(order_list[i].if_spay_price)
							{
								html_str += '<p><a href="javascript:void(0)" class="wtsc-btn-mini wtsc-btn-green mt10" uri="index.php?w=store_order&t=change_state&state_type=spay_price&order_sn='+order_list[i].order_sn+'&order_id='+order_list[i].order_id+'" dialog_width="480" dialog_title="调整费用" wt_type="dialog"  dialog_id="seller_order_adjust_fee" id="order'+order_list[i].order_id+'_action_adjust_fee" /><i class="icon-pencil"></i>修改价格</a></p>';
							}
							...
							if(order_list[i].if_send){
								html_str += '<p>';
								html_str += '<a class="wtsc-btn wtsc-btn-green mt10" href="index.php?w=store_deliver&t=send&order_id='+order_list[i].order_id+'">';
					   ...
					   //E 合并TD
						...
						if(order_list[i].zengpin_list && j == order_list[i].goods_list.length)
						{
							html_str += '<tr>';
							html_str += '  <td class="bdl"></td>';
							html_str += '  <td colspan="4" class="tl"><div class="wtsc-goods-gift">赠品：';
							html_str += '  <ul>';
...
{
    if(length > 0){
		var url = BASE_SITE_URL+'/index.php?w=order_call_print&t=index';
...
{
	if(length > 0){
		var url = BASE_SITE_URL+'/index.php?w=order_call_print&t=small';
	    window.open(url,'_blank');
	}
}
...
function select_type(obj)
{
	...
	$.ajax({
		type: "GET",
		url: BASE_SITE_URL+'/index.php?w=order_call&t=select_type&type='+type,
		dataType:"json",
		async: true,
		success: function(data){
			if(data == 1)
