...
class what_goodsModel extends Model{

    const TABLE_NAME = 'what_goods';
    const PK = 'commend_id';

    public function __construct(){
        parent::__construct('what_goods');
    }
    ...
    public function getListWithUserInfo($condition,$page=null,$order='',$field='*',$limit=''){
        $on = 'what_goods.commend_member_id = member.member_id';
        $result = $this->table('what_goods,member')->field($field)->join('left')->on($on)->where($condition)->page($page)->order($order)->limit($limit)->select();
        return $result;
    }
    ...
    public function getOneWithUserInfo($param){
        $on = 'what_goods.commend_member_id = member.member_id';
        $result = $this->table('what_goods,member')->join('left')->on($on)->where($param)->find();
        return $result;
    }
