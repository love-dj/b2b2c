...
<div class="eject_con">
  <form method="post" action="<?php echo CHAIN_SITE_URL?>/index.php?w=order&t=pickup_parcel" id="pickup_parcel_form">
    ...
    <div class="content">
      <div class="order-handle">
        <div class="title">
          ...
          <div class="no-pay"><i class="icon-quote-left"></i>该笔尚未付款，需支付<strong><?php echo wtPriceFormat($output['order_info']['order_amount']);?>元</strong>,门店收款后再进行提货验证。<i class="icon-quote-right"></i> </div>
          <?php } ?>
        </div>
        <label>
      ...
      <div class="order-info">
        ...
        <div class="tabs-panel tabs-hide">
          <table class="wtsc-default-table">
            ...
            <tbody>
              <?php foreach ($output['order_info']['extend_order_goods'] as $goods_content) { ?>
              <tr>
                <td></td>
                <td class="w60"><img src="<?php echo $goods_content['image_url'] ?>" style="width: 40px; height: 40px;"/></td>
                <td class="tl"><div class="goods-name"> <?php echo $goods_content['goods_name'];?></div>
                  <div class="goods-spec"><?php echo $goods_content['goods_spec'];?></div>
                  <div class="goods-type"><?php echo $goods_content['goods_type'] == 5 ? '赠品' : ''?></div></td>
                <td><em class="goods-price"><?php echo wtPriceFormat($goods_content['goods_price']); ?></em></td>
