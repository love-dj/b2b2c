...
class pointsModel extends Model {
    ...
    public function __construct(){
        parent::__construct();
        $this->stage_arr = array('regist'=>'注册','login'=>'登录','comments'=>'商品评论','order'=>'订单消费','system'=>'积分管理','pointorder'=>'礼品兑换','app'=>'积分兑换','signin'=>'签到','inviter'=>'邀请返利','rebate'=>'消费返利','share'=>'分享');
    }
    /**
     * 操作积分
     * @author ShopWT Develop Team
    ...
    function savePointsLog($stage,$insertarr,$if_repeat = true){
        ...
        switch ($stage){
            ...
            case 'share':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '分享获得积分';
                }
                $insertarr['pl_points'] = intval(C('points_share_onepoint'));
                break;
            case 'login':
                if (!$insertarr['pl_desc']){
            ...
            case 'pointstomoney':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = "积分抵用消费订单号:".$insertarr['order_sn'];
                }
                break;
            case 'other':
                break;
		...
		if ($insertarr['inviter_memberid']){
            $value_array['pl_points'] = $insertarr['inviter_memberid'];
        }
	...
	*
	*/
	public function getOrderInviteCount($memberid,$inviteid)
    {
		return $this->table('points_log')->where(array('pl_memberid' => $memberid,'inviter_memberid' => $inviteid,'pl_stage'=>'rebate'))->count();
    }
		/**
	*获取邀请积分总数
	*
	*/
    ...
    {
		return $this->table('points_log')->where(array('pl_memberid' => $memberid,'inviter_memberid' => $inviteid,'pl_stage'=>'rebate'))->sum('pl_points');
    }
