...
class navigationControl extends SystemControl{
            ...
            foreach ($navigation_list as $k => $v){
                switch ($v['nav_location']){
                    ...
                    case '2':
                        ...
                        break;
		    case '3':
		        $navigation_list[$k]['nav_location'] = $lang['navigation_index_hidden'];
                        break;
                }
            }
        }
     ...
     * 页面导航 添加
     */
    public function navigation_addWt(){
        $lang   = Language::getLangContent();
        $model_navigation = Model('navigation');
        if (chksubmit()){
            /**
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?w=navigation&t=navigation_add',
                            'msg'=>$lang['navigation_add_again'],
                        ),
                        array(
                            'url'=>'index.php?w=navigation&t=navigation',
     ...
     * 页面导航 编辑
     */
    public function navigation_editWt(){
        $lang   = Language::getLangContent();
        $model_navigation = Model('navigation');
        if (chksubmit()){
            /**
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?w=navigation&t=navigation_edit&nav_id='.intval($_POST['nav_id']),
     ...
     * 删除页面导航
     */
    public function navigation_delWt(){
        $lang   = Language::getLangContent();
        $model_navigation = Model('navigation');
        if (preg_match('/^[\d,]+$/', $_GET['nav_id'])) {
            ...
            showMessage($lang['navigation_index_del_succ'],'index.php?w=navigation&t=navigation');
        }else {
            showMessage($lang['navigation_index_choose_del'],'index.php?w=navigation&t=navigation');
        }
    }
