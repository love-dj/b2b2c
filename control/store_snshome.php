        ...
        if(!empty($strace_array) && is_array($strace_array)){
            foreach ($strace_array as $key=>$val){
                if($val['strace_content'] == ''){
                    ...
                    $strace_array[$key]['strace_content'] = str_replace("%siteurl%", BASE_SITE_URL.DS, $content);
                }
            }
        }
...
//      //查询会员信息
        ...
        if ($result){
            ...
            if ($_POST['showtype'] == 1){
                $js .="$('#tracereply_".$stid."').load('index.php?w=store_snshome&t=commenttop&id=".$stid."');";
            }else {
                $js .="$('#tracereply_".$stid."').load('index.php?w=store_snshome&t=commentlist&id=".$stid."');";
            }
    ...
    public function addforwardWt(){
        ...
        $insert_arr['trace_content']            = "<dl class=\"fd-wrap\">
                                                        <dt>
                                                            <h3><a href=\"index.php?w=store_snshome&sid=".$stracelog_info['strace_storeid']."\" target=\"_blank\">".$stracelog_info['strace_storename']."</a>".Language::get('wt_colon')."
    ...
    public function deltraceWt(){
        ...
        if ($result){
            ...
            $js = "$('[wt_type=\"tracerow_{$stid}\"]').remove();";
    ...
    public function delcommentWt(){
        ...
        if ($result){
            ...
            $js .="$('.comment-list [wt_type=\"commentrow_".$scid."\"]').remove();";
            showDialog(L('wt_common_del_succ'),'','succ',$js);
        }else {
            showDialog(L('wt_common_del_fail'),'','error');
        }
    }
    /**
     * 一条SNS动态及其评论
     */
    public function straceinfoWt(){
