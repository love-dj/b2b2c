...
<?php if ($output['isOwnShop']) { ?>
  <a class="wtbtn wtbtn-mint" href="<?php echo urlShop('store_sale_cou', 'cou_add');?>"><i class="icon-plus-sign"></i>添加活动</a>

<?php } else { ?>

  <?php if(!empty($output['current_cou_quota'])) { ?>
  <a class="wtbtn wtbtn-mint" style="right:100px" href="<?php echo urlShop('store_sale_cou', 'cou_add');?>"><i class="icon-plus-sign"></i>添加活动</a> <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_cou', 'cou_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_cou', 'cou_quota_add');?>" title=""><i class="icon-money"></i>购买套餐</a>
  <?php } ?>
...
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_cou_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['wt_colon'];?></strong><strong style="color:#F00;"><?php echo date('Y-m-d H:i:s', $output['current_cou_quota']['tend']);?></strong>
  <?php } else { ?>
...
<form method="get">
  <table class="search-form">
    <input type="hidden" name="w" value="store_sale_cou" />
    <input type="hidden" name="t" value="cou_list" />
    <tr>
      <td>&nbsp;</td>
      <th>状态</th>
  ...
  <tbody id="cou_list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr">
          ...
          <span>
              <a href="index.php?w=store_sale_cou&t=cou_edit&cou_id=<?php echo $val['id'];?>" class="btn-bluejeans">
                  <i class="icon-edit"></i>
          ...
          <span>
              <a href="javascript:;" wttype="btn_del_cou" data-cou-id=<?php echo $val['id'];?> class="btn-grapefruit">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[wttype="btn_del_cou"]').on('click', function() {
            if(confirm('<?php echo $lang['wt_ensure_del'];?>')) {
                var action = "<?php echo urlShop('store_sale_cou', 'cou_del');?>";
