...
class cartModel extends Model {
     ...
     * 添加数据库购物车
     *
     * @param unknown_type $goods_content
     * @param unknown_type $quantity
     * @return unknown
     */
    private function _addCartDb($goods_content = array(),$quantity) {
        //验证购物车商品是否已经存在
        $condition = array();
        $condition['goods_id'] = $goods_content['goods_id'];
        $condition['buyer_id'] = $goods_content['buyer_id'];
        if (isset($goods_content['bl_id'])) {
            $condition['bl_id'] = $goods_content['bl_id'];
        } else {
            $condition['bl_id'] = 0;
        }
        ...
        $array    = array();
        $array['buyer_id']  = $goods_content['buyer_id'];
        $array['store_id']  = $goods_content['store_id'];
        $array['goods_id']  = $goods_content['goods_id'];
        $array['goods_name'] = $goods_content['goods_name'];
        $array['goods_price'] = $goods_content['goods_price'];
        $array['goods_num']   = $quantity;
        $array['goods_image'] = $goods_content['goods_image'];
        $array['store_name'] = $goods_content['store_name'];
        $array['bl_id'] = isset($goods_content['bl_id']) ? $goods_content['bl_id'] : 0;
     ...
     * @return unknown
     */
    private function _addCartCookie($goods_content = array(), $quantity = null) {
        //去除斜杠
        ...
        if (in_array($goods_content['goods_id'],array_keys($cart_array))) return true;
        $cart_array[$goods_content['goods_id']] = array(
          'store_id' => $goods_content['store_id'],
          'goods_id' => $goods_content['goods_id'],
          'goods_name' => $goods_content['goods_name'],
          'goods_price' => $goods_content['goods_price'],
          'goods_image' => $goods_content['goods_image'],
          'goods_num' => $quantity
        );
        setWtCookie('cart',encrypt(base64_encode(serialize($cart_array))),24*3600);
        return true;
    }
    ...
    public function listCart($type, $condition = array(), $limit = '') {
        ...
        $this->cart_all_price = wtPriceFormat($cart_all_price);
    ...
    public function getCartNum($type, $condition = array()) {
        ...
        @setWtCookie('cart_goods_num',$this->cart_goods_num,2*3600);
    ...
    public function batchAddCart($cart_new_list, $member_id, $store_id=0){
        ...
        if (!empty($goods_list)){
            foreach ($goods_list as $goods_content){
                $goods_content['buyer_id'] = $member_id;
                $this->addCart($goods_content,'db',$cart_new_list[$goods_content['goods_id']]['goods_num']);
                if (C('fenxiao_isuse') == 1 && $goods_content['is_fx'] == 1) {
                    $goods_commonid = $goods_content['goods_commonid'];
                    $fx_member_id = cookie('fx_'.$goods_commonid);
                    if ($fx_member_id) Model('fx_goods')->addDisCart($goods_commonid,$fx_member_id,$member_id);
                }
            }
        }
    }
