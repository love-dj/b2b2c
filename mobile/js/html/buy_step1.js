...
$(function() {
    ...
    var _init = function (address_id) {
        ...
        $.ajax({//提交订单信息
            ...
            success:function(result){
				...
				member_points_payrate = result.datas.member_points_payrate;
				...
				pm_allow = parseInt((member_points_payrate*totals*0.01)/points_money_parity);
				if(pm_allow>member_points){
					pm_allow = member_points;
				}
				$('#show_mp_maxallow').html(pm_allow);
    ...
    $('input[id="isPoint"]').on('change',function(){
      if ($(this).prop('checked')) {
	     ...
	     $('#points_show').addClass('points_height');
        }else{
            $('#J_PointInput').val(0);
            calcPoint($('#J_PointInput'));
            $('.buy-point-input').hide();
	    $('#points_show').removeClass('points_height');
        }
    });
...
function calcPoint(inObj){
		...
		if(parseInt($(inObj).val())>parseInt(pm_allow))
        {
