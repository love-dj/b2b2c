...
class messageControl extends SystemControl{
    private $links = array(
        array('url'=>'w=message&t=seller_tpl', 'lang'=>'seller_tpl'),
        array('url'=>'w=message&t=member_tpl', 'lang'=>'member_tpl'),
    );
     ...
     * 商家消息模板编辑
     */
    public function seller_tpl_editWt() {
        if (chksubmit()) {
        ...
        $this->links[] = array('url'=>'w=message&t=seller_tpl_edit','lang'=>'seller_tpl_edit');
    ...
    private function seller_tpl_update_short() {
        ...
        $update['apicodeid'] = $_POST["apicodeid"];
        $result = Model('store_msg_tpl')->editStoreMsgTpl($where, $update);
        $this->seller_tpl_update_showmessage($result);
    }
    ...
    private function seller_tpl_update_showmessage($result) {
        if ($result) {
            showMessage(L('wt_common_op_succ'), urlAdminShop('message', 'seller_tpl'));
        } else {
     ...
     * 用户消息模板编辑
     */
    public function member_tpl_editWt() {
        if (chksubmit()) {
            $code = trim($_POST['code']);
            $type = trim($_POST['type']);
            if (empty($code) || empty($type)) {
        ...
        $this->links[] = array('url'=>'w=message&t=member_tpl_edit','lang'=>'member_tpl_edit');
    ...
    private function member_tpl_update_showmessage($result) {
        if ($result) {
            showMessage(L('wt_common_op_succ'), urlAdminShop('message', 'member_tpl'));
        } else {
            showMessage(L('wt_common_op_fail'));
        }
    }
