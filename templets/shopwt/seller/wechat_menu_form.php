<link rel="stylesheet" type="text/css" href="<?php echo STATIC_SITE_URL;?>/js/dialog/dialog.css" />
...
.dialog_close_button { display: block; width: 21px; height: 21px; background: url(<?php echo SHOP_TEMPLATES_URL;?>/keyword/images/member/ico.gif) 0 -761px; position: absolute; top: 10px; right: 10px; }
.dialog_close_button:hover { background: url(<?php echo SHOP_TEMPLATES_URL;?>/keyword/images/member/ico.gif) 0 -783px; }
...
.tab { width: 100%; height: 28px; line-height: 26px; background: url(<?php echo SHOP_TEMPLATES_URL;?>/keyword/images/member/tab.gif) repeat-x bottom; }
...
.eject_con .adds .submit .btn { border: 0; width: 120px; height: 32px; background: url(<?php echo SHOP_TEMPLATES_URL;?>/keyword/images/member/btn.gif) no-repeat 0 -253px; cursor: pointer; font-weight: bold; color: #3f3d3e; }

.sign_box { float: left; width: 300px; }
.eject_con .adds .sign { float: left; width: 150px; height: 50px; background: url(<?php echo SHOP_TEMPLATES_URL;?>/keyword/images/member/default.gif); margin-right: 5px; }
</style>
...
<ul class="tab">
    <li class="active"><?php if($output['t'] == 'add'){ ?>添加分类<?php }else{ ?>编辑分类<?php } ?></li>
</ul>
<div class="eject_con">
 <div class="adds">
        <div id="warning"></div>
        <form id="category_form" method="post" target="pop_warning" action="index.php?w=seller_wechat_menu&amp;t=<?php echo $output['t'];?><?php if($output['id']){ ?>&amp;id=<?php echo $output['id'];?><?php } ?>">
        <ul>
            <li>
                <h3>分类名称:</h3>
            ...
            <li>
                ...
                <p>
                 ...
                 <b style="color:red;">选择URL类型：关键词不用填,key值直接填链接<br>买什么 首页 先选择URL类型<br>
...
我的二维码 按钮 KEY值：qrcode  关键词：qrcode</b>
           ...
           <li>
            ...
            <li>
                <h3>显示:</h3>
                <p><label>
                 <input type="radio" name="if_show" value="1" <?php if ($output['gcategory']['if_show']) { echo 'checked="checked"'; } ?> />
                是</label>
                <label>
                <input type="radio" name="if_show" value="0" <?php if (!$output['gcategory']['if_show']) { echo 'checked="checked"'; } ?> />
                否</label></p>
            </li>
        </ul>
