...
.dialog-share .bottom .button { color: #FFF; background-color: #E4393C; height: 28px; width: 80px; border: solid 1px #B22D00; cursor: pointer;}
...
<div class="dialog-share">
  <form method="post" action="index.php?w=member_snsindex&t=sharegoods" id="sharegoods_form">
    <input type="hidden" name="form_submit" value="ok"/>
    <input type="hidden" id="choosegoodsid" name="choosegoodsid" value="<?php echo intval($output['goods_content']['goods_id']);?>" />
    <!-- 分享范围 -->    
    <div class="share-set">
      <div class="share-widget"><?php echo $lang['sharebind_alsoshareto'];?>
        <ul class="s-app">
          ...
          <li wt_type="appitem_<?php echo $k;?>" title="<?php echo $v['name'];?>" class="<?php echo $v['isbind']?'checked':'disable';?>"> <i class="i-<?php echo $k;?>" wt_type="bindbtn" data-param='{"apikey":"<?php echo $k;?>","apiname":"<?php echo $v['name'];?>"}' attr_isbind="<?php echo $v['isbind']?'1':'0';?>"></i>
      ...
      <div wt_type="formprivacydiv" class="share-privacy"><a href="javascript:void(0);" wt_type="formprivacybtn"><i class="icon-group"></i>可见范围</a>
        <div class="privacytab" wt_type="formprivacytab" style="display:none;"> <span class="arrow"></span>
          <ul>
            <li wt_type="formprivacyoption" data-param='{"v":"0","hiddenid":"gprivacy"}'><span class="selected"></span><?php echo $lang['sns_weiboprivacy_all'];?></li>
            <li wt_type="formprivacyoption" data-param='{"v":"1","hiddenid":"gprivacy"}'><span></span><?php echo $lang['sns_weiboprivacy_friend'];?></li>
            <li wt_type="formprivacyoption" data-param='{"v":"2","hiddenid":"gprivacy"}'><span></span><?php echo $lang['sns_weiboprivacy_self'];?></li>
          </ul>
        </div>
      </div>
    </div>
    <div class="share-content">
      ...
      <div class="share-goods">
        <div class="goods-thumb"><a target="_blank" href="<?php echo urlShop('goods','index',array('goods_id'=>$output['goods_content']['goods_id']));?>"> <img title="<?php echo $output['goods_content']['goods_name']; ?>" src="<?php echo thumb($output['goods_content'],240);?>" /></a></div>
        <dl class="goods-intro">
          <dt><a target="_blank" href="<?php echo urlShop('goods','index',array('goods_id'=>$output['goods_content']['goods_id']));?>"><?php echo $output['goods_content']['goods_name']; ?></a></dt>
          <dd><?php if (in_array($output['goods_content']['goods_sale_type'], array(1,2))) {?><span><?php echo $output['goods_content']['goods_sale_type'] == 1 ? '抢购' : '限时';?></span><?php }?><strong>￥<?php echo wtPriceFormat($output['goods_content']['goods_sale_price']); ?></strong></dd>
        </dl>
      </div>
      <div class="error form-error"></div>
    </div>
    <!-- 验证码 -->
    <div id="sg_seccode" class="seccode"><?php echo $lang['wt_checkcode'].$lang['wt_colon']; ?>
    ...
    <!-- 防止点击Enter键提交 -->
    <div class="bottom">
      <input name="<?php echo $lang['wt_snsshare'];?>" type="button" class="button" value="<?php echo $lang['wt_snsshare'];?>" id="weibobtn_goods" />
...
<div class="bottom"><a href="javascript:void(0);" wt_type="finishbtn" data-param='{"apikey":"@apikey"}' class="mini-btn"><?php echo $lang['sharebind_list_finishbind'];?></a><span><?php echo $lang['sharebind_list_unfinishedbind'];?><a target="_blank" href="<?php echo MEMBER_SITE_URL;?>/api.php?w=sharebind&type=@apikey"><?php echo $lang['sharebind_list_againbind'];?></a></span>
</div>
</div>
</textarea>
