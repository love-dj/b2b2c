...
class sale_boothControl extends SystemControl{
    public function __construct(){
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?w=sale_booth&sale_allow=1',
                    'msg'=>L('open'),
                ),
            );
    ...
    public function goods_listWt()
    {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_booth_goods.list');
    }
    ...
    public function goods_list_xmlWt()
    {
        ...
        foreach ($goods_list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="'
                    . urlAdminShop('sale_booth', 'del_goods', array(
...
EOB;
     ...
     * 套餐列表
     */
    public function booth_quota_listWt()
    {
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_booth_quota.list');
    }

    /**
     * 套餐列表XML
     */
    public function booth_quota_list_xmlWt()
    {
        $condition = array();
        if (strlen($q = trim($_REQUEST['query']))) {
            switch ($_REQUEST['qtype']) {
        ...
        foreach ($list as $val) {
            ...
            $i['operation'] = '<a class="btn green" href="' . urlAdminShop('sale_booth', 'goods_list', array(
                'store_id' => $val['store_id'],
     ...
     * 设置
     */
    public function booth_settingWt() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["sale_booth_price"], "require"=>"true", 'validator'=>'Number', "message"=>'请填写展位价格'),
                array("input"=>$_POST["sale_booth_goods_sum"], "require"=>"true", 'validator'=>'Number', "message"=>'不能为空，且不小于1的整数'),
            );
            ...
            $data['sale_booth_price'] = intval($_POST['sale_booth_price']);
            $data['sale_booth_goods_sum'] = intval($_POST['sale_booth_goods_sum']);
        ...
        Tpl::setDirquna('shop');
        Tpl::showpage('sale_booth.setting');
    }
}
