...
class snapshotControl extends BaseHomeControl {
    ...
    public function indexWt() {
        ...
        $order_goods_content = $model_order->getOrderGoodsInfo(array('rec_id' => $rec_id));
        if (empty($order_goods_content)) {
            showMessage('参数错误，或者不是本人购买的商品', '', 'html', 'error');
        }
        $order_goods_content['goods_type_cn'] = orderGoodsType($order_goods_content['goods_type']);
        $spec_array = array();
        ...
        $sp_hot_info = Model('order_snapshot')->getSnapshotInfoByRecid($rec_id,$order_goods_content['goods_id']);
        $sp_hot_info['goods_attr'] = unserialize($sp_hot_info['goods_attr']);
        Tpl::output('goods', array_merge($order_goods_content,$sp_hot_info));

        $order_info = $model_order->getOrderInfo(array('order_id' => $order_goods_content['order_id']));
