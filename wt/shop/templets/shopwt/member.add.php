...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?w=member&t=member" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['member_index_manage']?> - <?php echo $lang['wt_new']?>会员</h3>
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['wt_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?w=common&t=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
    ...
    $('#user_form').validate({
        ...
        rules : {
			member_name: {
				...
				maxlength: 20,
				remote   : {
                    url :'index.php?w=member&t=ajax&branch=check_user_name',
                    type:'get',
                    data:{
                        user_name : function(){
        ...
        messages : {
			...
			member_qq : {
				digits: '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['member_edit_qq_wrong']?>',
				minlength: '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['member_edit_qq_wrong']?>',
				maxlength: '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['member_edit_qq_wrong']?>'
			}
        }
    });
});
