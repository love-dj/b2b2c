...
class full_amount_give_queueControl extends SystemControl{
    /*private $links = array(
	...
	public function indexWt() {
        ...
        if($back_state = $_POST['back_state']){
            $where = ' order_full_give.status='."'".$back_state."'";
        }else{
        ...
        $count = $model->table('order_full_give')->where(array('status'=>1))->count();
        $on = 'order_full_give.member_id=member.member_id';
	    $list =  $model->table('order_full_give,member')->join('left')->on($on)->where($where)->field('order_full_give.*,member.member_name,member.member_mobile')->page(10,$count)->select();
        ...
        Tpl::showpage('full_amount_give_queue');
...
//冻结正常队列
	public function forzenWt(){
	    $data = $_POST['data'];
	    $arr_id =explode(',',$data);

	    foreach($arr_id as $v){
            $dot =  Model('order_full_give')->where(array('id'=>$v))->find();
            if($dot['status']==3)continue; //完成状态 不做操作
            $res = Model('order_full_give')->where(array('id'=>$v))->update(array('status'=>2,'interest_dot'=>0,'interest_dot_freeze'=>$dot['interest_dot']));
        }
        echo json_encode(array('status'=>1));
    }

    //删除权益队列
    public function del_queueWt(){
	    $id = $_GET['id'];
...
//导出Excel
    public function export_full_maountWt(){

        $model = Model();
        $count = $model->table('full_amount_give')->count();

        if($back_state = $_GET['back_state']){
            $where = ' order_full_give.status='."'".$back_state."'";
        }else{
            $where = 'order_full_give.status<>2';
        }
