...
class goods_classControl extends SystemControl{
    ...
    public function indexWt() {
       $this->goodsclass_listWt();
    }

    /**
     * 买什么商品(说说看)分类管理
     **/
    public function goodsclass_listWt() {
        $model_class = Model("what_goods_class");
        $list = $model_class->getList(TRUE);
        Tpl::output('list',$list);
        $this->show_menu_goods_class("goods_class_list");
        Tpl::setDirquna('what');
Tpl::showpage("what_goods_class.list");
    }

    /**
     * 买什么商品(说说看)分类添加
     **/
    public function goodsclass_addWt() {
        //取得一级分类列表
        $model_what_goods_class = Model('what_goods_class');
        $condition = array();
        $condition['class_parent_id'] = 0;
        $goods_class_list = $model_what_goods_class->getList($condition);
        ...
        Tpl::setDirquna('what');
Tpl::showpage('what_goods_class.add');
    }
     ...
     * 买什么商品(说说看)分类编辑
     **/
    public function goodsclass_editWt() {
...
Tpl::showpage("what_goods_class.add");
     ...
     * 买什么商品(说说看)分类保存
     **/
    public function goodsclass_saveWt() {
        ...
        if($result) {
            showMessage(Language::get('class_add_success'),"index.php?w=goods_class&t=goodsclass_list");
        } else {
            showMessage(Language::get('class_add_fail'),"index.php?w=goods_class&t=goodsclass_list",'','error');
        }
     ...
     * ajax修改分类排序
     */
    public function goodsclass_sort_updateWt() {
        if(intval($_GET['id']) <= 0) {
     ...
     * ajax修改分类名称
     */
    public function goodsclass_name_updateWt() {
     ...
     * 说说看分类删除
     **/
     public function goodsclass_dropWt() {

        $class_id = trim($_REQUEST['class_id']);
        $model_what_goods_class = Model('what_goods_class');
        $condition = array();
        $condition['class_parent_id'] = array('in',$class_id);
        $goods_class_list = $model_what_goods_class->getList($condition,'','','class_id');
        ...
        //删除分类图片
        $list = $model_what_goods_class->getList($condition);
        if(!empty($list)) {
        ...
        //删除绑定关系
        $model_what_goods_relation = Model('what_goods_relation');
        $model_what_goods_relation->drop($condition);

        //删除分类
        $result = $model_what_goods_class->drop($condition);
        if($result) {
     ...
     * 分类关键字和商品分类的绑定
     **/
    public function goodsclass_bindingWt() {
        ...
        Tpl::setDirquna('what');
Tpl::showpage('what_goods_class.binding');
     ...
     * 分类关键字和商品分类的绑定保存
     **/
    public function goodsclass_binding_saveWt() {
        ...
        if($result) {
            showMessage(Language::get('goods_relation_save_success'),self::what_CLASS_LIST);
        } else {
            showMessage(Language::get('goods_relation_save_fail'),self::what_CLASS_LIST,'','error');
        }
    }
     ...
     * 设为默认分类
     **/
    public function goodsclass_defaultWt() {
        $class_id = intval($_GET['class_id']);
        if($class_id <= 0) {
            showMessage(Language::get('param_error'),'','','error');
        }
    ...
    public function goodsclass_getWt() {
    ...
    private function show_menu_goods_class($menu_key) {
        $menu_array = array(
                'goods_class_list'=>array('menu_type'=>'link','menu_name'=>Language::get('wt_manage'),'menu_url'=>'index.php?w=goods_class&t=goodsclass_list'),
                'goods_class_add'=>array('menu_type'=>'link','menu_name'=>Language::get('wt_new'),'menu_url'=>'index.php?w=goods_class&t=goodsclass_add'),
        );
        if($menu_key == 'goods_class_edit') {
            $menu_array['goods_class_edit'] = array('menu_type'=>'link','menu_name'=>Language::get('wt_edit'),'menu_url'=>'###');
        }
        if($menu_key == 'goods_class_binding') {
            $menu_array['goods_class_binding'] = array('menu_type'=>'link','menu_name'=>Language::get('what_goods_class_binding'),'menu_url'=>'###');
        }
