...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?w=sns_strace&t=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?w=sns_strace&t=scomm_list';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?w=sns_strace&t=strace_del', {id:id}, function(data){
            if (data.state) {
