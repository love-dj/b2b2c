...
//注册表单验证
$(function(){
	var $list = $('div[wttype="list"]');
	var $choose = $('div[wttype="choose"]');
	var $ep = $('span[wttype="ep"]');
	$('span[wttype="able"]', $list).draggable({ 
		cancel: "a.ui-icon",
        revert: "invalid",
	...
	$('span[wttype="able"]', $choose).draggable({ 
		cancel: "a.ui-icon",
        revert: "invalid",
        containment: "document",
       	helper: "clone",
    ...
    function chooseTeg($item){
    	$item.fadeOut('fast',function(){
    		...
    		$item.append('<a href="javascript:void(0)" wttype="delTag"><input type="hidden" name="mid[]" value="'+data_param.id+'" />X</a>')
	...
	$('a[wttype="delTag"]').live('click', function(){
		recycleIeg($(this).parent());
	});

	$('div[wttype="list"]').find('span').live('click', function(){
		chooseTeg($(this));
	});
