...
var password,rcb_pay,pd_pay,payment_code;
		 ...
		 var readytoWXH5Pay = false;
		     ...
		     readytoWXH5Pay = true;
                 ...
                 for (var i=0; i<result.datas.pay_info.payment_list.length; i++) {
                    ...
                    if ((_payment_code == 'wxpay_jsapi' || _payment_code == 'wxpay') && readytoWXPay) {
						 $('#wxpay_jsapi').parents('label').show();
                         if (payment_code == '') {
                            payment_code = _payment_code;
                            $('#wxpay_jsapi').attr('checked', true).parents('label').addClass('checked');
                         }
                     }
             ...
             $('#toPay').click(function(){
                 ...
                 if (_use_password) {
                     ...
                     $.ajax({
                         type:'post',
                         url:ApiUrl+'/index.php?w=member_buy&t=check_pd_pwd',
                         dataType:'json',
                         ...
                         success:function(result){
                             ...
                             goToPayment(pay_sn,w == 'member_buy' ? 'pay_new' : 'vr_pay_new');
                         }
                     });
                 } else {
                	 goToPayment(pay_sn,w == 'member_buy' ? 'pay_new' : 'vr_pay_new');
 ...
 function goToPayment(pay_sn,t) {
	var params='?w=member_payment&t='+t+'&key=' + key + '&pay_sn=' + pay_sn + '&password=' + password + '&rcb_pay=' + rcb_pay + '&pd_pay=' + pd_pay + '&payment_code=mini_wxpay';
