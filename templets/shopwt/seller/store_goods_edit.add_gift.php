...
<form method="post" id="goods_gift" action="<?php echo urlShop('store_goods_online', 'save_gift');?>">
  ...
  <div class="wtsc-form-goods-gift" data-gid="<?php echo $value['goods_id'];?>">
    ...
    <div class="goods-summary">
      ...
      <dl>
        <dt>商品价格：</dt>
        <dd>￥<?php echo wtPriceFormat($value['goods_price']);?></dd>
      </dl>
      <dl>
        <dt>库&nbsp;&nbsp;存&nbsp;&nbsp;量：</dt>
      ...
      <dl>
        <dt>赠品捆绑：</dt>
        <dd>
          <ul class="goods-gift-list" wttype="choose_goods_list">
            ...
            <li>
              ...
              <a class="gift-del" wttype="del_choosed" href="javascript:void(0);" title="删除赠品">X</a></li>
            <?php }?>
            <?php }?>
          </ul>
          <?php if ($value['is_virtual'] == 0) {?>
          <a class="wtbtn-mini wtbtn-aqua" wttype="select_goods" href="javascript:void(0);"><i class="icon-gift"></i>选择赠品</a>
          <?php } else {?>
          <a class="wtbtn-mini" href="javascript:void(0);"><i class="icon-gift"></i>不能添加赠品</a>
          <?php }?>
        </dd>
      </dl>
    </div>
      ...
      <table class="search-form">
        <thead>
          <tr>
            ...
            <td class="tc w70"><a class="wtbtn" href="javascript:void(0);" wttype="search_gift"><i class="icon-search"></i>搜索</a></td>
            <td class="w10"></td>
          </tr>
        </thead>
      </table>
      <div class="search-result" wttype="gift_goods_list"></div>
...
$(function(){
    ...
    $('a[wttype="select_goods"]').click(function(){
        $(this).parents('.goods-summary:first').nextAll('.div-goods-select').show()
            ...
            .find('a[wttype="search_gift"]').click();
    });
    ...
    // 所搜商品
    $('a[wttype="search_gift"]').click(function(){
    ...
    // 分页
    $('div[wttype="gift_goods_list"]').on('click', 'a[class="demo"]', function(){
        $(this).parents('div[wttype="gift_goods_list"]').load($(this).attr('href'));
        return false;
    });

    // 删除
    $('ul[wttype="choose_goods_list"]').on('click', 'a[wttype="del_choosed"]', function(){
        $(this).parents('li:first').remove();
    });

    // 选择商品
    $('div[wttype="gift_goods_list"]').on('click', 'a[wttype="a_choose_goods"]', function(){
        _owner_gid = $(this).parents('.wtsc-form-goods-gift:first').attr('data-gid');
        ...
        _li = $('<li></li>')
            ...
            .append('<a class="gift-del" wttype="del_choosed" href="javascript:void(0);" title="删除赠品">X</a>');
        $(this).parents('.div-goods-select:first').prev().find('ul[wttype="choose_goods_list"]').append(_li);
    });
