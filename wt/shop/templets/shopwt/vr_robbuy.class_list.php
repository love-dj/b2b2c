  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        ...
        <li><a href="index.php?w=vr_robbuy&t=area_list">区域管理</a></li>
      </ul>
    </div>
  </div>
    ...
    <table class="flex-table">
      <thead>
        <tr>
          ...
          <th width="60" align="left"><?php echo $lang['wt_sort']; ?></th>
      ...
      <tbody>
        ...
        <tr class="hover edit" data-id="<?php echo $val['class_id']; ?>">
          ...
          <td class="handle"><a href="javascript:;" onclick="submit_delete(<?php echo $val['class_id']; ?>)" class="btn red"><i class="fa fa-trash-o"></i>删除</a> <span class="btn"> <em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em>
            <ul>
              <li><a href="index.php?w=vr_robbuy&t=class_edit&class_id=<?php echo $val['class_id']; ?>">编辑分类</a></li>
              <li><a href="index.php?w=vr_robbuy&t=class_add&parent_class_id=<?php echo $val['class_id']; ?>">新增下级</a></li>
            </ul>
            </span></td>
          <td class="sort"><span wt_type="class_sort" column_id="<?php echo $val['class_id']; ?>" title="可编辑" class="editable"><?php echo $val['class_sort'];?></span></td>
          <td class="name"><span wt_type="class_name" column_id="<?php echo $val['class_id']; ?>" title="可编辑" class="editable"><?php echo $val['class_name']; ?></span></td>
          <td></td>
        </tr>
        ...
        <tr class="<?php echo 'class_id'.$val['class_id'];?>" style="display:none;" data-id="<?php echo $val1['class_id']; ?>">
          <td></td>
          <td class="handle"><a href="javascript:;" onclick="submit_delete(<?php echo $val1['class_id']; ?>)" class="btn red"><i class="fa fa-trash-o"></i>删除</a> <a href="index.php?w=vr_robbuy&t=class_edit&class_id=<?php echo $val1['class_id']; ?>" class="btn blue"><i class="fa fa-pencil-square-o"></i>编辑</a></td>
          <td class="sort"><span wt_type="class_sort" column_id="<?php echo $val1['class_id']; ?>" title="可编辑" class="editable"><?php echo $val1['class_sort'];?></span></td>
          <td class="name"><span wt_type="class_name" column_id="<?php echo $val1['class_id']; ?>" title="可编辑" class="editable"><?php echo $val1['class_name']; ?></span></td>
          <td></td>
        </tr>
        <?php } ?>
        <?php } ?>
...
$(function() {
        ...
        buttons: [
            {
                ...
                title: '新增分类',
                onpress: function() {
                    location.href = '<?php echo urlAdminShop('vr_robbuy', 'class_add'); ?>';
                }
            },
            {
    ...
    //行内ajax编辑
    $('span[wt_type="class_sort"]').inline_edit({w: 'vr_robbuy', t: 'update_class_sort'});
    $('span[wt_type="class_name"]').inline_edit({w: 'vr_robbuy', t: 'update_class_name'});
...
function submit_delete(id) {
    if (confirm('<?php echo $lang['wt_ensure_del']; ?>')) {
        ...
        $('#list_form').attr('action','index.php?w=vr_robbuy&t=class_del');
        $('#class_id').val(id);
        $('#list_form').submit();
    }
}
