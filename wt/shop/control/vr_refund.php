...
class vr_refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'w=vr_refund&t=refund_manage','text'=>'待处理'),
            array('url'=>'w=vr_refund&t=refund_all','text'=>'所有记录'),
    );
    ...
    public function indexWt() {
        $this->refund_manageWt();
    }

    /**
     * 待处理列表
     */
    public function refund_manageWt() {
        $_GET['waiting'] = 1;
        $this->refund_allWt();
    }
    ...
    public function get_all_xmlWt() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            if ($_GET['waiting'] == 1 && $refund_info['admin_state'] == 1) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?w=vr_refund&t=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=vr_refund&t=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function editWt() {
        ...
        $order = $model_order_vr->getOrderInfo(array('order_id'=> $order_id));
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=vr_refund&t=refund_manage');
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
