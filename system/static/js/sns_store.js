$(function(){
    ...
    //展示和隐藏评论列表
	$("[wt_type='sd_commentbtn']").live('click',function(){
		var $this = $(this);
		$.get('index.php?w=index&t=login', function(result){
		    if(result=='0'){
				...
				if($('#tracereply_'+data.txtid).css("display")=='none'){
					//加载评论列表
			        $("#tracereply_"+data.txtid).load('index.php?w=store_snshome&t=commenttop&id='+data.txtid);
	...
	//评论提交
	$("[wt_type='scommentbtn']").live('click',function(){
		var data = $(this).attr('data-param');
        eval("data = "+data);
	...
	//删除评论
	$("[wt_type='scomment_del']").live('click',function(){
		var obj = $(this);
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			var data_str = $(obj).attr('data-param');
	        eval("data_str = "+data_str);
	        ajax_get_confirm('','index.php?w=store_snshome&t=delcomment&scid='+data_str.scid+'&stid='+data_str.stid);
			return false;
		});
	});
	...
	//展示和隐藏转发表单
	$("[wt_type='sd_forwardbtn']").live('click',function(){
		var $this = $(this);
		$.get('index.php?w=index&t=login', function(result){
		    if(result=='0'){
		    	login_dialog();
	...
	//转发提交
	$("[wt_type='s_forwardbtn']").live('click',function(){
		var data = $(this).attr('data-param');
		var form = $(this).parents('form:first');
		var vercode = $("#forwardseccode"+data.txtid);
        eval("data = "+data);
		if(form.valid()){
			var cookienum = $.cookie(COOKIE_PRE+'forwardnum');
			cookienum = parseInt(cookienum);
			if(!isNaN(cookienum) && cookienum >= MAX_RECORDNUM){
			    if (vercode.css('display') == 'none') {
	                //显示验证码
	                vercode.show();
	                
	                vercode.find("[name='codeimage']").attr('src','index.php?w=vercode&type=30x92&c=' + Math.random());
			    } else if(vercode.find("[name='captcha']").val() == ''){
	                showDialog('请填写验证码');
			    }
			}else{
				ajaxpost('forwardform_'+data.txtid, '', '', 'onerror');
				//隐藏验证码
				vercode.hide().find("[name='codeimage']").attr('src','').end().find("[name='captcha']").val('');
				//隐藏表单
				$('#forward_'+data.txtid).hide();
    ...
    //删除动态
	$("[wt_type='sd_del']").live('click',function(){
		var data_str = $(this).attr('data-param');
        eval("data_str = "+data_str);
        var url = "index.php?w=store_snshome&t=deltrace&id="+data_str.txtid;
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			ajaxget(url);
			return false;
		});
