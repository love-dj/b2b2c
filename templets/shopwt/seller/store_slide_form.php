...
<div class="wtsc-form-default"> <div class="alert">
      <ul>
  ...
  <form action="index.php?w=store_setting&t=store_slide" id="store_slide_form" method="post" onsubmit="ajaxpost('store_slide_form', '', '', 'onerror');return false;">
    <input type="hidden" name="form_submit" value="ok" />
    <!-- 图片上传部分 -->
    <ul class="wtsc-store-slider" id="goods_images">
      <?php for($i=0;$i<5;$i++){?>
      <li wt_type="handle_pic" id="thumbnail_<?php echo $i;?>">
        <div class="picture" wttype="file_<?php echo $i;?>">
          <?php if (empty($output['store_slide'][$i])) {?>
          <i class="icon-picture"></i>
          <?php } else {?>
          <img wttype="file_<?php echo $i;?>" src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_SLIDE.DS.$output['store_slide'][$i];?>" />
          <?php }?>
          <input type="hidden" name="image_path[]" wttype="file_<?php echo $i;?>" value="<?php echo $output['store_slide'][$i];?>" /><a href="javascript:void(0)" wttype="del" class="del" title="移除">X</a></div>
...
<script src="<?php echo STATIC_SITE_URL;?>/js/store_slide.js" charset="utf-8"></script>
<!-- 引入幻灯片JS --> 
