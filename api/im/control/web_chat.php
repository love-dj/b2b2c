...
class web_chatControl extends BaseControl {
    ...
    public function send_msgWt(){
        ...
        if (($member_id < 1) || ($member_id != $f_id)) $this->error(Language::get('wt_member_chat_login'));

        $member = $model_chat->getMember($t_id);

        if ($t_name != $member['member_name']) $this->error(Language::get('wt_member_chat_name_error'));
        ...
        if($chat_msg['m_id']) {
            ...
            $this->error(Language::get('wt_member_chat_add_error'));
    ...
    public function get_user_listWt(){
    ...
    public function get_seller_listWt(){
    ...
    public function get_infoWt(){
        ...
        if ($_SESSION['member_id'] < 1) $this->error(Language::get('wt_member_chat_login'));
    ...
    public function get_chat_logWt(){
    ...
    public function get_goods_contentWt(){
    ...
    public function get_goods_listWt(){
        ...
        if ($s_id > 0) {
            ...
            if(!empty($list) && is_array($list)) {
                ...
                foreach($list as $k => $v) {
                    ...
                    $v['goods_sale_price'] = wtPriceFormat($v['goods_sale_price']);
	...
	public function image_sendWt() {
		if(!isset($_SESSION['member_id'])||intval($_SESSION['member_id'])<1)
		{
			return false;
		}
			$member_id=$_SESSION['member_id'];
			...
			$upload->set('fprefix', 'smp'.$member_id);
            ...
            if (!empty($_FILES['_pic']['tmp_name'])){
                ...
                if ($result){
					echo UPLOAD_SITE_URL.'/chat/'.$upload->thumb_image;
                }else {
					echo '';
                }
            }
