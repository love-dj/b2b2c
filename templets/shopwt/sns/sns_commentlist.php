...
<div class="comment-widget">
  <div class="comment-edit">
    <form id="commentform_<?php echo $output['tid'];?>" method="post" action="index.php?w=member_snsindex&t=addcomment">
      ...
      <div class="comment-add">
        ...
        <!-- 防止点击Enter键提交 -->
        <div class="w"> <span class="skin-blue"><span class="btn"><a href="javascript:void(0);" wt_type="commentbtn" data-param='{"txtid":"<?php echo $output['tid'];?>"}'><?php echo $lang['sns_comment'];?></a></span></span> <span id="commentcharcount<?php echo $output['tid'];?>" style="float:right;"></span> <a class="face" wt_type="smiliesbtn" data-param='{"txtid":"comment<?php echo $output['tid'];?>"}' href="javascript:void(0);" ><?php echo $lang['sns_smiles'];?></a> </div>
      </div>
    </form>
  </div>
  <div class="clear"></div>
  ...
  <ul class="comment-list">
    ...
    <li wt_type="commentrow_<?php echo $v['comment_id']; ?>"> <a target="_blank" href="index.php?w=member_snshome&mid=<?php echo $v['comment_memberid'];?>"><img width="30" height="30" class="clogo" src="<?php if ($v['comment_memberavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['comment_memberavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" onload="javascript:DrawImage(this,30,30);"></a>
      <div class="detail">
        <p class="words"><a target="_blank" href="index.php?w=member_snshome&mid=<?php echo $v['comment_memberid'];?>" class="name"><?php echo $v['comment_membername'];?></a><?php echo $lang['wt_colon']; ?><?php echo parsesmiles($v['comment_content']);?></p>
        <p class="other">
          <?php if ($_SESSION['member_id'] == $v['comment_memberid']){?>
          <span class="opt"><a href="javascript:void(0);" wt_type="comment_del" data-param='{"cid":"<?php echo $v['comment_id'];?>"}'><?php echo $lang['wt_delete'];?></a></span>
          <?php }?>
  ...
  <div class="more"><a target="_blank" href="index.php?w=member_snshome&t=traceinfo&mid=<?php echo $output['master_info']['member_id'];?>&id=<?php echo $output['tid'];?>"><?php echo $lang['sns_comment_more'];?></a></div>
...
$(function(){
	//删除评论
	$("[wt_type='comment_del']").live('click',function(){
		var obj = $(this);
		showDialog('<?php echo $lang['wt_common_op_confirm'];?>','confirm', '', function(){
			var data_str = $(obj).attr('data-param');
	        eval("data_str = "+data_str);
	        ajax_get_confirm('','index.php?w=member_snsindex&t=delcomment&id='+data_str.cid);
			return false;
		});
	});
	//评论提交
	$("[wt_type='commentbtn']").die().live('click',function(){
		...
		if($("#commentform_"+data.txtid).valid()){			
			...
			if(cookienum >= <?php echo $output['max_recordnum'];?> && $("#commentseccode"+data.txtid).css('display')=="none"){
				...
				$("#commentseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?w=vercode&type=30x92&c=' + Math.random());
