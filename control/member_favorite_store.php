...
class member_favorite_storeControl extends BaseMemberControl{
     ...
     * 增加店铺收藏
     */
    public function favoritestoreWt(){
    ...
    private function _getStoreGoods($store_id) {
        ...
        $fieldstr = "goods_id,goods_name,goods_sale_price,goods_image";
        ...
        $condition = array();
        $condition['store_id'] = $store_id;
        $condition['goods_sale_type'] = array('neq', 0);
        $sale_goods_list = $model_goods->getGoodsListByColorDistinct($condition, $fieldstr, 'goods_id desc', 0, $num);
        ...
        return array('new'=>$new_goods_list, 'sale'=>$sale_goods_list, 'hot'=>$hot_goods_list, 'sign' => $sign);
    }
    /**
     * 店铺收藏列表查看 
     */
    public function moreWt() {
        ...
        $model_goods = Model('goods');
        $fieldstr = "goods_id,goods_name,goods_sale_price,goods_image,goods_addtime";
        ...
        $sale_count = $model_goods->getGoodsOnlineCount(array('store_id' => $store_id, 'goods_sale_type' => array('neq', 0)));
        Tpl::output('count', array('new' => $new_count, 'sale' => $sale_count, 'hot' => $salenum_count));
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),'index.php?w=member_favorite_store&t=fslist','succ');
                }
            }else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_store','menu_name'=>Language::get('wt_member_path_collect_store'), 'menu_url'=>'index.php?w=member_favorite_store&t=fslist')
        );
        if ($menu_key == 'more') {
