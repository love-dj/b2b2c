...
class goods_albumControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ($album_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick='fg_del(". $value['aclass_id'] .")'><i class='fa fa-trash-o'></i>删除</a><a class='btn green' href='index.php?w=goods_album&t=pic_list&aclass_id=".$value['aclass_id']."'><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['aclass_cover'] = "<a href='javascript:void(0);' class='pic-thumb-tip' onMouseOut='toolTip()' onMouseOver='toolTip(\"<img src=".($value['aclass_cover'] != '' ? cthumb($value['aclass_cover'], 60, $value['store_id']) : ADMIN_SITE_URL.'/templets/'.TPL_NAME.'/images/member/default_image.png').">\")'><i class='fa fa-picture-o'></i></a>";
    ...
    public function aclass_delWt(){
        ...
        $this->log(L('wt_delete,g_album_one').'[ID:'.intval($_GET['aclass_id']).']',1);
     ...
     *
     */
    public function del_album_picWt(){
        ...
        $this->log(L('wt_delete,g_album_pic_one').'[ID:'.$apic_id.']',1);
        exit('1');
    }
     ...
     *
     */
    public function del_more_picWt(){
        ...
        $this->log(L('wt_delete,g_album_pic_one').'[ID:'.implode(',',$_POST['delbox']).']',1);
        redirect();
    }
