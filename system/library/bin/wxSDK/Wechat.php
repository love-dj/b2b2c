...
class Wechat {
        ...
        if($token){
            ...
            if(IS_GET){
				if(isset($_GET['echostr'])){
					Handle('weixin_token')->get_access_token(array(),true);
					exit($_GET['echostr']);
				}else{
					exit('请绑定微信服务器');
				}
    ...
    /**
     * 回复图文消息，一条数组值代表一条信息
     * @param  array  $news   图文内容 array([标题，描述，URL，缩略图],[标题，描述，URL，缩略图])二维数组
     */
    public function replyNews($news){
        return $this->response($news, self::MSG_TYPE_NEWS);
    }
