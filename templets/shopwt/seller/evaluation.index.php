...
<form method="get">
  <table class="search-form">
    <input type="hidden" name="w" value="store_evaluate" />
...
<table class="wtsc-default-table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td rowspan="2" class="nscs-table-handle vt"><span><a wttype="btn_show_explain_dialog" data-geval-id="<?php echo $v['geval_id'];?>" data-geval-content="<?php echo $v['geval_content'];?>" href="javascript:;" class="btn-aqua"> <i class="icon-comments-alt "></i>
        <p><?php echo $lang['member_evaluation_explain'];?></p>
        </a></span></td>
    </tr>
    <tr class="bd-line">
      <td class="tl" colspan="20"><div <?php echo empty($v['geval_explain'])?'style="display:none;"':''?>> <strong>解释内容：</strong> <span wttype="explain"><?php echo $v['geval_explain'];?></span> </div></td>
    </tr>
    <?php if (!empty($v['geval_content_again'])) {?>
    <tr>
      <td rowspan="2"></td>
      <td class="tl"><strong>追加评价：</strong> <span><?php echo $v['geval_content_again'];?></span></td>
      <td rowspan="2" class="nscs-table-handle vt"><span> <a wttype="btn_show_explain_again_dialog" data-gaval-again-id="<?php echo $v['geval_id'];?>" data-geval-again-content="<?php echo $v['geval_content_again'];?>" href="javascript:;" class="btn-aqua"> <i class="icon-comments-alt"></i>
        <p><?php echo $lang['member_evaluation_explain'];?></p>
        </a> </span></td>
    </tr>
    <tr class="bd-line">
      <td class="tl" colspan="20"><div <?php echo empty($v['geval_explain_again']) ? 'style="display:none;"' : ''?>> <strong>解释内容：</strong> <span wttype="explain_again"><?php echo $v['geval_explain_again'];?></span> </div></td>
    </tr>
    <?php }?>
    <?php }?>
    <?php } else { ?>
...
$(document).ready(function(){
    ...
    $('[wttype="btn_show_explain_dialog"]').on('click', function() {
        $item = $(this).parents('tr').next('tr').find('[wttype="explain"]');
        ...
        $('#geval_explain').val('');
        $('#dialog_explain').wt_show_dialog({title:'解释评价'});
        });
    ...
    $('[wttype="btn_show_explain_again_dialog"]').on('click', function() {
        $item_again = $(this).parents('tr').next('tr').find('[wttype="explain_again"]');
        ...
        $('#dialog_explain_again').wt_show_dialog({title:'解释追评'});
        });
