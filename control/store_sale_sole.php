...
class store_sale_soleControl extends BaseSellerControl {
    ...
    public function indexWt() {
        $this->sole_goods_listWt();
    }

    /**
     * 套餐商品列表
     */
    public function sole_goods_listWt() {
        ...
        Tpl::showpage('store_sale_sole.goods_list');
    }

    /**
     * 选择商品
     */
    public function sole_select_goodsWt() {
        ...
        Tpl::showpage('store_sale_sole.select_goods', 'null_layout');
    }

    /**
     * 购买套餐
     */
    public function sole_quota_addWt() {
        if (chksubmit()) {
            $quantity = intval($_POST['sole_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_sole_price')); // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlShop('store_sale_sole', 'sole_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_sole = Model('p_sole');
            ...
            if ($return) {
                ...
                $this->recordSellerLog('购买'.$quantity.'套手机专享，单价'.intval(C('sale_sole_price')).'元');
                showDialog('购买成功', urlShop('store_sale_sole', 'sole_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_sole', 'sole_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('sole_quota_add', 'sole_quota_add');
        Tpl::showpage('store_sale_sole.quota_add');
    }

    /**
     * 套餐续费
     */
    public function sole_renewWt() {
        if (chksubmit()) {
            $model_sole = Model('p_sole');
            $quantity = intval($_POST['sole_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_sole_price')); // 扣款数
            ...
            if ($return) {
                ...
                $end_time = TIMESTAMP + 60 * 60 * 24 * 30 * $quantity;
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 10), true);
                $this->recordSellerLog('续费'.$quantity.'套手机专享，单价'.intval(C('sale_sole_price')).'元');
                showDialog('购买成功', urlShop('store_sale_sole', 'sole_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_sole', 'sole_quota_add'));
            }
        }
     ...
     * 选择商品
     */
    public function choosed_goodsWt() {
        $gid = $_GET['gid'];
        if ($gid <= 0) {
            Tpl::output('error', '参数错误');
        }
        $goods_content = Model('goods')->getGoodsInfoByID($gid);
        if (empty($goods_content) || $goods_content['store_id'] != $_SESSION['store_id']) {
            Tpl::output('error', '参数错误');
        }
        Tpl::output('goods_content', $goods_content);
        
        $solegoods_content = Model('p_sole')->getSoleGoodsInfo(array('store_id' => $_SESSION['store_id'], 'goods_id' => $gid));
        Tpl::output('solegoods_content', $solegoods_content);
        
        Tpl::showpage('store_sale_sole.choosed_goods', 'null_layout');
    }
    
    /**
     * 保存选择商品
     */
    public function choosed_goods_saveWt() {
        $model_sole = Model('p_sole');
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
        ...
        $sole_price = wtPriceFormat(floatval($_POST['sole_price']));
        if ($gid <= 0) {
        ...
        // 验证手机价格是否为最低价格
        if ($sole_price > $goods_content['goods_sale_price']) {
        ...
        $param['gc_id']        = $goods_content['gc_id'];
        
        $solegoods_content = $model_sole->getSoleGoodsInfo(array('store_id' => $_SESSION['store_id'], 'goods_id' => $goods_content['goods_id']));
        if (empty($solegoods_content)) {
            // 保存到推荐展位商品表
            $model_sole->addSoleGoods($param);
            $this->recordSellerLog('添加手机专享商品，商品id：'.$goods_content['goods_id']);
        } else {
            $model_sole->editSoleGoods($param, array('sole_goods_id' => $solegoods_content['sole_goods_id']));
            $this->recordSellerLog('编辑手机专享商品，商品id：'.$goods_content['goods_id']);
        }


        $goods_class = Model('goods_class')->getGoodsClassInfoById($goods_content['gc_id']);
        // 输出商品信息
        $goods_content['goods_image']  = thumb($goods_content);
        $goods_content['url']          = urlShop('goods', 'index', array('goods_id' => $goods_content['goods_id']));
        $goods_content['gc_name']      = $goods_class['gc_name'];
        $goods_content['sole_price']   = $sole_price;
        $goods_content['result']       = 'true';
        if (empty($solegoods_content)) {
            showDialog('添加手机专享促销成功', '', 'succ', 'CUR_DIALOG.close();insert_data(' . json_encode($goods_content) . ');');
        } else {
            showDialog('编辑手机专享促销成功', '', 'succ', 'CUR_DIALOG.close();update_data(' . json_encode($goods_content) . ');');
        }
    }
     ...
     * 删除选择商品
     */
    public function del_choosed_goodsWt() {
        $gid = $_GET['gid'];
        if ($gid <= 0) {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'sole_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'sole_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_sole', 'sole_goods_list'))
                );
                break;
            case 'sole_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'sole_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_sole', 'sole_goods_list')),
                    2=>array('menu_key'=>'sole_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlShop('store_sale_sole', 'sole_quota_add'))
                );
                break;
            case 'sole_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'sole_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_sole', 'sole_goods_list')),
                    2=>array('menu_key'=>'sole_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShop('store_sale_sole', 'sole_renew'))
                );
                break;
        }
