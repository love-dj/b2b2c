  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        <li><a href="index.php?w=brand&t=brand"><?php echo $lang['wt_manage'];?></a></li>
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?w=brand&t=get_xml&type=apply',
        colModel : [
...
function fg_apply(id) {
    ...
    $.getJSON('index.php?w=brand&t=brand_apply_set', {id:id}, function(data){
        if (data.state) {
...
function fg_del(id) {
    ...
    $.getJSON('index.php?w=brand&t=brand_del', {id:id}, function(data){
        if (data.state) {
            $("#flexigrid").flexReload();
        } else {
            showError(data.msg)
