...
class web_chatModel extends Model{
    ...
    public function addMsg($msg) {
        ...
        if ($m_id > 0) {
            ...
            $goods_id = 0;
            $goods_content = array();
            $pattern = '#'.BASE_SITE_URL.'/index.php\?w=goods&amp;t=index&amp;goods_id=(\d+)$#';
            preg_match($pattern, $t_msg, $matches);
            if (intval($matches[1]) < 1) {//伪静态
                $pattern = '#'.BASE_SITE_URL.'/goods-(\d+)\.html$#';
                preg_match($pattern, $t_msg, $matches);
            }
            $goods_id = intval($matches[1]);
            if ($goods_id >= 1) {
                $goods_content = $this->getGoodsInfo($goods_id);
                $goods_id = intval($goods_content['goods_id']);
            }
            $msg['goods_id'] = $goods_id;
            $msg['goods_content'] = $goods_content;
            return $msg;
        } else {
            return 0;
        }
    ...
    public function getGoodsInfo($goods_id) {
        ...
        $field = 'goods_id,store_id,goods_commonid,goods_name,goods_image,goods_price,goods_marketprice,goods_sale_price';
        ...
        if (is_array($goods) && !empty($goods)) {
            ...
            $goods['goods_sale_price'] = wtPriceFormat($goods['goods_sale_price']);
