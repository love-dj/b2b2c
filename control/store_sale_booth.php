...
class store_sale_boothControl extends BaseSellerControl {
    ...
    public function indexWt() {
        $this->booth_goods_listWt();
    }

    /**
     * 套餐商品列表
     */
    public function booth_goods_listWt() {
        ...
        Tpl::showpage('store_sale_booth.goods_list');
    }

    /**
     * 选择商品
     */
    public function booth_select_goodsWt() {
        ...
        Tpl::showpage('store_sale_booth.select_goods', 'null_layout');
    }

    /**
     * 购买套餐
     */
    public function booth_quota_addWt() {
        if (chksubmit()) {
            $quantity = intval($_POST['booth_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_booth_price')); // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlShop('store_sale_booth', 'booth_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_booth = Model('p_booth');
            ...
            if ($return) {
                ...
                $this->recordSellerLog('购买'.$quantity.'套推荐展位，单价'.intval(C('sale_booth_price')).'元');
                showDialog('购买成功', urlShop('store_sale_booth', 'booth_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_booth', 'booth_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('booth_quota_add', 'booth_quota_add');
        Tpl::showpage('store_sale_booth.quota_add');
    }

    /**
     * 套餐续费
     */
    public function booth_renewWt() {
        if (chksubmit()) {
            $model_booth = Model('p_booth');
            $quantity = intval($_POST['booth_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_booth_price')); // 扣款数
            ...
            if ($return) {
                ...
                $end_time = TIMESTAMP + 60 * 60 * 24 * 30 * $quantity;
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 4), true);
                $this->recordSellerLog('续费'.$quantity.'套推荐展位，单价'.intval(C('sale_bundling_price')).'元');
                showDialog('购买成功', urlShop('store_sale_booth', 'booth_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_booth', 'booth_quota_add'));
            }
        }
    ...
    public function choosed_goodsWt() {
        ...
        // 验证商品是否存在
        $goods_content = Model('goods')->getGoodsInfoByID($gid, 'goods_id,goods_name,goods_image,goods_price,store_id,gc_id');
        ...
        $bootgoods_content = $model_booth->getBoothGoodsList(array('store_id' => $_SESSION['store_id']), 'goods_id');
        // 已添加商品总数
        if (count($bootgoods_content) >= C('sale_booth_goods_sum')) {
            $data = array('result' => 'false', 'msg' => '只能添加'.C('sale_booth_goods_sum').'个商品');
            $this->_echoJson($data);
        }
        // 商品是否已经被添加
        $bootgoods_content = array_under_reset($bootgoods_content, 'goods_id');
        if (isset($bootgoods_content[$gid])) {
        ...
        $insert['gc_id'] = $goods_content['gc_id'];
        $model_booth->addBoothGoods($insert);

        $this->recordSellerLog('添加推荐展位商品，商品id：'.$goods_content['goods_id']);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_booth', 'booth_goods_list'))
                );
                break;
            case 'booth_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlShop('store_sale_booth', 'booth_quota_add'))
                );
                break;
            case 'booth_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShop('store_sale_booth', 'booth_renew'))
                );
                break;
        }
