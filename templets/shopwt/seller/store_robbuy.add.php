...
<div class="wtsc-form-default">
  <form id="add_form" action="index.php?w=store_robbuy&t=robbuy_save" method="post" enctype="multipart/form-data">
    <dl>
...
<?php if (!$output['isOwnShop']) { ?>
          <?php echo '抢购开始时间不能大于'.date('Y-m-d H:i', $output['current_robbuy_quota']['end_time']);?>
          </p>
<?php } ?>
    ...
    <dl>
      ...
      <div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
        ...
        <p class="hint"><?php echo $lang['robbuy_goods_explain'];?></br><span class="red">抢购生效后该商品的所有规格SKU都将执行统一的抢购价格</span></p>
        </dd>
    </dl>
    <dl wttype="robbuy_goods_content" style="display:none;">
      <dt><?php echo $lang['robbuy_index_store_price'].$lang['wt_colon'];?></dt>
      <dd> <?php echo $lang['currency'];?><span wttype="robbuy_goods_price"></span><input id="input_robbuy_goods_price" type="hidden"></dd>
    </dl>
    <dl>
    ...
    <dl>
      <dt><?php echo $lang['robbuy_class'].$lang['wt_colon'];?></dt>
      <dd>
        ...
        <span></span>
        <p class="hint"><?php echo $lang['robbuy_class_tip'];?></p>
      </dd>
    </dl>
    <dl>
...
$(document).ready(function(){
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlShop('store_robbuy', 'search_goods');?>";
    ...
    $('#div_goods_search_result').on('click', '[wttype="btn_add_robbuy_goods"]', function() {
        ...
        $.get('<?php echo urlShop('store_robbuy', 'robbuy_goods_content');?>', {goods_commonid: goods_commonid}, function(data) {
            if(data.result) {
                ...
                $('#input_robbuy_goods_price').val(data.goods_price);
    ...
    jQuery.validator.methods.lessThanGoodsPrice= function(value, element) {
        var goods_price = $("#input_robbuy_goods_price").val();
...
})($.parseJSON('<?php echo json_encode($output['robbuy_classes']); ?>'));
</script>
