...
<div id="dialog_item_edit_image" style="display:none;">
  <div class="s-tips"><i class="icon-lightbulb"></i>请按提示尺寸制作上传图片，以达到手机客户端及Wap手机商城最佳显示效果。</div>
...
<div id="dialog_item_edit_gg" style="display:none;">
  <div class="s-tips"><i class="icon-lightbulb"></i>请填写标题、内容</div>
  <div class="upload-thumb"> <img style="display: block;margin: 0 auto;" id="dialog_item_image" src="" alt=""></div>
  <input id="dialog_item_image_name" type="hidden">
  ...
  <form id="form_image" action="">
    <div class="wtap-form-default">
      ...
      <div class="bot"><a id="btn_save_itemjj" class="wtap-btn-big wtap-btn-green" href="javascript:;">保存</a></div>
    </div>
  </form>
</div>
...
<script id="item_image_templatejj" type="text/html">
    <div wttype="item_image" class="item">
        <input wttype="image_name" name="item_data[item][<%=image_name%>][image]" type="hidden" value="<%=image_name%>">
        <input wttype="image_data" name="item_data[item][<%=image_name%>][data]" type="hidden" value="<%=image_data%>">
        <a wttype="btn_del_item_imagejj" href="javascript:;">删除</a>
    </div>
</script>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        //添加公告
        $('[wttype="btn_add_item_gg"]').on('click', function() {
            //$dialog_item_image.hide();
            //$dialog_item_image_name.val('');
            $current_content = $(this).parent().find('[wttype="item_content"]');
            //$('#dialog_image_desc').text('推荐图片尺寸' + $(this).attr('data-desc'));
            ...
            //change_image_type_desc($('#dialog_item_image_type').val());
            $('#dialog_item_edit_gg').wt_show_dialog({
                width: 600,
                title: '添加'
            });
        });
        //添加简介
        $('[wttype="btn_add_item_jj"]').on('click', function() {
            //$dialog_item_image.hide();
            //$dialog_item_image_name.val('');
            $current_content = $(this).parent().find('[wttype="item_content"]');
            //$('#dialog_image_desc').text('推荐图片尺寸' + $(this).attr('data-desc'));
        ...
        //删除简介
        $('#item_edit_content').on('click', '[wttype="btn_del_item_imagejj"]', function() {
            $(this).parents('[wttype="item_image"]').remove();
        });
        //图片上传
