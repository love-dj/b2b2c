      ...
      <tbody>
        ...
        <tr class="<?php echo $val['class_parent_id'] == '0' ? '' : 'two'; ?> <?php echo 'parent' . $val['class_parent_id']; ?>" data-id="<?php echo $val['class_id']; ?>">
          ...
          <td class="handle"><a href="javascript:;" onclick="submit_delete(<?php echo $val['class_id']; ?>)" class="btn red"><i class="fa fa-trash-o"></i>删除</a>
            ...
            <a href="index.php?w=robbuy&t=class_add&parent_id=<?php echo $val['class_id']; ?>" class="btn green"><i class="fa fa-plus"></i><?php echo $lang['wt_add_sub_class']; ?></a>
            <?php } ?></td>
          <td class="sort"><span wt_type="class_sort" column_id="<?php echo $val['class_id']; ?>" title="可编辑" class="editable"><?php echo $val['sort'];?></span></td>
          <td class="name"><?php if ($val['class_parent_id'] != '0') { ?>
            <img fieldid="<?php echo $val['class_id'];?>" status="close" wt_type="flex" src="<?php echo ADMIN_TEMPLATES_URL;?>/images/tv-item1.gif">
            <?php } ?>
            <span wt_type="class_name" column_id="<?php echo $val['class_id']; ?>" title="可编辑" class="editable"><?php echo $val['class_name']; ?></span></td>
          <td></td>
        </tr>
        <?php } ?>
        <?php } else { ?>
...
$(function() {
        ...
        buttons: [
            {
                ...
                title: '新增分类',
                onpress: function() {
                    location.href = '<?php echo urlAdminShop('robbuy', 'class_add'); ?>';
                }
            },
            {
    ...
    // 行内ajax编辑
    $('span[wt_type="class_sort"]').inline_edit({w: 'robbuy', t: 'class_sort_update'});
    $('span[wt_type="class_name"]').inline_edit({w: 'robbuy', t: 'class_name_update'});
...
function submit_delete(id) {
    if (confirm('<?php echo $lang['wt_ensure_del'];?>')) {
        ...
        $('#list_form').attr('action','index.php?w=robbuy&t=class_drop');
        $('#class_id').val(id);
        $('#list_form').submit();
    }
}
