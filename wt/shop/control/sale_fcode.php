...
class sale_fcodeControl extends SystemControl{
    private $links = array(
        array('url'=>'w=sale_fcode&t=index','text'=>'商品列表'),
        array('url'=>'w=sale_fcode&t=fcode_quota_list','text'=>'套餐列表'),
        array('url'=>'w=sale_fcode&t=fcode_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?w=sale_fcode&sale_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function get_goods_xmlWt() {
        ...
        foreach ($goods_list as $value) {
            ...
            $operation .= "<li><a href='" . urlAdminShop('sale_fcode', 'download_f_code_excel', array('gid' => $value['goods_id'])) . "' target=\"_blank\">下载Ｆ码</a></li>";
     ...
     * 套餐列表
     */
    public function fcode_quota_listWt() {
        Tpl::output('top_link',$this->sublink($this->links,'fcode_quota_list'));
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_fcode.quota');
    }
     ...
     * 设置
     */
    public function fcode_settingWt() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["sale_fcode_price"], "require"=>"true", 'validator'=>'Number', "message"=>'请填写展位价格'),
            );
            ...
            $data['sale_fcode_price'] = intval($_POST['sale_fcode_price']);
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_fcode.setting');
    }
}
