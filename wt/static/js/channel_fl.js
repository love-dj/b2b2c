  ...
  titles["upload_show_a"] = '广告图片1';
  titles["upload_show_b"] = '广告图片2';
  titles["upload_show_c"] = '广告图片3';
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('index.php?w=web_channel&t=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
	...
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			$("#add_recommend_list").before('<div class="wtap-form-default" select_recommend_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'<p class="notic">该区域中部推荐商品模块选项卡名称，控制名称字符在4-8字左右，超出范围自动隐藏</p></dd></dl></div>'+
