...
class stat_goodsControl extends SystemControl{
    private $links = array(
        array('url'=>'w=stat_goods&t=pricerange','lang'=>'stat_goods_pricerange'),
        array('url'=>'w=stat_goods&t=hotgoods','lang'=>'stat_hotgoods'),
        array('url'=>'w=stat_goods&t=goods_sale','lang'=>'stat_goods_sale'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['t']) || in_array($this->search_arr['t'],array('pricerange','hotgoods','goods_sale','get_goods_xml'))){
     ...
     * 热卖商品列表
     */
    public function hotgoods_listWt(){
        $model = Model('stat');
        switch ($_GET['type']){
           case 'goodsnum':
               $sort_text = '下单量';
