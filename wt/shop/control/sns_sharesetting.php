...
class sns_sharesettingControl extends SystemControl{
     ...
     * 绑定接口列表
     */
    public function sharesettingWt(){
    ...
    public function setWt(){
        ...
        if ($result){
            $this->log(L('wt_edit,wt_binding_manage'),null);
    ...
    public function editWt(){
        ...
        if(chksubmit()){
            ...
            if ($result){
                $this->log(L('wt_edit,wt_binding_manage'),null);
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=sns_sharesetting&t=sharesetting');
            }else {
