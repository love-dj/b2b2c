...
class snstraceControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $operation .= "<li><a href='index.php?w=snstrace&t=commentlist&tid=". $value['trace_id'] ."'>查看评论</a></li>";
            ...
            $param['trace_title'] = str_replace("%siteurl%", BASE_SITE_URL.DS, $value['trace_title']);
            $param['trace_membername'] = $value['trace_membername'];
            $param['trace_memberid'] = $value['trace_memberid'];
            $param['trace_state'] = $value['trace_state'] ==  '0' ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function traceinfoWt() {
        ...
        if(!empty($trace_info['trace_title'])){
            //替换标题中的siteurl
            $trace_info['trace_title'] = str_replace("%siteurl%", BASE_SITE_URL.DS, $trace_info['trace_title']);
        }
        if(!empty($trace_info['trace_content'])){
            //替换内容中的siteurl
            $trace_info['trace_content'] = str_replace("%siteurl%", BASE_SITE_URL.DS, $trace_info['trace_content']);
            //将收藏商品和店铺连接剔除
    ...
    public function tracedelWt(){
        ...
        if($result){
            ...
            $this->log(L('wt_del,admin_snstrace_comment'),1);
    ...
    public function traceeditWt(){
        ...
        if($result){
            ...
            $this->log(L('wt_edit,admin_snstrace_comment'),1);
            showDialog(L('wt_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();');
        }else{
     ...
     * 输出XML数据
     */
    public function get_comment_xmlWt() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $param['comment_state'] = $value['comment_state'] ==  '1' ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function commentdelWt(){
        ...
        if($result){
            $this->log(L('wt_del,admin_snstrace_pl'),1);
            exit(json_encode(array('state'=>true,'msg'=>'删除成功')));
        }else{
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }
    ...
    public function commenteditWt(){
        ...
        if($result){
            $this->log(L('wt_edit,admin_snstrace_pl'),1);
            showDialog(L('wt_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();');
        }else{
            showDialog(L('wt_common_op_fail'));
        }
