...
class contractControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能','index.php?w=operating&t=setting','error');
        }
     ...
     * 保障服务项目列表XML
     */
    public function citemlist_xmlWt()
    {
        $model_contract = Model('contract');
     ...
     * 保障服务项目编辑
     */
    public function citemeditWt(){
        $itemid_id = intval($_GET['itemid']);
        if ($itemid_id <= 0){
            $itemid_id = intval($_POST['itemid_id']);
        }
        if ($itemid_id <= 0){
            showDialog(L('param_error'),'index.php?w=contract&t=citemlist');
        }
        $model_contract = Model('contract');
        //查询模板信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('wt_common_save_succ'),'index.php?w=contract&t=citemlist','succ');
            } else {
     ...
     * 店铺保障服务列表
     */
    public function contractlistWt(){
        //查询保障项目
     ...
     * 店铺保障服务列表XML
     */
    public function contractlist_xmlWt()
    {
        $where = array();
     ...
     * 店铺保障服务编辑
     */
    public function contracteditWt(){
        $ct_id = intval($_GET['ct_id']);
        if ($ct_id <= 0){
            $ct_id = intval($_POST['ct_id']);
        }
        if ($ct_id <= 0){
            showDialog(L('param_error'),'index.php?w=contract&t=contractlist');
        }
        $model_contract = Model('contract');
        //查询店铺保障服务信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if ($c_info['ct_closestate_key'] == $_POST['c_state']) {
                showDialog(L('wt_common_save_succ'),'index.php?w=contract&t=contractlist','succ');
            }
            try {
     ...
     * 服务加入申请列表XML
     */
    public function applylist_xmlWt()
    {
        $where = array();
        $sort = '';
        if ($_REQUEST['showanced']) {
     ...
     * 保障服务申请编辑
     */
    public function applyeditWt(){
        $aid = intval($_GET['aid']);
        if ($aid <= 0){
            $aid = intval($_POST['aid']);
        }
        if ($aid <= 0){
            showDialog(L('param_error'),'index.php?w=contract&t=applylist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['apply_state'] == $apply_info['cta_auditstate_key']) {
                showDialog('不能重复更改申请状态','index.php?w=contract&t=applylist');
            }
            if (!in_array($_POST['apply_state'],array_keys($curr_applystatearr))) {
                showDialog('编辑失败','index.php?w=contract&t=applyedit&aid='.$apply_info['cta_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('wt_common_save_succ'),'index.php?w=contract&t=applylist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    public function costeditWt(){
        ...
        if(chksubmit()){
            ...
            try {
                ...
                $price_msg .= (wtPriceFormat(abs($price)).L('currency_zh'));
                //新增保障项目日志
     ...
     * 服务退出申请列表XML
     */
    public function quitlist_xmlWt()
    {
        $where = array();
        $sort = '';
        if ($_REQUEST['showanced']) {
     ...
     * 保障服务申请编辑
     */
    public function quiteditWt(){
        $qid = intval($_GET['qid']);
        if ($qid <= 0){
            $qid = intval($_POST['qid']);
        }
        if ($qid <= 0){
            showDialog(L('param_error'),'index.php?w=contract&t=quitlist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['quit_state'] == $quit_info['ctq_auditstate_key']) {
                showDialog('不能重复编辑申请状态','index.php?w=contract&t=quitlist');
            }
            if (!in_array($_POST['quit_state'],array_keys($this->quit_auditstate_arr))) {
                showDialog('编辑失败','index.php?w=contract&t=quitedit&qid='.$quit_info['ctq_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('wt_common_save_succ'),'index.php?w=contract&t=quitlist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
     ...
     * 店铺保障服务详情
     */
    public function contractinfoWt(){
        $ct_id = intval($_GET['ct_id']);
        if ($ct_id <= 0){
            showDialog(L('param_error'),'index.php?w=contract&t=contractlist');
        }
     ...
     * 保障服务日志列表XML
     */
    public function contractlog_xmlWt()
    {
     ...
     * 保证金列表XML
     */
    public function costlist_xmlWt()
    {
        $item_id = intval($_GET['item_id']);
        $store_id = intval($_GET['store_id']);
        if ($item_id <= 0 || $store_id <= 0) {
     ...
     * 更新店铺商品保障服务状态
     */
    public function contractgoodsWt(){
    ...
    private function show_menu($menu_key=''){
        $menu_array = array(
            1=>array('menu_key'=>'applylist','menu_name'=>'服务加入申请', 'menu_url'=>'index.php?w=contract&t=applylist'),
            2=>array('menu_key'=>'quitlist','menu_name'=>'服务退出申请', 'menu_url'=>'index.php?w=contract&t=quitlist'),
            3=>array('menu_key'=>'contractlist','menu_name'=>'店铺保障服务', 'menu_url'=>'index.php?w=contract&t=contractlist'),
            4=>array('menu_key'=>'citemlist','menu_name'=>'保障服务管理', 'menu_url'=>'index.php?w=contract&t=citemlist')
        );
