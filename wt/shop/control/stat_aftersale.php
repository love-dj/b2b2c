...
class stat_aftersaleControl extends SystemControl{
    private $links = array(
        array('url'=>'w=stat_aftersale&t=refund','lang'=>'stat_refund'),
        array('url'=>'w=stat_aftersale&t=evalstore','lang'=>'stat_evalstore'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['t']) || in_array($this->search_arr['t'],array('refund','refundlist','get_refundlist_xml','get_refund_highcharts'))){
     ...
     * 退款统计
     */
    public function get_refund_highchartsWt(){
        $where = array();
     ...
     * 输出退款统计XML数据
     */
    public function get_refundlist_xmlWt(){
        $where = array();
        $model = Model('stat');
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where['add_time'] = array('between',$searchtime_arr);
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $out_array['refund_amount'] = wtPriceFormat($v['refund_amount']);
     ...
     * 输出店铺动态评分统计XML数据
     */
    public function get_evalstore_xmlWt(){
        $model = Model('stat');
        $where = array();
        if(intval($_GET['store_class']) > 0){
            $where['sc_id'] = intval($_GET['store_class']);
