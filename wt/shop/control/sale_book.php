...
class sale_bookControl extends SystemControl{
    private $links = array(
        array('url'=>'w=sale_book&t=index','text'=>'定金预售'),
        array('url'=>'w=sale_book&t=presell_goods_list', 'text'=>'全款预售'),
        array('url'=>'w=sale_book&t=book_quota_list','text'=>'套餐列表'),
        array('url'=>'w=sale_book&t=book_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?w=sale_book&sale_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function indexWt() {
        ...
        $this->book_goods_listWt();
    }

    /**
     * 活动商品列表
     */
    public function book_goods_listWt() {
        Tpl::output('top_link',$this->sublink($this->links,'index'));
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_book.book_goods');
    }

    /**
     * 活动商品管理XML
     */
    public function get_book_goods_xmlWt() {
        ...
        foreach ($goods_list as $value) {
            ...
            $param['book_total_payment'] = wtPriceFormat($value['book_down_payment'] + $value['book_final_payment']);
     ...
     * 删除订金预售商品活动
     */
    public function del_book_goodsWt() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 活动商品列表
     */
    public function presell_goods_listWt() {
        Tpl::output('top_link',$this->sublink($this->links,'presell_goods_list'));
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_book.presell_goods');
    }

    /**
     * 活动商品管理XML
     */
    public function get_presell_goods_xmlWt() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
     ...
     * 删除订金预售商品活动
     */
    public function del_presell_goodsWt() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function book_quota_listWt() {
        Tpl::output('top_link',$this->sublink($this->links,'book_quota_list'));
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_book.quota');
    }

    /**
     * 套餐列表XML
     */
    public function get_quota_xmlWt() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
     ...
     * 设置
     */
    public function book_settingWt() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["sale_book_price"], "require"=>"true", 'validator'=>'Number', "message"=>'请填写展位价格'),
            );
            ...
            $data['sale_book_price'] = intval($_POST['sale_book_price']);

            $return = $model_setting->updateSetting($data);
            if($return){
                $this->log(L('wt_set').' 推荐展位');
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_book.setting');
    }
}
