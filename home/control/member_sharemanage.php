...
class member_sharemanageControl extends BaseMemberControl {
    public function __construct(){
        ...
        //判断系统是否开启站外分享功能
        if (C('share_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?w=member_information&t=member','html','error');
        }
    }
    /**
     * 分享绑定列表
     ...
     * 解除绑定
     */
    public function unbindWt(){
        $type = $_GET['type'];
        if (empty($type)){
            showDialog(Language::get('wrong_argument'),'index.php?w=member_sharemanage','error');
        }
        $model = Model('sns_binding');
     ...
     * 分享绑定qqzone
     */
    public function share_qqzoneWt(){
        //判断系统是否开启站外分享功能
        if (C('share_qqzone_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?w=member_sharemanage','html','error');
        }
        include_once BASE_API_PATH.'/sns/qqzone/oauth/qq_callback.php';
        if (!$_SESSION['qqzone']){
        ...
        //获取qq账号信息
        require_once (BASE_API_PATH.DS.'sns'.DS.'qqzone'.DS.'user'.DS.'get_user_info.php');
     ...
     * 分享绑定sinaweibo
     */
    public function share_sinaweiboWt(){
        //判断系统是否开启站外分享功能
        if (C('share_sinaweibo_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?w=member_sharemanage','html','error');
        }
        include_once BASE_API_PATH.'/sns/sinaweibo/callback.php';
        ...
        //获取新浪微博账号信息
        require_once (BASE_API_PATH.DS.'sns'.DS.'sinaweibo'.DS.'saetv2.ex.class.php');
     ...
     * 分享绑定腾讯微博
     */
    public function share_qqweiboWt(){
        //判断系统是否开启站外分享功能
        if (C('share_qqweibo_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?w=member_sharemanage','html','error');
        }
        include_once BASE_API_PATH.'/sns/qqweibo/callback.php';
        if (!$_SESSION['qqweibo']){
            echo "<script>alert('".Language::get('sharebind_bind_fail')."');</script>";
            echo "<script>window.close();</script>";
            exit;
