...
class mb_specialControl extends SystemControl{
     ...
     * 输出专题列表XML数据
     */
    public function get_special_xmlWt() {
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $out_array['special_desc'] = '<span wt_type="edit_special_desc" column_id="'.$v['special_id'].
     ...
     * 保存专题
     */
    public function special_saveWt() {
        $model_mb_special = Model('mb_special');
        ...
        if($result) {
            ...
            showMessage(L('wt_common_save_succ'), urlAdminMobile('mb_special', 'special_list'));
        } else {
            $this->log('添加手机专题' . '[ID:' . $result. ']', 0);
            showMessage(L('wt_common_save_fail'), urlAdminMobile('mb_special', 'special_list'));
        }
    }
     ...
     * 编辑专题描述
     */
    public function update_special_descWt() {
        $model_mb_special = Model('mb_special');
    ...
    public function special_delWt() {
        ...
        if($result) {
            ...
            showMessage(L('wt_common_del_succ'), urlAdminMobile('mb_special', 'special_list'));
        } else {
            $this->log('删除手机专题' . '[ID:' . $_POST['special_id'] . ']', 0);
            showMessage(L('wt_common_del_fail'), urlAdminMobile('mb_special', 'special_list'));
        }
    }
...
Tpl::showpage('mb_special_item.list');
     ...
     * 编辑专题
     */
    public function special_editWt() {
        $model_mb_special = Model('mb_special');
...
Tpl::showpage('mb_special_item.list');
    ...
    public function special_item_saveWt() {
        ...
        if($result) {
            if($_POST['special_id'] == $model_mb_special::INDEX_SPECIAL_ID) {
                showMessage(L('wt_common_save_succ'), urlAdminMobile('mb_special', 'index_edit'));
            } else {
                showMessage(L('wt_common_save_succ'), urlAdminMobile('mb_special', 'special_edit', array('special_id' => $_POST['special_id'])));
            }
        } else {
     ...
     * 限时折扣商品列表
     */
    public function goods_xianshi_listWt() {
        $model_goods = Model('goods');
        $condition = array();
		...
		$goods_list = $model_goods->getGoodsOnlineListAndSaleByIdArray($goods_id_list);
        Tpl::output('goods_list', $goods_list);
        Tpl::output('show_page', $model_goods->showpage());
        Tpl::setDirquna('mobile');
        Tpl::showpage('mb_special_widget.goods', 'null_layout');
     ...
     * 抢购商品列表
     */
    public function goods_robbuy_listWt() {
        $model_goods = Model('goods');
        $condition = array();
        $condition['goods_name'] = array('like', '%' . $_GET['keyword'] . '%');
		$model_robbuy_goods = Model('robbuy');
		$goods_list_arr=$model_robbuy_goods->getRobbuyGoodsExtendIds($condition);			
		$goods_list=$model_goods->getGoodsOnlineListAndSaleByIdArray($goods_list_arr);
