...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'w=store&t=store','text'=>'管理'),
        array('url'=>'w=store&t=store_joinin','text'=>'开店申请'),
        array('url'=>'w=store&t=reopen_list','text'=>'续签申请'),
        array('url'=>'w=store&t=store_bind_class_applay_list','text'=>'经营类目申请'),
        array('url'=>'w=store&t=bill_cycle','text'=>'结算周期设置')
    );
    ...
    public function get_xmlWt() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?w=store&t=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('wt_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?w=store&t=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?w=store&t=del&store_id=" . $value['store_id'] . "'>删除该店铺</a></li><li><a href='index.php?w=store&t=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
            ...
            $operation .= "</ul></span><a class='btn blue' href='../../index.php?w=seller_loginstore&t=login_store&name=".$value["seller_name"]."&pass=".$pass["member_passwd"]."' target='_blank'>登录</a>";
     ...
     * 输出XML数据
     */
    public function get_bill_cycle_xmlWt() {
        $model_store = Model('store');
        $condition = array();
        if ($_GET['store_name'] != '') {
            $condition['store_name'] = array('like', '%' . $_GET['store_name'] . '%');
        ...
        foreach ($store_list as $value) {
            $param = array();
            $store_state = $this->getStoreState($value);
            $operation = "<a class='btn blue' href='index.php?w=store&t=bill_cycyle_edit&store_id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='". urlShop('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
    ...
    public function export_csvWt() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=store&t=index');
				Tpl::setDirquna('shop');
    ...
    public function store_editWt(){
	    ...
	    //编辑店铺地址 公司名称
            ...
            if ($result){
                $url = array(
                array(
                'url'=>'index.php?w=store&t=store',
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>'index.php?w=store&t=store_edit&store_id='.intval($_POST['store_id']),
     ...
     * 店铺结算周期编辑
     */
    public function bill_cycyle_editWt(){
        $lang = Language::getLangContent();
        ...
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage($lang['wt_common_save_succ'],'index.php?w=store&t=bill_cycle');
            }else {
                $this->log('设置店铺结算周期['.$_POST['store_name'].']',1);
                showMessage($lang['wt_common_save_fail'],'index.php?w=store&t=bill_cycle');
            }
        }
     ...
     * 店铺经营类目管理
     */
    public function store_bind_classWt() {
     ...
     * 添加经营类目
     */
    public function store_bind_class_addWt() {
     ...
     * 删除经营类目
     */
    public function store_bind_class_delWt() {
    ...
    public function store_bind_class_updateWt() {
        $bid = intval($_GET['id']);
        if($bid <= 0) {
    ...
    public function get_joinin_xmlWt() {
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"index.php?w=store&t=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-check-bbs\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?w=store&t=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
            $param['member_id'] = $value['member_id'];
            $param['member_name'] = $value['member_name'];
            $param['sg_id'] = $value['sg_name'];
            $param['paying_amount'] = wtPriceFormat($value['paying_amount']);
     ...
     * 经营类目申请列表
     */
    public function store_bind_class_applay_listWt(){
     ...
     * 输出XML数据
     */
    public function get_bind_class_applay_xmlWt() {
        $model_store_bind_class = Model('store_bind_class');
        // 设置页码参数名称
        $condition = array();
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = 'index.php?w=store&t=store_bind_class_applay_check&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-check-bbs\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = 'index.php?w=store&t=store_bind_class_applay_del&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
     ...
     * 审核经营类目申请
     */
    public function store_bind_class_applay_checkWt() {
        $model_store_bind_class = Model('store_bind_class');
        $condition = array();
     ...
     * 删除经营类目申请
     */
    public function store_bind_class_applay_delWt() {
        $model_store_bind_class = Model('store_bind_class');
        $condition = array();
        $condition['bid'] = intval($_GET['bid']);
    ...
    public function get_reopen_xmlWt() {
        ...
        foreach ($reopen_list as $value) {
            ...
            $param['re_store_id'] = $value['re_get_xmlstore_id'];
     ...
     * 审核店铺续签申请
     */
    public function reopen_checkWt() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_reopen = Model('store_reopen');
            $condition = array();
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('店铺入驻申请审核完成','index.php?w=store&t=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail,$paying_amount = 0) {
	    ...
	    $shop_array['is_person']= $joinin_detail['is_person'];
            ...
            if($state) {
                ...
                showMessage('店铺开店成功','index.php?w=store&t=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?w=store&t=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?w=store&t=store_joinin');
        }
    }
     ...
     * 提醒续费
     */
    public function remind_renewalWt() {
        $store_id = intval($_GET['store_id']);
        ...
        if (!empty($store_info) && $store_info['store_end_time'] < (TIMESTAMP + 864000) && cookie('remindRenewal'.$store_id) == null) {
            ...
            setWtCookie('remindRenewal'.$store_id, 1, 86400 * 10);  // 十天
            showMessage('消息发送成功');
        }
            showMessage('消息发送失败');
    }
