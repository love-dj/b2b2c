...
class member_refundControl extends mobileMemberControl {
     ...
     * 全部退款获取订单信息
     */
    public function refund_all_formWt(){
        ...
        if(!empty($order_info) && is_array($order_info)) {
            $book_amount = Handle('order_book')->getDepositAmount($order_info);//订金金额
            $order = array();
            $order['order_id'] = $order_info['order_id'];
            $order['order_type'] = $order_info['order_type'];
            $order['order_amount'] = wtPriceFormat($order_info['order_amount']);
            $order['order_sn'] = $order_info['order_sn'];
            $order['store_name'] = $order_info['store_name'];
            $order['store_id'] = $order_info['store_id'];
            $order['allow_refund_amount'] = wtPriceFormat($order_info['order_amount'] - $book_amount);//可退款金额
            $order['book_amount'] = wtPriceFormat($book_amount);
     ...
     * 全部退款保存数据
     */
    public function refund_all_postWt(){
        $model_refund = Model('refund_return');
        $member_id = $this->member_info['member_id'];
        ...
        if (empty($order_info) || $payment_code == 'offline' || $refund['refund_id'] > 0) {//检查数据,防止页面刷新不及时造成数据错误
            ...
            $book_amount = Handle('order_book')->getDepositAmount($order_info);//订金金额
            $allow_refund_amount = wtPriceFormat($order_info['order_amount'] - $book_amount);//可退款金额
            ...
            $refund_array['refund_amount'] = wtPriceFormat($allow_refund_amount);
    ...
    public function refund_formWt(){
        ...
        if($refund_state == 1 && $goods_id > 0) {
            ...
            $goods['store_id'] = $goods_content['store_id'];
            $goods['order_goods_id'] = $goods_content['rec_id'];
            $goods['goods_id'] = $goods_content['goods_id'];
            $goods['goods_name'] = $goods_content['goods_name'];
            $goods['goods_type'] = orderGoodsType($goods_content['goods_type']);
            $goods['goods_img_360'] = thumb($goods_content,360);
            $goods['goods_price'] = wtPriceFormat($goods_content['goods_price']);
            $goods['goods_spec'] = $goods_content['goods_spec'];
            $goods['goods_num'] = $goods_content['goods_num'];
            
            $goods_pay_price = $goods_content['goods_pay_price'];//商品实际成交价
            ...
            $goods['goods_pay_price'] = wtPriceFormat($goods_pay_price);
    ...
    public function refund_postWt(){
        ...
        if($refund_state == 1 && $goods_id > 0 && empty($refund)) {
            ...
            if (($goods_num < 0) || ($goods_num > $goods_content['goods_num'])) {
                $goods_num = 1;
            }
            ...
            $refund_array['seller_state'] = '1';//状态:1为待审核,2为同意,3为不同意
            $refund_array['refund_amount'] = wtPriceFormat($refund_amount);
            $refund_array['goods_num'] = $goods_num;
            $refund_array['buyer_message'] = $_POST['buyer_message'];
            $refund_array['add_time'] = time();
            
            $state = $model_refund->addRefundReturn($refund_array,$order_info,$goods_content);
            if ($state) {
                if ($order_info['order_state'] == $order_shipped) {
                    $model_refund->editOrderLock($order_id);
                }
     ...
     * 退款记录列表
     */
    public function get_refund_listWt() {
        $model_order = Model('order');
        $model_refund = Model('refund_return');
        $member_id = $this->member_info['member_id'];
        $refund_list = array();
        ...
        if(!empty($list) && is_array($list)) {
            ...
            foreach($list as $k => $v) {
                ...
                $val['refund_amount'] = wtPriceFormat($v['refund_amount']);
     ...
     *
     */
    public function get_refund_infoWt(){
        $model_refund = Model('refund_return');
        $member_id = $this->member_info['member_id'];
        $condition = array();
        $condition['buyer_id'] = $member_id;
        ...
        if(!empty($refund_info) && is_array($refund_info)) {
            ...
            $refund['refund_amount'] = wtPriceFormat($refund_info['refund_amount']);
            ...
            if(!empty($detail_info) && $detail_info['refund_state'] == 2) {
                ...
                $detail_array['pay_amount'] = wtPriceFormat($detail_info['pay_amount']);
                $detail_array['pd_amount'] = wtPriceFormat($detail_info['pd_amount']);
                $detail_array['rcb_amount'] = wtPriceFormat($detail_info['rcb_amount']);
            }
