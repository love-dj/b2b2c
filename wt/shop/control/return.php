...
class returnControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'w=return&t=return_manage','text'=>'待处理'),
            array('url'=>'w=return&t=return_all','text'=>'所有记录')
    );
    public function __construct(){
        parent::__construct();
        $model_refund = Model('refund_return');
        $model_refund->getRefundStateArray();
        if ($_GET['t'] == 'index') $_GET['t'] = 'return_manage';
        Tpl::output('top_link',$this->sublink($this->links,$_GET['t']));
    }

    public function indexWt() {
        $this->return_manageWt();
    }

    /**
     * 待处理列表
     */
    public function return_manageWt() {
		Tpl::setDirquna('shop');
    ...
    public function get_manage_xmlWt() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?w=return&t=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = wtPriceFormat($return_info['refund_amount']);
    ...
    public function get_all_xmlWt() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?w=return&t=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = wtPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function editWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
				...
				$wx_info = array();
				$wx_info['order_sn'] = $return['order_sn'];
				$wx_info['msg'] = $return['reason_info'];
				$wx_info['goods_name'] = $return['goods_name'];
				$wx_info['refund_amount'] = $return['refund_amount'];
				$param['param']['mp_array'] = Handle('wx_api')->getTemplateData($param['code'], $wx_info);
                QueueClient::push('sendMemberMsg', $param);

                showMessage(Language::get('wt_common_save_succ'),'index.php?w=return&t=return_manage');
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
