...
class dateControl extends BaseCronControl {
    ...
    public function indexWt() {
        ...
        //关闭退出分销超过N天的用户的分销中心
        $this->_fenxiao_member_close();
        //清理退出分销超过N天的商品
        $this->_fenxiao_goods_close();
    }
    ...
    private function _order_book_end_pay_notice() {
        ...
        foreach ($order_book_list as $order_book_info) {
            ...
            if (!empty($order_book_info['book_pay_time'])) {
				...
				$order_goods_info = $model_order->getOrderGoodsInfo(array('order_id'=>$order_book_info['book_order_id']));
                // 发送买家消息
                $param = array();
				...
				$wx_info = array();
				$wx_info['goods_name'] = $order_goods_info['goods_name'];
				$wx_info['goods_num'] = $order_goods_info['goods_num'];
				$wx_info['book_amount'] = $order_book_info['book_amount'];				
    ...
    private function _order_book_timeout_cancel() {
        ...
        foreach ($order_book_list as $order_book_info) {
            ...
            if (!empty($order_book_info['book_pay_time'])) {
                ...
                $result = $handle_order_book->changeOrderStateCancel($order_info,'system','系统','超期未支付尾款系统自动关闭订单');
                if (!$result['state']) {
    ...
    private function _order_auto_complete() {
        ...
        for ($i = 0; $i < 500; $i++){
            ...
            foreach ($order_list as $order_info) {
                $result = $handle_order->changeOrderStateSuccess($order_info['order_id']);
                if (!$result['state']) {
        ...
        for ($i = 0; $i < 500; $i++){
            ...
            if (empty($order_list)) break;
            foreach ($order_list as $order_info) {
                $result = $handle_order->changeOrderStateReceive($order_info,'system','系统','超期未收货系统自动完成订单');
                if (!$result['state']) {
    ...
    private function _vr_code_will_expire() {
        ...
        $list = Model('order_vr')->getCodeUnusedList($where,'order_id,min(buyer_id) as buyer_id,min(rec_id) as rec_id,min(vr_indate) as vr_indate');
        if (!empty($list)) {
            foreach ($list as $val) {
                $param = array();
                ...
                $param['param'] = array(
                    ...
                    'order_vr_url' => urlShop('member_order_vr', 'index')
                );
                QueueClient::push('sendMemberMsg', $param);
            }
        }
    ...
    private function _order_chain_timeout_cancel() {
        ...
        for ($i = 0; $i < 100; $i++){
            ...
            if (empty($order_list)) break;
            foreach ($order_list as $order_info) {
                $result = $handle_order->changeOrderStateCancel($order_info,'system','系统','超期未提货系统自动关闭订单',false,array('order_state'=>ORDER_STATE_NEW));
                if (!$result['state']) {
     ...
     * 更新商品促销到期状态(目前只有满即送)
     */
    private function _goods_sale_state_update() {
        //满即送过期
    ...
    private function _arrival_notice() {
        ...
        for ($i = 0; $i <= $times; $i++) {
            ...
            foreach ($notice_list as $val) {
				...
				$wx_info = array();
				$wx_info['goods_name'] = $val['goods_name'];
				$wx_info['goods_sale_price'] = $goodslist[$val['goods_id']]['goods_price'];
				$wx_info['an_addtime'] = date('Y-m-d H:i:s',$val['an_addtime']);				
				$param['param']['mp_array'] = Handle('wx_api')->getTemplateData($param['code'], $wx_info);
                QueueClient::push('sendMemberMsg', $param);
            }
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime+86400){
            ...
            for ($i=0; $i<$countnum; $i+=100){//每次查询100条
                ...
                if ($orderlog_list){
                    ...
                    //查询订单数据
                    $field = 'order_id,order_sn,store_id,buyer_id,buyer_name,add_time,payment_code,order_amount,shipping_fee,evaluation_state,order_state,refund_state,refund_amount,order_from,is_fx';
                    ...
                    //查询订单扩展数据
                    $field = 'order_id,reciver_province_id,reciver_city_id,reciver_area_id';
                    ...
                    //查询订单商品
                    $field = 'rec_id,order_id,goods_id,goods_name,goods_price,goods_num,goods_image,goods_pay_price,store_id,buyer_id,goods_type,sales_id,commis_rate,gc_id,is_fx';
                    ...
                    foreach ((array)$ordergoods_list as $k=>$v){
                        ...
                        $tmp['reciver_city_id'] = $order_common_list[$v['order_id']]['reciver_city_id'];
                        $tmp['reciver_area_id'] = $order_common_list[$v['order_id']]['reciver_area_id'];
    ...
    private function _create_bill() {
        ...
        $this->_model_order_vr = Model('order_vr');
    ...
    private function _create_vr_bill_cycle_by_day($start_unixtime,$store_info) {
        ...
        while (($time = strtotime('+'.$i.' day',$start_unixtime)) < $current_time) {
            ...
            try {
                $this->_model_order_vr->beginTransaction();
                //生成单个店铺订单出账单
                $data = array();
                $data['ob_store_id'] = $store_info['store_id'];
                $data['ob_start_date'] = $first_day_unixtime;
                $data['ob_end_date'] = $last_day_unixtime;
                $this->_create_order_vr_bill($data);
    
                $this->_model_order_vr->commit();
            } catch (Exception $e) {
                $this->log('虚拟账单:'.$e->getMessage());
                $this->_model_order_vr->rollback();
            }
            $start_unixtime = strtotime(date('Y-m-d 00:00:00', $last_day_unixtime+86400));
        }
    }
    ...
    /**
     * 取得结算开始时间
     * 从order_vr_bill表中取该店铺结算单中最大的ob_end_date作为本次结算开始时间
    ...
    private function _get_vr_start_date($store_id) {
        ...
        if ($bill_info['stime']){
            ...
            $condition['store_id'] = $store_id;
            $order_info = $this->_model_order_vr->getOrderInfo($condition,'min(add_time) as stime');
            if ($order_info['stime']) {
                $start_unixtime = $order_info['stime'];
            }
            if ($start_unixtime) {
    ...
    private function _calc_real_order_bill($data_bill){
        ...
        for ($i = 0; $i <= $order_count; $i = $i + 300){
            ...
            if (!empty($order_id_array)){
                ...
                $order_goods_content = $this->_model_order->getOrderGoodsInfo($order_goods_condition,$field);
        ...
        $cost_info = $this->_model_store_cost->getStoreCostInfo($cost_condition,'sum(cost_price) as cost_amount');
        $update['ob_store_cost_totals'] = floatval($cost_info['cost_amount']);
        
        $condition = array();
        $condition['store_id'] = $data_bill['ob_store_id'];
        $condition['log_state'] = 1;
        $condition['fx_pay_time'] = array(array('egt',$data_bill['ob_start_date']),array('elt',$data_bill['ob_end_date']),'and');
        $fx_pay = Model()->table('fx_pay')->field('sum(fx_pay_amount) as amount')->where($condition)->find();//分销佣金
        $update['ob_fx_pay_amount'] = floatval($fx_pay['amount']);
        ...
        $update['ob_commis_totals'] + $update['ob_commis_return_totals']- $update['ob_rf_rpt_amount'] - $update['ob_store_cost_totals'] - $update['ob_fx_pay_amount'];
     ...
     * @param int $data
     */
    private function _create_order_vr_bill($data){
        $data_bill['ob_start_date'] = $data['ob_start_date'];
        $data_bill['ob_end_date'] = $data['ob_end_date'];
        $data_bill['ob_state'] = 0;
        $data_bill['ob_store_id'] = $data['ob_store_id'];
        if (!$this->_model_vr_bill->getOrderBillInfo(array('ob_store_id'=>$data['ob_store_id'],'ob_start_date'=>$data['ob_start_date']))) {
            ...
            //对已生成空账单进行销量、退单、佣金统计
            $data_bill['ob_id'] = $insert;
            $update = $this->_calc_order_vr_bill($data_bill);
            if (!$update){
                throw new Exception('更新账单失败');
            }
     ...
     * @param array $data_bill
     */
    private function _calc_order_vr_bill($data_bill){
        ...
        $order_info =  $this->_model_order_vr->getOrderCodeInfo($order_condition, $fields);
        $update['ob_order_totals'] = floatval($order_info['order_amount']);
     ...
     * 关闭退出分销超过N天的用户的分销中心
     */
    private function _fenxiao_member_close(){
        $days = MEMBER_FENXIAO_CLOSE * 86400;
        $member_model = Model('member');
        $condition = array();
        $condition['quit_time'] = array('elt',TIMESTAMP - $days);
        $param = array();
        $param['fx_show'] = 0;
        $member_model->editMember($condition,$param);
    }

    /**
     * 清理退出分销超过N天的商品
     */
    private function _fenxiao_goods_close(){
        $days = GOODS_FENXIAO_DEL * 86400;
        $model_fx_goods = Model('fx_goods');
        $condition = array();
        $condition['fx_goods_state'] = 2;
        $condition['quite_time'] = array('elt',TIMESTAMP - $days);
        $model_fx_goods->dbDelFenxiaoGoods($condition);
    }
}