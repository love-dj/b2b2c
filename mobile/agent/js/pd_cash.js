$(function(){
	...
	var predepoit_=0;
	//获取区域代理佣金余额
	$.getJSON(ApiUrl + '/index.php?w=member_agent&t=my_asset', {'key':key}, function(result){
		
		$("#allmoney").html(result.datas.area_commission);
		predepoit_=result.datas.area_commission;
	});	
	...
	$('#btn').click(function(){
		...
		if($.sValid()){
	          $.ajax({
				type:'post',
				url:ApiUrl+"/index.php?w=member_agent&t=pd_cash_add",	
