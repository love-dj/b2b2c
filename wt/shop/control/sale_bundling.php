...
class sale_bundlingControl extends SystemControl{

    public function __construct(){
        parent::__construct();

        //读取语言包
        Language::read('sale_bundling');
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?w=sale_bundling&sale_allow=1',
     ...
     * 套餐管理
     */
    public function bundling_quotaWt()
    {
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_bundling_quota.list');
    }

    /**
     * 套餐管理XML
     */
    public function bundling_quota_xmlWt()
    {
        $condition = array();
    ...
    {
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_bundling.list');
    }

    /**
     * 活动管理XML
     */
    public function bundling_list_xmlWt()
    {
        $condition = array();
        ...
        foreach ($bundling_list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="'
                . urlAdminShop('sale_bundling', 'del_bundling', array(
                'bl_id' => $val['bl_id'],
     ...
     * 设置
     */
    public function bundling_settingWt() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["sale_bundling_price"], "require"=>"true", 'validator'=>'Number', "message"=>Language::get('bundling_price_error')),
                array("input"=>$_POST["sale_bundling_sum"], "require"=>"true", 'validator'=>'Number', "message"=>Language::get('bundling_sum_error')),
                array("input"=>$_POST["sale_bundling_goods_sum"], "require"=>"true", 'validator'=>'Number', "message"=>Language::get('bundling_goods_sum_error')),
            );
            ...
            $data['sale_bundling_price']       = intval($_POST['sale_bundling_price']);
            $data['sale_bundling_sum']         = intval($_POST['sale_bundling_sum']);
            $data['sale_bundling_goods_sum']   = intval($_POST['sale_bundling_goods_sum']);

            $return = $model_setting->updateSetting($data);
            if($return){
                $this->log(L('wt_set,wt_sale_bundling'));
        ...
        Tpl::setDirquna('shop');
        Tpl::showpage('sale_bundling.setting');
    }

    /**
     * 删除套餐活动
     */
    public function del_bundlingWt() {
