    ...
    public function get_xmlWt(){
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?w=article&t=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
                ...
                $list['article_show'] = $v['article_show'] == 0 ? '<span class="no"><i class="fa fa-ban"></i>否</span>' : '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>';
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?w=article&t=article',
                            'msg'=>"{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url'=>'index.php?w=article&t=article_add&ac_id='.intval($_POST['ac_id']),
     ...
     * 文章编辑
     */
    public function article_editWt(){
        $lang    = Language::getLangContent();
        $model_article = Model('article');
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
				...
				$update_array['article_time'] = time();
                ...
                if ($result){
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?w=article&t=article_edit&article_id='.intval($_POST['article_id']),
     ...
     * 文章图片上传
     */
    public function article_pic_uploadWt(){
        /**
         * 上传图片
         */
