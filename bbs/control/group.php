...
class groupControl extends BasebbsThemeControl{
    ...
    public function indexWt(){
        ...
        $theme_list = $model->table('bbs_theme')->where($where)->order('is_stick desc,lastspeak_time desc')->page(20)->select();
        ...
        $todaythcount = $model->table('bbs_theme')->where(array('theme_addtime'=>array('egt',$dayBegin), 'bbs_id'=>$this->c_id))->count();
        Tpl::output('todaythcount', $todaythcount);

        //展示形式，默认以图文展示 list/preview
        if($_GET['type'] != ''){
            $display_mode = ($_GET['type'] == 'list')?'list':'preview';
            setWtCookie('bbsDisplayMode', $display_mode, 30*24*60*60);
        }else{
            $display_mode = cookie('bbsDisplayMode') ? cookie('bbsDisplayMode') : 'preview';
        }
        ...
        $thclass_list = $model->table('bbs_thclass')->where($where)->order('thclass_sort asc')->select();
        ...
        $this->bbsSEO($this->bbs_info['bbs_name']);
    ...
    public function applyWt(){
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["apply_content"], "require"=>"true", "message"=>L('bbs_apply_content_null')),
                array("input"=>$_POST["intro"], "require"=>"true", "message"=>L('bbs_introduction_not_null')),
            );
            ...
            if($error != ''){
                ...
                $insert['cm_state']         = intval($this->bbs_info['bbs_joinaudit']) == 0 ? 1 : 0;
                $insert['is_identity']      = 3;
                $model->table('bbs_member')->insert($insert, true);
                if(intval($this->bbs_info['bbs_joinaudit']) == 0){
                    // Update the number of members
                    $update = array(
                                'bbs_mcount'=>array('exp', 'bbs_mcount+1')
                            );
                    ...
                    $model->table('bbs')->where(array('bbs_id'=>$this->c_id))->update($update);
                    showDialog(L('wt_apply_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
                }
            }
        }
        $this->bbsSEO(L('bbs_apply_join').$this->bbs_info['bbs_name']);
        Tpl::showpage('group_apply', 'null_layout');
    }
    /**
     * 退出社区
     */
    public function quitWt(){
        // 社区信息
        $this->bbsInfo();
        // 会员信息
        $this->memberInfo();
        if(in_array($this->identity, array(2,3))){
            // 删除会员
            Model()->table('bbs_member')->where(array('bbs_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->delete();

            $update = array();
            $update['bbs_mcount']= array('exp','bbs_mcount-1');

            // Whether to apply for management
            $rs = Model()->table('bbs_mapply')->where(array('bbs_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->find();
            if($rs){
                Model()->table('bbs_mapply')->where(array('bbs_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->delete();
     ...
     * 社区成员列表
     */
    public function group_memberWt(){
        // 社区信息
        $this->bbsInfo();
        ...
        $cm_list = $model->table('bbs_member')->where($where)->order('is_identity asc,cm_jointime desc')->page(15)->select();
        Tpl::output('show_page', $model->showpage(2));
        Tpl::output('cm_list', $cm_list);

        $this->bbsSEO(L('bbs_member_list').$this->bbs_info['bbs_name']);

        // breadcrumb navigation
        $this->breadcrumd(L('bbs_firend'));
        Tpl::showpage('group.member');
    }
    /**
     * 社区成员编辑
     */
    public function group_membereditWt(){
        $model = Model();
        $where = array();
        $where['bbs_id'] = $this->c_id;
        ...
        if(chksubmit()){
            ...
            if(!in_array($this->identity, array(1,2,3,6))){
                showDialog(L('bbs_member_no_join'), 'reload', 'error', 'CUR_DIALOG.close();');
            }
            $update = array();
            $update['cm_intro'] = $_POST['intro'];
            $model->table('bbs_member')->where($where)->update($update);
            showDialog(L('wt_deit_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
        }
        $member_info = $model->table('bbs_member')->where($where)->find();
    ...
    public function group_goodsWt(){
        ...
        // 成员商品列表
        $model = Model();
        $cmid_list  = $model->table('bbs_member')->field('member_id')->where(array('bbs_id'=>$this->c_id, 'cm_state'=>1))->select();
        ...
        $this->bbsSEO(L('bbs_member_like_and_show_goods').$this->bbs_info['bbs_name']);
     ...
     */
    public function manage_applyWt(){
        $model = Model();
        // bbs information
        $this->bbsInfo();
        // Verify membership
        $cm_info = $model->table('bbs_member')->where(array('bbs_id'=>$this->c_id, 'cm_state'=>1, 'is_identity'=>3))->find();
        if(empty($cm_info) || $this->bbs_info['mapply_open'] == 0 || $this->bbs_info['mapply_ml'] > $cm_info['cm_level'] || $cm_info['is_identity'] == 1){
            if(chksubmit()){
                showDialog(L('bbs_apply_error'), '', 'error', 'DialogManager.close(\'manage_apply\')');
            }else{
                echo '<script>showError("'.L('bbs_apply_error').'");DialogManager.close("manage_apply");</script>';exit;
            }
        }
        // Ban repeated application
        $mapply_info = $model->table('bbs_mapply')->where(array('bbs_id'=>$this->c_id, 'member_id'=>$_SESSION['member_id']))->find();
        if(!empty($mapply_info)){
            if(chksubmit()){
                showDialog(L('bbs_repeat_apply_error'), '', 'error', 'DialogManager.close(\'manage_apply\')');
            }else{
                echo '<script>showError("'.L('bbs_repeat_apply_error').'");DialogManager.close("manage_apply");</script>';exit;
            }
        }
        ...
        if(chksubmit()){
            ...
            $model->table('bbs_mapply')->insert($update);

            // Update the application for membership
            $model->table('bbs')->where(array('bbs_id'=>$this->c_id))->update(array('new_mapplycount'=>array('exp', 'new_mapplycount+1')));

            showDialog(L('wt_common_op_succ'), 'reload', 'succ', 'DialogManager.close(\'manage_apply\')');
        }
