...
class goodsModel extends Model{
     ...
     * @param string $table 表名
     */
    public function addGoodsStepPrice($insert) {
        return $this->table('goods_steps')->insert($insert);
    }
     ...
     * @return array
     */
	public function getGoodStepPrice($condition,$field = '*')
	{
		return $this->table('goods_steps')->field($field)->where($condition)->order('step_l_num asc')->select();
	}
     ...
     * @return boolean
     */
    public function delGoodsStepPrice($condition) {
        $result = $this->table('goods_steps')->where($condition)->delete();
        return true;
    }
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0) {
        ...
        if (C('dbdriver') == 'mysqli') {
            //shopwt v6.0 去掉重复的SKU
	    //$_field = "CONCAT(goods_commonid,',',color_id)";
	    $_field = "CONCAT(goods_commonid) as wt_distinct ," . $field;
            $_distinct = 'wt_distinct';
        }
        
        // 只查询固定条数不分页时，不计算商品总数
        if ($limit == 0) {
	     //shopwt v1.0 去掉重复的SKU
        ...
        $goods_list = array();
        if (C('dbdriver') == 'mysqli') {
            $goods_list = $this->getGoodsOnlineList($condition, $_field.' wt_distinct,'.$field, $page, $order, $limit, $_distinct, false, $count);
        }
        return $goods_list;
    }
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
            $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1, 'is_book' => 0), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_sale_price', 0, 'rand()', $limit, 'goods_commonid');
            if (!empty($goods_id_list)) {
                $tmp = array();
                foreach ($goods_id_list as $v) {
                    $tmp[] = $v['goods_id'];
                }
                $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in',$tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_sale_price', 0, 'rand()', $limit);
            }
     ...
     * @return boolean
     */
    public function editGoodsSalePrice($condition) {
        ...
        foreach ($goods_array as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                if ($v['is_book'] == '1') {
                    if ($v['book_down_time'] > time()) {
                        // 更新价格
                        $this->editGoodsById(array('goods_sale_price' => ($v['book_down_payment'] + $v['book_final_payment']), 'goods_sale_type' => 0), $k);
                    } else {
            ...
            // 查询抢购是否进行
            $robbuy = $model_robbuy->getRobbuyOnlineInfo(array('goods_commonid' => $key));
            if (!empty($robbuy)) {
                // 更新价格
                $this->editGoods(array('goods_sale_price' => $robbuy['robbuy_price'], 'goods_sale_type' => 1), array('goods_commonid' => $key, 'is_book' => 0));
                continue;
            }
            foreach ($val as $k => $v) {
                if ($v['is_book'] == '1') {
                ...
                if (!empty($xianshigoods)) {
                    // 更新价格
                    $this->editGoodsById(array('goods_sale_price' => $xianshigoods['xianshi_price'], 'goods_sale_type' => 2), $k);
                    continue;
                }

                // 没有促销使用原价
                $this->editGoodsById(array('goods_sale_price' => array('exp', 'goods_price'), 'goods_sale_type' => 0), $k);
            }
        }
        return true;
    }
    ...
    public function getGoodsOnlineInfoForShare($goods_id) {
        $goods_content = $this->getGoodsOnlineInfoAndSaleById($goods_id);
        if (empty($goods_content)) {
            return array();
        }
        //抢购
        if (isset($goods_content['robbuy_info'])) {
            $goods_content['sale_type'] = '抢购';
            $goods_content['sale_price'] = $goods_content['robbuy_info']['robbuy_price'];
        }

        if (isset($goods_content['xianshi_info'])) {
            $goods_content['sale_type'] = '限时折扣';
            $goods_content['sale_price'] = $goods_content['xianshi_info']['xianshi_price'];
        }
     ...
     * @return array
     */
    public function getGoodsOnlineInfoAndSaleById($goods_id) {
        $goods_content = $this->getGoodsInfoAndSaleById($goods_id);
        if (empty($goods_content) || $goods_content['goods_state'] != self::STATE1 || $goods_content['goods_verify'] != self::VERIFY1) {
            return array();
        }
        return $goods_content;
    }
     ...
     * @return array
     */
    public function getGoodsInfoAndSaleById($goods_id) {
        $goods_content = $this->getGoodsInfoByID($goods_id);
        if (empty($goods_content)) {
            return array();
        }
        // 手机专享
        if (C('sale_allow') && APP_ID == 'mobile') {
            $goods_content['sole_info'] = Model('p_sole')->getSoleGoodsInfoOpenByGoodsID($goods_content['goods_id']);
        }
        
        //抢购
        if (C('robbuy_allow') && empty($goods_content['sole_info'])) {
          	
            $robbuy_info = Model('robbuy')->getRobbuyInfoByGoodsCommonID($goods_content['goods_commonid']);
          	if(!empty($robbuy_info)){
               $goods_content['robbuy_info'] = $robbuy_info;
            }
        }
        
        //限时折扣
        if (C('sale_allow') && empty($goods_content['sole_info']) && empty($goods_content['robbuy_info'])) {
            $xianshi_info = Model('p_xianshi_goods')->getXianshiGoodsInfoByGoodsID($goods_content['goods_id']);
          	if(!empty($xianshi_info)){
               $goods_content['xianshi_info'] = $xianshi_info;
            }
        }
        
        // 加价购
        if (C('sale_allow')) {
            $jjg_info = Model('p_cou')->getCachedRelationalCouDetailBySingleSku($goods_content['goods_id']);
          	if(!empty($jjg_info)){
               $goods_content['jjg_info'] = $jjg_info;
            }
        }
     ...
     * @return array
     */
    public function getGoodsOnlineListAndSaleByIdArray($goodsid_array) {
        if (empty($goodsid_array) || !is_array($goodsid_array)) return array();

        $goods_list = array();
        foreach ($goodsid_array as $goods_id) {
            $goods_content = $this->getGoodsOnlineInfoAndSaleById($goods_id);
            if (!empty($goods_content)) $goods_list[] = $goods_content;
        }
    ...
    public function getVirtualGoodsOnlineInfoByID($goods_id) {
        $goods_content = $this->getGoodsInfoByID($goods_id,'*');
        return $goods_content['is_virtual'] == 1 && $goods_content['virtual_indate'] >= TIMESTAMP ? $goods_content : array();
    }
    ...
    public function getGoodsOnlineInfoByID($goods_id, $field = '*') {
        ...
        $goods_content = $this->getGoodsInfoByID($goods_id,trim($field,','));
        if ($goods_content['goods_state'] != self::STATE1 || $goods_content['goods_verify'] != self::VERIFY1) {
            $goods_content = array();
        }
        return $goods_content;
    }
    ...
    public function getGoodsInfoByID($goods_id, $fields = '*') {
        $goods_content = $this->_rGoodsCache($goods_id, $fields);
        if (empty($goods_content)) {
            $goods_content = $this->getGoodsInfo(array('goods_id'=>$goods_id));
            $this->_wGoodsCache($goods_id, $goods_content);
        }
        return $goods_content;
    }
     ...
     * @return boolean
     */
    private function _wGoodsCache($goods_id, $goods_content) {
        return wcache($goods_id, $goods_content, 'goods');
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $result1 = $this->getGoodsInfoAndSaleById($goods_id);
        ...
        $goods_content['spec_value'] = unserialize($goods_content['spec_value']);
        $goods_content['spec_name'] = unserialize($goods_content['spec_name']);
        $goods_content['goods_spec'] = unserialize($goods_content['goods_spec']);
        $goods_content['goods_attr'] = unserialize($goods_content['goods_attr']);
        $goods_content['goods_custom'] = unserialize($goods_content['goods_custom']);
        ...
        //视频 by sh o pw t . c o m
        if($goods_content['goods_video']){
            $video_path = goodsVideoPath($goods_content['goods_video'], $goods_content['store_id']);//shopwt
        }
        // 查询所有规格商品
        $spec_array = $this->getGoodsSpecListByCommonId($goods_content['goods_commonid']);
        ...
        // 商品多图
        $image_more = $this->getGoodsImageByKey($goods_content['goods_commonid'] . '|' . $goods_content['color_id']);
        $image_list = array();
        $goods_image = array();
	...
	//放大镜 shopwt 
                $image_list[] = array( '_small' => cthumb($val['goods_image'] , 60 , $goods_content['store_id']) , '_mid' => cthumb($val['goods_image'] , 360 , $goods_content['store_id']) , '_big' => cthumb($val['goods_image'] , 1280 , $goods_content['store_id']) );
                $goods_image[] = "{ title : '', levelA : '".cthumb($val['goods_image'], 60, $goods_content['store_id'])."', levelB : '".cthumb($val['goods_image'], 360, $goods_content['store_id'])."', levelC : '".cthumb($val['goods_image'], 360, $goods_content['store_id'])."', levelD : '".cthumb($val['goods_image'], 1280, $goods_content['store_id'])."'}";
                $goods_image_mobile[] = cthumb($val['goods_image'], 1280, $goods_content['store_id']);
            }
        } else {
            $image_list[] = array( '_small' => thumb($goods_content,60) , '_mid' => thumb($goods_content , 360) ,'_big' => thumb($goods_content , 1280) );
            $goods_image[] = "{ title : '', levelA : '".thumb($goods_content, 60)."', levelB : '".thumb($goods_content, 360)."', levelC : '".thumb($goods_content, 360)."', levelD : '".thumb($goods_content, 1280)."'}";
            $goods_image_mobile[] = thumb($goods_content, 1280);
        }
       

        if ($goods_content['is_book'] != '1') {
            //限时折扣
            if (!empty($goods_content['xianshi_info'])) {
                $goods_content['sale_type'] = 'xianshi';
                $goods_content['title'] = $goods_content['xianshi_info']['xianshi_title'];
                $goods_content['remark'] = $goods_content['xianshi_info']['xianshi_title'];
                $goods_content['sale_price'] = $goods_content['xianshi_info']['xianshi_price'];
                $goods_content['down_price'] = wtPriceFormat($goods_content['goods_price'] - $goods_content['xianshi_info']['xianshi_price']);
                $goods_content['lower_limit'] = $goods_content['xianshi_info']['lower_limit'];
                $goods_content['explain'] = $goods_content['xianshi_info']['xianshi_explain'];
				$goods_content['xs_time'] = $goods_content['xianshi_info']['end_time']-TIMESTAMP;
                unset($goods_content['xianshi_info']);
            }
            //抢购
            if (!empty($goods_content['robbuy_info'])) {
                $goods_content['sale_type'] = 'robbuy';
                $goods_content['title'] = '抢购';
                $goods_content['remark'] = $goods_content['robbuy_info']['remark'];
                $goods_content['sale_price'] = $goods_content['robbuy_info']['robbuy_price'];
                $goods_content['down_price'] = wtPriceFormat($goods_content['goods_price'] - $goods_content['robbuy_info']['robbuy_price']);
                $goods_content['upper_limit'] = $goods_content['robbuy_info']['upper_limit'];
				$goods_content['end_time'] = $goods_content['robbuy_info']['end_time']-TIMESTAMP;
                unset($goods_content['robbuy_info']);
            }
            // 手机专享
            if (!empty($goods_content['sole_info'])) {
                $goods_content['sale_type'] = 'sole';
                $goods_content['title'] = '手机专享';
                $goods_content['sale_price'] = $goods_content['sole_info']['sole_price'];
                unset($goods_content['sole_info']);
            }
            // 加价购
            if (!empty($goods_content['jjg_info'])) {
                $jjgFirstLevel = $goods_content['jjg_info']['firstLevel'];
                if ($jjgFirstLevel && $jjgFirstLevel['mincost'] > 0) {
                    $goods_content['jjg_explain'] = sprintf(
            ...
            // 验证是否允许送赠品
            if ($this->checkGoodsIfAllowGift($goods_content)) {
                $gift_array = Model('goods_gift')->getGoodsGiftListByGoodsId($goods_id);
                if (!empty($gift_array)) {
                    $goods_content['have_gift'] = 'gift';
                }
            }

            //满即送
            $mansong_info = ($goods_content['is_virtual'] == 1) ? array() : Model('p_mansong')->getMansongInfoByStoreID($goods_content['store_id']);
        }

        // 加入购物车按钮
        $goods_content['cart'] = 1;
        //虚拟、F码、预售不显示加入购物车，不显示门店
        if ($goods_content['is_virtual'] == 1 || $goods_content['is_fcode'] == 1 || $goods_content['is_presell'] == 1 || $goods_content['is_book'] == 1) {
            $goods_content['is_chain'] = 0;
            $goods_content['cart'] = 0;
        }
        
        // 立即购买按钮
        $goods_content['buynow'] = 1;
        // 加价购不显示立即购买按钮
        if (!empty($goods_content['jjg_info'])) {
            $goods_content['buynow'] = 0;
        }

        // 立即购买文字显示
        $goods_content['buynow_text'] = '立即购买';
        if ($goods_content['is_presell'] == 1) {
            $goods_content['buynow_text'] = '预售购买';
        } elseif ($goods_content['is_book'] == 1) {
            $goods_content['buynow_text'] = '支付定金';
        } elseif ($goods_content['is_fcode'] == 1) {
            $goods_content['buynow_text'] = 'F码购买';
        }
        ...
        // 商品受关注次数加1
        $goods_content['goods_click'] = intval($goods_content['goods_click']) + 1;
        if (C('cache_open')) {
            $this->_wGoodsCache($goods_id, array('goods_click' => $goods_content['goods_click']));
            wcache('updateRedisDate', array($goods_id => $goods_content['goods_click']), 'goodsClick');
        } else {
            $this->editGoodsById(array('goods_click' => array('exp', 'goods_click + 1')), $goods_id);
        }
        $this->editGoodsCommonById(array('click_count' => array('exp', 'click_count + 1')),$goods_content['goods_commonid']);
        $result = array();
        $result['goods_content'] = $goods_content;
        ...
        $result['video_path'] = $video_path;//shopw t .co m
