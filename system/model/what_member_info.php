...
class what_member_infoModel extends Model{

    public function __construct(){
        parent::__construct('what_member_info');
    }
    ...
    public function getListWithUserInfo($condition,$page=null,$order='',$field='*',$limit=''){
        $on = 'what_member_info.member_id = member.member_id';
        $result = $this->table('what_member_info,member')->field($field)->join('left')->on($on)->where($condition)->page($page)->order($order)->limit($limit)->select();
        return $result;
    }
    ...
    private function updateMemberInfo($member_id, $column, $type, $step = 1){
        ...
        $what_member_info = self::getOne($param);
        $new_count = 0;
        if(empty($what_member_info)) {
            ...
            if($type != '-') {
                ...
                $new_count = $what_member_info[$column] + $step;
            } else {
                if($what_member_info[$column] > $step) {
                    $update[$column] = array('exp',$column.'-'.$step);
                    $new_count = $what_member_info[$column] - $step;
                } else {
                    $update[$column] = 0;
