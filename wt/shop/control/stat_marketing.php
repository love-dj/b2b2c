...
class stat_marketingControl extends SystemControl{
    private $links = array(
        array('url'=>'w=stat_marketing&t=sale','lang'=>'stat_sale'),
        array('url'=>'w=stat_marketing&t=group','lang'=>'stat_group'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['t']) || in_array($this->search_arr['t'],array('sale','group','get_groupgoods_xml','groupgoods','get_grouplist_xml'))){
    ...
    public function saleWt(){
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('stat.marketing.sale');
    }
    /**
     * 促销销售趋势分析
     */
    public function saletrendWt(){
        //优惠类型数组
        $goodstype_arr = array(2=>'抢购',3=>'限时折扣',4=>'优惠套装');
     ...
     * 输出抢购统计XML数据
     */
    public function get_grouplist_xmlWt(){
        $model = Model('robbuy');
        $where = array();
        ...
        $keyword_type = array('robbuy_name','goods_name');
        if (trim($_POST['query']) != '' && in_array($_POST['qtype'],$keyword_type)) {
            $type = $_POST['qtype'];
            $where[$type] = array('like','%'.$_POST['query'].'%');
        }
        $order_type = array('start_time','end_time','goods_price','robbuy_price','views');
        ...
        $grouplist_tmp = $model->getRobbuyExtendList($where,$page,$orderby);
        $grouplist = array();
        if ($grouplist_tmp){
            foreach ((array)$grouplist_tmp as $k => $v){
                ...
                $v['goodsamount'] = wtPriceFormat(0);
                $grouplist[$v['robbuy_id']] = $v;
            }
            //查询抢购的订单
            $where = array();
            $where['order_isvalid'] = 1;//计入统计的有效订单
            $where['goods_type'] = 2;//抢购
            $where['sales_id'] = array('in',array_keys($grouplist));
            $field = 'sales_id,SUM(goods_num) as goodsnum,COUNT(DISTINCT order_id) as ordernum,SUM(goods_pay_price) as goodsamount';
            $order_list = Model('stat')->statByStatordergoods($where, $field, 0, 0, '', 'sales_id');
            foreach ((array)$order_list as $k => $v){
                $grouplist[$v['sales_id']]['goodsnum'] = $v['goodsnum'];
                $grouplist[$v['sales_id']]['ordernum'] = $v['ordernum'];
                if (intval($grouplist['views']) > 0){
                    $grouplist[$v['sales_id']]['orderrate'] = round(($v['ordernum']/$grouplist[$v['sales_id']]['views'])*100,2).'%';
                }
                $grouplist[$v['sales_id']]['goodsamount'] = $v['goodsamount'];
            }
        }
        $statlist = array();
        if (!empty($grouplist) && is_array($grouplist)){
            $fields_array = array('robbuy_name','start_time','end_time','robbuy_state_text','goods_name','goods_price',
                'robbuy_rebate','robbuy_price','views','ordernum','goodsnum','goodsamount','orderrate');
            foreach ($grouplist as $k => $v){
                $out_array = getFlexigridArray(array(),$fields_array,$v);
                $out_array['robbuy_name'] = '<a href="'.urlShop('robbuy', 'robbuy_detail', array('group_id' => $v['robbuy_id'])).'" target="_blank">'.$v['robbuy_name'].'</a>';
                ...
                $out_array['goods_price'] = wtPriceFormat($v['goods_price']);
                $out_array['robbuy_price'] = wtPriceFormat($v['robbuy_price']);
                $out_array['goodsamount'] = wtPriceFormat($v['goodsamount']);
                $statlist[] = $out_array;
            }
        }
        $data = array();
     ...
     * 输出抢购商品统计XML数据
     */
    public function get_groupgoods_xmlWt(){
        $model = Model('stat');
        $where = array();
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
