...
<?php } else { ?>
  ...
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_combo', 'combo_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-mint" href="javascript:void(0);" wttype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="wtbtn wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_combo', 'combo_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['wt_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['combo_quota']['cq_endtime']);?></strong>
<?php }?>
  <ul>
...
<table class="wtsc-default-table">
  ...
  <tbody wttype="choose_goods_list">
    <tr wttype="tr_no_sale" style="display:none;">
      <td colspan="20" class="norecord"><div class="no-sale"><i class="zw"></i><span>推荐组合商品列表暂无内容，请选择添加推荐组合商品。</span></div></td>
    </tr>
	...
	<td class="nscs-table-handle">
        <span>
          <a class="btn-bluejeans" href="<?php echo urlShop('store_sale_combo', 'choosed_goods', array('gid' => $val['goods_id']));?>">
            <i class="icon-edit"></i>
            <p>编辑</p>
          </a>
        </span>
	   <span>
            <a class="btn-grapefruit" href='javascript:void(0);' wttype="del_choosed" data-gid="<?php echo $val['goods_id'];?>">
...
$(function(){
    ...
    $('input[wttype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlShop('store_sale_combo', 'combo_select_goods');?>';
    ...
    $('#mainContent').on('click', 'a[wttype="a_choose_goods"]', function(){
        _gid = $(this).attr('data-gid');
        CUR_DIALOG =ajax_form('choose_goods', '设置推荐组合商品', '<?php echo urlShop('store_sale_combo', 'choosed_goods');?>&gid='+_gid, 1000);
    });

    // 删除商品
    $('a[wttype="del_choosed"]').click(function(){
        $this = $(this);
        _url = '<?php echo urlShop('store_sale_combo', 'del_choosed_goods');?>';
        _gid = $this.attr('data-gid');
