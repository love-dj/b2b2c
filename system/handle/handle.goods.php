...
class goodsHandle {
    ...
    public function saveLibGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc){
		...
		$common_array['areaid_3'] = 0;
		$common_array['is_bat']  = 0;
    ...
    public function updateGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc) {
        ...
        if ($common_array['is_virtual'] == 1) {
            ...
            QueueClient::push('clearSpecialGoodsSale', array('goods_commonid' => $common_id, 'goodsid_array' => $goodsid_array));
        } else {
            // 更新商品促销价格
            QueueClient::push('updateGoodsSalePriceByGoodsCommonId', $common_id);
        }
    ...
    private function _validParam($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array (
                "input" => $param["g_price"],
		"input" => $param["g_price_2"], //v6.5 批发价验证
                "require" => "true",
                "validator" => "Double",
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state) {
		...
		//是否设置商品阶梯价格 v6.5批发价
		$common_array['is_bat']            = intval($param['is_bat']);
        ...
        $common_array['is_independent_bonus'] = intval($param['is_independent_bonus']);
        $common_array['level_commission']   = $param['level_commission'];
        $common_array['is_own_shop']        = in_array($store_id, Model('store')->getOwnShopIds()) ? 1 : 0;
        // 选择商品视频后，添加商品视频链接
        $common_array['goods_video'] = $param['goods_video'];
    ...
    private function _addGoods($param, $common_id, $common_array) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
				...
				$goods['is_bat']       =  $common_array['is_bat'];//提取商品阶梯价格状态 v6.5
                $goods['goods_sale_price']=$value['price'];
                $goods['goods_marketprice'] = $value['marketprice'] == 0 ? $common_array['goods_marketprice'] : $value['marketprice'];
                $goods['goods_serial']      = (!empty($value['sku'])&&trim($value['sku'])!='')?trim($value['sku']):$common_array['goods_serial'];
				...
				//插入阶梯价格 v6.5
				$this->addUpdateInsertStepPriceSpec($value['sp_value'],$goods_id,$goods['is_bat']);		
			...
			$goods['is_bat']       		= $common_array['is_bat'];//提取商品阶梯价格状态 v6.5
            $goods['goods_sale_price']=$common_array['goods_price'];
            ...
            $goodsid_array[] = $goods_id;
		   //没有规格的阶梯价格 v6.5
			$this->addUpdateInsertStepPriceNoSpec($goods_id,$goods['is_bat']);
        }
    ...
    private function _editGoods($param, $common_id, $common_array, $store_id) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                $goods = $this->_initGoodsByCommonGoods($common_id, $common_array);
                $goods_content = $model_goods->getGoodsInfo(array('goods_id' => $value['goods_id'], 'goods_commonid' => $common_id, 'store_id' => $store_id), 'goods_id');
                if (!empty($goods_content)) {
                    $goods_id = $goods_content['goods_id'];
                    $goods['goods_name']        = $common_array['goods_name'] . ' ' . implode(' ', $value['sp_value']);
                    $goods['goods_price']       = $value['price'];
		    		$goods['is_bat']            = $common_array['is_bat'];//是否设置商品阶梯价格 v6.5
                    $goods['goods_marketprice'] = $value['marketprice'] == 0 ? $common_array['goods_marketprice'] : $value['marketprice'];
                    $goods['goods_serial']      = (!empty($value['sku'])&&trim($value['sku'])!='')?trim($value['sku']):$common_array['goods_serial'];
                    $goods['goods_storage_alarm']= intval($value['alarm']);
                    $goods['goods_spec']        = serialize($value['sp_value']);
		    ...
		    //修改有规格的阶梯价格 v6.3
		    $rsVal =$rsVal."u".$this->editUpdateInsertStepPriceSpec($value['sp_value'],$goods_id,$goods['is_bat']);
		    ...
		    //新增有规格的阶梯价格 v6.5
		    $rsVal =$rsVal."i".$this->editUpdateInsertStepPriceSpec($value['sp_value'],$goods_id,$goods['is_bat']);
                }
            ...
            $goods = $this->_initGoodsByCommonGoods($common_id, $common_array);
            $goods_content = $model_goods->getGoodsInfo(array($goods_spec_field_name => serialize(null), 'goods_commonid' => $common_id, 'store_id' => $store_id), 'goods_id');
            if (!empty($goods_content)) {
                $goods_id = $goods_content['goods_id'];
                $goods['goods_name']        = $common_array['goods_name'];
                $goods['goods_price']       = $common_array['goods_price']; 
                $goods['goods_costprice']   = $common_array['goods_costprice'];
				$goods['is_bat']            = $common_array['is_bat'];//是否设置商品阶梯价格 v6.5
                $goods['goods_marketprice'] = $common_array['goods_marketprice'];
                $goods['goods_serial']      = $common_array['goods_serial'];
                $goods['goods_storage_alarm']= $common_array['goods_storage_alarm'];
                ...
                $goods['goods_barcode']     = $common_array['goods_barcode'];
                $goods['color_id']          = 0;
                $goods['is_independent_bonus']     = $common_array['is_independent_bonus'];
                $goods['level_commission']     = $common_array['level_commission'];
                if ($common_array['is_virtual'] == 1) {
                    $goods['have_gift']    = 0;
                    Model('goods_gift')->delGoodsGift(array('goods_id' => $goods_id));
                    Model('goods_fcode')->delGoodsFCode(array('goods_id' => $goods_id));
                ...
                $model_goods->editGoodsById($goods, $goods_id);
				//无规格阶梯价格 v6.5
				$this->editUpdateInsertStepPriceNoSpec($goods_id,$goods['is_bat']);
    ...
    private function _recordLog($content = '', $seller_id, $seller_name, $store_id, $state = 1) {
        ...
        $log['log_url'] = 'goodsHandle&saveGoods';
        $log['log_state'] = $state;
    ...
    /**
     * 处理编辑没有规格商品阶梯价格 online edit
     */
	public function editUpdateInsertStepPriceNoSpec($goods_id,$isBat)
	{
		$model_goods = Model('goods');
		$re = $model_goods->delGoodsStepPrice(array('common_id' =>$goods_id));
		if($re)
		{
			//判断是否设置阶梯价格,无规格商品发布阶梯价格
			if($isBat==1) {
				//插入阶梯价格
				 $step_price_array = array();
				 $step_price_array['common_id'] = $goods_id;
				 for($s=1;$s<=intval($_POST['noSpecsnum']);$s++)
				 {
					 $h = $s + 1;
					 if(isset($_POST['g_step_price_'.$s])&&!empty($_POST['g_step_price_'.$s]))
					 {
						 $step_price_array['step_price']    = $_POST['g_step_price_'.$s];
						 $step_price_array['step_l_num']    = intval($_POST['g_step_num_'.$s]);
						 if(isset($_POST['g_step_num_'.$h]))
						 {
							//取得范围最大数量
							$step_price_array['step_h_num'] = intval($_POST['g_step_num_'.$h]);
						 }
						 else
						 {
							//如果是最后一个阶梯价格，则最高数量设置一个大数
							$step_price_array['step_h_num'] = 999999999;
						 }
						 $step_price_array['step_add_date'] = TIMESTAMP;
						 $step_id = $model_goods->addGoodsStepPrice($step_price_array);
					 }
				 }
			}
		}
    ...
    /**
     * 处理编辑有不同规格阶梯价格 online edit
     */
	public function editUpdateInsertStepPriceSpec($spValue,$goods_id,$isBat)
	{
		 $model_goods = Model('goods');
		 $spec_key="" ;
		 ksort($spValue);
		 foreach ($spValue as $key => $v)
		 {
			 $spec_key =$spec_key.$key; 
		 }
		$specFile="i_".$spec_key."";
		$linc=0;
		if(file_exists(BASE_UPLOAD_PATH."/spec/".$specFile))
		{	
			//清除之前的阶梯价格
			$re = $model_goods->delGoodsStepPrice(array('common_id' =>$goods_id));
			if($re)
			{
				...
				if($isBat==2)
				{
					//插入阶梯价格
					 ...
					 //读取临时保存的规格阶梯价格值
					$fileContent = @fopen(BASE_UPLOAD_PATH."/spec/".$specFile, "r");
					$spec=array();
					$specValue = array();
					$tmpValue = array();
					$i=0;
					//处理阶梯价格数据
					//$tmpfileContent = fgets($fileContent);
					while(!feof($fileContent))
					{
					 $spec[$i]= fgets($fileContent);//读取行数据
					 $specValue = explode("_",$spec[$i]);
					 $tmpValue[$specValue[2]] = $spec[$i];
					 $i++;
					}
					$linc =$i;
					fclose($fileContent);
					for($s=1;$s<=intval(count($tmpValue));$s++)
					 {
						 $h = $s + 1;
						 $_tmpValue = array();
						 if(!empty($tmpValue[$s]))
						 {
							 $_tmpValue = explode("_",$tmpValue[$s]);
							 $step_price_array['step_price']    = $_tmpValue[4];
							 $step_price_array['step_l_num']    = intval($_tmpValue[3]);
							 if(isset($tmpValue[$h]))
							 {
								//取得范围最大数量
								 $_tmpValue_h = array();
								 $_tmpValue_h = explode("_",$tmpValue[$h]);
								 $step_price_array['step_h_num'] = intval($_tmpValue_h[3]);
							 }
							 else
							 {
								//如果是最后一个阶梯价格，则最高数量设置一个大数
							 ...
							 $step_price_array['step_add_date'] = TIMESTAMP;
							 //showDialog(L('error') . $_tmpValue[3], urlShop('store_goods_online', 'index'));
							 $step_id = $model_goods->addGoodsStepPrice($step_price_array);
						 }
					 }
					 //删除临时文件
...
/**
     * 处理编辑没有规格商品阶梯价格 add
     */
	public function addUpdateInsertStepPriceNoSpec($goods_id,$isBat)
	{
		$model_goods = Model('goods');
		$re = $model_goods->delGoodsStepPrice(array('common_id' =>$goods_id));
		if($re)
     ...
     * 处理编辑有不同规格阶梯价格
     */
	public function addUpdateInsertStepPriceSpec($spValue,$goods_id,$isBat)
	{
		 $model_goods = Model('goods');
		 $spec_key="" ;
		 ksort($spValue);
