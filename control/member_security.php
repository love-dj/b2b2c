...
class member_securityControl extends BaseSellerControl {
     ...
     * 绑定邮箱 - 发送邮件
     */
    public function send_bind_emailWt() {
        ...
        $verify_url = urlShop('member_security', 'bind_email', array('uid' => $uid, 'hash' => md5($seed)));
	    ...
	    showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！如果您始终未收到邮件，请于60秒后重新发送','index.php?w=member_security&t=index','succ','',10);
    ...
    public function authWt() {
        ...
        if (chksubmit(false,true)) {
            if (!in_array($_POST['type'],array('modify_pwd','modify_mobile','modify_email','modify_paypwd','pd_cash'))) {
                redirect('index.php?w=member_security&t=index');
            }
            $member_common_info = $model_member->getMemberCommonInfo(array('member_id'=>$_SESSION['member_id']));
            ...
            $member_info['member_mobile_bind'] == '0') {
                showMessage('请先绑定邮箱或手机','index.php?w=member_security&t=index','html','error');
            }
     ...
     * 统一发送身份验证码
     */
    public function send_auth_codeWt() {
       ...
       if ($_GET['type'] == 'email') {
            ...
            $sms = new Sms();
            $result = $sms->send($member_info["member_mobile"],$message,$param);
        }
        if ($result) {
            $data = array();
    ...
    public function modify_pwdWt() {
        ...
        if (TIMESTAMP - $_SESSION['auth_modify_pwd'] > 1800) {
            showDialog('操作超时，请重新获得验证码','index.php?w=member_security&t=auth&type=modify_pwd','html','error');
        }
        ...
        showDialog($message,'index.php?w=member_security&t=index',$update ? 'succ' : 'error');
     ...
     * 设置支付密码
     */
    public function modify_paypwdWt() {
        $model_member = Model('member');

        //身份验证后，需要在30分钟内完成修改密码操作
        if (TIMESTAMP - $_SESSION['auth_modify_paypwd'] > 1800) {
            showMessage('操作超时，请重新获得验证码','index.php?w=member_security&t=auth&type=modify_paypwd','html','error');
        }
     ...
     * 绑定手机
     */
    public function modify_mobileWt() {
        $model_member = Model('member');
        ...
        if (chksubmit()) {
            ...
            showDialog('手机号绑定成功','index.php?w=member_security&t=index','succ');
        }
    }
     ...
     * 修改手机号 - 发送验证码
     */
    public function send_modify_mobileWt() {
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
		...
		$param['apicodeid'] = $tpl_info['apicodeid'];
        $sms = new Sms();
        $result = $sms->send($_GET["mobile"],$message,$param);
        if ($result) {
            $data = array();
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'modify_pwd':
                $menu_array = array(
                array('menu_key'=>'modify_pwd','menu_name'=>'修改登录密码','menu_url'=>'index.php?w=member_security&t=auth&type=modify_pwd'),
                );
                break;
            case 'modify_email':
                $menu_array = array(
                array('menu_key'=>'modify_email', 'menu_name'=>'邮箱验证','menu_url'=>'index.php?w=member_security&t=auth&type=modify_email'),
                );
                break;
            case 'modify_mobile':
                $menu_array = array(
                array('menu_key'=>'modify_mobile','menu_name'=>'手机验证','menu_url'=>'index.php?w=member_security&t=auth&type=modify_mobile'),
                );
                break;
            case 'modify_paypwd':
                $menu_array = array(
                array('menu_key'=>'modify_paypwd','menu_name'=>'设置支付密码','menu_url'=>'index.php?w=member_security&t=auth&type=modify_paypwd'),
                );
                break;
            case 'pd_cash':
                $menu_array = array(
               
                array('menu_key'=>'pd_cash','menu_name'=>'提现申请','menu_url'=>'index.php?w=member_security&t=auth&type=pd_cash'),
                );
                break;
        }
