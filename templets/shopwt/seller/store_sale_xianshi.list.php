...
<div class="tabmenu">
  ...
  <a class="wtbtn wtbtn-mint" href="<?php echo urlShop('store_sale_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a>
  <?php } else { ?>
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <a class="wtbtn wtbtn-mint" style="right:100px" href="<?php echo urlShop('store_sale_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a> <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
  <?php } ?>
  <?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
...
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['wt_colon'];?></strong><strong style="color:#F00;"><?php echo date('Y-m-d H:i:s', $output['current_xianshi_quota']['end_time']);?></strong>
  <?php } else { ?>
...
<form method="get">
  <table class="search-form">
    <input type="hidden" name="w" value="store_sale_xianshi" />
    <input type="hidden" name="t" value="xianshi_list" />
    <tr>
      <td>&nbsp;</td>
      <th>状态</th>
  ...
  <tbody id="xianshi_list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr"><?php if($val['editable']) { ?>
        <span> <a href="index.php?w=store_sale_xianshi&t=xianshi_edit&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-bluejeans"> <i class="icon-edit"></i>
        <p><?php echo $lang['wt_edit'];?></p>
        </a> </span>
        <?php } ?>
        <span> <a href="index.php?w=store_sale_xianshi&t=xianshi_manage&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-mint"> <i class="icon-cog"></i>
        <p><?php echo $lang['wt_manage'];?></p>
        </a> </span> <span> <a href="javascript:;" wttype="btn_del_xianshi" data-xianshi-id=<?php echo $val['xianshi_id'];?> class="btn-grapefruit"> <i class="icon-trash"></i>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[wttype="btn_del_xianshi"]').on('click', function() {
            if(confirm('<?php echo $lang['wt_ensure_del'];?>')) {
                var action = "<?php echo urlShop('store_sale_xianshi', 'xianshi_del');?>";
