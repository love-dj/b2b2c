...
class web_channelControl extends SystemControl{
    private $links = array(
        array('url'=>'w=web_channel&t=web_channel','text'=>'频道列表'),
        array('url'=>'w=web_channel&t=floor_list','text'=>'模块列表')
    );
     ...
     * 输出频道列表XML数据
     */
    public function get_channel_xmlWt() {
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['channel_show'] == 1) {
                    $out_array['channel_show'] = '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>';
                } else {
                ...
                $operation .= '<li><a href="index.php?w=web_channel&t=edit_channel&channel_id='.$v['channel_id'].'">基本设置</a></li>';
                $operation .= '<li><a href="index.php?w=web_channel&t=set_channel&channel_id='.$v['channel_id'].'">编辑模块</a></li>';
                $operation .= '</ul></span>';
     ...
     * 输出模块列表XML数据
     */
    public function get_floor_xmlWt() {
        $model_channel = Model('web_channel');
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['web_show'] == 1) {
                    $out_array['web_show'] = '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>';
                } else {
                    $out_array['web_show'] = '<span class="no"><i class="fa fa-ban"></i>否</span>';
                }
                $operation = '';
                $operation .= '<a class="btn purple" href="index.php?w=web_channel&t=edit_floor&web_id='.$v['web_id'].'"><i class="fa fa-cog"></i>设置</a>';
                $operation .= '<a class="btn orange" href="index.php?w=web_channel&t=edit_'.$v['web_page'].'&web_id='.$v['web_id'].'"><i class="fa fa-steam"></i>模块设计</a>';
                $out_array['operation'] = $operation;
     ...
     * 中部模块数据
     */
    public function get_channel_flWt() {
        $model_channel = Model('web_channel');
        $condition = array();
        if (!empty($_GET['name'])) {
    ...
    public function add_channelWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=web_channel&t=web_channel');
            } else {
    ...
    public function add_floorWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=web_channel&t=floor_list');
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
     ...
     * 编辑频道
     */
    public function edit_channelWt() {
        $model_channel = Model('web_channel');
        $condition = array();
     ...
     * 编辑模块
     */
    public function edit_floorWt() {
        $model_page_config = Model('page_config');
        $condition = array();
        $condition['web_id'] = intval($_GET['web_id']);
        $web_list = $model_page_config->getWebList($condition);
        $web = $web_list[0];
        if (chksubmit()) {
            $web_array = array();
            $web_array['web_name'] = $_POST['web_name'];
            ...
            $state = $model_page_config->updateWeb($condition, $web_array);
            if ($state) {
                $model_page_config->updateWebHtml($condition['web_id']);//更新前台显示的html内容
     ...
     * 编辑顶部模块
     */
    public function edit_channel_tpWt() {
        $model_page_config = Model('page_config');
        ...
        $floor_list = $model_page_config->getWebList(array('web_id'=>"$web_id"));
     ...
     * 编辑中部模块
     */
    public function edit_channel_flWt() {
        $model_page_config = Model('page_config');
        $web_id = intval($_GET["web_id"]);
        $code_list = $model_page_config->getCodeList(array('web_id'=>"$web_id"));
        if(is_array($code_list) && !empty($code_list)) {
     ...
     * 商品分类
     */
    public function get_category_listWt() {
        $model_class = Model('goods_class');
     ...
     * 检查分类是否已经绑定到频道
     */
    public function check_goods_classWt() {
        $model_channel = Model('web_channel');
     ...
     * 商品分类图片
     */
    public function upload_gc_picWt() {
     ...
     * 保存焦点区切换大图
     */
    public function channel_slideWt() {
        $code_id = intval($_POST['code_id']);
        $web_id = intval($_POST['web_id']);
        $model_page_config = Model('page_config');
        $code = $model_page_config->getCodeRow($code_id,$web_id);
     ...
     * 保存焦点区切换小图
     */
    public function channel_showWt() {
        $code_id = intval($_POST['code_id']);
        $web_id = intval($_POST['web_id']);
        $model_page_config = Model('page_config');
        $code = $model_page_config->getCodeRow($code_id,$web_id);
