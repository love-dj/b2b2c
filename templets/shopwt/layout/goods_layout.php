...
<![endif]-->
<!--[if IE 6]>
<script src="<?php echo STATIC_SITE_URL;?>/js/IE6_PNG.js"></script>
<script>
...
<div class="header-box">
  <header class="wt-head wrapper">
    ...
    <div class="search-m" id="search">
        <form class="search-form" method="get" action="<?php echo BASE_SITE_URL.'/';?>index.php" name="formSearch" id="formSearch">
          <input name="w" id="search_wt" value="search" type="hidden" />
          <input type="hidden" name="keyword" value="<?php echo $_GET['inkeyword'];?>" />
          <input name="inkeyword" id="inkeyword" type="text" class="input-text" value="<?php echo $_GET['inkeyword'];?>" x-webkit-speech lang="zh-CN" onwebkitspeechchange="foo()" x-webkit-grammar="builtin:search" placeholder="<?php echo $lang['wt_what_goods'];?>" />
...
<div class="wtsl-nav">
    ...
    <div id="nav" class="wts-nav">
	  ...
	  <?php if(!empty($output['goods_class_list'])){?>
		...
		<?php }?>
        <li class="<?php if($output['page'] == 'index'){?>active<?php }else{?>normal<?php }?>"><a href="<?php echo urlShop('show_store', 'index', array('store_id'=>$output['store_info']['store_id']));?>"><span><?php echo $lang['wt_store_index'];?><i></i></span></a></li>
...
$("img[rel='lazy']").scrollLoading();
		...
		$('<input type="hidden" value="<?php echo $output['store_info']['store_id'];?>" name="store_id" /> <input type="hidden" name="t" value="goods_all" />').appendTo("#formSearch");
		$('#formSearch').submit();
	});
