...
class consultingControl extends SystemControl{
    private $links = array(
            array('url'=>'w=consulting&t=consulting','text'=>'咨询管理'),
            array('url'=>'w=consulting&t=type_list','text'=>'类型设置'),
            array('url'=>'w=consulting&t=setting','text'=>'头部文字设置')
    );
    public function __construct(){
        parent::__construct();
        Language::read('consulting');
        if ($_GET['t'] == 'index') $_GET['t'] = 'consulting';
    ...
    public function get_consulting_xmlWt(){
    ...
    public function deleteWt(){
        ...
        if (preg_match('/^[\d,]+$/', $_GET['consult_id'])) {
            ...
            if ($del) {
                $this->log(L('wt_delete,consulting').'[ID:'.implode(',',$_GET['consult_id']).']',null);
    ...
    public function type_addWt() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('wt_common_save_succ'), urlAdminShop('consulting', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
                showMessage(L('wt_common_save_fail'));
            }
    ...
    public function type_editWt() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('wt_common_op_succ'), urlAdminShop('consulting', 'type_list'));
            }else {
