...
<script type="text/javascript" src="<?php echo ADMIN_STATIC_URL;?>/js/jquery.edit.js"></script> 
<script type="text/javascript">
function update_flex(){
    $("#flexigrid").flexigrid({
        url: 'index.php?w=mb_special&t=get_special_xml',
        colModel : [
...
//编辑专题描述
$('span[wt_type="edit_special_desc"]').live('click', function() {
    if($(this).attr("edit")>0) return;
    $(this).inline_edit({w: 'mb_special',t: 'update_special_desc'});
    $(this).attr("edit",1);
});
//添加专题
function fg_operation_add() {
    $('#dialog_add_mb_special').wt_show_dialog({title: '新增专题'});
}
//删除专题
...
$(function(){
    update_flex();
        ...
        $('#add_form').validate({
            ...
            messages : {
                special_desc : {
                    required : "<i class='fa fa-exclamation-bbs'></i>专题描述不能为空",
                    maxlength : "<i class='fa fa-exclamation-bbs'></i>专题描述最多20个字"
                }
            }
        });
});
