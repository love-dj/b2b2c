...
class mall_consultControl extends SystemControl{
    ...
    public function get_xmlWt(){
        ...
        foreach ($consult_list as $consult_info) {
            ...
            if ($consult_info['is_reply']) {
                $list['operation'] .= "<a class=\"btn green\" href=\"index.php?w=mall_consult&t=consult_reply&id={$consult_info['mc_id']}\"><i class=\"fa fa-list-alt\"></i>编辑</a>";
            } else {
                $list['operation'] .= "<a class=\"btn green\" href=\"index.php?w=mall_consult&t=consult_reply&id={$consult_info['mc_id']}\"><i class=\"fa fa-list-alt\"></i>回复</a>";
            }
     ...
     * 回复咨询
     */
    public function consult_replyWt() {
        $model_mallconsult = Model('mall_consult');
        if (chksubmit()) {
     ...
     * 批量删除平台客服咨询
     */
    public function del_consult_batchWt(){
        $ids = $_POST['id'];
        if(empty($ids)){
            showMessage(Language::get('wt_common_del_fail'));
        }
    ...
    public function type_addWt() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('wt_common_save_succ'), urlAdminShop('mall_consult', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
    ...
    public function type_editWt() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('wt_common_op_succ'), urlAdminShop('mall_consult', 'type_list'));
            }else {
