...
<div class="tabmenu">
  ...
  <a href="index.php?w=store_fx_goods&t=goods_list" class="wtbtn wtbtn-mint" title="添加商品"><i class="icon-plus-sign"></i>添加商品</a>
</div>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="tl" style="width:280px"><dl class="goods-name">
          ...
          <dd>价格<?php echo $lang['wt_colon'];?><?php echo $lang['currency'].wtPriceFormat($val['goods_price']); ?></dd>
        </dl></td>
      ...
      <td><span><?php echo date('Y-m-d H:i:s',$val['fx_add_time']);; ?></span></td>
      <td><input class="text w20" type="text" id="<?php echo $val['goods_commonid']; ?>" name="fx_commis_rate" maxlength="2" value="<?php echo $val['fx_commis_rate']; ?>" onchange="change_commis(<?php echo $val['goods_commonid']; ?>);" /><span style="line-height:32px;">&nbsp;%</span></td>
      <td class="nscs-table-handle">
        <span><a href="javascript:void(0)" onclick="ajax_get_confirm('您确定要删除吗?', '<?php echo FENXIAO_SITE_URL;?>/index.php?w=store_fx_goods&t=del_goods&id=<?php echo $val['goods_commonid']; ?>');" class="btn-grapefruit"><i class="icon-trash"></i>
        <p>取消分销</p>
        </a></span></td>
    </tr>
    <?php } ?>
...
function change_commis(common_id){
	...
	if (num>0 && num <= 30) {
	    var ajaxurl = 'index.php?w=store_fx_goods&t=edit_goods&num='+num+'&id='+common_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			async: false,
