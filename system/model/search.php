...
//                 showMessage($e->getMessage(),'','html','error');
    ...
    private function _setQueryXS($condition,$order) {
        ...
        if (isset($condition['robbuy'])) {
            $this->_xs_search->addQueryString('robbuy'.':'.$condition['robbuy']);
        }
        ...
        if (isset($condition['price_from']) || isset($condition['price_to'])) {
            $this->_xs_search->addRange('goods_sale_price',$condition['price_from'],$condition['price_to']);
        }
    ...
    private function _createXS($pagesize,$appname) {
        ...
        require_once(BASE_API_PATH.'/xs/lib/XS.php');
    ...
    private function _searchXS(){
        ...
        foreach ($docs as $doc) {
            foreach (array('goods_id','goods_name','goods_jingle','gc_name','brand_name','brand_id','is_book','attr_id',
                    'store_id','area_id','have_gift','goods_sale_price','goods_click','goods_salenum','goods_barcode') as $v) {
   ...
   public function getTagCategory($keyword = '') {
        if ($keyword != '') {
            ...
            if (!empty($tag_list) && is_array($tag_list)) {
                foreach($tag_list as $key => $val) {
                    $tag_value = str_replace(',', '==ShopWT==', $val['gc_tag_value']);
     ...
     * @return array 返回当前ID分类至根分类路径数组(最多取上三级分类)
     */
    public function getGoodsNavCurrHere($cate_id){
		if (intval($cate_id)> 0) {
            $cate_here = array();
            /**
             * 取当前类别信息
             */
            $class = Model('goods_class')->getGoodsClassInfoById(intval($cate_id));
            ...
            if ($class['gc_parent_id'] != 0) {
                $parent_1 = Model('goods_class')->getGoodsClassInfoById($class['gc_parent_id']);
				
                if ($parent_1['gc_parent_id'] != 0) {
                    $parent_2 = Model('goods_class')->getGoodsClassInfoById($parent_1['gc_parent_id']);
					
                }
             
            }
			$key_id=0;
			if (isset($parent_2)&&is_array($parent_2)) {
				$cate_here[$key_id]['cate_array']=Model('goods_class')->getGoodsClassListByParentId($parent_2['gc_parent_id']);
				$cate_here[$key_id]['curr_cate_id']=$parent_2['gc_id'];
				$cate_here[$key_id]['curr_cate_name']=$parent_2['gc_name'];
				$key_id++;
			}
			if (isset($parent_1)&&is_array($parent_1)) {
				$cate_here[$key_id]['cate_array']=Model('goods_class')->getGoodsClassListByParentId($parent_1['gc_parent_id']);
				$cate_here[$key_id]['curr_cate_id']=$parent_1['gc_id'];
				$cate_here[$key_id]['curr_cate_name']=$parent_1['gc_name'];
				$key_id++;
			}
			if (isset($class)&&is_array($class)) {
				$cate_here[$key_id]['cate_array']=Model('goods_class')->getGoodsClassListByParentId($class['gc_parent_id']);
				$cate_here[$key_id]['curr_cate_id']=$class['gc_id'];
				$cate_here[$key_id]['curr_cate_name']=$class['gc_name'];
				$key_id++;
			}
    ...
    public function indexerSearch($get = array(), $pagesize) {
        ...
        if (in_array($get['key'],array('1','2','3'))) {
            $order = array(str_replace(array('1','2','3'), array('goods_salenum','goods_click','goods_sale_price'), $get['key'])
