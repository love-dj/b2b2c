            ...
            foreach($data as $kkk=>$vvv){
                if(is_array($vvv['store_announcement']['item'])){
                	...
                	foreach($vvv['store_announcement']['item'] as $kk=>$vv){
                        $newss = $this->_news_list_extend($vv['article_id']);
                        $data[$kkk]['store_announcement']['item'][$kk]['article_publish_time'] = date("Y-m-d ", time($newss['article_publish_time']));
                        $data[$kkk]['store_announcement']['item'][$kk]['article_click'] = $newss['article_click'];
                		if(is_array($vv['article_image'])){
                			$data[$kkk]['store_announcement']['item'][$kk]['article_image'] = getNEWSArticleImageUrl($vv['article_attachment_path'], $vv['article_image']['name']);
                		}else{
