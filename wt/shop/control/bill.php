...
class billControl extends SystemControl{
    ...
    public function show_billWt(){
        ...
        } elseif ($_GET['query_type'] == 'fx') {
            $sub_tpl_name = 'bill_order_bill.show.fx_list';
        ...
        if ($_GET['query_type'] == 'refund') {
            ...
            foreach ($refund_list as $refund_info) {
                ...
                if ($refund_info['refund_type'] == 1) {
                    $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=refund&t=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                } else {
                    $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=return&t=view&return_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                }
                $list['refund_sn'] = $refund_info['refund_sn'];
                $list['order_sn'] = $refund_info['order_sn'];
                $list['refund_amount'] = wtPriceFormat($refund_info['refund_amount']);
                $list['commis_amount'] = wtPriceFormat($refund_info['commis_amount']);
                $list['rpt_amount'] = wtPriceFormat($refund_info['rpt_amount']);
            ...
            foreach ($store_cost_list as $store_cost_info) {
                ...
                $list['cost_price'] = wtPriceFormat($store_cost_info['cost_price']);
            ...
            $fx_pay_list = $model_fx_order->getDisPayList($condition,$_POST['rp']);
            $data = array();
            $data['now_page'] = $model_fx_order->shownowpage();
            $data['total_num'] = $model_fx_order->gettotalnum();
            foreach ($fx_pay_list as $_info) {
                $list = array();
                $list['goods_name'] = $_info['goods_name'];
                $list['order_sn'] = $_info['order_sn'];
                $list['pay_goods_amount'] = wtPriceFormat($_info['pay_goods_amount']);
                $list['refund_amount'] = wtPriceFormat($_info['refund_amount']);
                $list['fx_commis_rate'] = $_info['fx_commis_rate'].'%';
                $list['fx_pay_amount'] = wtPriceFormat($_info['fx_pay_amount']);
                $list['fx_pay_time'] = date('Y-m-d',$_info['fx_pay_time']);
                $data['list'][$store_cost_info['log_id']] = $list;
            }
            exit(Tpl::flexigridXML($data));
        } elseif ($_GET['query_type'] == 'book') {
            ...
            foreach ($order_list as $order_info) {
                ...
                $list['shipping_fee'] = wtPriceFormat($order_info['shipping_fee']);
                $list['deposit_amount'] = wtPriceFormat($tmp_book[$order_info['order_id']]['book_real_pay']);
            ...
            foreach ($order_list as $order_info) {
                ...
                $list['rpt_amount'] = wtPriceFormat($order_info['rpt_amount']);
                $list['add_time'] = date('Y-m-d',$order_info['add_time']);
    ...
    public function bill_payWt(){
		...
		if(!empty($store_info)){
			...
			if ($update){
				...
				$data['amount'] = $bill_info['ob_result_totals'];
				$data['admin_name'] = $admininfo['name'];
				$data['pdr_sn'] = $bill_info['ob_id'];
				$data['lg_desc'] = '实物结算时间：'.date('Y-m-d',$bill_info['ob_start_date']).'至'.date('Y-m-d',$bill_info['ob_end_date']);
				...
				showMessage('保存成功','index.php?w=bill');
			}else{
				$this->log('账单付款,账单号：'.$ob_id,1);
    ...
    public function export_refund_orderWt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?w=bill&t=show_bill&query_type=refund&ob_id='.$ob_id);
    ...
    public function get_bill_xmlWt(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?w=bill&t=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=bill&t=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }

            $list['ob_id'] = $bill_info['ob_id'];
            $list['ob_order_totals'] = wtPriceFormat($bill_info['ob_order_totals']);
            $list['ob_shipping_totals'] = wtPriceFormat($bill_info['ob_shipping_totals']);
            $list['ob_commis_totals'] = wtPriceFormat($bill_info['ob_commis_totals']);
            $list['ob_order_return_totals'] = wtPriceFormat($bill_info['ob_order_return_totals']);
            $list['ob_commis_return_totals'] = wtPriceFormat($bill_info['ob_commis_return_totals']);
            $list['ob_store_cost_totals'] = wtPriceFormat($bill_info['ob_store_cost_totals']);
            if(C('fenxiao_isuse') == 1) {
            $list['ob_fx_pay_amount'] = wtPriceFormat($bill_info['ob_fx_pay_amount']);
            }
