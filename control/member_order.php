...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexWt() {
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $_flag = ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['order_type'] == 1) || ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['order_type'] == 4) ||
            ...
            if ($_flag) {
                ...
                if ($order_info['order_type'] == 4) {//拼团订单
                    ...
                    $_info = $model_pingou->getOrderInfo(array('order_id'=> $order_info['order_id']));
                    if ($_info['end_time'] < TIMESTAMP) $order_group_list[$order_info['pay_sn']]['pay_amount'] = 0;//拼团订单过期后不能继续支付
                }
            }
    ...
    public function show_orderWt() {
        ...
        $result = $handle_order->getMemberOrderInfo($_GET['order_id'],$_SESSION['member_id']);
        if (!$result['state']) {
    ...
    private function _order_cancel($order_info, $post) {
        if (!chksubmit()) {
            ...
            if ($order_info['order_type'] != 2) {
                ...
                $result = $handle_order->changeOrderStateCancel($order_info,'buyer', $_SESSION['member_name'], $msg,true,$cancel_condition);
            } else {
                //取消预定订单
                $result = Handle('order_book')->changeOrderStateCancel($order_info,'buyer', $_SESSION['member_name'], $msg);
            }
            return $result;
        }
    }
    ...
    private function _order_receive($order_info, $post) {
        if (!chksubmit()) {
            ...
            return $handle_order->changeOrderStateReceive($order_info,'buyer',$_SESSION['member_name'],'签收了货物');
        }
    }
    ...
    private function _order_recycle($order_info, $get) {
        ...
        return $handle_order->changeOrderStateRecycle($order_info,'buyer',$state_type);
    }
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            array('menu_key'=>'member_order','menu_name'=>Language::get('wt_member_path_order_list'), 'menu_url'=>'index.php?w=member_order'),
            array('menu_key'=>'member_order_recycle','menu_name'=>'回收站', 'menu_url'=>'index.php?w=member_order&recycle=1'),
        );
