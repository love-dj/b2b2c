...
class order_vrControl extends SystemControl{
    /**
     * 每次导出订单数量
     * @var int
     */
    ...
    public function get_xmlWt(){
        ...
        $order_list = $model_order_vr->getOrderList($condition,!empty($_POST['rp']) ? intval($_POST['rp']) : 15,'*',$order);

        $data = array();
        $data['now_page'] = $model_order_vr->shownowpage();
        $data['total_num'] = $model_order_vr->gettotalnum();
        foreach ($order_list as $k => $order_info) {
            $list = array();$operation_detail = '';
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=order_vr&t=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $if_cancel = $model_order_vr->getOrderOperateState('system_cancel',$order_info);
            if ($if_cancel) {
                $operation_detail .= "<li><a href=\"javascript:void(0);\" onclick=\"fg_cancel({$order_info['order_id']})\">取消订单</a></li>";
            }
            $if_system_receive_pay = $model_order_vr->getOrderOperateState('system_receive_pay',$order_info);
            if ($if_system_receive_pay) {
                $operation_detail .= "<li><a href=\"index.php?w=order_vr&t=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">收到货款</a></li>";
            }
            if ($operation_detail) {
    ...
    private function _order_cancel($order_info) {
        ...
        $if_allow = $model_order_vr->getOrderOperateState('system_cancel',$order_info);
        if (!$if_allow) {
        ...
        $result = $handle_order_vr->changeOrderStateCancel($order_info,'store', '管理员('.$this->admin_info['name'].')关闭订单');
        if ($result['state']) {
    ...
    private function _order_receive_pay($order_info,$post) {
        $model_order_vr = Model('order_vr');
        $handle_order_vr = Handle('order_vr');
        $if_allow = $model_order_vr->getOrderOperateState('system_receive_pay',$order_info);
        if (!$if_allow) {
            return callback(false,'无权操作');
        }
        ...
        if (!chksubmit()) {
            ...
            $result = $handle_order_vr->changeOrderStatePay($order_info,'admin', $post);
            if ($result['state']) {
    ...
    public function export_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            $count = $model_order_vr->getOrderCount($condition);
            $array = array();
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=order_vr&t=index');
				Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
                $data = $model_order_vr->getOrderList($condition,'','*','order_id desc',self::EXPORT_SIZE);
                $this->createExcel($data);
            }
        }else{  //下载
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_order_vr->getOrderList($condition,'','*','order_id desc',"{$limit1},{$limit2}");
            $this->createExcel($data);
        }
    }
