...
.area-list {display:inline-block;width:200px;padding:5px 0px;}
  ...
  <form id="area_form" method="post">
    ...
    <input type="hidden" name="w" value="store_transport" />
    <input type="hidden" name="t" value="offpay_area" />
    <input type="hidden" name="county" id="county" value="" />

    <table class="wtsc-default-table order" id="table_area_box">
      <tbody>
...
<?php foreach ($areas['children'][$provinceId] as $cityId) { ?>
            <div class="area-list">
              <label>
                <input type="checkbox" wt_province="<?php echo $provinceId; ?>"<?php if ($output['city_checked_array'][$cityId]) echo ' checked="checked"';?> value="<?php echo $cityId; ?>" name="city[]" />
              ...
              <a city_id="<?php echo $cityId; ?>" wt_title="<?php echo $areas['name'][$cityId]; ?>" province_id="<?php echo $provinceId; ?>" wt_type="edit" href="javascript:void(0);" title="选择下级地区"><i class="icon-pencil"></i></a>
            </div>
<?php } ?>
<?php } ?>
          </td>
...
<?php } ?>
      ...
      <tfoot>
        <tr class="tfoot">
          ...
          <a href="javascript:void(0);" class="wtbtn wtbtn-mint" id="submitBtn"><span>提交保存</span></a>
          </div>
          </td>
        </tr>
      </tfoot>
...
var wt_a = $.parseJSON('<?php echo $output['area_array_json']; ?>');
...
$(function(){
    ...
    $('input[name="province[]"]').on('click',function(){
        if ($(this).attr('checked') == 'checked'){
            $('input[wt_province="' + $(this).val() + '"]').each(function(){
                $(this).attr('checked','checked');
                if (typeof wt_a[$(this).val()] == 'object') {
                    county_array = wt_a[$(this).val()];
                } else {
    ...
    //点击编辑事件
    $('a[wt_type="edit"]').on('click',function(){
            if (typeof CUR_COUNTY[$(this).attr('city_id')] == 'object'){
                cur_county = CUR_COUNTY[$(this).attr('city_id')];
            }else{
                cur_county = new Array();
            }
            var province_array = wt_a[$(this).attr('city_id')];
            if (typeof wt_a[$(this).attr('city_id')] == 'object'){
                county_array = wt_a[$(this).attr('city_id')];
            }else{
                county_array = new Array();
            }
            ...
            county_html += '</ul><div class="bottom"><a id="county_submit" class="wtbtn wtbtn-mint" href="javascript:void(0);"><span>确认</span></a> <span style="color:#f30;">确认后，还需要点击页面底部的“提交保存”按钮完成保存操作</span></div></div>';
            html_form('select_county', '选择 '+ $(this).attr('wt_title') +' 子地区', county_html, 500,1);
    });
    ...
    $('input[name="city[]"]').on('click',function(){
        if ($(this).attr('checked')) {
            ...
            count = county_array.length;
            if ($('input[wt_province="'+$(this).attr('wt_province')+'"]').size() == $('input[wt_province="'+$(this).attr('wt_province')+'"]:checked').size()) {
                $('input[value="'+$(this).attr('wt_province')+'"]').attr('checked',true);
            } else {
                $('input[value="'+$(this).attr('wt_province')+'"]').attr('checked',false);
            }
        } else {
            CUR_COUNTY[$(this).val()] = undefined;
            count = 0;
    ...
    $('body').on('click','#county_submit',function(){
        ...
        if ($('input[wt_province="'+cur_td.attr('province_id')+'"]').size() == $('input[wt_province="'+cur_td.attr('province_id')+'"]:checked').size()) {
