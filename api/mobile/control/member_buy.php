...
class member_buyControl extends mobileMemberControl {
    ...
    public function buy_step1Wt() {
        ...
        //得到购买数据
        $result = $handle_buy->buyStep1($cart_id, $_POST['ifcart'], $this->member_info['member_id'], $this->member_info['store_id'],null,$member_discount,$member_level);
        if(!$result['state']) {
        ...
        if ($result['address_info']) {
            $data_area = $handle_buy->changeAddr($result['freight_list'], $result['address_info']['city_id'], $result['address_info']['area_id'], $this->member_info['member_id']);
            if(!empty($data_area) && $data_area['state'] == 'success' ) {
                if (is_array($data_area['content'])) {
                    foreach ($data_area['content'] as $store_id => $value) {
                        $data_area['content'][$store_id] = wtPriceFormat($value);
                    }
                }
            } else {
                output_error('地区请求失败');
        ...
        foreach ($result['store_cart_list'] as $key => $value) {
            ...
            if (is_array($result['store_voucher_list'][$key]) && count($result['store_voucher_list'][$key]) > 0) {
                ...
                $store_cart_list[$key]['store_voucher_info']['voucher_price'] = wtPriceFormat($store_cart_list[$key]['store_voucher_info']['voucher_price']);
        ...
        if (is_array($result['rpt_list']) && !empty($result['rpt_list'])) {
            foreach ($result['rpt_list'] as $k => $v) {
                unset($result['rpt_list'][$k]['coupon_id']);
                unset($result['rpt_list'][$k]['coupon_end_date']);
                unset($result['rpt_list'][$k]['coupon_owner_id']);
                unset($result['rpt_list'][$k]['coupon_code']);
            }
        }
        ...
        if ($data_area['content']) {
            $store_total_list = Handle('buy_action')->reCalcGoodsTotal($store_total_list,$data_area['content'],'freight');
            //返回可用平台优惠券
            $result['rpt_list'] = Handle('buy_action')->getStoreAvailableRptList($this->member_info['member_id'],array_sum($store_total_list),'coupon_limit desc');
            reset($result['rpt_list']);
            if (is_array($result['rpt_list']) && count($result['rpt_list']) > 0) {
                $result['rpt_info'] = current($result['rpt_list']);
                unset($result['rpt_info']['coupon_id']);
                unset($result['rpt_info']['coupon_end_date']);
                unset($result['rpt_info']['coupon_owner_id']);
                unset($result['rpt_info']['coupon_code']);
            }
        }
        $buy_list['order_amount'] = wtPriceFormat(array_sum($store_total_list)-$result['rpt_info']['coupon_price']);
        ...
        foreach ($store_total_list as $store_id => $value) {
            $store_total_list[$store_id] = wtPriceFormat($value);
        }
        $buy_list['store_final_total_list'] = $store_total_list;
		//会员积分
		$buy_list['member_points'] =$member_info['member_points'];
		$buy_list['points_money_parity'] =C('points_money_parity');
		$buy_list['points_money_isuse'] =C('points_money_isuse');
		$buy_list['member_points_payrate'] =C('member_points_payrate');
    ...
    public function buy_step2Wt() {
        ...
        $result = $handle_buy->buyStep2($param, $this->member_info['member_id'], $this->member_info['member_name'], $this->member_info['member_email'],$member_discount,$member_level);
        if(!$result['state']) {
            output_error($result['msg']);
        }
