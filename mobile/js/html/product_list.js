...
$(function(){
    ...
    $('#product_list').on('click', '.goods-store a',function(){
        ...
        $.getJSON(ApiUrl + '/index.php?w=store&t=store_credit', {store_id:store_id}, function(result){
            var html = '<dl>'
...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/index.php?w=goods&t=goods_list' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.datas = [];
...
function search_show() {
    $.getJSON(ApiUrl + '/index.php?w=index&t=search_show', function(result) {
    	...
    	$('#search_submit').click(function(){
			...
			if ($('#robbuy')[0].className == 'current') {
				queryString += '&robbuy=1';
			}
    		...
    		window.location.href = WapSiteUrl + '/html/product_list.html' + queryString;
    	});
...
function bss_get_list() {
	...
	$.getJSON(ApiUrl + '/index.php?w=bbs_index&t=bbs_list' + window.location.search.replace('?','&'), param, function(result){
		if(!result) {
			result = [];
			result.datas = [];
			result.datas.goods_list = [];
