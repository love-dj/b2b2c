...
class stat_memberControl extends SystemControl{
    private $links = array(
        array('url'=>'w=stat_member&t=newmember','lang'=>'stat_newmember'),
        array('url'=>'w=stat_member&t=analyze','lang'=>'stat_memberanalyze'),
        array('url'=>'w=stat_member&t=scale','lang'=>'stat_scaleanalyze'),
        array('url'=>'w=stat_member&t=area','lang'=>'stat_areaanalyze'),
        array('url'=>'w=stat_member&t=buying','lang'=>'stat_buying'),
        array('url'=>'w=stat_member&t=area_xq','text'=>'县级分析'),
    );
    ...
    public function __construct(){
		...
		if(!$this->search_arr['areadeep']){
           $area_deep = intval($this->search_arr['areadeep']);
		   if(!in_array($area_deep,array(1,2,3)))
		   {
			   $this->search_arr['areadeep'] = '1';
		   }
        }
		
        //处理搜索时间
        if (!isset($this->search_arr['t']) || in_array($_REQUEST['t'],array('newmember','analyze','scale','area','area_xq','buying','get_scale_xml'))){
    ...
    public function newmemberWt(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            Tpl::output('actionurl','index.php?w=stat_member&t=newmember&search_type=day&search_time='.date('Y-m-d',$this->search_arr['day']['search_time']));
        }
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            Tpl::output('actionurl','index.php?w=stat_member&t=newmember&search_type=week&searchweek_year='.$this->search_arr['week']['current_year'].'&searchweek_month='.$this->search_arr['week']['current_month'].'&searchweek_week='.$this->search_arr['week']['current_week']);
        }
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            Tpl::output('actionurl','index.php?w=stat_member&t=newmember&search_type=month&searchmonth_year='.$this->search_arr['month']['current_year'].'&searchmonth_month='.$this->search_arr['month']['current_month']);
        }
     ...
     * 输出会员分析详细列表XML数据
     */
    public function get_analyzeinfo_xmlWt(){
        $model = Model('stat');
        $where = array();
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
        foreach ((array)$searchtime_arr_tmp as $k => $v){
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                if ($stat_field == 'orderamount') {
                    $out_array['orderamount'] = wtPriceFormat($out_array['orderamount']);
                }
     ...
     * 查看会员列表
     */
    public function showmemberWt(){
        Language::read('member');
        $model = Model('stat');
        $where = array();
        if (!empty($_GET['c'])){
            $searchtime_arr_tmp = explode('|',$_GET['c']);
            foreach ((array)$searchtime_arr_tmp as $k => $v){
                $searchtime_arr[] = intval($v);
            }
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            //data
            foreach ($member_list as $k => $v){
                $excel_data[$k+1][] = array('data'=>$v['member_name'].'('.L('member_index_true_name,wt_colon').$v['member_truename'].')');
                ...
                $excel_data[$k+1][] = array('data'=>L('member_index_available,wt_colon').$v['available_predeposit'].L('currency_zh').'('.L('member_index_frozen,wt_colon').$v['freeze_predeposit'].L('currency_zh').')');
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        $this->links[] = array('url'=>'w=stat_member&t=showmember','lang'=>'stat_memberlist');
    ...
    public function get_member_xmlWt(){
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $out_array['available_predeposit'] = wtPriceFormat($v['available_predeposit']);
                $out_array['freeze_predeposit'] = wtPriceFormat($v['freeze_predeposit']);
     ...
     * 输出会员规模XML数据
     */
    public function get_scale_xmlWt(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
    ...
    public function area_xqWt(){
        ...
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
		Tpl::output('areadeep',$area_deep);
        Tpl::output('searchtime',implode('|',$searchtime_arr));
        Tpl::output('top_link',$this->sublink($this->links, 'area_xq'));
		Tpl::setDirquna('shop');
        Tpl::showpage('stat.memberarea_xq');
    }
    /**
     * 区域分析之详细列表
     */
     ...
     * 输出区域分析详细XML数据
     */
    public function get_arealist_xmlWt(){
        $model = Model('stat');
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
     ...
     * 输出区域分析详细XML数据
     */
    public function get_arealist_xq_xmlWt(){
        $model = Model('stat');
		$area_deep = intval($this->search_arr['areadeep']);
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
		//$where['is_fx'] = 0;//分销订单也统计
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
        foreach ((array)$searchtime_arr_tmp as $k => $v){
            $searchtime_arr[] = intval($v);
        }
        $where['order_add_time'] = array('between',$searchtime_arr);
		if($area_deep ==1) 
			$area_field = 'reciver_province_id';
		
		if($area_deep ==2) {
			$area_field = 'reciver_city_id';
			$where['reciver_province_id'] = intval($this->search_arr['province_id']);
		}
		if($area_deep ==3) {
			$area_field = 'reciver_area_id';
			$where['reciver_city_id'] =  intval($this->search_arr['city_id']);
		}
        $field  = $area_field. ', COUNT(*) as ordernum,SUM(order_amount) as orderamount, COUNT(DISTINCT buyer_id) as membernum ';
        ...
        $orderby = $sortname.' '.$sortorder.','.$area_field.' asc';
        $page = intval($_POST['rp']);
        if ($page < 1) {
            $page = 15;
        }

        $count_arr = $model->getoneByStatorder($where, 'COUNT(DISTINCT '.$area_field.') as countnum');
        $countnum = intval($count_arr['countnum']);
        $list = $model->statByStatorder($where, $field, array($page,$countnum), 0, $orderby, $area_field);
        // 地区
        $province_array = Model('area')->getTopLevelAreas();
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                $province_id = intval($v[$area_field]);
                $out_array = array();
                $out_array['operation'] = '--';
				if($area_deep==1){
					$out_array['operation'] = '<a href="index.php?w=stat_member&t=area_xq&areadeep='.($area_deep+1).'&province_id='.$province_id.'" >查看市级</a>';
				}
				if($area_deep==2){
					$out_array['operation'] = '<a href="index.php?w=stat_member&t=area_xq&areadeep='.($area_deep+1).'&city_id='.$province_id.'" >查看县级</a>';
				}
                $out_array['provincename'] = ($t = $province_array[$province_id]) ? $t : '其他';
                $out_array = getFlexigridArray($out_array,$order_type,$v);
                $out_array['orderamount'] = wtPriceFormat($out_array['orderamount']);
                $statlist[$v[$area_field]] = $out_array;
            }
        }
