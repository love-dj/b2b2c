...
class qrcodeControl extends BaseHomeControl{
    ...
    public function real_orderWt(){
        ...
        if (empty($api_pay_amount)) {
            redirect(BASE_SITE_URL.'/index.php?w=buy&t=pay_ok&pay_sn='.$order_pay_info['data']['pay_sn'].'&pay_amount='.wtPriceFormat($pay_amount));
        }
    ...
    public function order_vrWt(){
        ...
        if ($api_pdrcb_pay_order_ids) {
            Model('order_vr')->editOrder(array('api_pay_time'=>TIMESTAMP),array('order_id'=>array('in',$api_pdrcb_pay_order_ids)));
        }
