...
class flea_classControl extends SystemControl{
	 ...
	 * 保存批量修改分类
	 */
	public function brach_edit_saveWt(){
		$lang	= Language::getLangContent();
		if ($_POST['gc_show'] == '-1'){
			showMessage($lang['goods_class_batch_edit_succ'],'index.php?w=flea_class&t=flea_class');
		}
		if ($_POST['form_submit'] == 'ok'){
			$model_class = Model('flea_class');
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?w=flea_class&t=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
							'msg'=>$lang['goods_class_add_again'],
						),
						array(
							'url'=>'index.php?w=flea_class&t=flea_class',
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					$url = array(
						array(
							'url'=>'index.php?w=flea_class&t=goods_class_edit&gc_id='.$_POST['gc_id'],
	...
	public function goods_class_importWt(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				 ...
				 * 重新生成缓存
				 */
				showMessage($lang['goods_class_import_succ'],'index.php?w=flea_class&t=flea_class');
			}else {
	...
	public function goods_class_delWt(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			showMessage($lang['goods_class_index_del_succ'],'index.php?w=flea_class&t=flea_class');
		}else {
			showMessage($lang['goods_class_index_choose_del'],'index.php?w=flea_class&t=flea_class');
		}
	}
	/**
	 * ajax操作
