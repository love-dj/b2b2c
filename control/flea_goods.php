...
class flea_goodsControl extends BaseFleaControl {
	...
	public function indexWt() {
		...
		if(!empty($_GET['succ'])){
			if($_GET['succ']=='succ'){
				$succ_link = 'location:index.php?w=flea_goods&goods_id='.$_GET['goods_id']."#flea_message";
				@header($succ_link);
			}
		}
		/**
	...
	public function save_consultWt(){
		...
		if($_POST['type_name']==''){
			...
			$goods_content	= $goods->listGoods($condition);
		}
		...
		if($consult->addConsult($input)){
			...
			$succ_link = 'index.php?w=flea_goods&goods_id='.intval($_POST['goods_id']).'&succ=succ';
