...
<div class="wrap">
  ...
  <form method="get" action="index.php" target="_self">
    <table class="wtm-search-table">
      <input type="hidden" name="w" value="member_order" />
  ...
  <table class="wtm-default-table order">
    ...
    <tbody<?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        <td colspan="19" class="pay-td"><span class="ml15">在线支付金额：<em>￥<?php echo wtPriceFormat($group_info['pay_amount']);?></em></span> <a class="wtbtn wtbtn-bittersweet fr mr15" href="index.php?w=buy&t=pay&pay_sn=<?php echo $order_pay_sn; ?>"><i class="icon-shield"></i>订单支付</a></td>
      </tr>
      <?php }?>
      <?php $p++;?>
      <tr>
        <th colspan="19"> <span class="ml10"> 
          <!-- order_sn --> 
          <?php echo $lang['member_order_sn'].$lang['wt_colon'];?><?php echo $order_info['order_sn']; ?>
          <?php if ($order_info['order_from'] == 2){?>
          <i class="icon-mobile-phone"></i>
          <?php }?>
          </span> 
          <!-- order_time --> 
          <span><?php echo $lang['member_order_time'].$lang['wt_colon'];?><?php echo date("Y-m-d H:i:s",$order_info['add_time']); ?></span> 
          ...
          <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $order_info['extend_store']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid=<?php echo $order_info['extend_store']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" style=" vertical-align: middle;" /></a>
          <?php }?>
          </span> <a href="javascript:void(0)" class="share-goods" wt_type="sharegoods" data-param='{"gid":"<?php echo $order_info['goods_list'][0]['goods_id'];?>"}'><i class="icon-share"></i><?php echo $lang['member_order_snsshare'];?></a>
          ...
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要删除吗?删除后该订单可以在回收站找回，或彻底删除', 'index.php?w=member_order&t=change_state&state_type=order_delete&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-trash"></i>删除</a>
          <?php } ?>
          
          <!-- 还原订单 -->
          
          <?php if ($order_info['if_restore']) { ?>
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要还原吗?', 'index.php?w=member_order&t=change_state&state_type=order_restore&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-refresh"></i>还原</a>
          <?php } ?>
        </th>
      </tr>
      ...
      <?php foreach ($order_info['goods_list'] as $k => $goods_content) {?>
      <?php $i++;?>
      <tr>
        <td class="bdl"></td>
        <td class="w70"><div class="wtm-goods-thumb"><a href="<?php echo $goods_content['goods_url'];?>" target="_blank"><img src="<?php echo $goods_content['image_60_url'];?>" onMouseOver="toolTip('<img src=<?php echo $goods_content['image_240_url'];?>>')" onMouseOut="toolTip()"/></a></div></td>
        <td class="tl"><dl class="goods-name">
            <dt><a href="<?php echo $goods_content['goods_url'];?>" target="_blank"><?php echo $goods_content['goods_name']; ?></a><span class="rec"><a target="_blank" href="<?php echo urlShop('snapshot', 'index', array('rec_id' => $goods_content['rec_id']));?>">[交易快照]</a></span></dt>
            <?php if (!empty($goods_content['goods_spec'])) { ?>
          <dd><?php echo $goods_content['goods_spec'];?></dd>
          <?php } ?>
            <!-- 消费者保障服务 -->
            <?php if($goods_content["contractlist"]){?>
           <dd class="goods-slogen"> 
              <!-- S消费者保障服务 -->              
              <?php foreach($goods_content["contractlist"] as $gcitem_v){?>
        ...
        <td><?php echo wtPriceFormat($goods_content['goods_price']);?>
          <p class="green">
            <?php if (is_array($order_info['refund_all']) && !empty($order_info['refund_all']) && $order_info['refund_all']['admin_time'] > 0) {?>
            <?php echo $goods_content['goods_pay_price'];?>(退)
            <?php } elseif ($goods_content['extend_refund']['admin_time'] > 0) { ?>
            <?php echo $goods_content['extend_refund']['refund_amount'];?>(退)
            <?php } ?>
          </p></td>
        <td><?php echo $goods_content['goods_num']; ?></td>
        <td><?php if (!empty($goods_content['goods_type_cn'])) { ?>
          <span class="goods-type"><?php echo $goods_content['goods_type_cn'];?></span>
          <?php } ?></td>
        <td><!-- 退款 -->
          
          <?php if ($goods_content['refund'] == 1){?>
          <p><a href="index.php?w=member_refund&t=add_refund&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_content['rec_id']; ?>">退款/退货</a></p>
          <?php }?>
          <?php if (is_array($order_info['refund_all']) && !empty($order_info['refund_all'])) {?>
          <dd><a target="_blank" href="index.php?w=member_refund&t=view&refund_id=<?php echo $order_info['refund_all']['refund_id'];?>">查看退款</a></dd>
          <?php }else if($goods_content['extend_refund']['refund_type'] == 1) {?>
          <dd><a target="_blank" href="index.php?w=member_refund&t=view&refund_id=<?php echo $goods_content['extend_refund']['refund_id'];?>">查看退款</a></dd>
          <?php }else if($goods_content['extend_refund']['refund_type'] == 2) {?>
          <dd><a target="_blank" href="index.php?w=member_return&t=view&return_id=<?php echo $goods_content['extend_refund']['refund_id'];?>">查看退货</a></dd>
          <?php } ?>
          
          <!-- 投诉 -->
          
          <?php if ($order_info['if_complain']){ ?>
          <p><a href="index.php?w=member_complain&t=complain_new&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_content['rec_id']; ?>">交易投诉</a></p>
          <?php } ?></td>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p class=""><strong><?php echo $order_info['order_amount']; ?></strong></p>
          ...
          <p title="<?php echo $lang['member_order_pay_method'].$lang['wt_colon'];?><?php echo $order_info['payment_name']; ?>"><?php echo $order_info['payment_name']; ?></p></td>
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p><?php echo $order_info['state_desc']; ?> <?php echo $order_info['evaluation_status'] ? $lang['member_order_evaluated'] : '';?></p>
          ...
          <p><a href="index.php?w=member_order&t=show_order&order_id=<?php echo $order_info['order_id']; ?>" target="_blank"><?php echo $lang['member_order_view_order'];?></a></p>
          
          <!-- 物流跟踪 -->
          
          <?php if ($order_info['if_deliver']){ ?>
          <p><a href='index.php?w=member_order&t=search_deliver&order_id=<?php echo $order_info['order_id']; ?>&order_sn=<?php echo $order_info['order_sn']; ?>' target="_blank"><?php echo $lang['member_order_show_deliver']?></a></p>
          <?php } ?></td>
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 --> 
          ...
          <p><a href="javascript:void(0)" class="wtbtn wtbtn-grapefruit" wt_type="dialog" dialog_width="480" dialog_title="<?php echo $lang['member_order_cancel_order'];?>" dialog_id="buyer_order_cancel_order" uri="index.php?w=member_order&t=change_state&state_type=order_cancel&order_id=<?php echo $order_info['order_id']; ?>"  id="order<?php echo $order_info['order_id']; ?>_action_cancel"><i class="icon-ban-bbs"></i> <?php echo $lang['member_order_cancel_order'];?></a></p>
          <?php } ?>
          
          <!-- 退款取消订单 -->
          
          <?php if ($order_info['if_refund_cancel']){ ?>
          <p><a href="index.php?w=member_refund&t=add_refund_all&order_id=<?php echo $order_info['order_id']; ?>" class="wtbtn"><i class="icon-legal"></i>订单退款</a></p>
          <?php } ?>
          
          <!-- 收货 -->
          
          <?php if ($order_info['if_receive']) { ?>
          <p><a href="javascript:void(0)" class="wtbtn" wt_type="dialog" dialog_id="buyer_order_confirm_order" dialog_width="480" dialog_title="<?php echo $lang['member_order_ensure_order'];?>" uri="index.php?w=member_order&t=change_state&state_type=order_receive&order_sn=<?php echo $order_info['order_sn']; ?>&order_id=<?php echo $order_info['order_id']; ?>" id="order<?php echo $order_info['order_id']; ?>_action_confirm"><?php echo $lang['member_order_ensure_order'];?></a></p>
          <?php } ?>
          
          <!-- 评价 -->
          
          <?php if ($order_info['if_evaluation']) { ?>
          <p><a class="wtbtn wtbtn-aqua" href="index.php?w=member_evaluate&t=add&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>
          
          <!-- 追加评价 -->
          
          <?php if ($order_info['if_evaluation_again']) { ?>
          <p><a class="wtbtn" href="index.php?w=member_evaluate&t=add_again&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i>追加评价</a></p>
          <?php } ?>
          <?php if ($order_info['if_drop']) { ?>
          <p><a href="javascript:void(0);" onclick="ajax_get_confirm('您确定要永久删除吗?永久删除后您将无法再查看该订单，也无法进行投诉维权，请谨慎操作！', 'index.php?w=member_order&t=change_state&state_type=order_drop&order_id=<?php echo $order_info['order_id']; ?>');" class="wtbtn wtbtn-grapefruit mt5"><i class="icon-trash"></i>永久删除</a></p>
          <?php } ?></td>
        <!-- E 合并TD -->
        <?php } ?>
      </tr>
      ...
      <tr>
        <td class="bdl"></td>
        <td colspan="6" class="tl"><div class="wtm-goods-gift"><strong>赠品：</strong>
...
<script charset="utf-8" type="text/javascript" src="<?php echo STATIC_SITE_URL;?>/js/sns.js" ></script> 
