      ...
      <tr>
        ...
        <td class="w100">
          <form name="select_sort" id="select_sort">
            <input type="hidden" name="w" value="store_video" />
            <input type="hidden" name="t" value="album_video_list" />
    ...
    <div class="wtsc-picture-list">
      <ul>
        ...
        <li>
          <dl>
            <dt>
                <div class="picture" title="播放视频">
                  <video loop controls width="160" height="160" id="video_<?php echo $v['video_id'];?>" src="<?php echo goodsVideoPath($v['video_cover'],$_SESSION['store_id']);?>"></video></div>
              
              <input id="<?php echo $v['video_id'];?>" class="editInput1" readonly value="<?php echo $v['video_name']?>" ></dt>
            <dd class="date">
              <p><?php echo '上传时间'.$lang['wt_colon'].date("Y-m-d",$v['upload_time'])?></p>
            </dd>
            <dd class="buttons">
              <a href="javascript:void(0)" onclick="ajax_get_confirm('您确定进行该操作吗?注意：使用中的视频也将被删除','index.php?w=store_video&t=album_video_del&id=<?php echo $v['video_id'];?>');"><i class="icon-trash"></i>删除视频</a> <a title="新窗口打开视频" target="_blank" href="<?php echo goodsVideoPath($v['video_cover'],$_SESSION['store_id']);?>"><i class="icon-film"></i>打开视频</a> </dd>
          </dl>
        </li>
        <?php }?>
      </ul>
