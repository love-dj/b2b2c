...
class stat_storeControl extends SystemControl{
    private $links = array(
        array('url'=>'w=stat_store&t=newstore','lang'=>'stat_newstore'),
        array('url'=>'w=stat_store&t=hotrank','lang'=>'stat_storehotrank'),
        array('url'=>'w=stat_store&t=storesales','lang'=>'stat_storesales'),
        array('url'=>'w=stat_store&t=degree','lang'=>'stat_storedegree'),
        array('url'=>'w=stat_store&t=storearea','lang'=>'stat_storearea'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['t'],array('hotrank','storesales','get_storesales_xml','storesales_list','storesales_trends'))){
     ...
     * 新增店铺
     */
    public function newstoreWt(){
        //导出excel连接地址
        $actionurl = 'index.php?w=stat_store&t=newstore';
        $where = array();
        //所属店铺分类
     ...
     * 热卖排行列表
     */
    public function hotrank_listWt(){
        $datanum = 30;
        $model = Model('stat');
        switch ($_GET['type']){
           case 'ordernum':
                ...
                if ($i <= $datanum){
                    ...
                    switch ($_GET['type']){
                       case 'ordernum':
                           ...
                           break;
                       case 'orderamount':
                           $tmp['orderamount'] = wtPriceFormat($store_arr[$k]['orderamount']);
                           break;
                    }
    ...
    public function showstoreWt(){
        ...
        $this->links[] = array('url'=>'w=stat_store&t=showstore','lang'=>'stat_storelist');
     ...
     * 店铺销售统计列表
     */
    public function storesales_listWt(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
     ...
     * 输出店铺销售统计XML数据
     */
    public function get_storesales_xmlWt(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                $out_array['operation'] = "<a class='btn green' href='javascript:void(0);' wt_type='showtrends' data-param='{\"storeid\":\"{$v['store_id']}\"}'><i class='fa fa-line-chart'></i>走势图</a>";
                $out_array['orderamount'] = wtPriceFormat($v['orderamount']);
                $statlist[$v['store_id']] = $out_array;
            }
        }
     ...
     * 销售走势
     */
    public function storesales_trendsWt(){
    ...
    public function storeareaWt(){
        ...
        Tpl::output('stat_json',$stat_json);
        Tpl::output('statlist',$statlist);
        $actionurl = 'index.php?w=stat_store&t=showstore&type=storearea';
        if (trim($_GET['search_time'])){
            $actionurl = $actionurl.'&c=0|'.strtotime($_GET['search_time']);
        }
        if ($search_sclass > 0){
