...
class stat_industryControl extends SystemControl{
    private $links = array(
        array('url'=>'w=stat_industry&t=scale','lang'=>'stat_industryscale'),
        array('url'=>'w=stat_industry&t=rank','lang'=>'stat_industryrank'),
        array('url'=>'w=stat_industry&t=price','lang'=>'stat_industryprice'),
        array('url'=>'w=stat_industry&t=general','lang'=>'stat_industrygeneral'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['t']) || in_array($this->search_arr['t'],array('scale','rank','price'))){
        ...
        if (!isset($this->search_arr['t']) || in_array($this->search_arr['t'],array('scale','general'))){//仅显示前两级分类
            $show_depth = 2;
        }
     ...
     * 概况总览
     */
    public function general_listWt(){
        //获得子分类ID
        $gc_childid = $gc_childarr = array();
        if ($this->choose_gcid > 0){//如果进行了分类搜索，则统计该分类下的子分类
            $gc_childdepth = $this->gc_arr[$this->choose_gcid]['depth'] + 1;
        ...
        if (!empty($gc_childarr) && is_array($gc_childarr)){
            ...
            foreach ($gc_childarr as $k => $v){
                ...
                $tmp['priceavg'] = wtPriceFormat(($t = $goods_list[$v['gc_id']]['priceavg'])?$t:0);
     ...
     * 输出概况总览XML数据
     */
    public function get_general_xmlWt(){
        //获得子分类ID
        $gc_childid = $gc_childarr = array();
        if ($this->choose_gcid > 0){//如果进行了分类搜索，则统计该分类下的子分类
            $gc_childdepth = $this->gc_arr[$this->choose_gcid]['depth'] + 1;
        ...
        if ($gc_childarr){
            ...
            foreach ($gc_childarr as $k => $v){
                ...
                $out_array['priceavg'] = wtPriceFormat(($t = $goods_list[$v['gc_id']]['priceavg'])?$t:0);
                ...
                $out_array['orderamount'] = wtPriceFormat(($t = $ordergoods_list[$v['gc_id']]['orderamount'])?$t:0);
