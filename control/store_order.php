...
class store_orderControl extends BaseSellerControl {
    ...
    public function show_orderWt() {
        ...
        if ($order_info['if_store_send'] && $order_info['pingou_info']['lock_state'] == 1) {//拼团订单
        ...
        if(!chksubmit()) {
            ...
            exit();
             ...
             if ($order_info['order_type'] == 2) {
                 //预定订单
                 return Handle('order_book')->changeOrderStateCancel($order_info,'seller',$_SESSION['seller_name'], $msg);
             } else {
                 $cancel_condition = array();
                 if ($order_info['payment_code'] != 'offline') {
                     $cancel_condition['order_state'] = ORDER_STATE_NEW;
                 }
                 return $handle_order->changeOrderStateCancel($order_info,'seller',$_SESSION['seller_name'], $msg,true,$cancel_condition);
             }
         }
    }
    ...
    private function _order_ship_price($order_info, $post) {
        ...
        if(!chksubmit()) {
            ...
            return $handle_order->changeOrderShipPrice($order_info,'seller',$_SESSION['seller_name'],$post['shipping_fee']);
        }
	...
	private function _order_spay_price($order_info, $post) {
	    ...
	    if(!chksubmit()) {
            ...
            return $handle_order->changeOrderSpayPrice($order_info,'seller',$_SESSION['member_name'],$post['goods_amount']); 
	    }
	}
    ...
    public function order_printWt() {
        ...
        if (!empty($order_info['extend_order_goods'])){
            ...
            foreach ($order_info['extend_order_goods'] as $k => $v){
                ...
                $v['goods_all_price'] = wtPriceFormat($v['goods_num'] * $v['goods_price']);
                $goods_total_price += $v['goods_all_price'];
                $goods_new_list[ceil($i/22)][$i] = $v;
                $i++;
            }
        }
        //优惠金额
        $sale_amount = $goods_total_price - $order_info['goods_amount'];
        //运费
        $order_info['shipping_fee'] = $order_info['shipping_fee'];
        Tpl::output('sale_amount',$sale_amount);
        Tpl::output('goods_all_num',$goods_all_num);
        Tpl::output('goods_total_price',wtPriceFormat($goods_total_price));
     ...
     * 打印发货单
     */
    public function order_print_smallWt() {
        Language::read('member_printorder');
        ...
        Tpl::output('goods_total_price',wtPriceFormat($goods_total_price));
        Tpl::output('goods_list',$goods_new_list);
        Tpl::showpage('store_order_small.print',"null_layout");
    }
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
            $menu_array = array(
            array('menu_key'=>'store_order',        'menu_name'=>Language::get('wt_member_path_all_order'), 'menu_url'=>'index.php?w=store_order'),
            array('menu_key'=>'state_new',          'menu_name'=>Language::get('wt_member_path_wait_pay'),  'menu_url'=>'index.php?w=store_order&t=index&state_type=state_new'),
            array('menu_key'=>'state_pay',          'menu_name'=>Language::get('wt_member_path_wait_send'), 'menu_url'=>'index.php?w=store_order&t=store_order&state_type=state_pay'),
            array('menu_key'=>'state_notakes',      'menu_name'=>'待自提', 'menu_url'=>'index.php?w=store_order&t=store_order&state_type=state_notakes'),
            array('menu_key'=>'state_send',         'menu_name'=>Language::get('wt_member_path_sent'),      'menu_url'=>'index.php?w=store_order&t=index&state_type=state_send'),
            array('menu_key'=>'state_success',      'menu_name'=>Language::get('wt_member_path_finished'),  'menu_url'=>'index.php?w=store_order&t=index&state_type=state_success'),
            array('menu_key'=>'state_cancel',       'menu_name'=>Language::get('wt_member_path_canceled'),  'menu_url'=>'index.php?w=store_order&t=index&state_type=state_cancel'),
            );
            break;
        }
