$(function (){
    ...
    if(!key){
        ...
        $(".minus").click(function(){
		    ...
		    window.location.href = WapSiteUrl + '/html/cart_list.html';
                }
            }
            addCookie('goods_cart',goodsarr.join('|'));
        });
        ...
        function initCartList(){
             $.ajax({
                url:ApiUrl+"/index.php?w=member_cart&t=cart_list",
                type:"post",
                dataType:"json",
                data:{key:key},
        ...
        function delCartList(cart_id){
            $.ajax({
                url:ApiUrl+"/index.php?w=member_cart&t=cart_del",
                type:"post",
	    ...
	    var goods_id = sPrents.attr("goods_id");
            ...
            $.ajax({
                url:ApiUrl+"/index.php?w=member_cart&t=cart_edit_quantity",
                type:"post",
        ...
        $('#cart-list-wp').on('click', ".check-out > a", function(){
            ...
            window.location.href = WapSiteUrl + "/html/order/buy_step1.html?ifcart=1&cart_id="+cart_id;
        });
...
function getGoods(goods_id, goods_num){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+'/index.php?w=goods&t=goods_detail&goods_id='+goods_id + "&quanity=" + goods_num,
        dataType:'json',
        async:false,
        success:function(result){
            if (result.datas.error) {
...
//v5.2 添加登录后，返回购物车
function car_login(){
	addCookie('redirect_uri','/html/cart_list.html');
