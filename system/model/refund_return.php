...
class refund_returnModel extends Model{
    ...
    public function addRefundReturn($refund_array, $order = array(), $goods = array()) {
        ...
        if (!empty($goods) && is_array($goods)) {
            ...
            $refund_array['is_fx'] = $goods['is_fx'];
            $refund_array['fx_commis_rate'] = $goods['fx_commis_rate'];
            $refund_array['goods_commonid'] = $goods['goods_commonid'];
        }
        ...
        if ($refund_id > 0) {
            ...
            try {
                ...
                if ($state && $order_state == $order_paid) {
                    Handle('order')->changeOrderStateCancel($order, 'system', $user, '商品全部退款完成取消订单',false);
                }
                if ($state) {
                    ...
                    $order_array['refund_amount'] = wtPriceFormat($refund_amount);
                    $order_array['delay_time'] = time();
                    $state = $model_order->editOrder($order_array,array('order_id'=> $order_id));//更新订单退款
                    if ($state && $order_array['refund_state'] == 2) {//全部退款完成时更新优惠券值（结算使用）
                        $refund_array = array();
                    ...
                    if ($state && $order_state == $order_shipped && $refund_amount >= $order_refund_amount) {
                        ...
                        Handle('order')->changeOrderStateReceive($order_array, 'system', $user, '商品全部退款，系统完成订单');
                    }
                }
    ...
    public function addDetail($refund,$order) {
        ...
        $detail_array['refund_amount'] = wtPriceFormat($refund['refund_amount']);
        ...
        if (!empty($order['trade_no']) && in_array($order['payment_code'],array('wxpay','wx_jsapi','wx_saoma'))) {//微信支付
            $api_file = BASE_API_PATH.DS.'refund'.DS.'wxpay'.DS.'WxPay.Config.php';
            if ($order['payment_code'] == 'wxpay') {
                $api_file = BASE_API_PATH.DS.'refund'.DS.'wxpay'.DS.'WxPayApp.Config.php';
            }
            include $api_file;
    ...
    public function getPayDetailInfo($detail_array) {
        ...
        $order['pay_amount'] = wtPriceFormat($order['order_amount']-$order['rcb_amount']-$order['pd_amount']);//在线支付金额=订单总价格-充值卡支付金额-预存款支付金额
        ...
        $order['pay_refund_amount'] = wtPriceFormat($refund_amount);
        $condition = array();
        ...
        foreach ($order_list as $key => $order_info) {//在线支付金额
            if(in_array($order_info['payment_code'],array('wxpay','wx_jsapi','wx_saoma'))) $pay_amount += wtPriceFormat($order_info['order_amount']-$order_info['rcb_amount']-$order_info['pd_amount']);
        }
        $order['pay_amount'] = wtPriceFormat($pay_amount);
        return $order;
    }
