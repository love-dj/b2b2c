...
class order_vrModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $fields = '*', $master = false,$lock = false) {
        $order_info = $this->table('order_vr')->field($fields)->where($condition)->master($master)->lock($lock)->find();
        if (empty($order_info)) {
            return array();
        }
    ...
    public function addOrder($data) {
        $insert = $this->table('order_vr')->insert($data);
        return $insert;
    }
...
//         $order_code[$i-1]['pay_price'] += $diff_amount;
        ...
        return $this->table('order_vr_code')->insertAll($order_code);
    }
    ...
    public function editOrder($data, $condition, $limit = '') {
        return $this->table('order_vr')->where($condition)->limit($limit)->update($data);
    }
    ...
    public function editOrderCode($data, $condition) {
        return $this->table('order_vr_code')->where($condition)->update($data);
    }
    ...
    public function getCodeList($condition = array(), $fields = '*', $pagesize = '',$order = 'rec_id desc',$limit = '', $group = '') {
        return $this->table('order_vr_code')->field($fields)->where($condition)->page($pagesize)->order($order)->limit($limit)->group($group)->select();
    }
    ...
    public function getOrderCodeInfo($condition = array(), $fields = '*',$master = false) {
        return $this->table('order_vr_code')->field($fields)->where($condition)->master($master)->find();
    }
    ...
    public function getOrderCodeCount($condition) {
        return $this->table('order_vr_code')->where($condition)->count();
    }
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = ''){
        $list = $this->table('order_vr')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->select();
    ...
    public function getOrderCount($condition) {
        return $this->table('order_vr')->where($condition)->count();
    }
