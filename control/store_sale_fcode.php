...
class store_sale_fcodeControl extends BaseSellerControl {
    ...
    public function fcode_listWt() {
        ...
        Tpl::showpage('store_sale_fcode.goods_list');
    }

    /**
     * 购买套餐
     */
    public function fcode_quota_addWt() {
        if (chksubmit()) {
            $quantity = intval($_POST['fcode_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_fcode_price')); // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlShop('store_sale_fcode', 'fcode_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_fcode = Model('p_fcode');
            ...
            if ($return) {
                ...
                $this->recordSellerLog('购买'.$quantity.'套F码商品活动，单价'.intval(C('sale_fcode_price')).'元');
                showDialog('购买成功', urlShop('store_sale_fcode', 'fcode_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_fcode', 'fcode_quota_add'));
            }
        }
        // 输出导航
        $this->profile_menu('fcode_quota_add', 'fcode_quota_add');
        Tpl::showpage('store_sale_fcode.quota_add');
    }

    /**
     * 套餐续费
     */
    public function fcode_renewWt() {
        if (chksubmit()) {
            $model_fcode = Model('p_fcode');
            $quantity = intval($_POST['fcode_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_fcode_price')); // 扣款数
            ...
            if ($return) {
                ...
                $this->recordSellerLog('续费'.$quantity.'套F码商品活动，单价'.intval(C('sale_bundling_price')).'元');
                showDialog('购买成功', urlShop('store_sale_fcode', 'fcode_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_fcode', 'fcode_quota_add'));
            }
        }
     ...
     * 选择商品
     */
    public function fcode_select_goodsWt() {
        ...
        Tpl::showpage('store_sale_fcode.select_goods', 'null_layout');
    }
    ...
    public function choosed_goodsWt() {
        ...
        $model_goods = Model('goods');
        // 验证商品是否存在
        $goods_content = $model_goods->getGoodsInfoByID($gid);
        if (empty($goods_content) || $goods_content['store_id'] != $_SESSION['store_id'] || !$model_goods->checkIsGeneral($goods_content)) {
            if ($_GET['inajax']) {
        ...
        if (chksubmit()) {
            ...
            if ($rs) {
                ...
                $goodsclass_info = Model('goods_class')->getGoodsClassInfoById($goods_content['gc_id']);
                $goods_content['gc_name'] = $goodsclass_info['gc_name'];
                $goods_content['goods_image'] = thumb($goods_content, '60');
                $goods_content['url'] = urlShop('goods', 'index', array('goods_id' => $goods_content['goods_id']));
                $this->recordSellerLog('添加F码商品，商品id：'.$gid);
                showDialog('操作成功', '', 'succ', 'CUR_DIALOG.close();choose_goods('.json_encode($goods_content).')');
            } else {
        ...
        $goodscommon_info = $model_goods->getGoodsCommonInfoByID($goods_content['goods_commonid'], 'spec_name,store_id');
        $spec_name = array_values((array)unserialize($goodscommon_info['spec_name']));
        $goods_spec = array_values((array)unserialize($goods_content['goods_spec']));
        Tpl::output('goods_spec', $goods_spec);
        Tpl::output('spec_name', $spec_name);
        Tpl::output('goods_content', $goods_content);
        Tpl::showpage('store_sale_fcode.choosed_goods', 'null_layout');
    }
     ...
     * 下载F码
     */
    public function download_f_code_excelWt() {
        $goods_id = $_GET['gid'];
        if ($goods_id <= 0) {
            showMessage(L('wrong_argument'), '', '', 'error');
        }
        $goods_content = Model('goods')->getGoodsInfoByID($goods_id);
        if (empty($goods_content) || $goods_content['store_id'] != $_SESSION['store_id']) {
            showMessage(L('wrong_argument'), '', '', 'error');
        }
        import('bin.excel');
        ...
        $excel_obj->addWorksheet($excel_obj->charset($goods_content['goods_name'],CHARSET));
        $excel_obj->generateXML($excel_obj->charset($goods_content['goods_name'],CHARSET).'-'.date('Y-m-d-H',time()));
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'fcode_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'fcode_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_fcode', 'fcode_goods_list'))
                );
                break;
            case 'fcode_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'fcode_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_fcode', 'fcode_goods_list')),
                    2=>array('menu_key'=>'fcode_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlShop('store_sale_fcode', 'fcode_quota_add'))
                );
                break;
            case 'fcode_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'fcode_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_fcode', 'fcode_goods_list')),
                    2=>array('menu_key'=>'fcode_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShop('store_sale_fcode', 'fcode_renew'))
                );
                break;
        }
