...
function buildUrl(type, data) {
    switch (type) {
        case 'keyword':
            return WapSiteUrl + '/html/product_list.html?keyword=' + encodeURIComponent(data);
        case 'special':
            return WapSiteUrl + '/special.html?special_id=' + data;
        case 'goods':
            return WapSiteUrl + '/html/product_detail.html?goods_id=' + data;
        case 'url':
            return data;
    }
    return WapSiteUrl;
...
function getSearchName() {
	...
	if (keyword == '') {
	    if(getCookie('deft_key_value') == null) {
	        $.getJSON(ApiUrl + '/index.php?w=index&t=search_hot_info', function(result) {
...
function getFreeVoucher(tid) {
    ...
    $.ajax({
        type:'post',
        url:ApiUrl+"/index.php?w=member_voucher&t=voucher_freeex",
...
function updateCookieCart(key) {
    ...
    if (cartlist) {
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?w=member_cart&t=cart_batchadd',
...
function getCartCount(key, expireHours) {
    ...
    if (getCookie('key') !== null && getCookie('cart_count') === null) {
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php?w=member_cart&t=cart_count',
            data:{key:key},
            dataType:'json',
            async:false,
            success:function (result) {
...
function checkSellerLogin(state) {
    if (state == 0) {
        location.href = WapSiteUrl + '/html/seller/login.html';
        return false;
    } else {
        return true;
    }
...
function getChatCount() {
    if ($('#header').find('.message').length > 0) {
        var key = getCookie('key');
        if (key !== null) {
            $.getJSON(ApiUrl+'/index.php?w=member_chat&t=get_msg_count', {key:key}, function(result){
                if (result.datas > 0) {
     ...
     */
        areaSelected: function(options) {
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'get',
                    url:ApiUrl+'/index.php?w=area&t=area_list',
                    data:{area_id:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
                        ...
                        if (typeof(myScrollArea) == 'undefined') {
                            ...
                            myScrollArea = new IScroll('#areaSelected .wtm-main-box-a', { mouseWheel: true, click: true });
                        } else {
        ...
        goodsClassSelected: function(options) {
            ...
            function _classinit() {
                ...
                var e = '<div id="areaSelected">' + '<div class="wtm-full-mask left">' + '<div class="wtm-full-mask-bg"></div>' + '<div class="wtm-full-mask-block">' + '<div class="header">' + '<div class="header-box">' + '<div class="header-l"><a href="javascript:void(0);"><i class="back"></i></a></div>' + '<div class="header-title">' + "<h1>选择商品分类</h1>" + "</div>" + '<div class="header-r"><a href="javascript:void(0);"><i class="close"></i></a></div>' + "</div>" + "</div>" + '<div class="wtm-main-box">' + '<div class="wtm-single-nav">' + '<ul id="filtrate_ul" class="area">' + '<li class="selected"><a href="javascript:void(0);">一级分类</a></li>' + '<li><a href="javascript:void(0);" >二级分类</a></li>' + '<li><a href="javascript:void(0);" >三级分类</a></li>' + "</ul>" + "</div>" + '<div class="wtm-main-box-a"><ul class="wtm-default-list"></ul></div>' + "</div>" + "</div>" + "</div>" + "</div>";
                $("body").append(e);
                _getGoodsClassList();
                _gcBindEvent();
                _gcclose()
            ...
            function _getGoodsClassList() {
                $.ajax({
                    type: "post",
                    url: ApiUrl + "/index.php?w=seller_goods_class&t=class_list",
                    data: {
                        gc_id: ASID,
                        deep:ASDEEP,
                        client:'mobile',
                    ...
                    success: function(e) {
                        ...
                        $("#areaSelected").find(".wtm-default-list").html(a);
                        if (typeof myScrollArea == "undefined") {
                            if (typeof IScroll == "undefined") {
                                $.ajax({
                                    url: WapSiteUrl + "/js/iscroll.js",
            ...
            function _gcBindEvent() {
                $("#areaSelected").find(".wtm-default-list").off("click", "li > a");
                $("#areaSelected").find(".wtm-default-list").on("click", "li > a",
                function() {
                    ASID = $(this).attr("data-id");
                    eval("ASID_" + ASDEEP + "=$(this).attr('data-id')");
                    ASNAME = $(this).attr("data-name");
...
function loadVercode(){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+"/index.php?w=vercode&t=makecodekey",
        async : false,
        dataType: 'json',
        success:function(result){
...
function favoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?w=member_favorites_store&t=favorites_add',
...
function dropFavoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?w=member_favorites_store&t=favorites_del',
        data: {key: key, store_id: store_id},
        dataType: 'json',
        async: false,
        success: function(result) {
...
function favoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?w=member_favorites&t=favorites_add',
...
function dropFavoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php?w=member_favorites&t=favorites_del',
