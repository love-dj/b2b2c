...
class pointcartControl extends BasePointShopControl {
    ...
    public function addWt() {
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>'index.php?w=pointprod&t=plist')); die;
                    break;
                default:
    ...
    public function step1Wt(){
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?w=pointprod','error');
        }
    ...
    public function step2Wt() {
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?w=pointcart','error');
        }
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?w=pointcart&t=step1','error');
        }
        $order_id = $data['data']['order_id'];
        @header("Location:index.php?w=pointcart&t=step3&order_id=".$order_id);
    }
    /**
     * 流程第三步
     */
    public function step3Wt($order_arr=array()) {
