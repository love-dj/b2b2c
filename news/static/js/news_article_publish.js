 ...
 //图片上传
    $("#article_image_upload").fileupload({
        dataType: 'json',
            url: "index.php?w=publish&t=article_image_upload",
            add: function(e,data) {
                ...
                if(result.status == "success") {
					...
					var image_handle = '<a wttype="btn_insert_article_image">插入</a>';
                    image_handle += '<a wttype="btn_set_article_image" image_name="'+result.file_name+'">封面</a>';
                    image_handle += '<a wttype="btn_drop_article_image" image_name="'+result.file_name+'">删除</a>';
    ...
    //设封面图
    $("[wttype='btn_set_article_image']").live("click",function(){
    ...
    //插入编辑器
    $("[wttype='btn_insert_article_image']").live("click",function(){
        var html = KE.html();
    ...
    //图片删除
    $("[wttype='btn_drop_article_image']").live("click",function(){
        var image_object = $(this).parent().parent();
        var image_name = $(this).attr("image_name");
        $.getJSON("index.php?w=publish&t=article_image_drop", { image_name: image_name }, function(result){
            if(result.status == "success") {
                image_object.remove();
            } else {
                showError(result.error);
    ...
    $("[wttype='news_tag']").live("click",function(){
        ...
        $("#article_tag").val(news_tag_selected.substring(0, news_tag_selected.length-1));
    });
        ...
        if($("#article_goods_list li").length < 3) { 
            ...
            if(search_keyword != "") {
                if(search_type == "goods_url") {
                    ...
                    $.getJSON("index.php?w=api&t=goods_content_by_url", { url: url}, function(data){
                        if(data.result == "true") {
                    ...
                    $("#div_goods_select").load("index.php?w=api&t=goods_list&search_type="+search_type+"&search_keyword="+search_keyword);
                }
            }
        }
    });
    ...
    //商品添加
    $("#goods_search_list [wttype='btn_goods_select']").live("click",function(){
        if($("#article_goods_list li").length < 3) { 
            var temp = '<li wttype="btn_goods_select">'+$(this).html();
    ...
    //商品删除
    $("#article_goods_list [wttype='btn_goods_select']").live("click",function(){
        $(this).remove();
    });
    ...
    $("#btn_article_search").click(function(){
        ...
        if(search_keyword != "") {
            $("#div_article_select").load("index.php?w=api&t=article_list&search_type="+search_type+"&search_keyword="+search_keyword);
        }
    });
    ...
    //文章添加
    $("#article_search_list [wttype='btn_article_select']").live("click",function(){
    ...
    //文章删除
    $("#article_link_list [wttype='btn_article_select']").live("click",function(){
        $(this).remove();
        refresh_article_link();
    });

    function refresh_article_link() {
        var news_article_selected = '';
        $("#article_link_list [wttype='btn_article_select']").each(function(){
            news_article_selected += $(this).attr("article_id") + ",";
        });
        $("#article_link").val(news_article_selected.substring(0, news_article_selected.length-1));
    } 
