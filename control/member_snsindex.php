...
class member_snsindexControl extends BaseMemberControl {
    ...
    public function addtraceWt(){
        ...
        if ($result){
            ...
            if (cookie('weibonum') != null && intval(cookie('weibonum')) >0){
                setWtCookie('weibonum',intval(cookie('weibonum'))+1,2*3600);//保存2小时
            }else{
                setWtCookie('weibonum',1,2*3600);//保存2小时
            }
            $js = "var obj = $(\"#weiboform\").find(\"[wt_type='formprivacytab']\");$(obj).find('span').removeClass('selected');$(obj).find('ul li:nth-child(1)').find('span').addClass('selected');";
            $js .= "$(\"#content_weibo\").val('');$(\"#privacy\").val('0');$('#friendtrace').lazyshow({url:\"index.php?w=member_snshome&t=tracelist&curpage=1\",'iIntervalId':true});";
    ...
    public function sharegoodsWt(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            $goods_content = $goods_model->getGoodsOnlineInfoForShare(intval($_POST['choosegoodsid']));
            ...
            if ($result){
                ...
                if (empty($snsgoods_content)){
                    ...
                    //更新SNS商品
                    $update_arr = array();
                    $update_arr['snsgoods_sharenum'] = intval($snsgoods_content['snsgoods_sharenum'])+1;
                ...
                $content_str .= "<div class=\"fd-media\">
                    ...
                    <div class=\"goodsinfo\">
                        <dl>
                            ...
                            <dd wttype=\"collectbtn_{$goods_content['goods_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_goods(\'{$goods_content['goods_id']}\',\'succ\',\'collectbtn_{$goods_content['goods_id']}\');\">".Language::get('sns_sharegoods_collect')."</a></dd>
                        </dl>
                      </div>
                 </div>";
                ...
                if (C('share_isuse') == 1){
                    ...
                    $params['url'] = urlShop('goods' , 'index', array('goods_id'=>$goods_content['goods_id']));
                    $params['comment'] = $goods_content['goods_name'].$_POST['comment'];
                    $params['images'] = thumb($goods_content, 240);
                    //分享之qqweibo
                ...
                //输出js
                $js = "DialogManager.close('sharegoods');var countobj=$('[wt_type=\'sharecount_{$goods_content['goods_id']}\']');$(countobj).html(parseInt($(countobj).text())+1);";
                $url = '';
                if ($_GET['irefresh']){
                    $js .= "$('#friendtrace').lazyshow({url:\"index.php?w=member_snsindex&t=tracelist&curpage=1\",'iIntervalId':true});";
                }else{
                    $url = 'reload';
                }
                ...
                $content_str .= "<div class=\"fd-media\">
                    ...
                    <div class=\"goodsinfo\">
                        <dl>
                            ...
                            <dd wttype=\"storecollectbtn_{$store_info['store_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_store(\'{$store_info['store_id']}\',\'succ\',\'storecollectbtn_{$store_info['store_id']}\');\">".Language::get('sns_sharestore_collect')."</a></dd>
                        </dl>
                      </div>
                 </div>";
                $insert_arr['trace_content'] = $content_str;
    ...
    public function deltraceWt(){
        ...
        if ($result){
            ...
            if ($_GET['type'] == 'href'){
                showDialog(Language::get('wt_common_del_succ'),'index.php?w=member_snshome&t=trace&mid='.$_SESSION['member_id'],'succ');
            }else {
                $js = "location.reload();";
                showDialog(Language::get('wt_common_del_succ'),'','succ',$js);
            }
        } else {
    ...
    public function editprivacyWt(){
        ...
        if ($result){
            ...
            $js = "var obj = $(\"#recordone_{$id}\").find(\"[wt_type='privacytab']\"); $(obj).find('span').removeClass('selected');$(obj).find('li:nth-child(".$privacy_item.")').find('span').addClass('selected');";
    ...
    public function delgoodsWt(){
        ...
        $sharegoods_content = $sharegoods_model->getSharegoodsInfo($condition);
        if (empty($sharegoods_content)){
            showDialog(Language::get('wt_common_del_fail'),'','error');
        }
        unset($condition);
        ...
        if ($result){
            ...
            if ($_GET['type'] == 'like'){
                $snsgoods_model = Model('sns_goods');
                $snsgoods_content = $snsgoods_model->getGoodsInfo(array('snsgoods_goodsid'=>"{$sharegoods_content['share_goodsid']}"));
                if (!empty($snsgoods_content)){
                    $update_arr = array();
                    $update_arr['snsgoods_likenum'] = (intval($snsgoods_content['snsgoods_likenum'])-1)>0?(intval($snsgoods_content['snsgoods_likenum'])-1):0;
                    ...
                    $snsgoods_model->editGoods($update_arr,array('snsgoods_goodsid'=>"{$snsgoods_content['snsgoods_goodsid']}"));
                }
            }
            $js = "location.reload();";
            showDialog(Language::get('wt_common_del_succ'),'','succ',$js);
     ...
     * 编辑分享店铺的可见权限(主人登录后操作)
     */
    public function storeprivacyWt(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showDialog(Language::get('wrong_argument'),'','error');
        }
    ...
    public function addcommentWt(){
        ...
        if ($originaltype == 1){
            //查询分享和喜欢商品信息
            $sharegoods_model = Model('sns_sharegoods');
            $sharegoods_content = $sharegoods_model->getSharegoodsInfo(array('share_id'=>"{$originalid}"));
            if (empty($sharegoods_content)){
                showDialog(Language::get('sns_comment_fail'),'','error');
            }
            $owner_id = $sharegoods_content['share_memberid'];
        }else {
            //查询原帖信息
            $tracelog_model = Model('sns_tracelog');
        ...
        if ($result){
            ...
            if (cookie('commentnum') != null && intval(cookie('commentnum')) >0){
                setWtCookie('commentnum',intval(cookie('commentnum'))+1,2*3600);//保存2小时
            }else{
                setWtCookie('commentnum',1,2*3600);//保存2小时
            }
            $js = "$(\"#content_comment{$originalid}\").val('');";
            if ($_POST['showtype'] == 1){
                $js .="$(\"#tracereply_{$originalid}\").load('index.php?w=member_snshome&t=commenttop&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }else {
                $js .="$(\"#tracereply_{$originalid}\").load('index.php?w=member_snshome&t=commentlist&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }
    ...
    public function delcommentWt(){
        ...
        if ($result){
            ...
            $js .="$('.comment-list [wt_type=\"commentrow_{$id}\"]').remove();";
            showDialog(Language::get('wt_common_del_succ'),'','succ',$js);
        }else {
            showDialog(Language::get('wt_common_del_fail'),'','error');
        }
    ...
    public function addforwardWt(){
        ...
        if ($tracelog_info['trace_originalid'] > 0 || $tracelog_info['trace_from'] != 1){
            ...
            $content_str ="<div class=\"title\"><a href=\"%siteurl%index.php?w=member_snshome&mid={$tracelog_info['trace_memberid']}\" target=\"_blank\" class=\"uname\">{$tracelog_info['trace_membername']}</a>";
            $content_str .= Language::get('wt_colon')."{$tracelog_info['trace_title']}</div>";
        ...
        if ($result){
            ...
            if ($_GET['irefresh']){
                ...
                if (cookie('forwardnum') != null && intval(cookie('forwardnum')) >0){
                    setWtCookie('forwardnum',intval(cookie('forwardnum'))+1,2*3600);//保存2小时
                }else{
                    setWtCookie('forwardnum',1,2*3600);//保存2小时
                }
                if ($_GET['type']=='home'){
                    $js = "$('#friendtrace').lazyshow({url:\"index.php?w=member_snshome&t=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else if ($_GET['type']=='snshome'){
                    $js = "$('#forward_".$originalid."').hide();$('#friendtrace').lazyshow({url:\"index.php?w=member_snshome&t=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else {
                    $js = "$('#friendtrace').lazyshow({url:\"index.php?w=member_snsindex&t=tracelist&curpage=1\",'iIntervalId':true});";
                }
     ...
     * 商品收藏页面和商品详细页面分享商品
     */
    public function sharegoods_oneWt(){
        Language::read('member_sharemanage');
        $gid = intval($_GET['gid']);
        if ($gid<=0){
            showDialog(Language::get('wrong_argument'),'','error');
        ...
        if ($_GET['dialog']){
            $js = "CUR_DIALOG = ajax_form('sharegoods', '".Language::get('sns_sharegoods_tofriend')."', 'index.php?w=member_snsindex&t=sharegoods_one&gid={$gid}', 480);";
            showDialog('','','js',$js);
        }
        //查询商品信息
        $goods_content = Model('goods')->getGoodsOnlineInfoForShare($gid);
     ...
     * 店铺收藏页面分享店铺
     */
    public function sharestore_oneWt(){
        Language::read('member_sharemanage');
        $sid = intval($_GET['sid']);
        if ($sid<=0){
            showDialog(Language::get('wrong_argument'),'','error');
        ...
        if ($_GET['dialog']){
            $js = "ajax_form('sharestore', '".Language::get('sns_sharestore')."', 'index.php?w=member_snsindex&t=sharestore_one&sid={$sid}', 480);";
            showDialog('','','js',$js);
        }
        //查询店铺信息
        $store_model = Model('store');
