...
class manageControl extends BasebbsManageControl{
    ...
    public function indexWt(){
        ...
        $this->bbsMemberInfo();
        if($this->identity == 2){
            @header("location: ".BBS_SITE_URL.'/index.php?w=manage&t=applying&c_id='.$this->c_id);
        }
        // 会员加入社区列表
        $this->memberJoinbbs();
        $model = Model();
        if(chksubmit()){
            $update = array();
            $update['bbs_id']    = $this->c_id;
            // 上传图片
            if (!empty($_FILES['c_img']['name'])){
                $upload = new UploadFile();
                $upload->set('default_dir', ATTACH_BBS.'/group');
                ...
                if ($result){
                    $update['bbs_img']   = $upload->thumb_image;
            ...
            $update['bbs_joinaudit'] = $_POST['c_joinaudit'];
            ...
            if($_POST['c_mapply'] == 1){
                ...
                $model->table('bbs_mapply')->where(array('bbs_id'=>$this->c_id))->delete();
            }

            $model->table('bbs')->where(array('bbs_id'=>$this->c_id))->update($update);
            showDialog(L('wt_common_op_succ'), 'reload', 'succ');
        }
        $bbs_info = $model->table('bbs')->where(array('bbs_id'=>$this->c_id))->find();
        Tpl::output('bbs_info', $bbs_info);

        // member level
        $ml_info = $model->table('bbs_ml')->where(array('bbs_id'=>$this->c_id))->find();
        if(empty($ml_info)){
            $mld_array = rkcache('bbs_level', true);
     ...
     * 成员管理
     */
    public function member_manageWt(){
        // 社区信息
        $this->bbsInfo();
        // 会员信息
        $this->bbsMemberInfo();
        ...
        $cm_list = $model->table('bbs_member')->where($where)
    ...
    public function settingWt(){
        ...
        if($key == 'is_allowspeak'){
            ...
            Model()->table('bbs_threply')->where($where)->update($update);
        }
        showDialog(L('wt_common_op_succ'), 'reload', 'succ');
    }
    /**
    ...
    public function delmemberWt(){
        ...
        if (chksubmit()) {
            ...
            // 更新社区成员数量
            $count = Model()->table('bbs_member')->where(array('bbs_id'=>$this->c_id, 'cm_state'=>1))->count();
            Model()->table('bbs')->where(array('bbs_id'=>$this->c_id))->update(array('bbs_mcount'=>$count));
    ...
    public function applyingWt(){
        ...
        $this->memberJoinbbs();
        // 成员列表
        $cm_list = Model()->table('bbs_member')->where(array('bbs_id'=>$this->c_id, 'cm_state'=>0))->order('is_identity asc,cm_jointime desc')->select();
        Tpl::output('cm_list', $cm_list);
     ...
     * 申请中成员操作
     */
    public function applying_manageWt(){
        // 身份验证
        ...
        $count = $model->table('bbs_member')->where(array('bbs_id'=>$this->c_id, 'cm_state'=>0))->count();
        $model->table('bbs')->where(array('bbs_id'=>$this->c_id))->update(array('new_verifycount'=>$count));
    ...
    public function classWt(){
        ...
        $model = Model();
        $thclass_list = $model->table('bbs_thclass')->where(array('bbs_id'=>$this->c_id))->order('thclass_sort asc')->select();
    ...
    public function class_addWt(){
        ...
        if(chksubmit()){
            // 超过10不能继续添加
            $count = Model()->table('bbs_thclass')->where(array('bbs_id'=>$this->c_id))->count();
            if($count >= 10){
                showDialog(L('bbs_tclass_max_10'));
            }
            ...
            $obj_validate->validateparam = array(
                    array("input"=>$_POST["name"], "require"=>"true", "message"=>L('bbs_tclass_name_not_null')),
                    array("input"=>$_POST["sort"], "require"=>"true", 'validator'=>'Number', "message"=>L('bbs_sort_error')),
            );
            ...
            if($error != ''){
                ...
                Model()->table('bbs_thclass')->insert($insert);
                showDialog(L('wt_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            }
        }
    ...
    public function class_editWt(){
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $update['bbs_id']        = $this->c_id;
                Model()->table('bbs_thclass')->where(array('thclass_id'=>intval($_POST['thc_id']), 'bbs_id'=>$this->c_id))->update($update);
                showDialog(L('wt_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            }
        }
        ...
        $thclass_info = Model()->table('bbs_thclass')->where(array('bbs_id'=>$this->c_id, 'thclass_id'=>$thc_id))->find();
    ...
    public function class_delWt(){
        ...
        Model()->table('bbs_thclass')->where($where)->delete();
        showDialog(L('wt_common_op_succ'), 'reload', 'succ');
    }
    /**
     * 友情社区
     */
    public function friendshipWt(){
        // 社区信息
        $this->bbsInfo();
        // 会员信息
        $this->bbsMemberInfo();
        ...
        $fs_list = Model()->table('bbs_fs')->where(array('bbs_id'=>$this->c_id))->order('friendship_sort asc')->select();
     ...
     * 添加友情社区
     */
    public function friendship_addWt(){
        // 身份验证
        $rs = $this->checkIdentity('c');
        if(!empty($rs)){
            showDialog($rs);
        ...
        if(chksubmit()){
            ...
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
                    array("input"=>$_POST["cid"], "require"=>"true", 'validator'=>'Number', "message"=>L('fbbs_please_choose')),
                    array("input"=>$_POST["sort"], "require"=>"true", 'validator'=>'Number', "message"=>L('bbs_sort_error')),
            );
            $error = $obj_validate->validate();
            if($error != ''){
                ...
                if($_POST['cid'] <= 0){
                    showDialog(L('fbbs_please_choose'));
                }
                $insert = array();
                $insert['bbs_id']        = $this->c_id;
                ...
                Model()->table('bbs_fs')->insert($insert);
                showDialog(L('wt_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            }
    ...
    public function search_bbsWt(){
        ...
        // 查询已加友情社区
        $bbs_list = $model->table('bbs_fs')->where(array('bbs_id'=>$this->c_id))->select();
        if (!empty($bbs_list)){
            $bbs_list = array_under_reset($bbs_list, 'friendship_id'); $bbs_array = array_keys($bbs_list);
        }

        $where = array();
        $where['bbs_name']   = array('like', '%'.$_GET['name'].'%');
        $where['bbs_status'] = 1;
        if(!empty($bbs_array)){
            $bbs_array[] = $this->c_id;
            $where['bbs_id'] = array('not in', $bbs_array);
        }else{
            $where['bbs_id'] = array('neq', $this->c_id);
        }
        $bbs_list = $model->table('bbs')->field('bbs_id,bbs_name')->where($where)->select();
        if (empty($bbs_list)) {
            echo 'false';die;
        }

        if (strtoupper(CHARSET) == 'GBK'){
            $bbs_list = Language::getUTF8($bbs_list);
        }
        echo json_encode($bbs_list);
    }
    /**
     * 编辑社区
     */
    public function friendship_editWt(){
        // 身份验证
        $rs = $this->checkIdentity('c');
        if(!empty($rs)){
            showDialog($rs);
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                Model()->table('bbs_fs')->where(array('bbs_id'=>$this->c_id, 'friendship_id'=>intval($_GET['fs_id'])))->update($update);
                showDialog(L('wt_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            }
        ...
        $friendship_info = Model()->table('bbs_fs')->where(array('bbs_id'=>$this->c_id, 'friendship_id'=>$fs_id))->find();
     ...
     * 删除友情社区
     */
    public function friendship_delWt(){
        // 身份验证
        $rs = $this->checkIdentity('c');
        if(!empty($rs)){
            showDialog($rs);
        ...
        Model()->table('bbs_fs')->where($where)->delete();
        showDialog(L('wt_common_op_succ'), 'reload', 'succ');
    }
    /**
     * 加精/取消加精
    ...
    public function ajaxWt(){
        ...
        Model()->table('bbs_theme')->where(array('theme_id'=>intval($_GET['id'])))->update($update);
        echo 'true';exit;
    }
    /**
     * 编辑话题
    ...
    public function edit_themeWt(){
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('bbs_theme')->where(array('theme_id'=>$t_id))->update($update);
            if($rs){
                $has_goods = 0; // 存在商品标记
                $has_affix = 0;// 存在附件标记
                // 删除原有商品
                $goods_list = Model()->table('bbs_thg')->where(array('theme_id'=>$t_id, 'reply_id'=>0))->delete();
                // 插入话题商品
                ...
                // 更新话题信息
                $affixe_count = $model->table('bbs_affix')->where(array('affix_type'=>1, 'theme_id'=>$t_id))->count();
                if($affixe_count > 0){
                    $has_affix = 1;
                }
                if($has_goods || $has_affix){
                    ...
                    $model->table('bbs_theme')->where(array('theme_id'=>$t_id))->update($update);
                }
                // Special theme
                if($_GET['sp'] == 1){
                    // Update the vote
                    ...
                    $model->table('bbs_thpoll')->where(array('theme_id'=>$t_id))->update($update);

                    // Update the voting options
                    if(!empty($_POST['polloption'])){
                        $insert_array = array();
                        foreach ($_POST['polloption'] as $key=>$val){
                            $option_info = $model->table('bbs_thpolloption')->where(array('pollop_id'=>$key, 'theme_id'=>$t_id))->find();
                ...
                showDialog(L('wt_common_op_succ'), BBS_SITE_URL.'/index.php?w=theme&t=theme_detail&c_id='.$this->c_id.'&t_id='.$t_id, 'succ');
            }else{
        ...
        // 话题信息
        $this->theme_info = $model->table('bbs_theme')->where(array('theme_id'=>$t_id, 'bbs_id'=>$this->c_id))->find();
        ...
        // 话题商品
        $goods_list = $model->table('bbs_thg')->where(array('theme_id'=>$t_id, 'reply_id'=>0))->select();
        $goods_list = tidyThemeGoods($goods_list, 'themegoods_id');
        Tpl::output('goods_list', $goods_list);

        // 话题附件
        $affix_list = $model->table('bbs_affix')->where(array('affix_type'=>1, 'theme_id'=>$t_id))->select();
        ...
        if($this->theme_info['theme_special'] == 1){
            $poll_info = $model->table('bbs_thpoll')->where(array('theme_id'=>$t_id))->find();
            Tpl::output('poll_info', $poll_info);
            $option_list = $model->table('bbs_thpolloption')->where(array('theme_id'=>$t_id))->order('pollop_sort asc')->select();
    ...
    public function delimgWt(){
        ...
        // 验证是否为该社区附件
        $theme_info = Model()->table('bbs_theme')->where(array('bbs_id'=>$this->c_id, 'theme_id'=>$affix_info['theme_id']))->find();
        if(empty($theme_info)){
            echo false;exit;
        }
    ...
    public function del_themeWt(){
        ...
        // 验证话题
        $t_id = intval($_GET['t_id']);
        $theme_info = $model->table('bbs_theme')->where(array('theme_id'=>$t_id, 'bbs_id'=>$this->c_id))->find();
        if(empty($theme_info)){
            showDialog(L('wrong_argument'));
        }

        // 删除附件
        $affix_list = $model->table('bbs_affix')->where(array('theme_id'=>$t_id))->select();
        if(!empty($affix_list)){
            foreach ($affix_list as $val){
                @unlink(themeImagePath($val['affix_filename']));
                @unlink(themeImagePath($val['affix_filethumb']));
            }
            $model->table('bbs_affix')->where(array('theme_id'=>$t_id))->delete();
        }

        // 删除商品
        $model->table('bbs_thg')->where(array('theme_id'=>$t_id))->delete();

        // 删除赞表相关
        $model->table('bbs_like')->where(array('theme_id'=>$t_id))->delete();

        // 删除回复
        $model->table('bbs_threply')->where(array('theme_id'=>$t_id))->delete();
        ...
        // 删除话题
        $model->table('bbs_theme')->where(array('theme_id'=>$t_id))->delete();
        ...
        showDialog(L('wt_common_op_succ'), BBS_SITE_URL.'/index.php?w=group&c_id='.$this->c_id, 'succ');
    }
    /**
     * 删除回复
     */
    public function del_replyWt(){
        ...
        $where['reply_id']  = $r_id;
        $where['bbs_id'] = $this->c_id;
        $reply_info = $model->table('bbs_threply')->where($where)->find();
        if(empty($reply_info)){
            showDialog(L('bbs_reply_not_exists'), 'reload');
        }

        // 删除附件
        $affix_list = $model->table('bbs_affix')->where(array('affix_type'=>2, 'theme_id'=>$t_id, 'reply_id'=>$r_id))->select();
        if(!empty($affix_list)){
            foreach ($affix_list as $val){
                @unlink(themeImagePath($val['affix_filename']));
                @unlink(themeImagePath($val['affix_filethumb']));
            }
            $model->table('bbs_affix')->where(array('affix_type'=>2 ,'theme_id'=>$t_id, 'reply_id'=>$r_id))->delete();
        }

        // 删除商品
        $model->table('bbs_thg')->where(array('theme_id'=>$t_id, 'reply_id'=>$r_id))->delete();
        ...
        // 删除回复
        $rs = $model->table('bbs_threply')->where(array('theme_id'=>$t_id, 'reply_id'=>$r_id))->delete();
        // 更新话题回复数
        $model->table('bbs_theme')->where(array('theme_id'=>$t_id))->update(array('theme_commentcount'=>array('exp', 'theme_commentcount-1')));
        // Experience
     ...
     * ajax禁言
     */
    public function ajax_nospeakWt(){
        // 身份验证
        $rs = $this->checkIdentity('cm');
        if(!empty($rs)){
            showDialog($rs);
        ...
        // 圈主和管理不能被禁言
        $m_info = Model()->table('bbs_member')->where($where)->find();
        if(in_array($m_info['is_identity'], array(1,2))){
            showDialog(L('bbs_manager_shutup_error'));
        }
