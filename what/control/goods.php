...
class goodsControl extends MircroShopControl{
    ...
    public function listWt() {
        ...
        Tpl::output('html_title',Language::get('wt_what_goods').'-'.Language::get('wt_what').'-'.C('site_name'));
    ...
    public function detailWt() {
        ...
        $detail = $model_what_goods->getOneWithUserInfo($condition);
        if(empty($detail)) {
            header('location: '.WHAT_SITE_URL);die;
        }
        ...
        if(empty($goods_image_list)) {
            $goods_content = $model_goods->getGoodsCommonInfoByID($detail['commend_goods_commonid']);
            $goods_image_list[0]['goods_image'] = $goods_content['goods_image'];
        }
        ...
        $model_what_goods->modify($update,$condition);
        ...
        $store_info['hot_sales_list'] = $model_store->getHotSalesList($detail['commend_goods_store_id'], 6);
        ...
        Tpl::output('html_title',$detail['commend_goods_name'].'-'.Language::get('wt_what_goods').'-'.Language::get('wt_what').'-'.C('site_name'));
