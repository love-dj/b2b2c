...
<div class="side-message">
  ...
  <div class="guest-form">
    <form id="send_form" action='<?php echo MEMBER_SITE_URL;?>/index.php?w=member_message&t=savemsg&type=sns_board' method="post">
      ...
      <div class="action"> <a class="face" href="javascript:void(0);" data-param='{"txtid":"msg"}' wt_type="smiliesbtn"><?php echo $lang['sns_smiles'];?></a><a href="javascript:void(0);" wttype="commentbtn" class="btn"><?php echo $lang['sns_speak'];?></a><span class="charcount"><em id="commentcharcount"></em>/140</span></div>
    </form>
  </div>
  <?php }?>
  <div wttype="message_list" class="message-list">
    <?php if(!empty($output['message_list'])){?>
    <?php foreach($output['message_list'] as $val){?>
    <dl wttype="dl<?php echo $val['message_id'];?>">
      <dt><a href="index.php?w=member_snshome&mid=<?php echo $val['from_member_id'];?>" data-param="{'id':<?php echo $val['from_member_id'];?>}" wttype="mcard"><?php echo $val['from_member_name'].$lang['wt_colon'];?></a><span><?php echo parsesmiles($val['message_body']);?></span></dt>
      <dd data-param="{'msgid':'<?php echo $val['message_id'];?>','fname':'<?php echo $val['from_member_name'];?>'}"><span class="time"><?php echo $val['message_time'];?></span><span class="handle">
        <?php if($output['relation'] == 3){?>
        <a href="javascript:void(0);" wttype="reply_msg"><?php echo $lang['sns_reply'];?></a>|
        <?php }?>
        <?php if($output['relation'] == 3 || $val['from_member_id'] == $_SESSION['member_id']){?>
        <a  onclick="ajax_get_confirm('<?php echo $lang['wt_common_op_confirm'];?>', '<?php echo MEMBER_SITE_URL;?>/index.php?w=member_message&t=dropcommonmsg&drop_type=sns_msg&message_id=<?php echo $val['message_id'];?>');" href="javascript:void(0);"><?php echo $lang['wt_delete'];?></a>
        <?php }?>
        </span> </dd>
      ...
      <dd>
        ...
        <dl class="re-content">
          <dt><a href="index.php?w=member_snshome&mid=<?php echo $v['from_member_id'];?>"><?php echo $v['from_member_name'];?><?php echo $lang['sns_reply'].$lang['wt_colon'];?></a><span><?php echo parsesmiles($v['message_body']);?></span></dt>
...
$(function(){
	...
	// 回复提交
	$("[wttype='commentbtn']").live('click',function(){
		if($("#send_form").valid()){
	...
	// 回复
	$('a[wttype="reply_msg"]').live('click', function(){
		...
		if(!p_dd.next().is('dd[nctyoe="replyform"]')){
			$('<dd nctyoe="replyform" class="re-msg"></dd>')
				.append('<i></i>')
				.append('<form id="replyform'+data.msgid+'" action="<?php echo MEMBER_SITE_URL;?>/index.php?w=member_message&t=savereply" method="post"></form>').children('form')
				...
				.append('<a class="face" wt_type="smiliesbtn" data-param=\'{"txtid":"msg'+data.msgid+'"}\' href="javascript:void(0);"><?php echo $lang['sns_smiles'];?></a>')
				.append('<a wt_type="commentbtn'+data.msgid+'" class="btn" href="javascript:void(0);"><?php echo $lang['wt_submit'];?></a>')
			...
			// 回复提交
			$("[wt_type='commentbtn"+data.msgid+"']").live('click',function(){
...
function leaveMsgSuccess(data){
	$('<dl></dl>')
		.append('<dt><a href="index.php?w=member_snshome&mid='+data.from_member_id+'">'+data.from_member_name+'<?php echo $lang['wt_colon'];?></a><span>'+data.msg_content+'</span></dt>')
		.append('<dd data-param="{\'msgid\':\''+data.msg_id+'\',\'fname\':\''+data.from_member_name+'\'}"><span class="time"><?php echo $lang['sns_just'];?></span><span class="handle"><a href="javascript:void(0);"onclick="ajax_get_confirm(\'<?php echo $lang['wt_common_op_confirm'];?>\', \'<?php echo MEMBER_SITE_URL;?>/index.php?w=member_message&t=dropcommonmsg&drop_type=sns_msg&message_id='+data.msg_id+'\');"><?php echo $lang['wt_delete'];?></a></span></dd>')
		.prependTo('div[wttype="message_list"]');
	$('div[wttype="message_list"]').children('div').hide();
}

function replyMsgSuccess(data){
	$('dd[nctyoe="replyform"]').remove();
	var to = $('dl[wttype="dl'+data.message_parent_id+'"]').children('dd:last');
	$('<dl class="re-content"></dl>')
		.append('<dt><a href="index.php?w=member_snshome&mid='+data.from_member_id+'">'+data.from_member_name+'<?php echo $lang['sns_reply'].$lang['wt_colon'];?></a><span>'+data.msg_content+'</span></dt>')
