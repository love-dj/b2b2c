...
$(function() {
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + "/index.php?w=store&t=store_info",
        ...
        success: function(result) {
			if (result.datas.error) {
					...
					setTimeout("location.href = WapSiteUrl+'/shop.html'",3000);
					return false;
             }
        	var htmla = '';
            var data = result.datas;
                var urlstore = WapSiteUrl+"/html/store.html?store_id=" + data.store_info.store_id;
	    ...
	    if (!data.store_info.node_chat) {
		    $('.store_kefu').hide();
		    $('.wtm-store-bottom li').css('width','30%');
	    }
            //联系客服
            $('#store_kefu').click(function(){
                window.location.href = WapSiteUrl+'/html/member/chat_info.html?t_id=' + result.datas.store_info.member_id;
            });
			//联系客服
...
if(data.store_info.mb_store_menu.on == 1){
    ...
    $('#goods_rank_tab').find('a').click(function(){
        ...
        $.ajax({
            type: 'post',
            url: ApiUrl + '/index.php?w=store&t=store_goods_rank',
    ...
    function getStoreNewGoods(){
        ...
        load_class_newgoods.loadInit({'url':ApiUrl + '/index.php?w=store&t=store_new_goods','getparam':param,'tmplid':'newgoods_tpl','containerobj':$("#newgoods"),'iIntervalId':true,'resulthandle':'tidyStoreNewGoodsData'});
    }
    //加载店铺促销活动
    function getStoreActivity(){
        $.ajax({
            type: 'post',
            url: ApiUrl + "/index.php?w=store&t=store_sale",
    ...
    $("#store_voucher").click(function(){
        if (!$("#store_voucher_con").html()) {
            $.ajax({
                type: 'post',
                url: ApiUrl + '/index.php?w=voucher&t=voucher_tpl_list',
