...
.release-tab li em { /* background:url(../images/member/wtus_public.png) no-repeat scroll; */ _background-image: url(../images/ie6/wtus_public.gif)/*IE6*/; float:left; height: 26px; width: 26px;}
...
.release-content .arrow { /* background: url(../images/member/wtus_public.png) scroll no-repeat -90px -400px; */ _background-image: url(../images/ie6/wtus_public.gif)/*IE6*/; width: 18px; height: 10px; position: absolute; z-index: 1; top: 6px; left: 48px;}
...
.release-content .smile em { /* background: url(../images/member/wtus_public.png) no-repeat scroll -182px -380px; */ _background-image: url(../images/ie6/wtus_public.gif)/*IE6*/; width: 20px; height: 20px; float: left;}
...
<?php if ($output['is_owner']){ ?>
    ...
    <div class="release-content"><span class="arrow"></span>
      <form id="weiboform" method="post" action="index.php?w=member_snsindex&t=addtrace">
        <textarea name="content" id="content_weibo" wt_type="contenttxt" class="textarea"resize="none"></textarea>
        <span class="error"></span>
        <div class="smile"><em></em><a href="javascript:void(0)" wt_type="smiliesbtn" data-param='{"txtid":"weibo"}'>表情</a></div>
        <div id="weibocharcount" class="weibocharcount"></div>
        <div id="weiboseccode" class="weiboseccode">
          <label for="captcha" class="ml5 fl"><strong><?php echo $lang['wt_checkcode'].$lang['wt_colon'];?></strong></label>
          <input name="captcha" class="w40 fl text" type="text" id="captcha" size="4" maxlength="4"/>
          <a href="javascript:void(0)" class="ml5 fl"><img src="" title="<?php echo $lang['wrong_checkcode_change'];?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?w=vercode&type=30x92&c=' + Math.random()" /></a>
          <input type="hidden" name="wthash" value="<?php echo $output['wthash'];?>"/>
        </div>
        <div class="handle">
          <div wt_type="formprivacydiv" class="privacy-module"><span class="privacybtn" style="width:55px;" wt_type="formprivacybtn"><i></i>所有人</span>
            <div class="privacytab" wt_type="formprivacytab" style="display:none;">
              <ul class="menu-bd">
                <li wt_type="formprivacyoption" data-param='{"v":"0"}'><span class="selected">所有人可见</span></li>
                <li wt_type="formprivacyoption" data-param='{"v":"1"}'><span>仅好友可见</span></li>
                <li wt_type="formprivacyoption" data-param='{"v":"2"}'><span>仅自己可见</span></li>
              </ul>
            </div>
          </div>
        ...
        <!-- 防止点击Enter键提交 -->
        <input name="<?php echo $lang['wt_snsshare'];?>" type="button" class="button" value="<?php echo $lang['wt_snsshare'];?>" id="weibobtn" />
      </form>
      <!-- 表情弹出层 -->
...
$(function(){
	...
	$('#friendtrace').lazyshow({url:"index.php?w=member_snshome&t=tracelist&mid=<?php echo $output['master_info']['member_id'];?>&page=1",'iIntervalId':true});
	...
	$("#weibobtn").bind('click',function(){			
		if($("#weiboform").valid()){
			...
			if(cookienum >= max_recordnum && $("#weiboseccode").css('display') == 'none'){
				//显示验证码
				$("#weiboseccode").show();
				$("#weiboseccode").find("#codeimage").attr('src','index.php?w=vercode&type=30x92&c=' + Math.random());
