...
class channelControl extends BaseHomeControl {
     ...
     *
     */
    public function get_right_listWt(){
        $gc_id = intval($_GET['gc_id']);
        $condition = array();
        $condition['gc_id'] = $gc_id;
        $condition['goods_sale_type'] = 1;//促销类型 0无促销，1抢购，2限时折扣
        ...
        $condition['is_vr'] = 0;
        $group_list = $model_robbuy->getRobbuyOnlineList($condition, 5, 'recommended desc');
        Tpl::output('group_list', $group_list);

        if (empty($group_list)) {//无抢购数据时调用限时折扣
            $condition = array();
            $condition['gc_id'] = $gc_id;
            $condition['goods_sale_type'] = 2;//促销类型 0无促销，1抢购，2限时折扣
            $model_goods = Model('goods');
            $field = 'goods_id';
            $goods_list = array();
