...
class store_goods_classControl extends BaseSellerControl {
     ...
     * @return
     */
    public function goods_class_saveWt() {
        $model_class    = Model('store_goods_class');
        if(isset($_POST['stc_id'])) {
            ...
            if($state) {
                showDialog(Language::get('wt_common_save_succ'),'index.php?w=store_goods_class&t=index','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            } else {
     ...
     * @return
     */
    public function drop_goods_classWt() {
        $model_class    = Model('store_goods_class');
        ...
        if ($drop_state){
            showDialog(Language::get('wt_common_del_succ'),'index.php?w=store_goods_class&t=store_goods_class','succ');
        }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'store_goods_class':
                $menu_array = array(
                1=>array('menu_key'=>'store_goods_class','menu_name'=>'店铺分类',   'menu_url'=>'index.php?w=store_goods_class&t=store_goods_class'));
                break;
        }
