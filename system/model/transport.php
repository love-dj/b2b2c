...
class transportModel extends Model {
    ...
    public function isUsing($id){
        ...
        $goods_content = $this->table('goods')->where(array('transport_id'=>$id))->field('goods_id')->find();
        return $goods_content ? true : false;
    }
            ...
            if (!empty($_array)) {
                ...
                if ($transport['goods_fee_type']==1 && $_array['xnum']>0) {//运费承担:1买家承担2是商家承担
                    foreach ($goods_list as $goods_content) {
                        $_n = intval($goods_content['n']);//购买数量
                        $_v = $goods_content['v'];//商品(重量/体积)
                        if ($_type==1) $_v = 1;
                        $_m += $_n*$_v;
                    }
     ...
     * 商品运费
     */
    public function goods_trans_calc($goods_content, $area_id) {
        ...
        if (empty($transport) || empty($extend_list)) {
            ...
            $calc_total = false;
            $_n = intval($goods_content['goods_n']);//购买数量
            $_v = $goods_content['goods_trans_v'];//商品(重量/体积)
            $_type = $transport['goods_trans_type'];//计费规则:1是按件数2是按重量3是按体积
