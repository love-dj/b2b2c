...
class team_levelControl extends SystemControl{
    ...
    public function get_xmlWt() {
    	$model = Model('team_level');
    	$condition = array();
    	if ($_POST['query'] != '') {
    		$condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
    	}
    	$order = '';
    	$param = array('id', 'level_weight', 'level_name', 'layer_rate', 'commission_layers', 'same_layers', 'same_layer_rate');
        ...
        foreach ($list as $value) {            
        	...
        	$operation .= "<li><a href='index.php?w=team_level&t=edit_level&id=".$value['id']."'>编辑</a></li>";            
        	$operation .= "</ul></span>";
        	$param['operation']     = $operation;
        	$param['level_weight']  = $value['level_weight'];
        	$param['level_name']    = $value['level_name'];
        	$param['layer_rate']     = $value['layer_rate'];
        	$param['commission_layers']     = $value['commission_layers'];
            $param['same_layers']   = $value['same_layers'];
            $param['same_layer_rate']   = $value['same_layer_rate'];
            // $param['same_layer_condition']   = $value['same_layer_condition'];
    ...
    public function add_levelWt(){
    	if(chksubmit()){
    		...
    		$obj_validate->validateparam = array(
    			...
    			array("input"=>$_POST["layer_rate"], "require"=>"true"),
    			array("input"=>$_POST["commission_layers"], "require"=>"true"),
                array("input"=>$_POST["same_layers"], "require"=>"true"),
    			array("input"=>$_POST["same_layer_rate"], "require"=>"true"),
                array("input"=>$_POST["same_layer_condition"], "require"=>"true"),
    		);
    		...
    		if ($error != ''){
                ...
                $param['layer_rate']       = trim($_POST['layer_rate']);
                $param['commission_layers']       = trim($_POST['commission_layers']);
                $param['same_layers']     = trim($_POST['same_layers']);
                $param['same_layer_rate']    = trim($_POST['same_layer_rate']);
                $param['same_layer_condition'] = serialize($_POST['upgrade_type']);
                $param['condition_value'] = serialize($_POST['upgrade_value']); 

                $team_level_model = Model('team_level');
    			$result = $team_level_model->insert($param);
    			if ($result){
    				$url = array(
    					array(
    						'url'=>'index.php?w=team_level&t=index',
    						'msg'=>'返回',
    					),
    					array(
    						'url'=>'index.php?w=team_level&t=add_level',
    						'msg'=>'继续新增分销等级',
    					),
    				);
    				dkcache('team_level');
    ...
    public function edit_levelWt(){
    	if(chksubmit()){
    		...
    		if ($error != ''){
    			...
    			$team_model             = Model('team_level');
    			$result = $team_model->where(array('id'=>$_POST['id']))->update($param);
    			if ($result){
    				$url = array(
    					array(
    						'url'=>'index.php?w=team_level&t=index',
    					...
    					array(
    						'url'=>'index.php?w=team_level&t=edit_level&id='.intval($_POST['id']),
    						'msg'=>'重新编辑该等级',
    					),
    				);
    				dkcache('team_level');
        ...
        if($level_array['same_layer_condition']){
            $upgrade_type_array = unserialize($level_array['same_layer_condition']); 
        }
