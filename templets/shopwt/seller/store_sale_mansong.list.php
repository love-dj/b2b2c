...
<?php if ($output['isOwnShop']) { ?>
  <a class="wtbtn wtbtn-mint" href="<?php echo urlShop('store_sale_mansong', 'mansong_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['mansong_add'];?></a>

<?php } else { ?>

  <?php if(!empty($output['current_mansong_quota'])) { ?>
  <a class="wtbtn wtbtn-mint" style="right:100px" href="<?php echo urlShop('store_sale_mansong', 'mansong_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['mansong_add'];?></a> <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_mansong', 'mansong_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_mansong', 'mansong_quota_add');?>" title=""><i class="icon-money"></i>购买套餐</a>
  <?php } ?>
<?php } ?>
...
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_mansong_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['wt_colon'];?></strong><strong style="color: #F00;"><?php echo date('Y-m-d H:i:s', $output['current_mansong_quota']['end_time']);?></strong>
  </li>
  <?php } else { ?>
...
<form method="get">
  <table class="search-form">
    <input type="hidden" name="w" value="store_sale_mansong" />
    <input type="hidden" name="t" value="mansong_list" />
    <tr>
      <td>&nbsp;</td>
...
<table class="wtsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle"><span><a href="index.php?w=store_sale_mansong&t=mansong_detail&mansong_id=<?php echo $val['mansong_id'];?>" class="btn-bluejeans"><i class="icon-th-list"></i>
        <p><?php echo $lang['wt_detail'];?></p>
        </a></span> <span><a wttype="btn_mansong_del" data-mansong-id="<?php echo $val['mansong_id'];?>" href="javascript:return void(0)" class="btn-grapefruit"><i class="icon-trash"></i>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('[wttype="btn_mansong_del"]').on('click', function() {
            if(confirm('<?php echo $lang['wt_ensure_cancel'];?>')) {
                var action = "<?php echo urlShop('store_sale_mansong', 'mansong_del');?>";
