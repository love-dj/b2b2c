...
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="w" value="order" />
...
<table class="wtsc-default-table">
  ...
  <tbody>
    ...
    <?php foreach ($order_info['extend_order_goods'] as $goods_content) { ?>
    <tr>
    <td class="bdl"></td>
      <td class="w70">
        <div class="goods-thumb"><a href="<?php echo $goods_content['goods_url'];?>" target="_blank"><img src="<?php echo $goods_content['image_url'] ?>"/></a></div></td>
        <td>
        <dl class="goods-content">
          <dt class="goods-name"><a href="<?php echo $goods_content['goods_url'];?>" target="_blank"><?php echo $goods_content['goods_name'];?></a></dt>
          <dd class="goods-spec"><?php echo $goods_content['goods_spec'];?></dd>
          <dd class="goods-type"><?php echo $goods_content['goods_type'] == 5 ? '赠品':''?></dd>
        </dl>
        </td>
      <td><em class="goods-price"><?php echo $goods_content['goods_price'];?></em></td>
      <td><?php echo $goods_content['goods_num'];?></td>
      <?php if ($_i == 0) { ?>
      <td rowspan="<?php echo $_cont;?>" class="bdl"><em class="order-amount"><?php echo wtPriceFormat($order_info['order_amount']);?></em></td>
      ...
      <td rowspan="<?php echo $_cont;?>" class="nscs-table-handle bdl bdr"><?php if ($order_info['order_state'] != ORDER_STATE_SUCCESS) { ?>
        ...
        <span><a href="javascript:void(0);" class="btn-bluejeans" onclick="javascript:ajax_form('pickup_parcel', '付款并自提', 'index.php?w=order&t=pickup_parcel&order_id=<?php echo $order_info['order_id'];?>&payment_code=<?php echo $order_info['payment_code']?>',900)"><i class="icon-truck"></i>
        <p>付款自提</p>
        </a></span>
        <?php } else { ?>
        <span><a href="javascript:void(0);" class="btn-bluejeans" onclick="javascript:ajax_form('pickup_parcel', '自提', 'index.php?w=order&t=pickup_parcel&order_id=<?php echo $order_info['order_id'];?>',900)"><i class="icon-truck"></i>
        <p>自提</p>
        </a></span>
        <?php } ?>
        <?php } ?></td>
