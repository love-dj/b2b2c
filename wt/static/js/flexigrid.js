	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if (!data)
					{
                    $(t).append('<table><tbody><tr><td colspan="100" class="no-data"><i class="fa fa-exclamation-bbs"></i>' + p.nomsg + '</td></tr></tbody></table>');
					return false;
					}
				...
				if (p.dataType=='json')
				{
				    if (typeof(data.rows) == 'undefined') {
	                    $(tbody).append('<tr><td colspan="100" class="no-data"><i class="fa fa-exclamation-bbs"></i>' + p.nomsg + '</td></tr>');
				    } else {
    					$.each
    					(
    					 data.rows,
		...
		if (p.columnControl && $('th',g.hDiv).length)
		{
			...
			.html('<span class="hBtn"><i class="fa fa-cog"></i></span>')
			.hover ( function () {
			 	$(g.nDiv).show(); 
