...
class brandControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ($brand_list as $value) {
            ...
            if ($_GET['type'] == 'apply') {
                ...
                $operation .= "<a class='btn blue' href='index.php?w=brand&t=brand_edit&brand_id=" . $value['brand_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            }
            ...
            $param['brand_recommend'] = $value['brand_recommend'] ==  '1' ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?w=brand&t=brand_add',
                            'msg'=>$lang['brand_add_again'],
                        ),
                        array(
                            'url'=>'index.php?w=brand&t=brand',
                            'msg'=>$lang['brand_add_back_to_list'],
                        )
                    );
                    $this->log(L('wt_add,brand_index_brand').'['.$_POST['brand_name'].']',1);
            ...
            if ($error != ''){
                ...
                if ($result){
					if (!empty($_POST['brand_bgpic']) && !empty($brand_info['brand_bgpic'])){
                        @unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.'/'.DS.$brand_info['brand_bgpic']);
                    }
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?w=brand&t=brand_edit&brand_id='.intval($_POST['brand_id']),
                    ...
                    $this->log(L('wt_edit,brand_index_brand').'['.$_POST['brand_name'].']',1);
                    showMessage($lang['wt_common_save_succ'],$url);
                }else {
                    $this->log(L('wt_edit,brand_index_brand').'['.$_POST['brand_name'].']',0);
                    showMessage($lang['wt_common_save_fail']);
                }
            }
        }
     ...
     * 删除品牌
     */
    public function brand_delWt(){
        $brand_id = intval($_GET['id']);
        if ($brand_id <= 0) {
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }
        Model('brand')->delBrand(array('brand_id' => $brand_id));
        $this->log(L('wt_delete,brand_index_brand').'[ID:'.$brand_id.']',1);
     ...
     * 审核 申请品牌操作
     */
    public function brand_apply_setWt(){
        $brand_id = intval($_GET['id']);
        if ($brand_id <= 0) {
    ...
    public function export_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?w=brand&t=brand');
