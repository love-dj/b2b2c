  ...
  <div class="wtm-favorite-store">
    <div class="store-info">
      ...
      <dl>
        ...
        <dd><?php echo $lang['favorite_message'].$lang['wt_colon'];?><span member_id="<?php echo $favorites['store']['member_id'];?>"></span>
          ...
          <?php if(!empty($favorites['store']['store_ww'])){?>
           <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $favorites['store']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid=<?php echo $favorites['store']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" style=" vertical-align: middle;" /></a>
          <?php }?>
        </dd>
        <dd>所在地：<?php echo $favorites['store']['area_info'];?></dd>
      </dl>
      <div class="handle"><a href="javascript:void(0);" wt_type="sharestore" data-param='{"sid":"<?php echo $favorites['store']['store_id'];?>"}' title="<?php echo $lang['wt_snsshare'];?>"><i class="icon-share-alt"></i></a><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['wt_ensure_del'];?>', 'index.php?w=member_favorite_store&t=delfavorites&type=store&fav_id=<?php echo $favorites['fav_id'];?>');" class="ml10" title="<?php echo $lang['wt_del'];?>"><i class="icon-trash"></i></a></div>
    </div>
    <div class="store-goods"><a href="<?php echo urlShop('member_favorite_store', 'more', array('store_id' => $favorites['store']['store_id']));?>" class="more">查看更多></a>
      <div class="show-tab" data-sid="<?php echo $favorites['store']['store_id'];?>"> 
        ...
        <a href="javascript:void(0)" class="<?php if ($favorites['goods']['sign'] == 'sale') {?>current<?php }?> <?php if (empty($favorites['goods']['sale'])) {?>disabled<?php }?>">优惠促销</a>
      ...
      <?php if (!empty($favorites['goods']['new'])) {?>
        <ul>
         <?php foreach ($favorites['goods']['new'] as $goods) {?>
          <li>
            <div class="goods-thumb"><a href="index.php?w=goods&goods_id=<?php echo $goods['goods_id'];?>" title="<?php echo $goods['goods_name'];?>" target="_blank"><img src="<?php echo thumb($goods, 240);?>" /></a></div>
            <p>￥<?php echo $goods['goods_sale_price'];?></p>
          </li>
          <?php }?>
        </ul>
      <?php }?>
      </div>
      <div class="show-list" <?php if ($favorites['goods']['sign'] != 'sale') {?>style="display:none;"<?php }?>>
      <?php if (!empty($favorites['goods']['sale'])) {?>
        <ul>
         <?php foreach ($favorites['goods']['sale'] as $goods) {?>
          <li>
            <div class="goods-thumb"><a href="index.php?w=goods&goods_id=<?php echo $goods['goods_id'];?>" title="<?php echo $goods['goods_name'];?>" target="_blank"><img src="<?php echo thumb($goods, 240);?>" /></a></div>
            <p>￥<?php echo $goods['goods_sale_price'];?></p>
          </li>
