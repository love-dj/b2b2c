...
class connect_apiHandle {
    ...
    public function getStateInfo() {
		...
		$state_array['connect_wap_wxauto'] = C('wap_weixin_auto');//wap的自动微信登录
    ...
    public function getWxOAuth2Url(){
        ...
        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$weixin_appid.
            '&redirect_uri='.urlencode(MOBILE_SITE_URL.'/index.php?w=connect&t=index').
     ...
     * @return string
     */
    public function getWxminiOAuth2Url(){
        $weixin_appid = C('wap_weixin_appid');
        
        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$weixin_appid.
            '&redirect_uri='.urlencode(MOBILE_SITE_URL.'/index.php?w=connect&t=get_wxmini').
            '&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect';
        return $api_url;
    }
        ...
        if($state == true) {
            ...
            switch ($log_type) {
                case '1':
                    ...
                    $log_msg .= '您的注册验证码是：'.$captcha.'，如非本人操作，请忽略本短信。';
		    	$s_data['apicodeid'] = 'SMS_130235151';
                    break;
                case '2':
                    if(C('sms_login') != 1) {
                        $state = false;
                ...
                case '3':
                    ...
                    $log_msg .= '您正在重置登录密码，验证码：'.$captcha.'，如非本人操作，请忽略本短信。';
                    $log_array['member_id'] = $member['member_id'];
                    $log_array['member_name'] = $member['member_name'];
		    $s_data['apicodeid'] = 'SMS_130235150';
                    break;
                default:
                    $state = false;
                    $msg = '参数错误';
            ...
            if($state == true){
                $sms = new Sms();
                $result = $sms->send($phone,$log_msg,$s_data);
                if($result){
    ...
    public function smsRegister($phone, $captcha, $password, $client){
        ...
        if($state == true){
            ...
            $num = substr($phone,-4);
            $member_name = $this->getMemberName('wt', $num);
			//ShopWT分销 会员邀请
    ...
    public function getSinaOAuth2Url($sina_type = ''){
        ...
        require_once (BASE_API_PATH.DS.'sina'.DS.'saetv2.ex.class.php');
        $oauth2 = new SaeTOAuthV2($client_id, $client_secret);
        
        $api_url = BASE_SITE_URL.'/api/sina/return_url.php';
        $sina_url = '';
    ...
    public function getQqOAuth2Url($qq_type = ''){
        ...
        require_once (BASE_API_PATH.DS.'qq'.DS.'oauth.qq.class.php');
        $oauth2 = new OauthQq($client_id, $client_secret);
        
        $api_url = BASE_SITE_URL.'/api/qq/return_url.php';
        $qq_url = '';
