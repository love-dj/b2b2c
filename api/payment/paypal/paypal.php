...
class paypal{
    ...
    public function submit(){
        ...
        $data_return_url    =SHOP_SITE_URL."/api/payment/paypal/return_url.php";//返回URL
        $data_pay_account   = $this->payment['payment_config']['paypal_account'];
        $currency_code      = $this->payment['payment_config']['paypal_currency'];
        $data_notify_url    = SHOP_SITE_URL."/api/payment/paypal/notify_url.php";
        $cancel_return      = SHOP_SITE_URL.'/';
        $invoice  = $data_order_id;

        if(empty($this->payment['payment_config']['sandbox']))
        {
