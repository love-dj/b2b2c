...
class goods_browseModel extends Model{
    ...
    public function getViewedGoodsList($member_id = 0,$shownum = 0) {
        ...
        if ($browselist){
            $goods_list_tmp = Model('goods')->getGoodsList(array('goods_id' => array('in', array_keys($browselist))), 'goods_id, goods_name, goods_sale_price, goods_sale_type, goods_marketprice, goods_image, store_id, gc_id, gc_id_1, gc_id_2, gc_id_3');
            $goods_list = array();
            ...
            foreach ((array)$browselist as $k=>$v){
                if ($goods_list[$k]){
                    $browselist_new[] = array(
                        ...
                        "goods_sale_price"=> $goods_list[$k]['goods_sale_price'],
                        "goods_sale_type"=>$goods_list[$k]['goods_sale_type'],
    ...
    public function saveViewedGoods($goods_id,$member_id = 0,$store_id = 0) {
        ...
        $goods_content = $model_goods->getGoodsInfoByID($goods_id, 'goods_id,gc_id,gc_id_1,gc_id_2,gc_id_3,store_id');
        if (!$store_id) {
            //查询会员的店铺信息
        ...
        if((!$goods_content) || $store_id == $goods_content['store_id']){//店铺浏览自己的商品不加入浏览历史
            return false;
        }
        $result = $this->addViewedGoodsToDatabase($goods_content,$member_id);
        if ($result) {
     ...
     * 浏览过的商品加入浏览历史数据库
     *
     * @param array $goods_content 商品详情
     * @param int $member_id 会员ID（一般传递$_SESSION['member_id']）
     * @return array
     */
    public function addViewedGoodsToDatabase($goods_content,$member_id) {
        if ((!$goods_content) || $member_id <= 0){
            return false;
        }
        $insert_arr = array();
        $insert_arr['goods_id']  = $goods_content['goods_id'];
        $insert_arr['member_id'] = $member_id;
        $insert_arr['browsetime'] = time();
        $insert_arr['gc_id']    = $goods_content['gc_id'];
        $insert_arr['gc_id_1']  = $goods_content['gc_id_1'];
        $insert_arr['gc_id_2']  = $goods_content['gc_id_2'];
        $insert_arr['gc_id_3']  = $goods_content['gc_id_3'];
        //删除当天已存在的该goods_id记录
        $stime = strtotime(date('Y-m-d',time()));
        $etime = strtotime(date('Y-m-d',time()))+86400;
        $this->delGoodsbrowse(array('goods_id'=>$goods_content['goods_id'],'member_id'=>$member_id,'browsetime'=>array('between',array($stime,$etime))));
        //新增浏览记录
    ...
    public function addViewedGoodsToCookie($goods_id){
        ...
        setWtCookie('viewed_goods', $vg_ca);
    }
}
