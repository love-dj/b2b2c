...
//布局换色设置
        ...
        $("div.bgSelector").toggle(300, function() {
            if ($(this).html() == '') {
                $(this).sColor({
                    ...
                    defaultItem: ($.cookie('bgColorSelectorPosition') != null) ? $.cookie('bgColorSelectorPosition') : 30  // 可选，第几个颜色的索引作为初始颜色，默认第1个颜色
                });
            }
        });//切换显示
    });    
    ...
    $('.nav-tabs').each(function(){
        $(this).find('dl > dt > a').each(function(i){
            ...
            $(this).click(function(){
                if ($('.admiwtp-container').hasClass('fold')) {
                    return;
                }
                $('.sub-menu').hide();
    ...
    $('#foldSidebar > i').click(function(){
        if ($('.admiwtp-container').hasClass('unfold')) {
            $(this).addClass('fa-indent').removeClass('fa-outdent');
            $('.sub-menu').removeAttr('style');
            $('.admiwtp-container').addClass('fold').removeClass('unfold');
        } else {
            ...
            $('.admiwtp-container').addClass('unfold').removeClass('fold');
        }
    });
    ...
    // 顶部各个模块切换
    $('.wt-module-menu').find('a').click(function(){
        if ($('.admiwtp-container').hasClass('unfold')) {
            $('.sub-menu').hide();
        }
        $('.wt-module-menu').find('li').removeClass('active');
    ...
    if ($.cookie('workspaceParam') == null) {
        // 默认选择第一个菜单
        $('.wt-module-menu').find('li:first > a').click();
    } else {
        openItem($.cookie('workspaceParam'));
    }

    // 导航菜单  显示
    $('a[wttype="map_on"],a[class="add-menu"]').click(function(){
        $('div[wttype="map_nav"]').show();
    });
    // 导航菜单 隐藏
    $('a[wttype="map_off"]').click(function(){
        $('div[wttype="map_nav"]').hide();
    });
    // 导航菜单切换
    ...
    $('div[data-param^="map-"]').find('i').click(function(){
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php?w=common&t=common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
                    $this.parent().removeClass('selected');
                    $('ul[wttype="quick_link"]').find('a[onclick="openItem(\'' + _value + '\')"]').parent().remove();
                }
            });
        } else {
            var _name = $this.prev().html();
            $.getJSON('index.php?w=common&t=common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
                    $this.parent().addClass('selected');
                    $('ul[wttype="quick_link"]').append('<li><a onclick="openItem(\'' + _value + '\')" href="javascript:void(0);">' + _name + '</a></li>');
                }
            });
        }
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : 'index.php?w=common&t=ajax_pending_matters',
                async : false,
    ...
    if (_commonPendingMatters > 0) {
        $('li[wttype="pending_matters"]').show().find('em').html($.cookie('commonPendingMatters'));
    }
});

// 点击菜单，iframe页面跳转
function openItem(param) {
    $('.sub-menu,.wt-module-menu').find('li').removeClass('active');
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '/' + data_str[0] + '/index.php?w=' + data_str[1]);
