...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?w=goods_recommend&t=index" title="返回商品分类推荐列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
  ...
  <form id="goods_form" method="post" action='index.php?w=goods_recommend&t=save'>
    <input type="hidden" name="form_submit" value="ok" />
    <input type="hidden" name="gc_name" id="gc_name" value="<?php echo $output['rec_info']['rec_gc_name']?>" />
    <div class="wtap-form-default" id="explanation">
      <dl class="row">
        <dt class="tit">
...
$(function(){
	$("#submitBtn").click(function(){
		if ($('input[name="goods_id_list[]"]').size() == 0) {
			$('#valid_recommend').rules('add',{
				 required: true,
				 messages: {required : '<i class="fa fa-exclamation-bbs"></i>请选择推荐商品'}
			});
		}else{
    ...
    $('#goods_search').on('click',function(){
    	...
    	if($("#goods_form").valid()){
    		...
    		<?php } ?>
    	    $('#show_recommend_goods_list').load('index.php?w=goods_recommend&t=get_goods_list&gc_id='+gc_id+'&goods_name='+$('#goods_name').val());
    	}
    });
	...
	$('#goods_form').validate({
        ...
        messages : {
        	gc_id : {
                required : '<i class="fa fa-exclamation-bbs"></i>请将分类选择完整'
            }
        }
    });
