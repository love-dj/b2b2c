...
class SystemControl{
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. ADMIN_SITE_URL .'/index.php?w=login&t=login');exit;
        }else {
    ...
    protected final function systemSetKey($user, $avatar = '', $avatar_compel = false) {
        setWtCookie('sys_key',encrypt(serialize($user),MD5_KEY),3600,'',null);
        if ($avatar_compel || $avatar != '') {
            setWtCookie('admin_avatar',$avatar,86400 * 365,'',null);
        }
    }
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        if (!defined('MODULE_NAME')) return true;
        if (is_array($permission[MODULE_NAME]) && in_array($w, $permission[MODULE_NAME])){
            return true;
        }
        showMessage(Language::get('wt_assign_right'),'','html','succ',0);
    }
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        foreach ($links as $k=>$v) {
            ...
            if (empty($array['t'])) $array['t'] = 'index';
            if (!$this->checkPermission($array)) continue;
            $href = ($array['t'] == $actived ? null : "href=\"{$file}?{$v['url']}\"");
            $class = ($array['t'] == $actived ? "class=\"current\"" : null);
