...
class expressControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ($list as $value) {
            ...
            $operation .= "<li><a href='index.php?w=express&t=edit_express&id=".$value['id']."'>编辑快递公司</a></li>";
            ...
            $param['e_state'] = $value['e_state'] == 1 ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $param['e_order'] = $value['e_order'] == 1 ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $param['e_zt_state'] = $value['e_zt_state'] == 1 ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function ajaxWt(){
        switch ($_GET['column']){
            case 'e_state':
                ...
                dkcache('express');
                $this->log(L('wt_edit,express_name,express_state').'[ID:'.intval($_GET['id']).']',1);
    ...
    function add_expressWt(){
		...
		$param['e_code_kdniao'] = trim($_POST['e_code_kdniao']);
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?w=express&t=index',
                            'msg'=>'返回快递公司列表',
                        ),
                        array(
                            'url'=>'index.php?w=express&t=add_express',
                            'msg'=>'继续新增快递公司',
                        ),
                    );
                    dkcache('express');
		...
		$param['e_code_kdniao'] = trim($_POST['e_code_kdniao']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?w=express&t=edit_express&id='.intval($_POST['id']),
                            'msg'=>'重新编辑该快递公司',
                        ),
                    );
                    dkcache('express');
