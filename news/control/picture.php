...
class pictureControl extends NEWSHomeControl{
     ...
     * 图刊详细页
     */
    public function picture_detailWt() {
        ...
        if(empty($picture_detail)) {
            showMessage(Language::get('picture_not_exist'), NEWS_SITE_URL, '', 'error');
        }
        ...
        $news_tag_list = $model_tag->getList(TRUE, null, 'tag_sort asc', '', 10);
        $news_tag_list = array_under_reset($news_tag_list, 'tag_id');
        Tpl::output('news_tag_list', $news_tag_list);
     ...
     * 图刊评论
     */
    public function picture_comment_detailWt() {
        $picture_id = intval($_GET['picture_id']);
        if($picture_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
     ...
     * 图刊详细页(图片列表)
     */
    public function picture_detail_imageWt() {
        $picture_id = intval($_GET['picture_id']);
        if($picture_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
     ...
     * 图刊搜索
     */
    public function picture_searchWt() {
        $condition = array();
     ...
     * 根据标签搜索
     */
    public function picture_tag_searchWt() {
        $picture_list = array();
