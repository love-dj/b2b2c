...
function show($ap_id, $type = 'js'){
	...
	$list = $ap_info['show_list'];unset($ap_info['show_list']);
	extract($ap_info);
	if($is_use !== '1'){
	    return ;
	}
	$show_list = array();
	$show_info = array();//异步调用的数组格式
    ...
    if(empty($show_list)){
        if($ap_class == '1'){//文字广告
	        ...
	        $content .= UPLOAD_SITE_URL."/".ATTACH_SHOW."/".$default_content;
            $content .= "' alt='' rel='lazy' />";
	        $content .= "</a>";
	        $show_info['show_title'] = $ap_name;
	        $show_info['show_img'] = UPLOAD_SITE_URL."/".ATTACH_SHOW."/".$default_content;
	        $show_info['show_url'] = '';
        }
    }else {
        $select = 0;
        if($ap_display == '1'){//多广告展示
            $select = array_rand($show_list);
        }
        $show_select = $show_list[$select];
        extract($show_select);
		//图片广告
		if($ap_class == '0'){
			$width   = $ap_width;
			$height  = $ap_height;
			$pic_content = unserialize($show_content);
		...
		//文字广告
		if($ap_class == '1'){
			$word_content = unserialize($show_content);
			$word    = $word_content['show_word'];
			$url     = $word_content['show_word_url'];
			$content .= "<a href='http://".$pic_content['show_word_url']."' target='_blank'>";
			$content .= $word;
			$content .= "</a>";
		}
        //Flash广告
		if($ap_class == '3'){
			$width   = $ap_width;
			$height  = $ap_height;
			$flash_content = unserialize($show_content);
			...
			$content .= UPLOAD_SITE_URL."/".ATTACH_SHOW."/".$flash;
			...
			$content .= STATIC_SITE_URL."/js/expressInstall.swf'/><!-- 下一个对象标签用于非 IE 浏览器。所以使用 IECC 将其从 IE 隐藏。 --><!--[if !IE]>--><object type='application/x-shockwave-flash' data='";
			$content .= UPLOAD_SITE_URL."/".ATTACH_SHOW."/".$flash;
