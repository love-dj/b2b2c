...
class commision_listControl extends BaseSellerControl {
	...
	public function indexWt() {
	   ...
	   foreach($CLists as $key=>$val){
	   		$goods_content = $model_goods->getGoodsInfo(array('goods_id'=>$val['goods_id']));
	   		$CLists[$key]['goods_name'] = $goods_content['goods_name'];
	   }
	...
	public function getStoreId($goods_id){
		...
		$model_goods = Model('goods');
		$goods_content = $model_goods->getGoodsInfo(array('goods_id'=>$goods_id));
		return $goods_content['store_id'];
	} 
}
