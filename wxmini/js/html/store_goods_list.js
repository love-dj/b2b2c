...
var load_class_storegoodslist = new wtScrollLoad();
var isload_goods = false;

$(function() {
    // 商品展示形式
    $('#show_style').click(function(){
        var product_content_obj = $('[wt_type="product_content"]');
    ...
    $('#product_list').on('click', '[wt_type="goods_more_link"]',function(){
        ...
        $.ajax({
            type: 'post',
            url: ApiUrl + "/index.php?w=member_favorites&t=favorites_info",
            data: {key:key, fav_id:goods_id},
            dataType: 'json',
            success: function(result) {
                if (result.datas.favorites_info) {
                    $("#goods_more_"+goods_id+" [wt_type='goods_cancelfav']").show();
                    $("#goods_more_"+goods_id+" [wt_type='goods_addfav']").hide();
                }else{
                    $("#goods_more_"+goods_id+" [wt_type='goods_cancelfav']").hide();
                    $("#goods_more_"+goods_id+" [wt_type='goods_addfav']").show();
                }
    ...
    }).on('click', '[wt_type="goods_more_con"]', function(){
        var goods_id = $(this).attr('param_id');
        $("#goods_more_"+goods_id).hide();
    }).on('click', '[wt_type="goods_addfav"]',function(){
        var goods_id = $(this).attr('param_id');
        favoriteGoods(goods_id);
        $(this).hide();
        $("#goods_more_"+goods_id+" [wt_type='goods_cancelfav']").show();
    }).on('click', '[wt_type="goods_cancelfav"]',function(){
        var goods_id = $(this).attr('param_id');
    ...
    $('#search_submit').click(function(){
        ...
        //隐藏筛选页面
        $(".wtm-full-mask").addClass('hide').removeClass('left');
    });
...
function get_list(change_param, is_init){
    ...
    load_class_storegoodslist.loadInit({
        'url':ApiUrl + '/index.php?w=store&t=store_goods',
        'getparam':param,
