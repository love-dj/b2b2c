...
<div class="addshare">
<div class="goods-pic"><span class="thumb size120"><i></i><img src="<?php echo cthumb($output['sharegoods_content']['snsgoods_goodsimage'], 240,$output['sharegoods_content']['snsgoods_storeid']);?>"/></span></div>
  <ul id="goods_images">
    <?php for($i=0;$i<5;$i++){?>
    <li wt_type="handle_pic" data-param="{'apid':'<?php echo isset($output['pic_list'][$i])?$output['pic_list'][$i]['ap_id']:0;?>'}" >
      ...
      <div class="bg" wt_type="handler" style="display:none;"><span class="delete" title="<?php echo $lang['wt_delete'];?>" wttype="drop_image"><?php echo $lang['wt_delete'];?></span></div>
...
$(function(){
	...
	$('span[wttype="drop_image"]').click(function(){
		var p_li = $(this).parents('li:first');
		var data_str; eval( "data_str = "+p_li.attr('data-param'));
		$.getJSON('index.php?w=member_snshome&t=del_sharepic', {apid:data_str.apid}, function(data){
			if(data.type == 'false'){
				...
				p_li.find('*[wt_type="handler"]').hide().css('z-index', '0');
				img_handle();
			}
		});
	});
...
function ajaxFileUpload(id){
	...
	$.ajaxFileUpload
	({
		url:SITEURL + '/index.php?w=member_snshome&t=image_upload',
		secureuri:false,
		fileElementId:id,
		dataType: 'json',
