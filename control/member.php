...
class memberControl extends BaseMemberControl{
    ...
    public function ajax_load_member_infoWt() {
    ...
    public function ajax_load_order_infoWt() {
    ...
    public function ajax_load_goods_contentWt() {
        //商品收藏
        ...
        if (!empty($favorites_list) && is_array($favorites_list)){
            ...
            $field = 'goods_id,goods_name,store_id,goods_image,goods_sale_price';
        ...
        Tpl::showpage('member_home.goods_content','null_layout');
    }

    public function ajax_load_sns_infoWt() {
        //我的足迹
        ...
        //我的社区
        $model = Model();
        $bbsmember_array = $model->table('bbs_member')->where(array('member_id'=>$_SESSION['member_id']))->select();
        if(!empty($bbsmember_array)) {
            $bbsmember_array = array_under_reset($bbsmember_array, 'bbs_id');
            $bbsid_array = array_keys($bbsmember_array);
            $bbs_list = $model->table('bbs')->where(array('bbs_id'=>array('in', $bbsid_array)))->limit(6)->select();
