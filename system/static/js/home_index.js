(function($) {
	...
	function update_screen_focus(){
	    ...
	    if (ap_ids != '') {
    		$.ajax({
    			type: "GET",
    			url: BASE_SITE_URL+'/index.php?w=show&t=get_show_list'+ap_ids,
    			dataType:"jsonp",
    			async: true,
    		    success: function(show_list){
            	    $(".full-screen-slides li[ap_id]").each(function(){
            	        ...
            	        if (typeof show_list[ap_id] !== "undefined") {
            	            var show = show_list[ap_id];
            	            obj.css("background",color+' url('+show['show_img']+') no-repeat center top');
            	            obj.find("a").attr("title",show['show_title']);
            	            obj.find("a").attr("href",show['show_url']);
    					}
            	    });
            	    $(".jfocus-trigeminy a[ap_id]").each(function(){
            	        var obj = $(this);
            	        ...
            	        if (typeof show_list[ap_id] !== "undefined") {
            	            var show = show_list[ap_id];
            	            obj.attr("title",show['show_title']);
            	            obj.attr("href",show['show_url']);
            	            obj.find("img").attr("alt",show['show_title']);
            	            obj.find("img").attr("src",show['show_img']);
    					}
            	    });
    		    }
    		});
...
$(function(){
    ...
    //首页促销区Tab标签卡滑门切换
    $(".text-ctn > li > h3").bind('mouseover', (function(e) {
    	if (e.target == this) {
    		var tabs = $(this).parent().parent().children("li");
    		var panels = $(this).parent().parent().parent().parent().parent().children(".tabs-panel");
    		var index = $.inArray(this, $(this).parent().parent().find("h3"));
    		if (panels.eq(index)[0]) {
    			tabs.removeClass("tabs-selected").eq(index).addClass("tabs-selected");
    			panels.addClass("tabs-hide").eq(index).removeClass("tabs-hide");
	...
	$("#nav_box li").each(function(a) {
		$(this).click(function() {
			$("html,body").animate({
				scrollTop: $(".i-floor-con").eq(a).offset().top - 20 + "px"
			}, 500)
		}).mouseover(function() {
