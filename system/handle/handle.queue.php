...
class queueHandle {
    ...
    /**
     * 更新抢购信息
     * @param unknown $robbuy_info
     * @throws Exception
     */
    public function editRobbuySaleCount($robbuy_info) {
        $model_robbuy = Model('robbuy');
        $data = array();
        $data['buyer_count'] = array('exp','buyer_count+1');
        $data['buy_quantity'] = array('exp','buy_quantity+'.$robbuy_info['quantity']);
        $update = $model_robbuy->editRobbuy($data,array('robbuy_id'=>$robbuy_info['robbuy_id']));
        if (!$update) {
            return callback(false,'更新抢购信息失败robbuy_id:'.$robbuy_info['robbuy_id']);
        } else {
            return callback(true);
        }
    }
    ...
    public function editRptState($input_rpt_info, $pay_sn) {
        $model_rpt = Model('coupon');
        $update = $model_rpt->editCoupon(array('coupon_id'=>$input_rpt_info['coupon_id']),array('coupon_state'=>2,'coupon_order_id'=>$pay_sn),$input_rpt_info['coupon_owner_id']);
        if ($update) {
            $update = $model_rpt->editRptTemplate(array('coupon_t_id'=>$input_rpt_info['coupon_t_id']), array('coupon_t_used'=>array('exp','coupon_t_used+1')));
            if ($update) {
                $send = new sendMemberMsg();
                // 发送用户店铺消息
                $send->set('member_id', $input_rpt_info['coupon_owner_id']);
                $send->set('code', 'rpt_use');
                $param = array();
                $param['coupon_code'] = $input_rpt_info['coupon_code'];
                $param['coupon_url'] = urlMember('member_coupon', 'index');
                $send->send($param);
            } else {
                return callback(false,'更新优惠券状态失败tpl:'.$input_rpt_info['coupon_t_id']);
            }
        } else {
            return callback(false,'更新优惠券状态失败vcode:'.$input_rpt_info['coupon_code']);
        }
        return callback(true);
    }
     ...
     */
    public function createOrderUpdateStorage($goods_buy_quantity, $goods_sale=array()) {
        ...
        foreach ($goods_buy_quantity as $goods_id => $quantity) {
            ...
            if(!empty($goods_sale)){
                $model_goods->editGoodsCommon(array('sale_count' => array('exp','sale_count+'.$quantity)),array('goods_commonid'=>$goods_sale[$goods_id]));
            }
        }
        if (!$result) {
     ...
     * @param unknown $goods_buy_quantity
     */
    public function cancelOrderUpdateStorage($goods_buy_quantity, $goods_sale=array()) {
        $model_goods = Model('goods');
        foreach ($goods_buy_quantity as $goods_id => $quantity) {
            $data = array();
            $data['goods_storage'] = array('exp','goods_storage+'.$quantity);
     ...
     * @return boolean
     */
    public function updateGoodsSalePriceByGoodsId($goods_id) {
        $update = Model('goods')->editGoodsSalePrice(array('goods_id' => array('in', $goods_id)));
        if (!$update) {
     ...
     * @return boolean
     */
    public function updateGoodsSalePriceByGoodsCommonId($goods_commonid) {
        $update = Model('goods')->editGoodsSalePrice(array('goods_commonid' => array('in', $goods_commonid)));
        if (!$update) {
    ...
    public function createGoodsQRCode($param) {
        ...
        foreach ($param['goodsid_array'] as $goods_id) {
            // 生成商品二维码
            $PhpQRCode->set('date',WAP_SITE_URL.DS.html.DS.product_detail.'.'.html.'?'.goods_id.'='.$goods_id);
     ...
     * 清理特殊商品促销信息
     */
    public function clearSpecialGoodsSale($param) {
        // 抢购
        Model('robbuy')->delRobbuy(array('goods_commonid' => $param['goods_commonid']));
        // 显示折扣
        ...
        // 更新促销价格
        Model('goods')->editGoods(array('goods_sale_price' => array('exp', 'goods_price'), 'goods_sale_type' => 0), array('goods_commonid' => $param['goods_commonid']));
        return callback(true);
    }
    ...
    public function sendVrCode($param) {
        ...
        $code_list = Model('order_vr')->getOrderCodeList($condition,'vr_code,vr_indate');
        ...
        $message    = wtReplaceText($tpl_info['content'],$data);
		$data['apicodeid'] = $tpl_info['apicodeid'];
        $sms = new Sms();
        $result = $sms->send($param["buyer_phone"],$message,$data);
        if (!$result) {
    ...
    public function sendPickupcode($param) {
		...
		$data['apicodeid'] = $tpl_info['apicodeid'];
        $sms = new Sms();
        $result = $sms->send($dorder_info['reciver_mobphone'],$message,$data);
        if (!$result) {
    ...
    public function sendChainCode($order_info) {
		...
		$data['apicodeid'] = $tpl_info['apicodeid'];
        $sms = new Sms();
        $result = $sms->send($order_info['buyer_phone'],$message,$data);
        if (!$result) {
     ...
     * 生成卡密优惠券
     */
    public function buildPwdCoupon($t_id){
        $t_id = intval($t_id);
        if($t_id <= 0){
            return callback(false,'参数错误');
        }
        ...
        //优惠券状态
        $coupon_state_arr = $model_coupon->getCouponState();
        
        //查询代金券详情
        $where = array();
        $where['coupon_t_id'] = $t_id;
        $where['coupon_t_gettype'] = $gettype_arr['pwd']['sign'];
        $where['coupon_t_isbuild'] = 0;
        $where['coupon_t_state'] = 1;
        $t_info = $model_coupon->getRptTemplateInfo($where);
        $t_total = intval($t_info['coupon_t_total']);
        if($t_total <= 0){
            return callback(false,'优惠券模板信息错误');
        }
        while($t_total > 0){
            $is_succ = false;
            $insert_arr = array();
            $step = $t_total > 1000 ? 1000 : $t_total;
            for($t = 0; $t < $step; $t++){
                $code = $model_coupon->get_rpt_code(0);
                if(!$code){
                    continue;
                }
                $pwd_arr = $model_coupon->create_rpt_pwd($t_info['coupon_t_id']);
                if(!$pwd_arr){
                    continue;
                }
                $tmp = array();
                $tmp['coupon_code'] = $code;
                $tmp['coupon_t_id'] = $t_info['coupon_t_id'];
                $tmp['coupon_title'] = $t_info['coupon_t_title'];
                $tmp['coupon_desc'] = $t_info['coupon_t_desc'];
                $tmp['coupon_start_date'] = $t_info['coupon_t_start_date'];
                $tmp['coupon_end_date'] = $t_info['coupon_t_end_date'];
                $tmp['coupon_price'] = $t_info['coupon_t_price'];
                $tmp['coupon_limit'] = $t_info['coupon_t_limit'];
                $tmp['coupon_state'] = $coupon_state_arr['unused']['sign'];
                $tmp['coupon_active_date'] = 0;
                $tmp['coupon_owner_id'] = 0;
                $tmp['coupon_owner_name'] = '';
                $tmp['coupon_order_id'] = 0;
                $tmp['coupon_pwd'] = $pwd_arr[0];//md5
                $tmp['coupon_pwd2'] = $pwd_arr[1];
                $tmp['coupon_customimg'] = $t_info['coupon_t_customimg'];
                $insert_arr[] = $tmp;
                $t_total--;
            }
    
            $result = $model_coupon->addCouponBatch($insert_arr);
            if($result && $is_succ == false){
                $is_succ = true;
            }
        }
        //更新优惠券模板
        if($is_succ){
            $model_coupon->editRptTemplate(array('coupon_t_id'=>$t_info['coupon_t_id']),array('coupon_t_isbuild'=>1));
            return callback(true);
        }else{
            return callback(false);
        }
    ...
    public function createSphot($order_id_list = array()) {
        ...
        if ($goods_list) {
            ...
            foreach ($goods_list as $goods_content) {
                $model_sp->createSphot($goods_content['rec_id'], $goods_content['goods_id']);
            }
        }
    }
    ...
    public function createVrSphot($order_info = array()) {
        return Model('order_vr_snapshot')->createSphot($order_info['order_id'], $order_info['goods_id']);
    }
