...
<div class="wrap">
  <form id="voucher_list_form" method="get">
    <table class="wtm-search-table">
      <input type="hidden" id='w' name='w' value='fx_bill' />
  ...
  <table class="wtm-default-table">
    ...
    <tbody>
      ...
      <tr class="bd-line">
        <td></td>
        <td><div class="wtm-goods-thumb"><a href="javascript:void(0);"><img src="<?php echo cthumb($val['goods_image'], 60,$val['store_id']);?>" onMouseOver="toolTip('<img src=<?php echo cthumb($val['goods_image'], 360,$val['store_id']);?>>')" onMouseOut="toolTip()" /></a></div>
        </td>
        ...
        <td class="goods-price">
          <?php echo wtPriceFormatForList($val['pay_goods_amount']);?>
        </td>
        <td class="goods-price">
          <?php echo wtPriceFormatForList($val['refund_amount']);?>
        </td>
        <td class="goods-time"><?php echo $val['fx_commis_rate'];?>%</td>
        <td class="goods-price"><?php echo wtPriceFormatForList($val['fx_pay_amount']);?></td>

        <td class="goods-time"><?php echo $val['fx_pay_time']?date("Y-m-d",$val['fx_pay_time']):'';?></td>
