...
var focus_show_list = new Array();//四张联动区广告数据
...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        <li><a href="index.php?w=page_config&t=page_config"><?php echo '板块区';?></a></li>
  ...
  <div class="homepage-focus" id="homepageFocusTab">
    ...
    <form id="upload_screen_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?w=page_config&t=screen_pic" target="upload_pic">
          ...
          <dd class="opt">
            <div class="full-screen-slides">
              <ul>
                ...
                <div class="title"><h4>广告调用</h4><a class="wtap-btn-mini del" href="JavaScript:del_screen(<?php echo $val['pic_id'];?>);" title="<?php echo $lang['wt_del'];?>"><i class="fa fa-trash"></i>删除</a></div>
                ...
                <?php }else { ?>
                <li ap="0" screen_id="<?php echo $val['pic_id'];?>" title="可上下拖拽更改显示顺序"><div class="title"><h4>图片调用</h4><a class="wtap-btn-mini del" href="JavaScript:del_screen(<?php echo $val['pic_id'];?>);"><i class="fa fa-trash"></i>删除</a></div>
                  <div class="focus-thumb" onclick="select_screen(<?php echo $val['pic_id'];?>);" style="background-color:<?php echo $val['color'];?>;" title="点击编辑选中区域内容"> <img title="<?php echo $val['pic_name'];?>" src="<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>"/></div>
                  <input name="screen_list[<?php echo $val['pic_id'];?>][pic_id]" value="<?php echo $val['pic_id'];?>" type="hidden">
                  <input name="screen_list[<?php echo $val['pic_id'];?>][pic_name]" value="<?php echo $val['pic_name'];?>" type="hidden">
            ...
            <div class="mt20"><a class="wtap-btn" href="JavaScript:add_screen('pic');"><?php echo '图片调用';?></a>
              <?php if(!empty($output['screen_show_list']) && is_array($output['screen_show_list'])){ ?>
              <a class="wtap-btn" href="JavaScript:add_screen('show');"><?php echo '广告调用';?></a>
              <?php } ?>
            </div>
          </dd>
        </dl>
      </div>
      <div id="ap_screen" class="wtap-form-default" style="display:none; overflow: visible;">
        <div class="title">
          <h3>新增/选中区域内容设置详情</h3>
        </div>
        <dl class="row">
      ...
      <div id="upload_screen" class="wtap-form-default" style="display:none; overflow: visible;">
        <div class="title">
          <h3>新增/选中区域内容设置详情</h3>
        </div>
        <dl class="row">
      ...
      <div class="wtap-form-default">
        <div class="bot"><a href="JavaScript:void(0);" onclick="$('#upload_screen_form').submit();" class="wtap-btn-big wtap-btn-green"><?php echo $lang['page_config_save'];?></a> <a href="index.php?w=page_config&t=html_update&web_id=<?php echo $output['code_screen_list']['web_id'];?>" class="wtap-btn-big wtap-btn-green"><?php echo $lang['page_config_web_html'];?></a> <span class="web-save-succ" style="display:none;"><?php echo $lang['wt_common_save_succ'];?></span> </div>
      </div>
    </form>
    <form id="upload_focus_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?w=page_config&t=focus_pic" target="upload_pic" style="display:none;">
      <input type="hidden" name="form_submit" value="ok" />
        ...
        <dl class="row">
          ...
          <dd class="opt focus-trigeminy">
              ...
              <div focus_id="<?php echo $key;?>" class="focus-trigeminy-group" title="<?php echo '可上下拖拽更改图片组显示顺序';?>">
                ...
                <div class="title"><h4>广告调用</h4><a class="wtap-btn-mini del" href="JavaScript:del_focus(<?php echo $key;?>);" title="<?php echo $lang['wt_del'];?>"><i class="fa fa-trash"></i>删除</a></div>
                <ul>
                  <?php foreach($val['pic_list'] as $k => $v) { ?>
                  <li list="show" pic_id="<?php echo $k;?>" onclick="select_focus(<?php echo $key;?>,this);" title="<?php echo '可左右拖拽更改图片排列顺序';?>">
                ...
                <?php }else { ?>
                <div class="title"><h4>图片调用</h4><a class="wtap-btn-mini del" href="JavaScript:del_focus(<?php echo $key;?>);"><i class="fa fa-trash"></i>删除</a></div>
                <ul>
                  <?php foreach($val['pic_list'] as $k => $v) { ?>
            ...
            <div class="mt20"> <a class="wtap-btn" href="JavaScript:add_focus('pic');"><?php echo '图片组';?></a>
              <?php if(!empty($output['focus_show_list']) && is_array($output['focus_show_list'])){ ?>
              <a class="wtap-btn" href="JavaScript:add_focus('show');"><?php echo '广告组';?></a>
              <?php } ?>
            </div>
          </dd>
        </dl>
      </div>
      <div id="ap_focus" class="wtap-form-default" style="display:none;">
        <div class="title">
          <h3>新增/选中区域内容设置详情</h3>
        </div>
        <dl class="row">
...
<script src="<?php echo ADMIN_STATIC_URL?>/js/web_focus.js"></script>
