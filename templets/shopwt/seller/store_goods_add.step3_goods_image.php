...
<div class="goods-gallery" wttype="gallery-<?php echo $output['color_id'];?>"> <a class="sample_demo" href="index.php?w=store_album&t=pic_list&item=goods_image&color_id=<?php echo $output['color_id'];?>" style="display:none;"><?php echo $lang['wt_submit'];?></a>
...
$(document).ready(function(){
	$('div[wttype="gallery-<?php echo $output['color_id'];?>"]').find('.demo').unbind().ajaxContent({
		event:'click', //mouseover
		loaderType:'img',
		loadingMsg:'<?php echo SHOP_TEMPLATES_URL;?>/images/loading.gif',
		target:'div[wttype="album-<?php echo $output['color_id'];?>"]'
	});
	$('div[wttype="gallery-<?php echo $output['color_id'];?>"]').find('select[name="jumpMenu"]').unbind().change(function(){
		var $_select_submit = $('div[wttype="gallery-<?php echo $output['color_id'];?>"]').find('.sample_demo');
