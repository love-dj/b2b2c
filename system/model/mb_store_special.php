...
class mb_store_specialModel extends Model{
    ...
    public function __construct() {
        parent::__construct('mb_store_special');
    }
     ...
     *
     */
    public function getMbSpecialItemListByID($special_id,$store_id) {
        $condition = array();
        $condition['special_id'] = $special_id;
        if(!empty($store_id)){
...
//   *
//   */
//  public function addMbStoreItemListByID($store_id) {
//      $condition = array();
//      $cache = array('adv_list','home1','home2');
//      $condition['store_id'] = $store_id;  
//      $condition['special_id'] = '0'; 
//      $condition['item_type'] = '0'; 
//      $condition['item_data'] = '0'; 
//      $condition['item_usable'] = '0'; 
//      $condition['item_sort'] = '0'; 
//      $item_list = $this->table('mb_store_special_item')->insert($condition);            
//      return $this->_getMbSpecialItemList($condition);
//  }    
     ...
     *
     */
    public function getMbSpecialItemUsableListByID($store_id,$special_id) {
        $prefix = 'mb_store_special';
     ...
     * 首页专题
     */
    public function getMbSpecialIndex($store_id) {
        return $this->getMbSpecialItemUsableListByID($store_id,self::INDEX_SPECIAL_ID);
    }
    ...
    private function _getMbSpecialItemList($condition, $order = 'item_sort asc') {
        $item_list = $this->table('mb_store_special_item')->where($condition)->order($order)->select();
        foreach ($item_list as $key => $value) {
            $item_list[$key]['item_data'] = $this->_initMbSpecialItemData($value['item_data'], $value['item_type'],$value['article_content'],$condition['store_id']);
    ...
    public function isMbSpecialItemExist($condition) {
        $item_list = $this->table('mb_store_special_item')->where($condition)->select();
        if($item_list) {
            return true;
        } else {
            return false;
     ...
     *
     */
    public function getMbSpecialItemInfoByID($item_id,$store_id) {
        ...
        $condition['store_id'] = $store_id;
        $item_info = $this->table('mb_store_special_item')->where($condition)->find();
        
        $item_info['item_data'] = $this->_initMbSpecialItemData($item_info['item_data'], $item_info['item_type'], $item_info['article_content'],$store_id);
     ...
     *
     */
    private function _initMbSpecialItemData($item_data, $item_type,$article_content,$store_id) {
        if(!empty($item_data)) {
            $item_data = unserialize($item_data);
            if($item_type == 'goods') {
                $item_data = $this->_initMbSpecialItemGoodsData($item_data, $item_type,$store_id);
            }
            if($item_type == 'store_introduction'){
            	$item_data = $this->_initMbSpecialItemjjData($item_data, $item_type,$article_content,$store_id);
            }           
            if($item_type == 'store_announcement'){
            	$item_data = $this->_initMbSpecialItemGGData($item_data, $item_type,$store_id);
            }
        } else {
    ...
    /**
     * 处理store_introduction内容
     */
    private function _initMbSpecialItemjjData($item_data, $item_type,$article_content,$store_id) {
        $goods_id_string = '';
        if(!empty($item_data)) {
        
            //$item_data = unserialize($item_data);
         
            $item_data['article_content'] = $article_content;
        }
     ...
     * 处理announcement类型内容
     */
    private function _initMbSpecialItemGGData($item_data, $item_type,$store_id) {
        $goods_id_string = '';
        if(!empty($item_data['item'])) {
            ...
            $model_goods = Model('article');
            $goods_list = $model_goods->getGoodsList($condition, 'article_id,article_title,article_abstract,article_image,article_attachment_path');
           
            foreach ($goods_list as $kk => $va) {
            	
            	$cc[$va['article_id']]['article_id'] = $va['article_id'];
            	$cc[$va['article_id']]['article_title'] = $va['article_title'];
            	$cc[$va['article_id']]['article_abstract'] = $va['article_abstract'];
            	$cc[$va['article_id']]['article_image'] = unserialize($va['article_image']);
            	$cc[$va['article_id']]['article_attachment_path'] = $va['article_attachment_path'];
            }
     ...
     * 处理goods类型内容
     */
    private function _initMbSpecialItemGoodsData($item_data, $item_type,$store_id) {
        $goods_id_string = '';
        if(!empty($item_data['item'])) {
            foreach ($item_data['item'] as $value) {
                $goods_id_string .= $value . ',';
            ...
            $goods_list = $model_goods->getGoodsList($condition, 'goods_id,goods_name,goods_sale_price,goods_image');
            $goods_list = array_under_reset($goods_list, 'goods_id');
    ...
    public function addMbSpecialItem($param) {
        ...
        $param['item_sort'] = 255;
        $result = $this->table('mb_store_special_item')->insert($param);
        //删除缓存
        if($result) {
            //删除缓存
     ...
     *
     */
    public function editMbSpecialItemByID($update, $item_id, $special_id,$store_id) {
        ...
        return $this->table('mb_store_special_item')->where($condition)->update($update);
    }
    public function editMbSpecialItemByIDa($update, $updatea,$item_id, $special_id,$store_id) {
        if(isset($update['item_data'])) {
            $update['item_data'] = serialize($update['item_data']);
            $update['article_content'] = trim($updatea['article_content']);
     ...
     *
     */
    public function editMbSpecialItemUsableByID($usable, $item_id, $special_id,$store_id) {
        $update = array();
        if($usable == 'usable') {
        ...
        return $this->editMbSpecialItemByID($update, $item_id, $special_id,$store_id);
    }
    ...
    public function delMbSpecialItem($condition, $special_id) {
        ...
        return $this->table('mb_store_special_item')->where($condition)->delete();
    }
    ...
    public function getMbSpecialModuleList() {
        $module_list = array();
        $module_list['adv_list'] = array('name' => 'adv_list' , 'desc' => '幻灯片版块');
        ...
        $module_list['store_introduction'] = array('name' => 'store_introduction' , 'desc' => '简介版块');
        //$module_list['store_announcement'] = array('name' => 'store_announcement' , 'desc' => '新闻版块');//临时不支持
        return $module_list;
    }
