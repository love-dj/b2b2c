...
class d_centerControl extends BaseDeliveryCenterControl{
     ...
     * 修改密码
     */
    public function change_passwordWt() {
        if (chksubmit()) {
     ...
     * 从第三方取快递信息
     */
    public function ajax_get_expressWt(){
     ...
     * 取件通知
     */
    public function arrive_pointWt() {
     ...
     * 提货验证
     */
    public function pickup_parcelWt() {
        if (chksubmit()) {
            ...
            if ($result) {
                ...
                if ($order_info['order_state'] != ORDER_STATE_SUCCESS) {
                    Handle('order')->changeOrderStateReceive($order_info, 'buyer', '物流自提服务站', '物流自提服务站确认收货');
                }
