...
class member_informControl extends BaseMemberControl{
     ...
     * 提交举报商品
     */
    public function inform_submitWt() {
        ...
        //获取商品详细信息
        $goods_content = $this->get_goods_content_byid($goods_id);
    ...
    public function inform_saveWt() {
        ...
        $input['inform_goods_name'] = $goods_content['goods_name'];
        $input['inform_goods_image'] = $goods_content['goods_image'];
        ...
        $input['inform_store_id'] = $goods_content['store_id'];
        $input['inform_store_name'] = $goods_content['store_name'];
        ...
        if($model_inform->saveInform($input)) {
            showDialog(Language::get('inform_success'),'index.php?w=member_inform&t=inform_list','succ');
        }
        else {
            showDialog(Language::get('inform_fail'),'index.php?w=member_inform&t=inform_list','error');
        }
    }
     ...
     * 取消用户提交的商品举报
     */
    public function inform_cancelWt() {
     ...
     * 商品举报详细
     */
    public function inform_infoWt() {

        $inform_id = intval($_GET['inform_id']);
        $inform_info = $this->get_inform_info($inform_id);
        Tpl::output('inform_info', $inform_info);
        // 商品信息
        $goods_content = Model('goods')->getGoodsInfoByID($inform_info['inform_goods_id']);
        Tpl::output('goods_content', $goods_content);
        // 投诉类型
     ...
     * 根据id获取商品详细信息
     */
    private function get_goods_content_byid($goods_id) {
     ...
     * 根据举报类型id获取，举报具体列表
     */
    public function get_subject_by_typeidWt() {
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'inform_list','menu_name'=>'违规举报', 'menu_url'=>'index.php?w=member_inform&t=inform_list'),
        );
