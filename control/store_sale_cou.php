...
class store_sale_couControl extends BaseSellerControl
{
    const LINK_COU_LIST = 'index.php?w=store_sale_cou&t=cou_list';
    ...
    public function cou_listWt()
    {
        ...
        Tpl::showpage('store_sale_cou.list');
    }

    /**
     * 加价购套餐购买
     */
    public function cou_quota_addWt()
    {
        // 输出导航
        $this->profile_menu('cou_quota_add');
        Tpl::showpage('store_sale_cou.quota_add');
    }

    /**
     * 加价购套餐购买保存
     */
    public function cou_quota_add_saveWt()
    {
        ...
        // 获取当前价格
        $current_price = intval(C('sale_cou_price'));
        ...
        $this->recordSellerLog('购买' . $cou_quota_quantity . '份加价购套餐，单价' . $current_price . $lang['wt_yuan']);
    ...
    public function cou_addWt()
    {
        ...
        Tpl::showpage('store_sale_cou.add');
    }

    /**
     * 保存添加的加价购活动
     */
    public function cou_add_saveWt()
    {
        // 验证输入
        ...
        if ($result) {
            ...
            $editUrl = 'index.php?w=store_sale_cou&t=cou_edit&cou_id=' . $result;
    ...
    public function cou_editWt()
    {
        ...
        Tpl::showpage('store_sale_cou.edit');
    }

    /**
     * 保存编辑的加价购活动
     */
    public function cou_edit_saveWt()
    {
        // 验证输入
        ...
        try {
            ...
            showDialog(Language::get('wt_common_op_fail') . ': ' . $ex->getMessage());
        }

        $this->recordSellerLog('编辑加价购活动，活动名称：' . $cou_name . '，活动编号：' . $couId);
        showDialog(Language::get('wt_common_op_succ'), self::LINK_COU_LIST, 'succ', '', 3);
    }

    /**
     * 加价购活动删除
     */
    public function cou_delWt()
    {
        ...
        if ($result) {
            ...
            showDialog(L('wt_common_op_succ'), self::LINK_COU_LIST, 'succ');
        } else {
     ...
     * 异步设置加价购活动规则中的换购商品
     */
    public function cou_level_skuWt()
    {
        ...
        Tpl::showpage('store_sale_cou.cou_level_sku', 'null_layout');
    }

    /**
     * 异步设置参与加价购活动的商品
     */
    public function cou_skuWt()
    {
        $model_goods = Model('goods');
        ...
        Tpl::showpage('store_sale_cou.cou_sku', 'null_layout');
    }
    ...
    private function profile_menu($menu_key='')
    {
        $menu_array = array(
            1 => array(
                'menu_key' => 'cou_list',
                'menu_name' => '活动列表',
                'menu_url' => 'index.php?w=store_sale_cou&t=cou_list',
            ),
        );
        switch ($menu_key) {
            case 'cou_quota_add':
                $menu_array[] = array(
                    ...
                    'menu_name' => '购买套餐',
                    'menu_url' => 'index.php?w=store_sale_cou&t=cou_add',
                );
                break;
            case 'cou_add':
                $menu_array[] = array(
