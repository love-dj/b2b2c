...
<div class="wtsc-form-default">
  <form id="add_form" action="index.php?w=store_sale_cou&t=cou_edit_save" method="post">
    <input type="hidden" name="cou_id" value="<?php echo $info['id']; ?>" />
    <dl>
      <dt><i class="required">*</i>活动名称<?php echo $lang['wt_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt>开始时间<?php echo $lang['wt_colon'];?></dt>
      <dd><?php echo date('Y-m-d H:i', $info['tstart']); ?></dd>
    </dl>
    <dl>
      <dt>结束时间<?php echo $lang['wt_colon'];?></dt>
...
if (empty($g)) continue; ?>
            <tr class="off-shelf" data-product-id="<?php echo $g['goods_id']; ?>">
              ...
              <td class="goods-price w90"><?php echo wtPriceFormat($g['goods_price']); ?></td>
              <td class="nscs-table-handle w50"><span> <a href="javascript:;" class="btn-bittersweet" data-cou-sku-remove-button="<?php echo $g['goods_id']; ?>"> <i class="icon-ban-bbs"></i>
                <p>移除</p>
                </a> </span></td>
            </tr>
<?php } ?>
          ...
          <tbody id="cou-sku-item-tpl" style="display:none;">
            <tr class="off-shelf" data-product-id="__id">
              ...
              <td class="nscs-table-handle w50"><span> <a href="javascript:;" class="btn-bittersweet" data-cou-sku-remove-button="__id"> <i class="icon-ban-bbs"></i>
                <p>移除</p>
                </a> </span></td>
            </tr>
          </tbody>
        </table>
        <a id="cou-sku-choose-btn" href="index.php?w=store_sale_cou&t=cou_sku&cou_id=<?php echo $info['id']; ?>" class="wtbtn wtbtn-aqua">选择活动商品</a>
...
<?php foreach ((array) $output['data']['levels'] as $k => $v) { ?>
        <div data-cou-level-item="<?php echo (string) $k; ?>" class="wtsc-cou-rule">
          <div class="rule-note">
            <h5>规则<?php echo $k; ?>：<a href="javascript:;" class="wtbtn-mini wtbtn-grapefruit" data-cou-level-remove="<?php echo (string) $k; ?>"><i class="icon-trash"></i>删除</a></h5>
            <span>购买同一加价购活动商品消费满
            ...
            件（0为不限）优惠商品，换购商品如下：<a data-cou-level-sku-choose-button="<?php echo (string) $k; ?>" href="javascript:;" class="wtbtn"><i class="icon-gift"></i>添加换购商品</a></span></div>
...
if (empty($g)) continue; ?>
              <tr class="off-shelf" data-cou-level-selected-sku="<?php echo (string) $k2; ?>" data-level="<?php echo (string) $k; ?>">
                ...
                <td class="tl"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $g['goods_id'], )); ?>"> <?php echo $g['goods_name']; ?> </a></td>
                <td class="goods-price w90" wttype="bundling_data_price"><s> <?php echo wtPriceFormat($g['goods_price']); ?> </s></td>
                <td class="w90"><input type="text" class="w50" name="cou_level[<?php echo (string) $k; ?>][skus][<?php echo (string) $k2; ?>]" value="<?php echo $v2['price']; ?>" data-max-price="<?php echo wtPriceFormat($g['goods_price']); ?>" /></td>
                <td class="nscs-table-handle w50"><span> <a href="javascript:;" class="btn-bittersweet" data-cou-level-sku-remove="<?php echo (string) $k2; ?>"> <i class="icon-ban-bbs"></i>
                  <p>移除</p>
                  </a> </span></td>
              </tr>
<?php } ?>
...
<?php } ?>
        ...
        <a href="javascript:;" id="cou-level-add-button" class="wtbtn wtbtn-aqua mt10"> <i class="icon-plus-sign"></i> 添加规则 </a>
    ...
    <div class="bottom">
      <label class="submit-border">
        <input id="submit_button" type="submit" class="submit" value="<?php echo $lang['wt_submit'];?>">
      </label>
    </div>
  </form>
</div>
...
<div id="cou-level-newly" style="display:none;">
        <div data-cou-level-item="__level" class="wtsc-cou-rule">
          <div class="rule-note">
            <h5>新增规则：<a href="javascript:;" class="wtbtn-mini wtbtn-grapefruit" data-cou-level-remove="__level"><i class="icon-trash"></i>删除</a></h5>
            <span>购买同一加价购活动商品消费满
            ...
            件（0为不限）优惠商品，换购商品如下：<a data-cou-level-sku-choose-button="__level" href="javascript:;" class="wtbtn"><i class="icon-gift"></i>添加换购商品</a></span></div>
...
<table style="display:none;">
  <tbody id="cou-level-sku-newly">
    <tr class="off-shelf" data-cou-level-selected-sku="__id" data-level="__level">
      ...
      <td class="tl"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => '__id', )); ?>"> __name </a></td>
      <td class="goods-price w90" wttype="bundling_data_price"><s> __price </s></td>
      <td class="w90"><input type="text" class="w50" name="cou_level[__level][skus][__id]" value="__price" data-max-price="__price" /></td>
      <td class="nscs-table-handle w50"><span> <a href="javascript:;" class="btn-bittersweet" data-cou-level-sku-remove="__id"> <i class="icon-ban-bbs"></i>
        <p>移除</p>
        </a> </span></td>
    </tr>
  </tbody>
...
$(function() {
    ...
    var couLevelSkuChooseTriggered = function(id, url) {
        $("[data-cou-level-sku-choose-container='"+id+"']").load(
            url || 'index.php?w=store_sale_cou&t=cou_level_sku&level='+id,
            function() {
    ...
    $('[data-cou-level-sku-choose-container] a[wttype="search_a"]').live('click', function() {
        var id = $(this).parents('[data-cou-level-sku-choose-container]').attr('data-cou-level-sku-choose-container');
        var url = this.href;
