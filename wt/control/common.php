...
// 				$save_file2 = BASE_UPLOAD_PATH.'/'.$_POST['filename'];
    ...
    private function get_pending_matters() {
        ...
        // 抢购数量
        $statistics['robbuy_verify'] = Model('robbuy')->getRobbuyCount(array('state'=>10));
        // 积分订单
        ...
        if (C('news_isuse')) {
            // 文章审核
            $statistics['news_article_verify'] = Model('news_article')->getCmsArticleCount(array('article_state' => 2));
            // 图刊审核
            $statistics['news_picture_verify'] = Model('news_picture')->getCmsPictureCount(array('picture_state' => 2));
        }
        /**
         * 社区
         */
        if (C('bbs_isuse')) {
            $statistics['bbs_verify'] = Model('bbs')->getbbsUnverifiedCount();
        }
	/**
         * 分销
         */
        if(C('fenxiao_isuse')){
            //分销认证
            $statistics['fx_verify'] = Model('member')->getMemberCount(array('fx_state'=>1));

            //佣金提现
            $statistics['fx_cash'] = Model('fx_trad')->getFenxiaoCashCount(array('tradc_payment_state' => 0));
        }
        return $statistics;
    }
}
