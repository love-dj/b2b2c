...
class store_sale_bookControl extends BaseSellerControl {
    ...
    public function book_listWt() {
        ...
        Tpl::showpage('store_sale_book.goods_list');
    }

    /**
     * 购买套餐
     */
    public function book_quota_addWt() {
        if (chksubmit()) {
            $quantity = intval($_POST['book_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_book_price')); // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlShop('store_sale_book', 'book_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_book = Model('p_book');
            ...
            if ($return) {
                ...
                $this->recordSellerLog('购买'.$quantity.'套预定商品活动，单价'.intval(C('sale_book_price')).'元');
                showDialog('购买成功', urlShop('store_sale_book', 'book_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_book', 'book_quota_add'));
            }
        }
        // 输出导航
        $this->profile_menu('book_quota_add', 'book_quota_add');
        Tpl::showpage('store_sale_book.quota_add');
    }

    /**
     * 套餐续费
     */
    public function book_renewWt() {
        if (chksubmit()) {
            $model_book = Model('p_book');
            $quantity = intval($_POST['book_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_book_price')); // 扣款数
            ...
            if ($return) {
                ...
                $this->recordSellerLog('续费'.$quantity.'套预定商品活动，单价'.intval(C('sale_book_price')).'元');
                showDialog('购买成功', urlShop('store_sale_book', 'book_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_book', 'book_quota_add'));
            }
        }
     ...
     * 选择商品
     */
    public function book_select_goodsWt() {
        ...
        Tpl::showpage('store_sale_book.select_goods', 'null_layout');
    }
    ...
    public function choosed_goodsWt() {
        ...
        if (chksubmit()) {
            ...
            if ($rs) {
                ...
                if ($_POST['type'] == 'book') {
                    $goods_content['book_down_payment'] = floatval($_POST['down_payment']);
                    $goods_content['book_final_payment'] = floatval($_POST['final_payment']);
                    $goods_content['book_down_time'] = date('Y-m-d', $update['book_down_time']);
                } else if ($_POST['type'] == 'presell') {
                    $goods_content['presell_deliverdate'] = date('Y-m-d', $update['presell_deliverdate']);
                }
                $this->recordSellerLog('添加预定商品，商品id：'.$gid);
                showDialog('操作成功', '', 'succ', 'CUR_DIALOG.close();choose_goods('.json_encode($goods_content).', "'. $_POST['type'] .'")');
            } else {
        ...
        Tpl::showpage('store_sale_book.choosed_goods', 'null_layout');
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'book_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'book_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_book', 'book_goods_list'))
                );
                break;
            case 'book_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'book_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_book', 'book_goods_list')),
                    2=>array('menu_key'=>'book_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlShop('store_sale_book', 'book_quota_add'))
                );
                break;
            case 'book_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'book_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_book', 'book_goods_list')),
                    2=>array('menu_key'=>'book_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShop('store_sale_book', 'book_renew'))
                );
                break;
        }
