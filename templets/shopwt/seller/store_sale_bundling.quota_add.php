...
<div class="wtsc-form-default">
  ...
  <form id="add_form" action="index.php?w=store_sale_bundling&t=bundling_quota_add" method="post">
  <?php } else {?>
  <form id="add_form" action="index.php?w=store_sale_bundling&t=bundling_renew" method="post">
  <?php }?>
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
      <dt><i class="required">*</i><?php echo $lang['bundling_quota_add_quantity'].$lang['wt_colon'];?></dt>
      <dd>
        <input id="bundling_quota_quantity" name="bundling_quota_quantity" type="text" class="text w50"/><em class="add-on"><?php echo $lang['wt_month'];?></em><span></span>
        <p class="hint"><?php echo $lang['bundling_price_explain1'];?></p>
        <p class="hint"><?php printf($lang['bundling_price_explain2'], intval(C('sale_bundling_price')));?></p>
...
$(document).ready(function(){
	...
	$("#add_form").validate({
		...
		submitHandler:function(form){
			var unit_price = parseInt(<?php echo C('sale_bundling_price');?>);
