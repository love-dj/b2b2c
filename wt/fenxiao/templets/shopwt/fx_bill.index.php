...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=fx_bill&t=get_bill_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=fx_bill&t=get_bill_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?w=fx_bill&t=get_bill_xml',
        colModel : [
            ...
            {display: '退款金额', name : 'refund_amount', width : 60, sortable : true, align: 'center'},
			      {display: '分销佣金比例', name : 'fx_commis_rate', width: 70, sortable : true, align : 'center'},
			      {display: '分销佣金', name : 'fx_pay_amount', width: 70, sortable : true, align : 'center'}, 
            {display: '结算时间', name : 'fx_pay_time', width : 80, sortable : true, align: 'center'},
            ...
            {display: '分销员ID', name : 'fx_member_id', width : 90, sortable : true, align: 'center'}
            ],
        buttons : [
