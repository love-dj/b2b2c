...
<div class="page">
  ...
  <div id="stat_tabs" class="  ui-tabs" style="min-height:500px">
    <ul class="tab-base wt-row">
      <li><a href="#membernum_div" wt_type="showmap" data-param='{"type":"membernum"}'>下单会员数</a></li>
      <li><a href="#ordernum_div" wt_type="showmap" data-param='{"type":"ordernum"}'>下单量</a></li>
      <li><a href="#orderamount_div" wt_type="showmap" data-param='{"type":"orderamount"}'>下单金额</a></li>
    </ul>
...
$(function () {
    ...
    //加载统计列表
    $("#statlist").flexigrid({
        url: 'index.php?w=stat_member&t=get_arealist_xml&c=<?php echo $output['searchtime'];?>',
        colModel : [
...
function fg_operation(name, bDiv){
    var stat_url = 'index.php?w=stat_member&t=area_list&exporttype=excel&c=<?php echo $output['searchtime'];?>';
    get_excel(stat_url,bDiv);
}
//加载统计地图
function getStatMap(type){
	$('#'+type+'_div').load('index.php?w=stat_member&t=area_map&type='+type+'&c=<?php echo $output['searchtime'];?>');
}
</script>