...
class store_sale_comboControl extends BaseSellerControl {
    ...
    public function combo_listWt() {
        ...
        Tpl::showpage('store_sale_combo.goods_list');
    }
    
    /**
     * 保存推荐组合
     */
    public function save_comboWt() {
        if (! chksubmit()) {
        ...
        // 验证
        $goods_content = $model_goods->getGoodsOnlineInfoByID($goods_id, 'goods_id,goods_commonid,store_id');
        // 商品是否存在，及是否为本店铺商品
        if (empty($goods_content) || $goods_content['store_id'] != $_SESSION['store_id']) {
            showDialog(L('wrong_argument'));
        }
        // 验证商品是否为普通商品
        if (!$model_goods->checkIsGeneral($goods_content)) {
        ...
        if (!empty($data)) {
            foreach ($data as $key => $val) {
                ...
                foreach ($val as $v) {
                    ...
                    $array['goods_commonid'] = $goods_content['goods_commonid'];
        ...
        showDialog(L('wt_common_save_succ'), 'reload', 'succ');
    }

    /**
     * 购买套餐
     */
    public function combo_quota_addWt() {
        if (chksubmit()) {
            $quantity = intval($_POST['combo_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_combo_price')); // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlShop('store_sale_combo', 'combo_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_combo = Model('p_combo_quota');
            ...
            if ($return) {
                ...
                $this->recordSellerLog('购买'.$quantity.'套推荐组合活动，单价'.intval(C('sale_combo_price')).'元');
                showDialog('购买成功', urlShop('store_sale_combo', 'combo_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_combo', 'combo_quota_add'));
            }
        }
        // 输出导航
        $this->profile_menu('combo_quota_add', 'combo_quota_add');
        Tpl::showpage('store_sale_combo.quota_add');
    }

    /**
     * 套餐续费
     */
    public function combo_renewWt() {
        if (chksubmit()) {
            $model_combo = Model('p_combo_quota');
            $quantity = intval($_POST['combo_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_combo_price')); // 扣款数
            ...
            if ($return) {
                ...
                $this->recordSellerLog('续费'.$quantity.'套推荐组合活动，单价'.intval(C('sale_bundling_price')).'元');
                showDialog('购买成功', urlShop('store_sale_combo', 'combo_list'), 'succ');
            } else {
                showDialog('购买失败', urlShop('store_sale_combo', 'combo_quota_add'));
            }
        }
     ...
     * 选择商品
     */
    public function combo_select_goodsWt() {
        $model_goods = Model('goods');
        $condition = array();
        ...
        Tpl::showpage('store_sale_combo.select_goods', 'null_layout');
    }
    ...
    public function choosed_goodsWt() {
        ...
        $goods_content['is_general'] = $model_goods->checkIsGeneral($goods_content);
        ...
        Tpl::showpage('store_sale_combo.choosed_goods');
    }
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'combo_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'combo_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_combo', 'combo_goods_list'))
                );
                break;
            ...
            case 'combo_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'combo_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_combo', 'combo_goods_list')),
                    2=>array('menu_key'=>'combo_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlShop('store_sale_combo', 'combo_quota_add'))
                );
                break;
            case 'combo_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'combo_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShop('store_sale_combo', 'combo_goods_list')),
                    2=>array('menu_key'=>'combo_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShop('store_sale_combo', 'combo_renew'))
                );
                break;
        }
