...
class paymentControl extends BaseHomeControl{
     ...
     * 实物商品订单
     */
    public function real_orderWt(){
        $pay_sn = $_POST['pay_sn'];
        $payment_code = $_POST['payment_code'];
        $url = 'index.php?w=member_order';
        ...
        $order_pay_info = $handle_payment->getRealOrderInfo($pay_sn, $_SESSION['member_id']);
        ...
        if (!empty($order_list)) {
            foreach ($order_list as $order_info) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) {
                    ...
                    //减去积分抵用部分
		    	                $api_pay_amount = floatval(wtPriceFormat($api_pay_amount));
					$api_pay_amount -=$order_info['points_money'];
        ...
        $result = $handle_payment->getPaymentInfo($payment_code);
        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        $payment_info = $result['data'];

        $order_pay_info['data']['api_pay_amount'] = wtPriceFormat($api_pay_amount);
     ...
     * 虚拟商品购买
     */
    public function order_vrWt(){
        $order_sn = $_POST['order_sn'];
        $payment_code = $_POST['payment_code'];
        $url = 'index.php?w=member_order_vr';
        ...
        //计算所需支付金额等支付单信息
        $result = Handle('payment')->getVrOrderInfo($order_sn, $_SESSION['member_id']);
        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        ...
        $api_pay_amount = 0;
        if ($order_info['order_state'] == ORDER_STATE_NEW) {
            $api_pay_amount = floatval(wtPriceFormat($order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount']));
			//减去积分抵用部分
			$api_pay_amount -=$order_info['points_money'];
        }
        ...
        $result = Model('order_vr')->editOrder(array('api_pay_time'=>TIMESTAMP),array('order_id'=>$order_info['order_id']));
        if(!$result) {
            showMessage('更新订单信息发生错误，请重新支付', $url, 'html', 'error');
        }

        $result = Handle('payment')->getPaymentInfo($payment_code);
        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        $payment_info = $result['data'];

        $order_info['api_pay_amount'] = wtPriceFormat($api_pay_amount);
    ...
    public function pd_orderWt(){
        ...
        $result = $handle_payment->getPdOrderInfo($pdr_sn,$_SESSION['member_id']);
        if(!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
    ...
    private function _pd_pay($order_list, $post) {
        ...
        try {
            ...
            if (!empty($post['rcb_pay'])) {
                $order_list = $handle_buy_action->rcbPay($order_list, $post, $buyer_info);
            }

            //使用预存款支付
            if (!empty($post['pd_pay'])) {
                $order_list = $handle_buy_action->pdPay($order_list, $post, $buyer_info);
            }

            //特殊订单站内支付处理
            $handle_buy_action->extendInPay($order_list);
    ...
    private function _pd_vr_pay($order_info, $post) {
        ...
        try {
            $model_member->beginTransaction();
            $handle_buy = Handle('buy_vr');
            //使用充值卡支付
            if (!empty($post['rcb_pay'])) {
                $order_info = $handle_buy->rcbPay($order_info, $post, $buyer_info);
            }

            //使用预存款支付
            if (!empty($post['pd_pay'])) {
                $order_info = $handle_buy->pdPay($order_info, $post, $buyer_info);
            }
    ...
    private function _api_pay($order_info, $payment_info) {
    	...
    	} elseif($payment_info['payment_code'] == 'paypal') {
			$payment_api->submit();
        } else {
    ...
    public function notifyWt(){
        ...
        //取得支付方式
        $result = $handle_payment->getPaymentInfo($_GET['payment_code']);
        if (!$result['state']) {
            exit($fail);
        }
        $payment_info = $result['data'];
        ...
        //购买商品
        if ($order_type == 'real_order') {
            $result = $handle_payment->updateRealOrder($out_trade_no, $payment_info['payment_code'], $order_list, $trade_no);
        } elseif($order_type == 'order_vr'){
            $result = $handle_payment->updateVrOrder($out_trade_no, $payment_info['payment_code'], $order_pay_info, $trade_no);
        } elseif ($order_type == 'pd_order') {
            $result = $handle_payment->updatePdOrder($out_trade_no,$trade_no,$payment_info,$order_pay_info);
        }
        if ($result['state']) {
            //记录消费日志
            if ($order_type == 'real_order') {
    ...
    public function returnWt(){
        ...
        $url = BASE_SITE_URL.'/index.php?w='.$w;
        ...
        if ($order_type == 'real_order') {
            ...
            if (!empty($order_list)) {
                foreach ($order_list as $order_info) {
					...
					//减去积分抵用部分
					$result['data']['api_pay_amount'] -=$order_info['points_money'];
                }
            }
        ...
        //支付成功后跳转
        if ($order_type == 'real_order') {
            $pay_ok_url = BASE_SITE_URL.'/index.php?w=buy&t=pay_ok&pay_sn='.$out_trade_no.'&pay_amount='.wtPriceFormat($api_pay_amount);
        } elseif ($order_type == 'order_vr') {
            $pay_ok_url = BASE_SITE_URL.'/index.php?w=buy_vr&t=pay_ok&order_sn='.$out_trade_no.'&order_id='.$order_pay_info['order_id'].'&order_amount='.wtPriceFormat($api_pay_amount);
        } elseif ($order_type == 'pd_order') {
     ...
     * 二维码显示(微信扫码支付)
     */
    public function qrcodeWt() {
        $data = base64_decode($_GET['data']);
        $data = decrypt($data,MD5_KEY,30);
        require_once BASE_STATIC_PATH.'/phpqrcode/phpqrcode.php';
        QRcode::png($data);
    }
     ...
     */
    public function wxpay_returnWt() {
        $result = Handle('payment')->getPaymentInfo('wxpay');
        if (!$result['state']) {
     ...
     * 支付成功，更新订单状态
     */
    public function wxpay_notifyWt() {
        $result = Handle('payment')->getPaymentInfo('wxpay');
        if (!$result['state']) {
            Log::record('wxpay not found','RUN');
        }
    ...
    public function query_stateWt() {
        if ($_GET['pay_id'] && intval($_GET['pay_id']) > 0) {
            ...
            $info = Model('order_vr')->getOrderInfo(array('order_id'=>intval($_GET['order_id']),'buyer_id'=>intval($_GET['buyer_id'])));
