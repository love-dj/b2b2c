...
<?php } else { ?>
  ...
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_fcode', 'fcode_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-mint" href="javascript:void(0);" wttype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="wtbtn wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_fcode', 'fcode_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['wt_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['fcode_quota']['fcq_endtime']);?></strong>
<?php }?>
  <ul>
...
<table class="wtsc-default-table">
  ...
  <tbody wttype="choose_goods_list">
    <tr wttype="tr_no_sale" style="display:none;">
      <td colspan="20" class="norecord"><div class="no-sale"><i class="zw"></i><span>F码商品列表暂无内容，请选择添加平台推荐展位商品。</span></div></td>
    </tr>
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle">
        <span><a class="btn-bittersweet" href='<?php echo urlShop('store_sale_fcode', 'download_f_code_excel', array('gid' => $val['goods_id']));?>' title="下载F码"><i class="icon-download"></i><p>下载</p></a></span>
...
$(function(){
    ...
    $('input[wttype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlShop('store_sale_fcode', 'fcode_select_goods');?>';
    ...
    $('div[wttype="div_goods_select"]').on('click', 'a[wttype="a_choose_goods"]', function(){
        ...
        CUR_DIALOG =ajax_form('choose_goods', 'F码商品规则设定', '<?php echo urlShop('store_sale_fcode', 'choosed_goods');?>&gid='+_gid, 640);
    });

    // 删除商品
    $('tbody[wttype="choose_goods_list"]').on('click','a[wttype="del_choosed"]', function(){
        $this = $(this);
        _url = '<?php echo urlShop('store_sale_fcode', 'del_choosed_goods');?>';
...
function choose_goods(data) {
    ...
    $('<tr class="bd-line"></tr>')
        ...
        .append('<td class="nscs-table-handle"><span><a class="btn-bittersweet" href="<?php echo urlShop('store_sale_fcode', 'download_f_code_excel');?>&gid=' + data.goods_id + '" title="下载F码"><i class="icon-download"></i><p>下载</p></a></span><span><a class="btn-grapefruit" href="javascript:void(0);" data-param="{gid:'+ data.goods_id +'}" wttype="del_choosed"><i class="icon-trash"></i><p>删除</p></a></span></td>')
        .appendTo('tbody[wttype="choose_goods_list"]');
    // 验证是否已经选择商品
    checked_no_sale();
}
// 验证是否已经选择商品
function checked_no_sale() {
