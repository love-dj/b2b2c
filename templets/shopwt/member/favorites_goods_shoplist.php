...
<div class="wrap">
  ...
  <div class="wtm-favorite-goods-store">
    <div class="store-info">
      ...
      <dl>
        ...
        <dd>联系：<i member_id="<?php echo $goods_list[0]['goods']['member_id'];?>"></i>
          ...
          <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $goods_list[0]['goods']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid=<?php echo $goods_list[0]['goods']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" style=" vertical-align: middle;" /></a>
          <?php }?>
        </dd>
        <dd>
          ...
          <?php }else{?>
          <a href="javascript:collect_store('<?php echo $goods_list[0]['goods']['store_id'];?>','store','')" title="<?php echo $lang['favorite_collect_store'];?>" wt_store="<?php echo $goods_list[0]['goods']['store_id'];?>"><i class="icon-heart-empty"></i>收藏该店铺</a>
          <?php }?>
        </dd>
    ...
    <div class="favorite-goods-list">
      <ul>
        ...
        <li class="favorite-pic-list<?php if (!isset($favorites['goods']) || !$favorites['goods']['state']) {?> disable<?php }?>">
          <div class="favorite-goods-thumb"><a href="index.php?w=goods&goods_id=<?php echo $favorites['goods']['goods_id'];?>" title="<?php echo $favorites['goods']['goods_name'];?>" target="_blank"><img src="<?php echo thumb($favorites['goods'], 240);?>" /></a></div>
          <div class="handle">
            <a href="javascript:void(0)" class="fr ml5" onclick="ajax_get_confirm('<?php echo $lang['wt_ensure_del'];?>', 'index.php?w=member_favorite_goods&t=delfavorites&type=goods&fav_id=<?php echo $favorites['fav_id'];?>');" title="<?php echo $lang['wt_del_&nbsp'];?>"><i class="icon-trash"></i></a>
            ...
            <a href="javascript:void(0)" class="fr" title="立即购买" wttype="buy_now" data-gid="<?php echo $favorites['goods']['goods_id'];?>"><i class="icon-shopping-cart"></i></a>
            <?php } else {?>
            <a href="javascript:void(0)" class="fr" title="加入购物车" wttype="add_cart" data-gid="<?php echo $favorites['goods']['goods_id'];?>"><i class="icon-shopping-cart"></i></a>
            <?php }?>
            <a href="javascript:void(0)" class="fl" wt_type="sharegoods" data-param='{"gid":"<?php echo $favorites['goods']['goods_id'];?>"}' title="<?php echo $lang['favorite_snsshare_goods'];?>"><i class="icon-share"></i> <?php echo $lang['favorite_snsshare_goods'];?> </a>
          </div>
          <dl class="favorite-goods-content">
            <dt><a href="index.php?w=goods&goods_id=<?php echo $favorites['goods']['goods_id'];?>" title="<?php echo $favorites['goods']['goods_name'];?>" target="_blank"><?php echo $favorites['goods']['goods_name'];?></a></dt>
            <?php if (!isset($favorites['goods']) || !$favorites['goods']['state']) {?>
            <dd class="lose"><em></em>商品已失效</dd>
            <?php } else {?>
            <dd class="goods-price">
              <?php if ($favorites['goods']['goods_sale_price'] < $favorites['log_price']) {?>
              <em class="down"></em>&yen;<strong><?php echo wtPriceFormat($favorites['goods']['goods_sale_price']);?></strong><span>&yen;<?php echo wtPriceFormat($favorites['log_price']);?></span>
              <?php } else {?>
              &yen;<strong><?php echo wtPriceFormat($favorites['goods']['goods_sale_price']);?></strong>
              <?php }?>
            </dd>
            <?php }?>
          </dl>
...
<script src="<?php echo STATIC_SITE_URL;?>/js/sns.js"></script> 
...
$(function(){	
    ...
    $(window).resize(function() {
        $('.favorite-pic-list').on('click','a[wttype="add_cart"]', function() {
            flyToCart($(this));
        });
    });
    $('.favorite-pic-list').on('click','a[wttype="add_cart"]', function() {
