...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=fx_order&t=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=fx_order&t=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?w=fx_order&t=get_xml',
        colModel : [
            ...
            {display: '订单分销金额', name : 'fx_order_amount', width : 150, sortable : true, align: 'left'}
          ],
        buttons : [
