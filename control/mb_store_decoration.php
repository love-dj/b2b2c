...
const ARTICLE_STATE_PUBLISHED = 3;
     ...
     * 店铺装修设置
     */
    public function decoration_settingWt() {
     ...
     * 店铺装修设置保存
     */
    public function decoration_setting_saveWt() {
        $model_store_decoration = Model('mb_store_decoration');
        ...
        if($result) {
            showDialog(L('wt_common_save_succ'), '', 'succ');
        } else {
     ...
     * 专题项目添加
     */
    public function special_item_addWt() {
     ...
     * 专题项目删除
     */
    public function special_item_delWt() {
        $model_mb_special = Model('mb_store_special');
     ...
     * 专题项目编辑
     */
    public function special_item_editWt() {
        $model_mb_special = Model('mb_store_special');
     ...
     * 专题项目保存
     */
    public function special_item_saveWt() {
        $model_mb_special = Model('mb_store_special');
        ...
        if($result) {
               showMessage(L('wt_common_save_succ'), urlShop('mb_store_decoration', 'decoration_edit'));
     ...
     * 更新项目排序
     */
    public function update_item_sortWt() {
     ...
     * 更新项目启用状态
     */
    public function update_item_usableWt() {
        $model_mb_special = Model('mb_store_special');
     ...
     * 导航装修
     */
    public function decoration_albumWt() {
        $model_store_decoration = Model('mb_store_decoration');
        $store_decoration_info = $model_store_decoration->getStoreDecorationInfo(array('store_id' => $_SESSION['store_id']));
    ...
    public function store_mb_sliders_dropWt()
    {
        try {
            $id = (int) $_REQUEST['id'];
    ...
    public function store_mb_slidersWt()
    {
        try {
     ...
     * 底部菜单
     */
    public function decoration_menuWt() {
        $model_store_decoration = Model('mb_store_decoration');
        $store_decoration_info = $model_store_decoration->getStoreDecorationInfo(array('store_id' => $_SESSION['store_id']));
     ...
     * 图片上传
     */
    public function decoration_album_uploadWt() {
        $store_id = $_SESSION ['store_id'];
     ...
     * 图片删除
     */
    public function decoration_album_delWt() {
     ...
     * 编辑首页
     */
    public function decoration_editWt() {
        $model_mb_special = Model('mb_store_special');
        $store_id = $_SESSION['store_id'];
     ...
     * 保存店铺装修背景设置
     */
    public function decoration_background_setting_saveWt() {
     ...
     * 装修导航保存
     */
    public function decoration_nav_saveWt() {
        $decoration_id = intval($_POST['decoration_id']);
        $nav = array();
     ...
     * 装修banner保存
     */
    public function decoration_banner_saveWt() {
        $decoration_id = intval($_POST['decoration_id']);
        $banner = array();
     ...
     * 装修添加块
     */
    public function decoration_block_addWt() {
        $decoration_id = intval($_POST['decoration_id']);
     ...
     * 装修块删除
     */
    public function decoration_block_delWt() {
     ...
     * 装修块保存
     */
    public function decoration_block_saveWt() {
        $block_id = intval($_POST['block_id']);
     ...
     * 装修块排序
     */
    public function decoration_block_sortWt() {
     ...
     * 商品搜索
     */
    public function goods_searchWt() {
    ...
    public function goods_listWt() {
        ...
        $condition['store_id'] = $_SESSION['store_id'];
        $condition['goods_name'] = array('like', '%' . $_GET['keyword'] . '%');
        $goods_list = $model_goods->getGoodsOnlineList($condition, 'goods_id,goods_name,goods_sale_price,goods_image', 10);
        Tpl::output('goods_list', $goods_list);
        Tpl::output('show_page', $model_goods->showpage());
     ...
     * 更新商品模块的商品价格
     */
    private function _update_module_goods_content($decoration_id, $store_id) {
        $model_store_decoration = Model('store_decoration');
        ...
        if(!empty($block_list_goods) && is_array($block_list_goods)) {
            foreach ($block_list_goods as $block) {
                ...
                foreach ($goods_array as $goods_key => $goods_value) {
                    ...
                    //商品信息
                    $goods_content = Model('goods')->getGoodsOnlineInfoByID($goods_value['goods_id']);
                    $new_goods_price = $goods_content['goods_price'];

                    //抢购
                    if (C('robbuy_allow')) {
                        $robbuy_info = Model('robbuy')->getRobbuyInfoByGoodsCommonID($goods_content['goods_commonid']);
                        if (!empty($robbuy_info)) {
                            $new_goods_price = $robbuy_info['robbuy_price'];
                        }
                    }

                    //限时折扣
                    if (C('sale_allow') && empty($robbuy_info)) {
     ...
     * 装修预览
     */
    public function decoration_previewWt() {
     ...
     * 装修静态文件生成
     */
    public function decoration_buildWt() {
        //静态文件路径
        ...
        //更新商品数据
        $this->_update_module_goods_content($decoration_id, $_SESSION['store_id']);
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            ...
            2=>array('menu_key'=>'decoration_album','menu_name'=>'首页导航设置','menu_url'=>urlShop('mb_store_decoration', 'decoration_album')),
            3=>array('menu_key'=>'decoration_menu','menu_name'=>'底部菜单设置','menu_url'=>urlShop('mb_store_decoration', 'decoration_menu')),//menu
        );
