...
$(function() {
	...
	var  fragment = uid[1];
	if(fragment){
		if (fragment.indexOf("WT") == 0) {
				addCookie("uid", "0");
			}else {
				addCookie("uid", fragment);
		}
	}
	var share_member_id = getQueryString('smid');
	
	if(share_member_id){
		addCookie("uid", share_member_id);
	}
	//自动登录
	var uawx = navigator.userAgent.toLowerCase();
	var wxout = getCookie('wxout');
	if(uawx.indexOf('micromessenger') > -1) {
		if (!wxout){
			$.getJSON(ApiUrl + '/index.php?w=connect&t=get_state', function(result){
				var allow_login = 0;
				if (result.datas.connect_wap_wx == '1' && result.datas.connect_wap_wxauto == '1') {
					allow_login = 1;
					if (allow_login) {
						var key = getCookie('key');
						var uid = window.location.href.split("#WT");
						...
						if(!key){
							location.href = ApiUrl+'/index.php?w=connect&t=index';
						}
					}
				}
			});
...
function toconfigure(name) {
    var key = '';
    $.ajax({
        type: 'post',
        url:ApiUrl+'/index.php?w=gain_content&t=configure',
        //url:ApiUrl+'/index.php?w=upload&t=up',
        data:{name:name},
        dataType: 'json',
        async: false,
        success:function(data){
