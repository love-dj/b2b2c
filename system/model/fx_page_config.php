...
class fx_page_configModel extends Model{
    /**
     * 读取模块内容记录
     *
     * @param
    ...
    public function getCodeRow($code_id,$web_id){
        ...
        $param['web_id']    = $web_id;
        $result = $this->table('fx_web_code')->where($param)->find();
        return $result;
    }
    ...
    public function getCodeList($condition = array()){
        $result = $this->table('fx_web_code')->where($condition)->order('web_id')->select();
        return $result;
    }
    ...
    public function updateCode($condition,$data){
        ...
        if (is_array($data)){
            $result = $this->table('fx_web_code')->where($condition)->update($data);
            return $result;
        } else {
            return false;
        }
    ...
    public function getWebList($condition = array('web_page' => 'index'),$page = '',$order = 'web_sort'){
        $result = $this->table('fx_web')->where($condition)->order($order)->page($page)->select();
        return $result;
    }
    ...
    public function updateWeb($condition,$data){
        ...
        if (is_array($data)){
            $result = $this->table('fx_web')->where($condition)->update($data);
            return $result;
        } else {
            return false;
        }
    ...
    public function updateWebHtml($web_id = 1,$web_array = array()){
        ...
        if(!empty($code_list) && is_array($code_list)) {
            Language::read('page_config,home_index_index');
            ...
            switch ($web_page) {
                case 'index':
                    $style_file = BASE_DATA_PATH.DS.'static'.DS.'fx_page_config'.DS.'default.php';
                    break;
                case 'index_pic':
                    $style_file = BASE_DATA_PATH.DS.'static'.DS.'fx_page_config'.DS.'focus.php';
                    break;
                case 'index_right_pic':
                    $style_file = BASE_DATA_PATH.DS.'static'.DS.'fx_page_config'.DS.'right.php';
                    break;
                default:
                    $style_file = BASE_DATA_PATH.DS.'static'.DS.'fx_page_config'.DS.'default.php';
                    break;
     ...
     *
     */
    public function getShowList($type = 'screen'){
        $condition = array();
        ...
        $result = $this->table('show_position')->where($condition[$type])->order('ap_id desc')->select();
        return $result;
    }
    ...
    public function getStyleList($style_id = 'index'){
        $style_data = array(
            'red' => Language::get('page_config_style_red'),
            'pink' => Language::get('page_config_style_pink'),
            'orange' => Language::get('page_config_style_orange'),
            'green' => Language::get('page_config_style_green'),
            'blue' => Language::get('page_config_style_blue'),
            'purple' => Language::get('page_config_style_purple'),
            'brown' => Language::get('page_config_style_brown'),
            'default' => Language::get('page_config_style_default')
            );
    ...
    public function getGoodsList($condition = array(),$order = 'goods_id desc',$page = ''){
        ...
        $field = 'goods_id,goods_commonid,goods_name,goods_image,goods_price,goods_marketprice,goods_sale_price';
        ...
        if (!empty($goods_list) && is_array($goods_list)) {
            ...
            foreach ($goods_list as $key => $value) {
                ...
                $value['goods_type'] = 1;
                $value['goods_price'] = $value['goods_sale_price'];
                $list[$goods_id] = $value;
            }
            $goods_ids = array_keys($list);//商品ID数组
            if (C('sale_allow')) {//限时折扣
            ...
            if (C('robbuy_allow')) {//最终以抢购价为准
                $robbuy_list = Model('robbuy')->getRobbuyListByGoodsCommonIDString(implode(',', $common_ids));
                if (!empty($robbuy_list) && is_array($robbuy_list)) {
                    foreach ($robbuy_list as $key => $value) {
                        $goods_commonid = $value['goods_commonid'];
                        $goods_price = $value['robbuy_price'];
