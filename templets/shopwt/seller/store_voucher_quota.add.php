<?php defined('ShopWT') or exit('Access Denied By ShopWT'); ?>
  ...
  <div class="wtsc-form-default">
    <form id="add_form" action="index.php?w=store_voucher&t=quotaadd" method="post">
    	<input type="hidden" name="form_submit" value="ok"/>
      <dl>
        <dt><i class="required">*</i><?php echo $lang['voucher_apply_addnum'].$lang['wt_colon'];?></dt>
        <dd><input id="quota_quantity" name="quota_quantity" type="text" class="text w50"/><em class="add-on"><?php echo $lang['wt_month'];?></em><span></span>
          <p class="hint"><?php echo $lang['voucher_apply_add_tip1'];?></p>
          <p class="hint"><?php echo sprintf($lang['voucher_apply_add_tip2'],C('sale_voucher_price'));?>,<?php echo sprintf($lang['voucher_apply_add_tip3'],C('sale_voucher_storetimes_limit'));?></p>
...
$(document).ready(function(){
    ...
    $("#add_form").validate({
    	...
    	submitHandler:function(form){
            var unit_price = parseInt(<?php echo C('sale_voucher_price');?>);
