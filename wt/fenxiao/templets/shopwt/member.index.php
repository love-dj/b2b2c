...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?w=fx_member&t=get_xml&mem_state=<?php echo intval($output['mem_stat']);?>',
        colModel : [
            ...
            <?php if(intval($output['mem_stat']) == 1){?>
             {display: '申请状态', name : 'fx_stat', width : 60, sortable : true, align: 'left'},
             ...
             {display: '申请时间', name : 'fx_time', width : 80, sortable : true, align: 'center'},
             {display: '通过时间', name : 'fx_handle_time', width : 80, sortable : true, align: 'center'}
            <?php }else{?>
            {display: '会员邮箱', name : 'member_email', width : 150, sortable : true, align: 'left'},
            {display: '会员手机', name : 'member_mobile', width : 80, sortable : true, align: 'center'}, 
            ...
            {display: '分销总额(元)', name : 'fx_amount', width : 100, sortable : true, align: 'center'},
            <?php }?>
            ],
        buttons : [
...
function fg_del(id) {
    if(confirm('清退后将不能恢复且该分销员及其的所分销的商品都将失效，确认清退该分销员吗？')){
        $.getJSON('index.php?w=fx_member&t=member_cancle', {member_id:id}, function(data){
            if (data.state) {
