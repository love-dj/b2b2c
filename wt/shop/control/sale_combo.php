...
class sale_comboControl extends SystemControl{
    private $links = array(
        array('url'=>'w=sale_combo&t=index','text'=>'商品列表'),
        array('url'=>'w=sale_combo&t=combo_quota_list','text'=>'套餐列表'),
        array('url'=>'w=sale_combo&t=combo_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?w=sale_combo&sale_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function indexWt() {
        ...
        $this->combo_goods_listWt();
    }

    /**
     * 活动商品列表
     */
    public function combo_goods_listWt() {
        Tpl::output('top_link',$this->sublink($this->links,'index'));
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_combo.goods');
    }

    /**
     * 活动商品管理XML
     */
    public function get_combo_goods_xmlWt() {
        $condition = array();
        if ($_POST['query'] != '') {
     ...
     * 删除订金预售商品活动
     */
    public function del_combo_goodsWt() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function combo_quota_listWt() {
        Tpl::output('top_link',$this->sublink($this->links,'combo_quota_list'));
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_combo.quota');
    }
     ...
     * 设置
     */
    public function combo_settingWt() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["sale_combo_price"], "require"=>"true", 'validator'=>'Number', "message"=>'请填写套餐价格'),
            );
            ...
            $data['sale_combo_price'] = intval($_POST['sale_combo_price']);

            $return = $model_setting->updateSetting($data);
            if($return){
                $this->log(L('wt_set').'推荐组合');
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_combo.setting');
    }
}
