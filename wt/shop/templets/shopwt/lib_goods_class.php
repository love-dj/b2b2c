...
.bottom .wtbtn { font-size: 14px; vertical-align: top; padding: 8px 19px; margin: 10px auto;}
</style>
<div class="page wrapper_search">
        ...
        <div class="text" id="commSelect">
            ...
            <div class="select_list" id="commListArea">
              <ul>
                ...
                <li  data-param="{stapleid:<?php echo $val['staple_id']?>}"><span wttype="staple_name"><?php echo $val['staple_name']?></span><a href="JavaScript:void(0);" wttype="del-comm-cate" title="<?php echo $lang['wt_delete'];?>">X</a></li>
                <?php }?>
                <?php }?>
        ...
        <div class="wp_category_list">
          <div id="class_div_1" class="category_list">
            <ul>
              ...
              <li class="" wttype="selClass" data-param="{gcid:<?php echo $val['gc_id'];?>,deep:1,tid:<?php echo $val['type_id'];?>}"> <a class="" href="javascript:void(0)"><i class="icon-double-angle-right"></i><?php echo $val['gc_name'];?></a></li>
              <?php }?>
              <?php }?>
            </ul>
          </div>
  ...
  <div class="wp_confirm">
    <form method="get">
      <input type="hidden" name="w" value="lib_goods" />
      <?php if ($output['goods_id']) {?>
      <input type="hidden" name="t" value="goods_edit" />
      <input type="hidden" name="goods_id" value="<?php echo $output['goods_id'];?>" />
      <?php } else {?>
      <input type="hidden" name="t" value="goods_add" />
      <?php }?>
      <input type="hidden" name="class_id" id="class_id" value="" />
      <input type="hidden" name="t_id" id="t_id" value="" />
      <div class="bottom tc">
      <label class="submit-border"><input disabled="disabled" wttype="buttonNextStep" value="下一步，填写商品信息" type="submit" class="submit"style=" width: 200px;" /></label>
      </div>
    </form>
  </div>
</div>
...
function selClass($this){
    ...
    $.getJSON(ADMIN_SITE_URL+'/index.php?w=lib_goods&t=ajax_goods_class', {gc_id : data_str.gcid, deep: deep}, function(data) {
        if (data != null) {
            $('input[wttype="buttonNextStep"]').attr('disabled', true);
...
function disabledButton() {
    if ($('#class_id').val() != '') {
        $('input[wttype="buttonNextStep"]').attr('disabled', false).css('cursor', 'pointer');
    } else {
        $('input[wttype="buttonNextStep"]').attr('disabled', true).css('cursor', 'auto');
    }
}
...
$(function(){
    ...
    // ajax选择分类
    $('li[wttype="selClass"]').click(function(){
        selClass($(this));
    });

    // 返回分类选择
    $('a[wt_type="return_choose_sort"]').unbind().click(function(){
        $('#class_id').val('');
        $('#t_id').val('');
        $("#commodityspan").show();
        $("#commoditydt").hide();
    ...
    // 常用分类选择
    $('#commListArea').find('span[wttype="staple_name"]').die().live('click',function() {
        $('#dataLoading').show();
        $('.wp_category_list').addClass('blank');
        $this = $(this);
        eval('var data_str = ' + $this.parents('li').attr('data-param'));
        $.getJSON(ADMIN_SITE_URL+'/index.php?w=lib_goods&t=ajax_show_comm&stapleid=' + data_str.stapleid, function(data) {
            if (data.done) {
                ...
                // 绑定ajax选择分类事件
                $('#class_div').find('li[wttype="selClass"]').click(function(){
                    selClass($(this));
                });
    ...
    // ajax删除常用分类
    $('#commListArea').find('a[wttype="del-comm-cate"]').die().live('click',function() {
        $this = $(this);
        eval('var data_str = ' + $this.parents('li').attr('data-param'));
        $.getJSON(ADMIN_SITE_URL+'/index.php?w=lib_goods&t=ajax_stapledel&staple_id='+ data_str.stapleid, function(data) {
            if (data.done) {
