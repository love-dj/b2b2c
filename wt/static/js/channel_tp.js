...
$(function(){
	...
	$(".wtap-channel-category").sortable({ items: 'dl' });
	$(".wtap-channel-category ul").sortable({ items: 'li' });

	slide_obj = $("#upload_slide_form");//初始化焦点大图区数据
    upload_obj = $("#upload_slide");
	slide_obj.find("ul").sortable({ items: 'li' });

	show_obj = $("#upload_show_form");//初始化三张联动区数据
	show_obj.find(".focus-trigeminy").sortable({ items: 'div[focus_id]' });
	show_obj.find("ul").sortable({ items: 'li' });
    show_upload_obj = $("#upload_show");
...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'index.php?w=web_channel&t=code_update',
...
function get_goods_class(){//查询子分类
	...
	if (gc_id>0) {
		...
		$.get('index.php?w=web_channel&t=get_category_list&id='+gc_id, function(data){
		  $(".wtap-channel-category").append(data);
		  $(".wtap-channel-category").sortable({ items: 'dl' });
		  $(".wtap-channel-category ul").sortable({ items: 'li' });
		  upload_gc(gc_id);
		});
...
function add_slide() {//增加图片
	for (var i = 1; i <= slide_max; i++) {//防止数组下标重复
		if (slide_obj.find("li[slide_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			var add_html = '';
			add_html = '<li slide_id="'+i+'" title="可上下拖拽更改显示顺序"><div class="title"><h4></h4><a class="wtap-btn-mini del" href="JavaScript:del_slide('+i+
...
//焦点区切换小图上传
function add_show() {//增加
	for (var i = 1; i <= show_max; i++) {//防止数组下标重复
		if (show_obj.find("div[focus_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			var text_type = '图片调用';
			add_html = '<div focus_id="'+i+'" class="focus-trigeminy-group" title="可上下拖拽更改显示顺序"><div class="title"><h4></h4>'+
			'<a class="wtap-btn-mini del" href="JavaScript:del_show('+i+');"><i class="fa fa-trash"></i>删除</a></div><ul></ul></div>';
			show_obj.find("#add_list").before(add_html);
			for (var pic_id = 1; pic_id <= pic_max; pic_id++) {
			    var text_append = '';
			    text_append += '<li pic_id="'+pic_id+'" onclick="select_show('+i+',this);" title="可左右拖拽更改图片排列顺序">';
				text_append += '<div class="focus-thumb">';
			    text_append += '<img title="" src="'+ADMIN_TEMPLATES_URL+'/images/picture.gif"/>';
				text_append += '</div>';
        	    text_append += '<input name="channel_show['+i+'][pic_list]['+pic_id+'][pic_id]" value="'+pic_id+'" type="hidden">';
        	    text_append += '<input name="channel_show['+i+'][pic_list]['+pic_id+'][pic_name]" value="" type="hidden">';
        	    text_append += '<input name="channel_show['+i+'][pic_list]['+pic_id+'][pic_url]" value="" type="hidden">';
        	    text_append += '<input name="channel_show['+i+'][pic_list]['+pic_id+'][pic_img]" value="" type="hidden">';
			    text_append += '</li>';
			    show_obj.find("div[focus_id='"+i+"'] ul").append(text_append);
			}
			show_obj.find("div ul").sortable({ items: 'li' });
			show_obj.find("div[focus_id='"+i+"'] li[pic_id='1']").trigger("click");//默认选中第一个图片
			break;
		}
	}
}
function select_show(focus_id,pic) {//选中图片
    var obj = $(pic);
    var pic_id = obj.attr("pic_id");
    var list = obj.attr("list");
    show_obj.find("li").removeClass("selected");
    obj.addClass("selected");
    var pic_name = obj.find("input[name*='[pic_name]']").val();
    var pic_url = obj.find("input[name*='[pic_url]']").val();

    show_obj.find("input[name='slide_id']").val(focus_id);
    show_obj.find("input[name='pic_id']").val(pic_id);
    show_obj.find("input[name='focus_pic[pic_name]']").val(pic_name);
    show_obj.find("input[name='focus_pic[pic_url]']").val(pic_url);
    show_obj.find(".type-file-file").val('');
    show_obj.find(".type-file-text").val('');
    show_upload_obj.show();
}
function focus_pic(pic_id,pic_img) {//更新图片
	if (pic_img!='') {
	    var focus_id = show_obj.find("input[name='slide_id']").val();
	    var pic_name = show_obj.find("input[name='focus_pic[pic_name]']").val();
	    var pic_url = show_obj.find("input[name='focus_pic[pic_url]']").val();
	    var obj = show_obj.find("div[focus_id='"+focus_id+"'] li[pic_id='"+pic_id+"']");
	    obj.find("img").attr("src",UPLOAD_SITE_URL+'/'+pic_img);
	    obj.find("img").attr("title",pic_name);
	...
	show_obj.find('.web-save-succ').show();
	setTimeout("show_obj.find('.web-save-succ').hide()",2000);
}
function del_show(focus_id) {//删除切换组
    if (show_obj.find("div[focus_id]").size()<2) {
         return;//保留一个
    }
	show_obj.find("div[focus_id='"+focus_id+"']").remove();
	var slide_id = show_obj.find("input[name='slide_id']").val();
	if (focus_id==slide_id) {
    	show_obj.find("input[name='slide_id']").val('');
    	show_upload_obj.hide();
	}
}