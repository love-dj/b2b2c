...
class informControl extends SystemControl{
    private $links = array(
            array('url'=>'w=inform&t=inform_list','text'=>'未处理'),
            array('url'=>'w=inform&t=inform_handled_list','text'=>'已处理'),
            array('url'=>'w=inform&t=inform_subject_type_list','text'=>'类型设置'),
            array('url'=>'w=inform&t=inform_subject_list','text'=>'主题设置')
    );
    public function __construct(){
        parent::__construct();
        Language::read('inform');
        if ($_GET['t'] == 'index') $_GET['t'] = 'inform_list';
     ...
     * 已处理的举报列表
     */
    public function inform_handled_listWt(){
		Tpl::setDirquna('shop');
        Tpl::showpage('inform.list');
    }
    ...
    public function get_xmlWt(){
        ...
        if ($inform_list && is_array($inform_list)) {
            ...
            foreach ($inform_list as $k => $v){
                ...
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?w=inform&t=show_handle_page&inform_id={$v['inform_id']}&inform_goods_name={$v['inform_goods_name']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
                } else {
                    $list['operation'] = '--';
                }
     ...
     * 举报类型列表
     */
    public function inform_subject_type_listWt() {
     ...
     * 举报主题列表
     */
    public function inform_subject_listWt(){
     ...
     * 添加举报类型页面
     */
    public function inform_subject_type_addWt(){
Tpl::setDirquna('shop');
     ...
     * 保存添加的举报类型
     */
    public function inform_subject_type_saveWt(){
        ...
        if ($error != ''){
            ...
            $this->log(L('wt_add,inform_type').'['.$_POST['inform_type_name'].']',1);
            showMessage(Language::get('wt_common_save_succ'),'index.php?w=inform&t=inform_subject_type_list');
        }
    }
    ...
    public function inform_subject_type_dropWt(){
        ...
        $this->log(L('wt_del,inform_type').'[ID:'.$_POST['inform_type_id'].']',1);
        showMessage(Language::get('wt_common_del_succ'),'index.php?w=inform&t=inform_subject_type_list');
     ...
     * 添加举报主题页面
     */
    public function inform_subject_addWt(){
     ...
     * 保存添加的举报主题
     */
    public function inform_subject_saveWt(){
        ...
        if ($error != ''){
            ...
            $this->log(L('wt_add,inform_subject').'['.$input['inform_subject_type_name'].']',1);
            showMessage(Language::get('wt_common_save_succ'),'index.php?w=inform&t=inform_subject_list');
        }
    }
     ...
     * 删除举报主题,伪删除只是修改标记
     */
    public function inform_subject_dropWt(){
        ...
        $this->log(L('wt_del,inform_subject').'['.$_POST['inform_subject_id'].']',1);
        showMessage(Language::get('wt_common_del_succ'),'index.php?w=inform&t=inform_subject_list');
    }

    /*
     * 显示处理举报
     */
    public function show_handle_pageWt() {
     ...
     * 处理举报
     */
    public function inform_handleWt(){
        ...
        if($model_inform->updateInform($update_array,$where_array)) {
            ...
            showMessage(Language::get('wt_common_op_succ'),'index.php?w=inform&t=inform_list');
        }
        else {
    ...
    private function denyGoods($goods_id) {
        ...
        $goods_content = $model_goods->getGoodsInfoByID($goods_id, 'goods_commonid');
        if (empty($goods_content)) {
            return true;
        }
        return Model('goods')->editProducesLockUp(array('goods_stateremark' => '商品被举报，平台禁售'),array('goods_commonid' => $goods_content['goods_commonid']));
    }
}
