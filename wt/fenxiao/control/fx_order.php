...
class fx_orderControl extends SystemControl{
    ...
    public function get_xmlWt(){
        ...
        $order_list = $model_fx_order->getDisOrderList($condition,$page,'',$order);
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $list = array();
            //$list['operation'] = "<a class=\"btn green\" href=\"index.php?w=fx_order&t=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['operation'] = '--';
            ...
            $list['fx_order_amount'] = wtPriceFormat($order_info['fx_order_amount']);
    ...
    public function export_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=fx_order&t=index');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
                $data = $model_fx_order->getDisOrderList($condition,'',self::EXPORT_SIZE,$order);
                $this->createExcel($data);
            }
        }else{  //下载
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_fx_order->getDisOrderList($condition,'',"{$limit1},{$limit2}",$order);
            $this->createExcel($data);
        }
    }
    ...
    private function createExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$order_info){
            ...
            $tmp[] = array('data'=>$list['fx_order_amount']);
            $excel_data[] = $tmp;
        }
        $excel_data = $excel_obj->charset($excel_data,CHARSET);
        $excel_obj->addArray($excel_data);
        $excel_obj->addWorksheet($excel_obj->charset(L('exp_od_order'),CHARSET));
        $excel_obj->generateXML('fx_order-'.$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
