...
$(function(){
    ...
    var connentNode = function(data){
        ...
        resourceSiteUrl = data.static_site_url;
        if (!data.node_chat) {
            $.sDialog({
                skin:"red",
                content:'在线聊天系统暂时未启用',
...
//                }
                function node_send_msg(data){
                    if(connect === 1) {
                        $.ajax({
                            type:'post',
                            url:ApiUrl+'/index.php?w=member_chat&t=send_msg',
                            data:data,
                            dataType:'json',
...
//                }
        ...
        $('#chat_msg_log').click(function(){
            $.ajax({
                type:'post',
                url:ApiUrl+'/index.php?w=member_chat&t=get_chat_log&page=50',
        ...
        function insert_html(msgData) {
            ...
            if (!$.isEmptyObject(msgData.goods_content)) {
                ...
                var html = '<div class="wtm-chat-product"> <a href="' + WapSiteUrl + '/html/product_detail.html?goods_id=' + goods.goods_id + '" target="_blank"><div class="goods-pic"><img src="' + goods.pic36 + '" alt=""/></div><div class="goods-content"><div class="goods-name">' + goods.goods_name + '</div><div class="goods-price">￥' + goods.goods_sale_price + '</div></div></a> </div>';
                $("#chat_msg_html").append(html);
            }
		...
		$('input[name="_pic"]').ajaxUploadImage({
				url: ApiUrl + "/index.php?w=member_chat&t=image_send",
				data: {
					key: key,
					name: "_pic"
				},
