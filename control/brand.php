...
class brandControl extends BaseHomeControl {
            ...
            foreach ($brand_c_list as $key=>$brand_c){
                ...
                if (empty($gc_array)) {
                    if ($brand_c['show_type'] == 1){
                        $brand_listnew[99999]['text'][] = $brand_c;
                    } else {
                        $brand_listnew[99999]['image'][] = $brand_c;
                    }
                    $brand_class[99999]['brand_class'] = '其他';
                } else {
                    if ($brand_c['show_type'] == 1){
    ...
    public function listWt(){
        ...
        if (!empty($_GET['key'])) {
            ...
            switch ($order_tmp) {
                ...
                case '2' : // 浏览量
                    ...
                    break;
                case '3' : // 价格
                    $order = 'goods_sale_price' . ' ' . $sequence;
                    break;
            }
        }
	    ...
	    //处理商品消费者保障服务信息
            ...
            //处理商品消费者保障服务信息
            $goods_list = $model_goods->getGoodsContract($goods_list, $contract_item);
			$goods_num= $model_goods->getGoodsOnlineCount($where);
        }
