$(document).ready(function(){
	...
	$('span[wt_type="inline_edit"]').click(function(){
		...
		$('<input type="text">')
                        ...
                        if(event.keyCode == 13)
                        {
							...
							//column 修改字段名
							//value 修改内容
                            $.get('index.php?w='+h+'&t=ajax',{branch:ajax_branch,id:s_id,column:s_name,value:$(this).attr('value')},function(data){
                                if(data === 'false')
                                {
                                    alert('名称已经存在，请您换一个');
	...
	$('img[wt_type="inline_edit"]').click(function(){
		...
		$.get('index.php?w='+h+'&t=ajax',{branch:ajax_branch,id:i_id,column:i_name,value:i_val},function(data){
		if(data == 'true')
			{
				if(i_val == 0)
				{
	...
	$('a[wt_type="inline_edit"]').click(function(){
		var i_id    = $(this).attr('fieldid');
		var i_name  = $(this).attr('fieldname');
		var i_src   = $(this).attr('src');
		var i_val   = ($(this).attr('fieldvalue'))== 0 ? 1 : 0;
...
//新的inline_edit调用方法
//javacript
//$('span[wt_type="class_sort"]').inline_edit({h: 'what',t: 'update_class_sort'});
//html
 ...
 $.fn.inline_edit= function(options) {
     ...
     function onClick() {
         ...
         .blur(function(){
             ...
             if(new_value != '') {
                 $.get('index.php?w='+settings.h+'&t='+settings.t+'&branch=ajax',{id:column_id,value:new_value},function(data){
                     data = $.parseJSON(data);
                     if(data.result) {
