        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $url = array(
                            array(
                                    'url'=>'index.php?w=sns_member&t=tag_add',
                                    'msg'=>Language::get('sns_member_add_once_more'),
                            ),
                            array(
                                    'url'=>'index.php?w=sns_member&t=member_tag',
                                    'msg'=>Language::get('sns_memner_return_list'),
                            )
                    );
                    $this->log(L('wt_add,sns_member_tag').'['.$_POST['membertag_name'].']',1);
                    showMessage(Language::get('wt_common_op_succ'),$url);
                }else {
                    showMessage(Language::get('wt_common_op_fail'));
                }
            }
...
Tpl::setDirquna('shop');
    ...
    public function tag_editWt(){
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $this->log(L('wt_edit,sns_member_tag').'['.$_POST['membertag_name'].']',1);
                    showMessage(Language::get('wt_common_op_succ'),'index.php?w=sns_member&t=member_tag');
                }else {
                    showMessage(Language::get('wt_common_op_fail'));
                }
            }
    ...
    public function tag_delWt(){
        ...
        if ($result){
            $this->log(L('wt_del,sns_member_tag').'[ID:'.$mtag_id.']',1);
     ...
     * 推荐标签
     */
    public function tag_recommendWt() {
        // 验证
        $mtag_id = intval($_GET['id']);
        if($mtag_id <= 0){
    ...
    public function get_xmlWt() {
        ...
        foreach ($tag_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href='index.php?w=sns_member&t=tag_del&id=" . $value['mtag_id'] . "'><i class='fa fa-trash-o'></i>删除</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('wt_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?w=sns_member&t=tag_edit&id=" . $value['mtag_id'] . "'>编辑标签</a></li><li><a href='index.php?w=sns_member&t=tag_member&id=".$value['mtag_id']."'>标签会员</a></li><li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('sns_member','tag_recommend',array('id'=> $value['mtag_id'], 'recommend' => ($value['mtag_recommend'] == 1 ? 0 : 1))) . "\")'>" . ($value['mtag_recommend'] == 1 ? '取消推荐' : '推荐标签') . "</a></li></ul></span>";
            ...
            $param['mtag_recommend'] = $value['mtag_recommend'] ==  '1' ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
     ...
     * 推荐标签会员
     */
    public function tag_mrecommendWt() {
        // 验证
        $mtag_id = intval($_GET['id']);
    ...
    public function get_tm_xmlWt() {
        ...
        foreach ($tag_list as $value) {
            ...
            $param['recommend'] = $value['recommend'] ==  '1' ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $data['list'][$value['mtag_id']] = $param;
        }
        echo Tpl::flexigridXML($data);exit();
    }
