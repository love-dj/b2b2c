...
class voucherControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('voucher_allow') != 1 || C('points_isuse')!=1){
            showMessage(Language::get('admin_voucher_unavailable'),'index.php?w=operation&t=point','html','succ',1,4000);
        }
    ...
    public function settingWt(){
        ...
        if (chksubmit()){
            ...
            $validate_arr[] = array('input'=>$_POST['sale_voucher_price'],'require'=>'true','validator'=>'IntegerPositive','message'=>Language::get('admin_voucher_setting_price_error'));
            $validate_arr[] = array('input'=>$_POST['sale_voucher_storetimes_limit'],'require'=>'true','validator'=>'IntegerPositive','message'=>Language::get('admin_voucher_setting_storetimes_error'));
            $validate_arr[] = array('input'=>$_POST['sale_voucher_buyertimes_limit'],'require'=>'true','validator'=>'IntegerPositive','message'=>Language::get('admin_voucher_setting_buyertimes_error'));
            ...
            //每月代金劵软件服务单价
            $sale_voucher_price = intval($_POST['sale_voucher_price']);
            if($sale_voucher_price < 0) {
                $sale_voucher_price = 20;
            }
            //每月店铺可以发布的代金劵数量
            $sale_voucher_storetimes_limit = intval($_POST['sale_voucher_storetimes_limit']);
            if($sale_voucher_storetimes_limit <= 0) {
                $sale_voucher_storetimes_limit = 20;
            }
            //买家可以领取的代金劵总数
            $sale_voucher_buyertimes_limit = intval($_POST['sale_voucher_buyertimes_limit']);
            if($sale_voucher_buyertimes_limit <= 0) {
                $sale_voucher_buyertimes_limit = 5;
            }
            $update_array = array();
            $update_array['sale_voucher_price'] = $sale_voucher_price;
            $update_array['sale_voucher_storetimes_limit'] = $sale_voucher_storetimes_limit;
            $update_array['sale_voucher_buyertimes_limit'] = $sale_voucher_buyertimes_limit;
            $result = $setting_model->updateSetting($update_array);
            if ($result){
                $this->log(L('admin_voucher_setting,wt_voucher_price_manage'));
     ...
     * 代金券面额列表XML
     */
    public function pricelist_xmlWt()
    {
        $model = Model();
    ...
    public function priceaddWt(){
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('wt_add,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('wt_common_save_succ'),'index.php?w=voucher&t=pricelist');
                }else {
                    showMessage(Language::get('wt_common_save_fail'),'index.php?w=voucher&t=priceadd');
                }
            }
        }else {
     ...
     * 编辑代金券面额
     */
    public function priceeditWt(){
        $id = intval($_GET['priceid']);
        if ($id <= 0){
            $id = intval($_POST['priceid']);
        }
        if ($id <= 0){
            showMessage(Language::get('param_error'),'index.php?w=voucher&t=pricelist');
        }
        $model = Model();
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('wt_edit,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('wt_common_save_succ'),'index.php?w=voucher&t=pricelist');
                }else {
                    showMessage(Language::get('wt_common_save_fail'),'index.php?w=voucher&t=pricelist');
                }
            }
        }else {
    ...
    public function pricedropWt(){
        ...
        if ($rs){
            $this->log(L('wt_del,admin_voucher_priceadd').'[ID:'.$voucher_price_id.']');
            showMessage(Language::get('wt_common_del_succ'),'index.php?w=voucher&t=pricelist');
        }else{
            showMessage(Language::get('wt_common_del_fail'),'index.php?w=voucher&t=pricelist');
        }
    }
     ...
     * 套餐管理XML
     */
    public function quotalist_xmlWt()
    {
        $condition = array();
     ...
     * 代金券列表XML
     */
    public function templatelist_xmlWt()
    {
        $condition = array();
     ...
     * 代金券模版编辑
     */
    public function templateeditWt(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            $t_id = intval($_POST['tid']);
        }
        if ($t_id <= 0){
            showMessage(Language::get('param_error'),'index.php?w=voucher&t=templatelist','','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
            showMessage(Language::get('param_error'),'index.php?w=voucher&t=templatelist','html','error');
        }
        if(chksubmit()){
            ...
            if($rs){
                $this->log(L('wt_edit,wt_voucher_price_manage,admin_voucher_styletemplate').'[ID:'.$t_id.']');
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=voucher&t=templatelist','succ');
            }else{
                showMessage(Language::get('wt_common_save_fail'),'index.php?w=voucher&t=templatelist','error');
            }
        }else{            
    ...
    private function show_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                3=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?w=voucher&t=templatelist'),
                2=>array('menu_key'=>'quotalist','menu_name'=>Language::get('admin_voucher_quota_manage'), 'menu_url'=>'index.php?w=voucher&t=quotalist'),
                5=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?w=voucher&t=pricelist'),
                4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?w=voucher&t=setting'),
                );
                break;
            case 'priceedit':
                $menu_array = array(
                1=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?w=voucher&t=setting'),
                2=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?w=voucher&t=pricelist'),
                3=>array('menu_key'=>'priceedit','menu_name'=>Language::get('admin_voucher_priceedit'), 'menu_url'=>'')
                );
                break;
            case 'templateedit':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?w=voucher&t=templatelist'),
