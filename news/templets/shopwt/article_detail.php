    ...
    <article class="article-detail-content">
      <header id="articleTool-holder">
        ...
        <ul class="article-toolbar" id="articleTool">
          ...
          <li class="none"><a href="Javascript: void(0);" onmousedown="document.execCommand('saveAs');" title="<?php echo $lang['news_text_saveas'];?>" class="saveas"><?php echo $lang['news_text_saveas'];?></a></li>
          <li><a href="Javascript: void(0);" onmousedown="window.print();" class="print" title="<?php echo $lang['news_text_print'];?>"><?php echo $lang['news_text_print'];?></a></li>
          <li><a href="Javascript: void(0);" onmousedown="window.close();" class="close" title="<?php echo $lang['news_text_close'];?>"><?php echo $lang['news_text_close'];?></a></li>
        </ul>
      </header>
      ...
      <section class="article-body" id="text"><?php echo $output['article_detail']['article_content'];?>
        ...
        <div class="article-related-goods"> 
          <!-- 相关商品 -->
          <h3><?php echo $lang['news_article_goods'];?></h3>
          ...
          <dl>
            ...
            <dd class="goods-price"><?php echo $lang['news_price'];?><?php echo $lang['wt_colon'];?><em><?php echo $value['price'];?></em></dd>
            <dd class="goods-handle"><a target="_blank" href="<?php echo $value['url']?>"><?php echo $lang['news_goods_detail'];?></a></dd>
          </dl>
          <?php } ?>
        </div>
        <?php } ?>
        ...
        <div class="article-tag"><?php echo $lang['news_keyword'];?><?php echo $lang['wt_colon'];?>
          ...
          <a href="<?php echo NEWS_SITE_URL.DS;?>index.php?w=article&t=article_search&keyword=<?php echo $value;?>" target="_blank"><?php echo $value;?></a>
          <?php } ?>
          <?php } ?>
        </div>
      </section>
      <section class="article-related-articles"> 
        ...
        <h3><?php echo $lang['news_other_article'];?></h3>
        <ul>
          <?php foreach($output['article_link_list'] as $value) { ?>
          <li><a target="_blank" href="<?php echo getNEWSArticleUrl($value['article_id']);?>"> <?php echo $value['article_title'];?> </a> </li>
          <?php } ?>
        </ul>
          <?php } ?>
      </section>
      <?php if(intval(C('news_attitude_flag')) === 1 && intval($output['article_detail']['article_attitude_flag']) === 1) { ?>
      <section  class="article-attitude"> 
        <!-- 心情 -->
        <?php require('article_attitude.php');?>
      </section>
      <?php } ?>
      <?php if(intval(C('news_comment_flag')) === 1 && intval($output['article_detail']['article_comment_flag']) === 1) { ?>
