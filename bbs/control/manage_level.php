...
class manage_levelControl extends BasebbsManageControl{
    ...
    public function levelWt(){
        ...
        if(chksubmit()){
            ...
            if($_POST['levelset'] == 'custom'){
                ...
                if($levelset) $mlref_info = Model()->table('bbs_mlref')->where(array('mlref_id'=>$levelset))->find();
                if(!empty($mlref_info)){
            ...
            $rs = Model('bbs_level')->levelInsert($insert, true);
            showDialog(L('wt_common_op_succ'), 'reload', 'succ');
        }
        $model = Model();
        // Defaule Member Title list
        $mldefault_list = rkcache('bbs_level', true);
        if (!empty($mldefault_list)){
            ...
            $mlref_list = $model->table('bbs_mlref')->where(array('mlref_status'=>1))->select();
