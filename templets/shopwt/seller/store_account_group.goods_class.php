...
<div class="wtsc-account-container">
  <h4>
    <input id="<?php echo $key;?>" value="1" name="cate[<?php echo $key;?>]" <?php if (in_array($key,$output['gc_id_use_list'])) echo 'checked="checked"'; ?> class="checkbox" wttype="btn_select_module_class" type="checkbox" />
  ...
  <ul class="wtsc-account-container-list">
    ...
    <li>
      ...
      )</span> <a cate_id="<?php echo $key2; ?>" wt_title="<?php echo $value2['gc_name']; ?>" parent_id="<?php echo $key; ?>" wt_type="edit" href="javascript:void(0);" title="选择下级分类"><i class="icon-pencil"></i></a> 
      <!-- 三级分类循环到隐藏域 -->
...
$(document).ready(function(){
    ...
    $('input[deep_type="2"]').on('click',function(){
        if ($(this).attr('checked') == 'checked') {
            ...
            //父级选框是否也选中功能注释
//             tmp = $(this).parents('.wtsc-account-container').find('input[deep_type="2"]');
...
//         	$('#'+$(this).attr('parent_id')).prop('checked',false);
	...
	$('a[wt_type="edit"]').on('click',function(){
        ...
        cate3_html += '</ul><div class="bottom"><label for="cate_3_selall" class="fl ml10 mt5"><input type="checkbox" id="cate_3_selall"> 全选</label><a id="cate_3_submit" class="wtbtn wtbtn-mint" href="javascript:void(0);">确认</a> <span style="color:#f30;">确认后，还需要点击页面底部的“提交保存”按钮完成保存操作</span></div></div>';
        html_form('select_cate', '选择 '+ $(this).attr('wt_title') +' 子分类', cate3_html, 600,1);
	});
	...
	$('body').on('click','#cate_3_submit', function(){
		...
		//父级选框是否也选中功能注释
//         tmp = $('#'+cur_edit_id).parents('.wtsc-account-container').find('input[deep_type="2"]');
//         if (tmp.size() == tmp.filter(':checked').size()){
