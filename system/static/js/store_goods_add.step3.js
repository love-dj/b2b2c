$(function(){
    // 商品图片ajax上传
    $('.wtsc-upload-btn').find('input[type="file"]').unbind().bind('change', function(){
        var id = $(this).attr('id');
        ajaxFileUpload(id);
    });
    //凸显鼠标触及区域、其余区域半透明显示
    ...
    // 关闭相册
    $('a[wttype="close_album"]').click(function(){
        $(this).hide();
        $(this).prev().show();
        $(this).parent().next().html('');
    });
    // 绑定点击事件
    $('div[wttype^="file"]').each(function(){
    ...
    $.ajaxFileUpload({
        ...
        success : function (data, status) {
                    if (typeof(data.error) != 'undefined') {
                        ...
                        $('input[wttype="' + id + '"]').val(data.name);
                        $('img[wttype="' + id + '"]').attr('src', data.thumb_name);
                        selectDefaultImage($('div[wttype="' + id + '"]'));      // 选择默认主图
                        checkDefaultImage($('div[wttype="' + id + '"]'));
                    }
                    $.getScript(SHOP_TEMPLATES_URL+ '/js/store_goods_add.step3.js');
                },
...
function selectDefaultImage($this) {
    ...
    // 删除
    $this.parents('li:first').find('a[wttype="del"]').click(function(){
...
function insert_img(name, src, color_id) {
    $_thumb = $('ul[wttype="ul'+ color_id +'"]').find('.upload-thumb').each(function(){
        if ($(this).find('input').val() == '') {
