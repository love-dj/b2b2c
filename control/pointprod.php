...
class pointprodControl extends BasePointShopControl{
    ...
    public function plistWt(){
        //查询会员及其附属信息
        $result = parent::pointsMInfo(true);
     ...
     * 积分礼品详细
     */
    public function pinfoWt() {
        $pid = intval($_GET['id']);
        if (!$pid){
            showDialog(L('pointprod_parameter_error'),urlShop('points','index'),'error');
        }
        $model_pointprod = Model('pointprod');
        //查询兑换礼品详细
        ...
        if (!in_array($pid, $tm_tm_visite_pgoods)){//如果已经浏览过该商品则不重复累计浏览次数
            ...
            if ($result['state'] == true){//累加成功则cookie中增加该商品ID
                ...
                setWtCookie('tm_visite_pgoods',implode(',', $tm_tm_visite_pgoods));
            }
        }
