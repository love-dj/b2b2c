...
.pw-box-ft { background: url("<?php echo SHOP_TEMPLATES_URL;?>/images/payment/icon-red.png") no-repeat scroll 50px 8px #ff7674; height: 44px; padding: 8px 0 8px 125px; }
.p-w-sidebar { background: url("<?php echo SHOP_TEMPLATES_URL;?>/images/payment/phone-bg.png") no-repeat scroll 50px 0 rgba(0, 0, 0, 0); float: left; height: 421px; margin-top: -20px; padding-left: 50px; width: 379px; }
...
<div class="wtc-main">
  ...
  <div class="wtc-receipt-info">
    <div class="wtc-receipt-info-title">
      <h3> 订单提交成功，请您尽快付款。 应付金额：<strong><?php echo wtPriceFormat($output['api_pay_amount']);?></strong>元 </h3>
    </div>
  </div>
  ...
  <div class="wxpayment"> 
    ...
    <div class="pay-weixin">
      ...
      <div class="p-w-bd">
        <div class="p-w-box">
          <div class="pw-box-hd"> <img width="298" src="<?php echo BASE_SITE_URL?>/index.php?w=payment&t=qrcode&data=<?php echo urlencode($output['pay_url']);?>"> </div>
          <div class="pw-box-ft">
            <p>请使用微信扫一扫</p>
            <p>扫描二维码支付</p>
          </div>
...
function queryOrderState(){
    $.ajax({
        type: "GET",
        url: "<?php echo BASE_SITE_URL?>/index.php?w=payment&t=query_state&<?php echo $output['args'];?>&m="+Math.random(),
        data: "",
        dataType: "json",
        timeout: 4000,
        success: function(result) {
            if(result.state==1){
                if (result.type == 'r') {
                	top.window.location = '<?php echo BASE_SITE_URL.'/index.php?w=member_order'?>';
				} else if (result.type == 'pd') {
					top.location.href = '<?php echo MEMBER_SITE_URL.'/index.php?w=predeposit&t=index'?>';
                } else {
                	top.window.location = '<?php echo BASE_SITE_URL.'/index.php?w=member_order_vr&t=index'?>';
                }
            }
        }
    });
