...
<a class="btn green confirm-del-on-click" href="javascript:;" data-href="index.php?w=rechargecard&t=del_card&id={$val['id']}"><i class="fa fa-trash"></i>删除</a>
EOB;
    ...
    public function export_step1Wt()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                Tpl::output('murl', 'index.php?w=rechargecard&t=index');
				Tpl::setDirquna('shop');
     ...
     * 充值卡使用明细XML
     */
    public function log_list_xmlWt()
    {
     ...
     * 导出使用日志
     */
    public function log_export_step1Wt()
    {
        $model = Model('rcb_log');
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?w=rechargecard&t=log_list');
				Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                return;
