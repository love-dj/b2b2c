...
$(function(){
    ...
    $("#sale_list_form .sale-box").sortable({//促销区拖动排序保存
        items: 'dl',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		default:
			...
			$("#upload_show_form ul").sortable({ items: 'li' });
			break;
	}
}
...
function update_category() {//更新分类
	...
	if (get_text=='1') {
		$(".home-templates-board-box .category-list ul").html('');
		$("#category_list_form .category-list").each(function(){
		  ...
		  $(".home-templates-board-box .category-list ul").append(''+text_append+'');
		});
...
function update_brand() {//更新品牌
	...
	if (get_text=='1') {
		...
		$("img[select_brand_id]").each(function() {
		  ...
		  $(".wt-btbrand ul.brands").append('<li><span>'+text_append+'</span></li>');
		});
...
function show_recommend_pic_dialog(id) {//弹出框
    ...
    if(obj.find("div[select_recommend_pic_id='"+id+"']").size()==0) {//不存在时添加
        var text_append = '<div select_recommend_pic_id="'+id+'" class="right-floor">'+
							...
							'<a href="javascript:void(0);" recommend_pic_id="31" class="left-c">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="32" class="left-d">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="33" class="left-e">140*140</a>'+
							'<a href="javascript:void(0);" recommend_pic_id="34" class="left-f">140*140</a>'+
            	  '</div>';
...
function recommend_pic(pic_id,pic_img) {//更新图片
	if (pic_img!='') {
        ...
        $(".middle dl[recommend_id='"+id+"'] dd").html('<div class="right-floor">'+text_append+'</div>');//页面中的图片显示
        $(".middle dl[recommend_id='"+id+"'] dd div.right-floor a").css("opacity","");
	    $(".middle dl[recommend_id='"+id+"'] dt h4").html(recommend_name);
	}
}
//切换图片上传
function add_slide_show() {//增加图片
	for (var i = 1; i <= slide_pic_max; i++) {//防止数组下标重复
		if ($("#upload_show_form ul li[slide_show_id='"+i+"']").size()==0) {//编号不存在时添加
    	    var text_input = '';
    	    text_input += '<input name="show['+i+'][pic_id]" value="'+i+'" type="hidden">';
    	    text_input += '<input name="show['+i+'][pic_name]" value="" type="hidden">';
    	    text_input += '<input name="show['+i+'][pic_url]" value="" type="hidden">';
			text_input += '<input name="show['+i+'][pic_sname]" value="" type="hidden">';
    	    text_input += '<input name="show['+i+'][pic_surl]" value="" type="hidden">';
			text_input += '<input name="show['+i+'][pic_simg]" value="" type="hidden">';
    	    text_input += '<input name="show['+i+'][pic_img]" value="" type="hidden">';
			var add_html = '';
			add_html = '<li slide_show_id="'+i+'"><div class="show-pic"><span class="ac-ico" onclick="del_slide_show('+i+
			');"></span><span class="thumb size-106x106" onclick="select_slide_show('+i+');"><img src="'+ADMIN_TEMPLATES_URL+
			'/images/picture.gif" /></span></div>'+text_input+'</li>';
			$("#upload_show_form ul").append(add_html);
			select_slide_show(i);
			break;
		}
	}
}
function select_slide_show(pic_id) {//选中图片
    var obj = $("#upload_show_form ul li[slide_show_id='"+pic_id+"']");
    var pic_name = obj.find("input[name='show["+pic_id+"][pic_name]']").val();
    var pic_url = obj.find("input[name='show["+pic_id+"][pic_url]']").val();
	var pic_sname = obj.find("input[name='show["+pic_id+"][pic_sname]']").val();
    var pic_surl = obj.find("input[name='show["+pic_id+"][pic_surl]']").val();
	var pic_simg = obj.find("input[name='show["+pic_id+"][pic_simg]']").val();
    var pic_img = obj.find("img").attr("src");
    $("#upload_show_form ul li").removeClass("selected");
    ...
    $("#upload_show_form .type-file-file").val('');
    $("#upload_show_form .type-file-text").val('');
    $("#upload_slide_show").show();
    obj.addClass("selected");
}
function slide_show(pic_id,pic_img) {//更新图片
    var obj = $("#upload_show_form ul li[slide_show_id='"+pic_id+"']");
	if (pic_img!='') {
        ...
        obj.find("input[name='show["+pic_id+"][pic_name]']").val(pic_name);
        obj.find("input[name='show["+pic_id+"][pic_url]']").val(pic_url);
		obj.find("input[name='show["+pic_id+"][pic_sname]']").val(pic_sname);
        obj.find("input[name='show["+pic_id+"][pic_surl]']").val(pic_surl);
		obj.find("input[name='show["+pic_id+"][pic_simg]']").val(pic_simg);
        obj.find("input[name='show["+pic_id+"][pic_img]']").val(pic_img);

        var pic_img = $("#upload_show_form ul li").first().find("img").attr("src");
        $("#picture_show").html('<img src="'+pic_img+'"/>');
    }
}
function del_slide_show(pic_id) {//删除图片
    if ($("#upload_show_form ul li").size()<2) {
         return;//保留一个
    }
	$("#upload_show_form li[slide_show_id='"+pic_id+"']").remove();
	var slide_id = $("input[name='slide_id']").val();
	if (pic_id==slide_id) {
    	$("input[name='slide_id']").val('');
    	$("#upload_slide_show").hide();
	}
}
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_sale_goods_list").load('index.php?w=page_config&t=goods_list&'+$.param({'id':gc_id,'goods_order':goods_order,'goods_name':goods_name }));
	}
}
...
function add_sale_list() {//增加商品推荐
	for (var i = 1; i <= sale_max; i++) {//防止数组下标重复
		if ($("dl[sale_id='"+i+"']").size()==0) {//编号不存在时添加
			var add_html = '';
			add_html = '<dl sale_id="'+i+'"><dt class="title"><h4>商品推荐</h4><a href="JavaScript:del_sale_list('+i+');" class="wtap-btn-mini del"><i class="fa fa-trash"></i>删除</a><a href="JavaScript:show_sale_dialog('+i+
    			');" class="wtap-btn-mini edit"><i class="fa fa-pencil-square-o"></i>编辑</a><input name="sale_list['+i+'][recommend][name]" value="" type="hidden"></dt>'+
    			'<dd><ul class="goods-list"><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li>'+
    			'<li><span><i class="icon-gift"></i></span></li><li><span><i class="icon-gift"></i></span></li></ul></dd></dl>';
			$("#add_list").before(add_html);
			$("#select_sale_list").before('<div class="wtap-form-default" select_sale_id="'+i+'"><dl class="row"><dt class="tit">商品推荐模块标题名称</dt>'+
    			...
    			'</dd></dl></div><div class="wtap-form-all" select_sale_id="'+i+'"><dl class="row"><dt class="tit">已选商品</dt><dd class="opt"><ul class="dialog-goodslist-s1 goods-list">'+
    			'</ul></dd></dl></div>');
...
function select_sale_goods(goods_id){//商品选择
	...
	text_append += '</span></div>';
	text_append += '<div class="goods-name">';
	text_append += '<a href="'+SITEURL+'/index.php?w=goods&goods_id='+goods_id+'" target="_blank">';
  	text_append += goods_name+'</a>';
	text_append += '</div>';
