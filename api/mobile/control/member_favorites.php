...
class member_favoritesControl extends mobileMemberControl {
     ...
     * 收藏列表
     */
    public function favorites_listWt() {
        ...
        $field = 'goods_id,goods_name,goods_sale_price,goods_image,store_id';
     ...
     * 添加收藏
     */
    public function favorites_addWt() {
        ...
        $goods_content = $goods_model->getGoodsInfoByID($goods_id);
        $seller_info = Model('seller')->getSellerInfo(array('member_id'=>$this->member_info['member_id']));
        if ($goods_content['store_id'] == $seller_info['store_id']) {
     ...
     * 删除收藏
     */
    public function favorites_delWt() {
     ...
     * 收藏详情
     */
    public function favorites_infoWt() {
        $param = $_POST;
        ...
        $goods_content = Model('goods')->getGoodsInfo(array(
            'goods_id' => $favorites_info['fav_id'],
            'is_book' => 0,// 默认不显示预订商品
        ), $field);
        $favorites_info = array_merge($favorites_info,$goods_content);
