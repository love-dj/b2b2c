...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?w=article&t=article" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['article_index_manage'];?> - <?php echo $lang['wt_edit'];?>文章“<?php echo $output['article_array']['article_title'];?>”</h3>
  ...
  <form id="article_form" method="post">
    ...
    <div class="wtap-form-default">
      ...
      <dl class="row" wttype="article_position" style="display: none">
        <dt class="tit">
          <label>显示位置:</label>
        </dt>
        <dd class="opt">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="article_show1" class="cb-enable <?php if($output['article_array']['article_show'] == '1'){ ?>selected<?php } ?>" ><?php echo $lang['wt_yes'];?></label>
            <label for="article_show0" class="cb-disable <?php if($output['article_array']['article_show'] == '0'){ ?>selected<?php } ?>" ><?php echo $lang['wt_no'];?></label>
      ...
      <dl class="row">
        ...
        <dd class="opt">
		  ...
		  <?php showEditor('article_content', $value=$output['article_array']['article_content'], $width='700px', $height='300px', $style='visibility:hidden;',$upload_state="true", $media_open=true, $type='all');?>
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
...
$(document).ready(function(){
	$('#ac_id').on('change',function(){
		if($(this).val() == '1') {
			$('dl[wttype="article_position"]').show();
		}else{
			$('dl[wttype="article_position"]').hide();
		}
	});
	...
	$('#article_form').validate({
        ...
        messages : {
            article_title : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['article_add_title_null'];?>'
            },
			ac_id : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['article_add_class_null'];?>'
            },
			article_url : {
				url : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['article_add_url_wrong'];?>'
            },
			article_content : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['article_add_content_null'];?>'
            },
            article_sort  : {
                number   : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['article_add_sort_int'];?>'
            }
        }
    });
    // 图片上传
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?w=article&t=article_pic_upload&item_id=<?php echo $output['article_array']['article_id'];?>',
...
function del_file_upload(file_id)
{
    ...
    $.getJSON('index.php?w=article&t=ajax&branch=del_file_upload&file_id=' + file_id, function(result){
        if(result){
            $('#' + file_id).remove();
        }else{
