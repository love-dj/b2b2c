...
class compareControl extends BaseHomeControl{
    ...
    public function indexWt() {
        ...
        if($gid_arr){
            ...
            for ($j=0; $j<self::MAXNUM; $j++){
                $compare_list[$i][] = $compare_listtmp[$j]?$compare_listtmp[$j]['goods_sale_price']:'';
            }
            $i++;
            //品牌
    ...
    public function addcompareWt(){
        ...
        setWtCookie('comparegoods', $comparegoods_str);
        exit(json_encode(array('done'=>true,'msg'=>'')));
    }
    /**
     * 显示对比商品
     */
    public function showcompareWt(){
        ...
        if($gid_arr){
            ...
            $goods_list = array();
            $goodsid_arr = array();
            $goods_listtmp = $model->getGoodsList(array('goods_id'=>array('in',$gid_arr),'goods_state'=>1,'goods_verify'=>1),'goods_id,goods_commonid,goods_name,store_id,gc_id,gc_id_1,gc_id_2,gc_id_3,goods_sale_price,goods_sale_type,goods_image');
            if ($goods_listtmp){
                foreach ($goods_listtmp as $k=>$v){
                    $goods_list[$v['goods_id']] = $v;
                }
                ...
                //更新cookie数据
                setWtCookie('comparegoods', $goodsid_arr?implode(',',$goodsid_arr):'');
            } else {
                //更新cookie数据
                setWtCookie('comparegoods','',-3600);
            }
        }
     ...
     * 获得已加入对比的商品ID
     */
    public function checkcompareWt(){
        $gids = cookie('comparegoods');
        $gid_arrtmp = $gids?explode(',',$gids):array();
        if(!empty($gid_arrtmp)){
            foreach($gid_arrtmp as $k=>$v){
        ...
        if($gid_arr){
            //查询商品信息
            $goods_list = array();
            $goods_list = $model->getGoodsList(array('goods_id'=>array('in',$gid_arr),'goods_state'=>1,'goods_verify'=>1),'goods_id,goods_commonid,goods_name,store_id,gc_id,gc_id_1,gc_id_2,gc_id_3,goods_sale_price,goods_sale_type,goods_image');
            if ($goods_list){
                foreach ($goods_list as $k=>$v){
    ...
    public function delcompareWt(){
        ...
        //更新cookie数据
        if($gid_str){
            setWtCookie('comparegoods', $gid_str);
        } else {
            setWtCookie('comparegoods','',-3600);
        }
