$(function() {
	...
	$.ajax({
		type: 'post',
		url: ApiUrl + '/index.php?w=member_address&t=address_info',
		data: {
			key: key,
	...
	$('.btn').click(function() {
		if($.sValid()){
			...
			$.ajax({
				type: 'post',
				url: ApiUrl + "/index.php?w=member_address&t=address_edit",
				data: {
					key: key,
					true_name: true_name,
					mob_phone: mob_phone,
				...
				success: function(result) {
					if (result) {
						location.href = WapSiteUrl + '/html/member/address_list.html';
					} else {
