...
class sale_couControl extends SystemControl
{
    public function __construct()
    {
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                array(
                    'url'=>'index.php?w=sale_cou&sale_allow=1',
    ...
    public function cou_listWt()
    {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_cou.list');
    }

    /**
     * 活动列表
     */
    public function cou_list_xmlWt()
    {
        $condition = array();
        ...
        foreach ($couList as $val) {
            $u = urlAdminShop('sale_cou', 'cou_detail', array(
                'id' => $val['id'],
                'store_id' => $val['store_id'],
            ));

            $o = '<a class="btn red confirm-on-click" href="' . urlAdminShop('sale_cou', 'cou_del', array(
                'cou_id' => $val['id'],
            ...
            if ($val['tend'] > TIMESTAMP) {
                switch ($val['state']) {
                    case $couModel::COU_STATE_NORMAL:
                        $o .= '<li><a class="confirm-on-click" href="' . urlAdminShop('sale_cou', 'cou_cancel', array(
                            'cou_id' => $val['id'],
                        )) . '">取消活动</a></li>';
                        break;

                    case $couModel::COU_STATE_CANCEL:
                        $o .= '<li><a class="confirm-on-click" href="' . urlAdminShop('sale_cou', 'cou_reopen', array(
                            'cou_id' => $val['id'],
                        )) . '">开启活动</a></li>';
                        break;
                }
...
EOB;
    ...
    public function cou_detailWt()
    {
        ...
        Tpl::showpage('sale_cou.detail', 'null_layout');
    }

    /**
     * 活动取消
     */
    public function cou_cancelWt()
    {
    ...
    public function cou_settingWt()
    {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_cou.setting');
    }

    public function cou_setting_saveWt()
    {
        $sale_cou_price = max(0, intval($_POST['sale_cou_price']));

        $model_setting = Model('setting');
        $update_array = array();
        $update_array['sale_cou_price'] = $sale_cou_price;

        $result = $model_setting->updateSetting($update_array);
        if ($result) {
            $this->log('修改加价购活动价格为'.$sale_cou_price.'元');
            showMessage('设置保存成功', '');
        } else {
            showMessage('设置保存失败', '');
        }
    ...
    public function cou_quotaWt()
    {
        $this->showMenu('cou_quota');
		Tpl::setDirquna('shop');
        Tpl::showPage('sale_cou.quota');
    }

    public function cou_quota_xmlWt()
    {
        $condition = array();
    ...
    private function showMenu($menu_key)
    {
        $menu_array = array(
            'cou_list' => array(
                'menu_type' => 'link',
                'menu_name' => '活动列表',
                'menu_url'=>'index.php?w=sale_cou&t=cou_list',
            ),
            'cou_detail' => array(
                'menu_type' => 'link',
                'menu_name' => '活动详情',
            ...
            'cou_quota' => array(
                'menu_type' => 'link',
                'menu_name' => '套餐管理',
                'menu_url'=>'index.php?w=sale_cou&t=cou_quota',
            ),
            'cou_setting' => array(
                'menu_type' => 'link',
                'menu_name' => '设置',
                'menu_url'=>'index.php?w=sale_cou&t=cou_setting',
            ),
        );
