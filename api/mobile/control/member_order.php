...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listWt() {
        ...
        foreach ($order_list_array as $value) {
			...
			if(!empty($value['extend_order_goods']) && is_array($value['extend_order_goods'])){
				//商品图
				foreach ($value['extend_order_goods'] as $k => $goods_content) {
					$value['extend_order_goods'][$k]['goods_image_url'] = cthumb($goods_content['goods_image'], 240, $value['store_id']);
					...
					unset($value['extend_order_goods'][$k]['sales_id']);
            ...
            if ($value['order_type'] == 4) {//拼团订单
                ...
                $_info = $model_pingou->getOrderInfo(array('order_id'=> $value['order_id']));
                $value['pingou_info'] = $_info;
            }
		...
		if ($value['order_type'] == 4 && $value['pingou_info']['end_time'] < TIMESTAMP) $order_group_list[$value['pay_sn']]['pay_amount'] = 0;//拼团订单过期后不能继续支付
            }
     ...
     * 取消订单
     */
    public function order_deleteWt() {
        $model_order = Model('order');
        $handle_order = Handle('order');
        $order_id = intval($_POST['order_id']);
