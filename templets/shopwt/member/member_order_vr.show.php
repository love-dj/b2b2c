...
<div class="wtm-oredr-show">
  <div class="wtm-order-info">
      ...
      <div class="content">
        <dl>
          ...
          <dd><?php echo $output['order_info']['buyer_phone'];?>
            ...
            <a href="javascript:void(0);" class="wtbtn-mini wtbtn-bittersweet" dialog_id="vr_code_resend" dialog_title="发送电子兑换码" dialog_width="480" wt_type="dialog" uri="<?php echo urlShop('member_order_vr', 'resend',array('buyer_phone'=>$output['order_info']['buyer_phone'],'order_id'=>$output['order_info']['order_id']));?>"><i class="icon-mobile-phone"></i>重新发送</a>
            <?php } ?>
          </dd>
        </dl>
        <dl class="line">
        ...
        <dl class="line">
          <dt>商&#12288;&#12288;家：</dt>
          <dd><?php echo $output['order_info']['store_name'];?><a href="javascript:void(0)" class="wtbtn" wt_type="dialog" dialog_width="800" dialog_title="查看地图" dialog_id="show_map" uri="index.php?w=show_map&t=index&width=440&height=400&store_id=<?php echo $output['order_info']['store_id'];?>"><i class="icon-map-marker"></i>商家所在信息</a></dd>
        </dl>
      </div>
    </div>
    ...
    <div class="wtm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?w=buy_vr&t=pay&order_id=<?php echo $output['order_info']['order_id']; ?>" class="wtbtn-mini wtbtn-bittersweet"><i></i>支付订单</a>以确保及时获取电子兑换码。</li>
        <li>2. 如果您不想购买此订单，请点击 <a href="#order-list" class="wtbtn-mini" >取消订单</a>。 </li>
    ...
    <div class="wtm-order-condition">
      ...
      <ul>
        ...
        <li>2. 您尚有&nbsp;<strong class="green"><?php echo $output['order_info']['extend_order_vr_code'][0]['vr_code_valid_count'];?></strong>&nbsp;组电子兑换码未被使用；有效期为
    ...
    <div class="wtm-order-condition">
      ...
      <ul>
        ...
        <li>2. 去商城看看有没有新的<a href="<?php echo urlShop('search','index');?>" class="wtbtn-mini wtbtn-mint" target="_blank">虚拟商品</a>。</li>
      </ul>
    </div>
    <?php }?>
  ...
  <div class="wtm-order-step">
    ...
    <?php if (!empty($output['order_info']['extend_order_vr_code'])){ ?>
    <div class="code-list tip" title="如列表过长超出显示区域时可滚动鼠标进行查看"><i class="arrow"></i>
      <h5>电子兑换码</h5>
      <div id="codeList">
        <ul>
          <?php foreach($output['order_info']['extend_order_vr_code'] as $code_info){ ?>
  ...
  <?php }?>
  <div class="wtm-order-contnet" id="order-list">
    <table class="wtm-default-table order">
      ...
      <tbody>
        <tr>
          <th colspan="20"><span class="ml10" title="虚拟订单号">虚拟订单号：<?php echo $output['order_info']['order_sn'];?></span><span>下单时间：<?php echo date("Y-m-d H:i",$output['order_info']['add_time']);?></span><span><a href="<?php echo urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']));?>" title="<?php echo $output['order_info']['store_name'];?>"><?php echo $output['store_info']['store_name'];?></a></span>
            ...
            <a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&uid=<?php echo $output['store_info']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid=<?php echo $output['store_info']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>" alt="Wang Wang" style=" vertical-align: middle;" /></a>
            <?php }?>
            </span></th>
        </tr>
        <tr>
          ...
          <td class="tl"><dl class="goods-name">
              ...
              <?php } ?>
              <dd><span class="sale-type"><?php if ($output['order_info']['order_sale_type'] == 1) {?>抢购，<?php } ?>使用时效：即日起 至 <?php echo date("Y-m-d",$output['order_info']['vr_indate']);?>
          ...
          <td class="tl"><?php echo wtPriceFormat($output['order_info']['goods_price']);?></td>
          <td><?php echo $output['order_info']['goods_num'];?></td>
          <td> <?php if($output['order_info']['if_refund']){ ?>
              <a href="index.php?w=member_vr_refund&t=add_refund&order_id=<?php echo $output['order_info']['order_id']; ?>">退款</a>
              <?php } ?></td>
          <td class="bdl"><?php echo $output['order_info']['state_desc'];?></td>
          <td class="bdl bdr">

          <?php if ($output['order_info']['if_cancel']){ ?>
            <a href="javascript:void(0)" class="wtbtn wtbtn-grapefruit" wt_type="dialog" dialog_width="480" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="index.php?w=member_order_vr&t=change_state&state_type=order_cancel&order_id=<?php echo $output['order_info']['order_id'];?>"  id="order_action_cancel"><i class="icon-ban-bbs"></i>取消</a>
            <?php }?>

            <!-- 评价 -->
            <?php if ($output['order_info']['if_evaluation']) { ?>
            <p><a class="wtbtn wtbtn-aqua" href="index.php?w=member_evaluate&t=add_vr&order_id=<?php echo $output['order_info']['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a></p>
            <?php } ?>
            ...
            <p><a href="javascript:void(0)" wt_type="sharegoods" data-param='{"gid":"<?php echo $output['order_info']['goods_id'];?>"}'>分享商品</a></p>
            <?php } ?>
          </td>
        </tr>
      </tbody>
      <tfoot>
        ...
        <tr>
          <th colspan="20"><dl class="wtm-store-sales">
              <?php if(!empty($output['order_info']['voucher_code'])){ ?>
