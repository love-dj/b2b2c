...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?w=document&t=document" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['document_index_document'];?> - <?php echo $lang['wt_edit'];?>文章“<?php echo $output['doc']['doc_title'];?>”</h3>
          ...
          <div id="thumbnails" class="wtap-thumb-list">
            ...
            <ul>
              ...
              <li id="<?php echo $v['upload_id'];?>">
                ...
                <a href="javascript:del_file_upload('<?php echo $v['upload_id'];?>');" class="del" title="<?php echo $lang['wt_del'];?>">X</a><a href="javascript:insert_editor('<?php echo $v['upload_path'];?>');" class="inset"><i class="fa fa-clipboard"></i>插入图片</a> </li>
              <?php } ?>
              <?php } ?>
            </ul>
          </div>
...
$(document).ready(function(){
	$('#doc_form').validate({
        ...
        messages : {
            doc_title : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['document_index_title_null'];?>'
            },
			doc_content : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['document_index_content_null'];?>'
            }
        }
    });
    // 图片上传
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?w=document&t=document_pic_upload&item_id=<?php echo $output['doc']['doc_id'];?>',
...
function del_file_upload(file_id)
{
    ...
    $.getJSON('index.php?w=document&t=ajax&branch=del_file_upload&file_id=' + file_id, function(result){
        if(result){
            $('#' + file_id).remove();
        }else{
