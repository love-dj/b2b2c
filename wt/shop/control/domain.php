...
class domainControl extends SystemControl{
     ...
     * @return
     */
    public function store_domain_settingWt(){
        /**
         * 读取语言包
         */
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('wt_edit,wt_domain_manage'),1);
     ...
     * 店铺二级域名列表
     */
    public function store_domain_listWt(){
    ...
    public function get_xmlWt() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='index.php?w=domain&t=store_domain_edit&store_id=" . $value['store_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
     ...
     * 二级域名编辑
     */
    public function store_domain_editWt(){
        ...
        if (chksubmit()){
            ...
            $this->log(L('wt_edit,wt_domain_manage').'['.$store_domain.']',1);
            showMessage(Language::get('wt_common_save_succ'),'index.php?w=domain&t=store_domain_list');//保存成功
        }
