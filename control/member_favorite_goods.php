...
class member_favorite_goodsControl extends BaseMemberControl{
     ...
     * 增加商品收藏
     */
    public function favoritegoodsWt(){
        ...
        $goods_content = $goods_model->getGoodsInfoByID($fav_id, 'store_id');
    ...
    public function fglistWt(){
        ...
        if (!empty($favorites_list) && is_array($favorites_list)){
            ...
            $goods_model = Model('goods');
            $fields = 'goods.goods_id,goods.goods_name,goods.store_id,goods.goods_image,goods.goods_sale_price,goods.goods_state,goods.goods_verify,goods.evaluation_count,goods.goods_salenum,goods.goods_collect,'
     ...
     * @return
     */
    public function delfavoritesWt(){
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),'index.php?w=member_favorite_goods&t=fglist&show='.$_GET['show'],'succ');
                }
            }else {
    ...
    public function log_msgWt(){
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                showDialog(Language::get('wt_common_save_succ'),'reload','succ','CUR_DIALOG.close();');
            } else {
                showDialog(Language::get('wt_common_save_fail'),'reload','error','CUR_DIALOG.close();');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_goods','menu_name'=>Language::get('wt_member_path_collect_list'), 'menu_url'=>'index.php?w=member_favorite_goods&t=fglist'),
            array('menu_key'=>'fav_goodsstore','menu_name'=>'同店商品', 'menu_url'=>'index.php?w=member_favorite_goods&t=fglist&show=store'),
        );
