...
class p_soleModel extends Model {
    ...
    public function getSoleGoodsInfoOpenByGoodsID($goods_id) {
        $goods_content = $this->_rGoodsSoleCache($goods_id);
        if (empty($goods_content)) {
            $goods_content = $this->getSoleGoodsInfo(array('goods_id'=>$goods_id, 'sole_state' => self::STATE1));
            $this->_wGoodsSoleCache($goods_id, $goods_content);
        }
