	...
	var audio_info = '<object width="1" height="1" style="position: absolute; left: -1px;" id="msg_audio" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">'+
								'<param value="'+CHAT_SITE_URL+'/static/audio.swf" name="movie"><param value="always" name="allowscriptaccess">'+
								'<embed width="1" height="1" allowscriptaccess="always" src="'+CHAT_SITE_URL+'/static/audio.swf" name="msg_audio"></object>';
    $(function(){
		...
		if(user['u_id'] != '') {
			...
			web_info['msg_dialog'] = '<div id="new_msg_dialog" class="msg-windows"><div class="user-tab-bar"><ul class="user-list" id="user_list"></ul></div>'+
									...
									'<a href="javascript:void(0)" class="chat_sendimg"><input type="file" hidefocus="true" size="1" class="input-file" id="chat_send_img" name="chat_send_img" />图片</a>' +
			...
			var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>'+
									...
									'</div></div>';
			var ajaxurl = CHAT_SITE_URL+'/index.php?w=web_chat&t=get_user_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
			    ...
			    success: function(u_list){
			  	for (var u_id in u_list){
					...
					setInterval( function () {
						$.get(CHAT_SITE_URL+'/index.php?w=web_chat&t=get_session&key=member_id');
					}, time_max*60000);
	        ...
	        if ( w_t == 'goods_index' ) {//异步调用商品信息
				var ajaxurl = CHAT_SITE_URL+'/index.php?w=web_chat&t=get_goods_content&goods_id='+chat_goods_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
			...
			$("#chat_show_user").click(function() {
			    $('div[wttype="a-barLoginBox"]').trigger("click");
			});
		}
		//发送图片
		$('#chat_send_img').live('change', function() {
			...
			$.each($('#chat_send_img')[0].files, function(i, file) {
				data.append('_pic', file);
			});
			$.ajax({
				type: "POST",
				url: CHAT_SITE_URL + '/index.php?w=web_chat&t=image_send',
				data: data,
				cache: false,
				contentType: false,
				processData: false,
    ...
    if (typeof JSON !== 'object') {//兼容IE7
        $.getScript(STATIC_SITE_URL+'/js/json2.js');
    }
	...
	function get_state(list){//返回会员的状态并在页面显示
		...
		if(layout == 'layout/store_layout.php' || w_t == 'goods_index') {//店铺页面18
		    var store_id = 0;
		    var store_name = '';
	...
	function chat_goods_buy(){
		$('a[wttype="buynow_submit"]').trigger("click");
	}
	function send_focus(){
		$("#send_alert").html('');
	}
	...
	function send_msg(){//发消息
		...
		$.ajax({
			type: "POST",
			url: CHAT_SITE_URL+'/index.php?w=web_chat&t=send_msg',
			dataType:"jsonp",
			data: msg,
			async: false,
	...
	function get_msg(list){//接收消息
		...
		for (var k in list){
			...
			if ( typeof user_list[u_id]['avatar'] === "undefined" ) {//当没获得会员信息时调用一次
				var ajaxurl = CHAT_SITE_URL+'/index.php?w=web_chat&t=get_info&u=member&u_id='+u_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
	...
	function get_chat_msg(t){
		var ajaxurl = CHAT_SITE_URL+'/index.php?w=web_chat&t=get_chat_log&page=30&f_id='+user['u_id']+'&t_id='+chat_log['u_id']+'&c='+chat_log['time_from'];
	...
	function get_goods_content(msg){//显示商品图片和名称
	    if ( typeof msg['goods_id'] === "undefined" ) {//没有定义商品编号时调用检测一次
	        var t_msg = msg['t_msg'];
	        var shop_site = BASE_SITE_URL;
	        var re = new RegExp(shop_site+"/index.php\\?w=goods&t=index&goods_id=(\\d+)$","g");
	        var arr = re.exec(t_msg);
	        goods_id = RegExp.$1;
	        if ( arr == null) {
	            re = new RegExp(shop_site+"/goods-(\\d+)\\.html$","g");
	            arr = re.exec(t_msg);
	            goods_id = RegExp.$1;
	            if ( arr == null) {
	                goods_id = 0;
    	    ...
    	    if ( goods_id > 0 ) {
    	        if ( typeof goods_list[goods_id] === "undefined" ) {
    				var ajaxurl = CHAT_SITE_URL+'/index.php?w=web_chat&t=get_goods_content&goods_id='+goods_id;
    				$.ajax({
    					type: "GET",
    					url: ajaxurl,
    					dataType:"jsonp",
				    ...
				    msg['goods_id'] = goods_id;
				    msg['goods_content'] = goods_list[goods_id];
				    show_msg_goods(msg);
				}
    	    }
	    } else {
	...
	function get_goods_list(s_id){
	    if(layout == 'layout/seller_layout.php' || w_t == 'goods_index') {//商家中心不显示推荐商品
	        return ;
	    }
		...
		var ajaxurl = CHAT_SITE_URL+'/index.php?w=web_chat&t=get_goods_list&s_id='+s_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			dataType:"jsonp",
		    ...
		    success: function(list){
		        ...
		        for (var k in list){
		            ...
		            text_goods += '<div class="goods-price">¥'+goods['goods_sale_price']+'</div>';
		            text_goods += '</li>';
	...
	function show_msg(u_id){//显示会员的消息
		...
		if(user_show != u_id) {
			...
			if (typeof user_info['s_name'] !== "undefined" && user_info['s_name'] !== "") {
			    ...
			    store_html = '<a target="_blank" href="'+BASE_SITE_URL+'/index.php?w=show_store&t=index&store_id='+
	...
	function show_msg_goods(msg){
	    ...
	    if ( goods_id != '' && goods_id > 0 ) {
	        var goods = msg['goods_content'];
	        var text_append = '';
    		text_append += '<div class="goods_content">';
    		text_append += '<div class="goods_name"><a class="goods_pic" target="_blank" href="'+goods['url']+'">';
    		text_append += goods['goods_name']+'</a></div>';
    		text_append += '<div class="goods_price">¥'+goods['goods_sale_price']+'</div>';
	...
	function play_audio(){//提示声音
	    if ( chat_audio === 1 ) {
	        ...
	        if ( typeof swf.pplay === "function" ) {
	            swf.load(CHAT_SITE_URL+'/static/msg.mp3');
	...
	function update_chat_msg(msg){
		if (typeof smilies_array !== "undefined") {
			...
			for(var i in smilies_array[1]) {
				...
				var smilieimg = '<img width="28" height="28" title="'+s[6]+'" alt="'+s[6]+'" src="'+STATIC_SITE_URL+'/js/smilies/images/'+s[2]+'">';
				msg = msg.replace(re,smilieimg);
			}
		}
		rr = new RegExp('\\[SIMG:(.*?)\\]', "g");
		msg = msg.replace(rr, "<a href='$1' target='_blank'><img src=$1 /></a>");
		return msg;
	}
	...
	function update_user(u_id){
		if($("#dialog_chat_right").html() == '') {
    		...
    		text_append += '</div>';
    		if(layout != 'layout/seller_layout.php' && w_t != 'goods_index') {//商家中心页面和商品详细页面不显示推荐商品
			  ...
			  socket.on('connect', function () {
			    ...
			    socket.on('get_msg', function (msg_list) {
				  ...
				  socket.on('disconnect', function () {
				    connect = 0;
				    $("#web_chat_dialog").hide();
				    interval = setInterval( getconnect, 60000);//断开1分钟后重新连接服务器1104
				  });
			  });
		  }
		});
