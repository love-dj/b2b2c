...
<style type="text/css">
  ...
  #focus .pre { left: 0; background: url(<?php echo SHOP_TEMPLATES_URL;?>/images/member/ad_prev.png) no-repeat left center; }
  #focus .next { right: 0; background: url(<?php echo SHOP_TEMPLATES_URL;?>/images/member/ad_next.png) no-repeat right center; }
...
<div class="tabmenu"><!--shopwt-->
  <?php include template('layout/submenu');?>
</div>
<div id="pictureFolder" class="wtsc-picture-folder">
  <dl class="wtsc-album-intro">
    <dt class="albume-name"><a href="index.php?w=store_video&t=album_video_list&id=<?php echo $output['class_info']['video_class_id']?>"><?php echo $output['class_info']['video_class_name']?></a></dt>
  ...
  <div id="gallery" class="ad-gallery">
    ...
    <li class="item">
      <div class="wt-shop-goodslist" wt_type="goods_1">
        <div class=" jcarousel-skin-tango">
          <div class="jcarousel-container jcarousel-container-horizontal" style="position: relative; display: block;">
            <div class="jcarousel-clip jcarousel-clip-horizontal" style="position: relative;">
              <ul class="jcarousel-list jcarousel-list-horizontal" wt_type="jcarousel" style="overflow: hidden; position: relative; top: 10px; margin: 0px; padding: 0px; left: 30px; width:900px;">
    ...
    <div class="ad-image-date">
      ...
      <dt>视频属性</dt>
      <dd>
        <p id="upload_time"><b><?php echo '上传时间'.$lang['wt_colon'];?></b><?php echo date('Y-m-d',$output['video_info']['upload_time']);?></p>
        <p><b><?php echo '视频名称'.$lang['wt_colon'];?></b><?php echo str_cut($output['class_info']['video_class_name'],20);?></p>
        <p id="default_size"><b><?php echo '原图大小'.$lang['wt_colon'];?></b><?php echo $output['video_info']['video_size'];?>MB</p>
      </dd>
    </div>
...
<script>
  $(function() {
    $('[wt_type="jcarousel"]').jcarousel({visible: 7});
...
function ajax_change_videomessage(url){
	$.getJSON("<?php echo BASE_SITE_URL; ?>/index.php?w=store_video&t=ajax_change_videomessage", {'url':url}, function(data){
		$("#video_name").html(data.video_name);
		$("#upload_time").html('<b><?php echo '上传时间'.$lang['wt_colon'];?></b>'+data.upload_time);
		$("#default_size").html('<b><?php echo '原图大小'.$lang['wt_colon'];?></b>'+data.default_size+'MB');
	});
}
</script>