	...
	<form id="schema_form_edit" action="" name="form1" enctype="multipart/form-data" method="post">
		...
		<div class="ncap-form-default">
			...
			<dl class="row">
				...
				<?php 
				$conditions = unserialize($output['level_array']['level_condition']);
				$condition =$conditions['upgrade_value'];
				$type =$conditions['upgrade_type'];
					...
					<div class="col-xs-12 col-sm-9 col-md-10">
						<div class="input-group row" style="padding-buttom:50px;">
							<label class="radio-inline col-xs-12 col-sm-6">
								<input type="checkbox" name="upgrade_type[first_cost_count]" value="1" <?php if($type['first_cost_count']){ ?> checked <?php } ?>>
								下一级用户消费满
						...
						<div class="input-group row">
							<label class="radio-inline col-xs-12 col-sm-6">
								<input type="checkbox" name="upgrade_type[order_money]" value="1" <?php if($type['order_money']){ ?> checked <?php } ?>>
								分销订单金额满
							...
							<label class="radio-inline  col-xs-12 col-sm-6" style="margin-left:50px;">
								<input type="checkbox" name="upgrade_type[order_count]" value="1" <?php if($type['order_count']){ ?> checked <?php } ?>>
								分销订单数量满
						...
						<div class="input-group row">
							<label class="radio-inline col-xs-12 col-sm-6">
								<input type="checkbox" name="upgrade_type[first_order_money]" value="1" <?php if($type['first_order_money']){ ?> checked <?php } ?>>
								一级分销订单金额满
								<input type="text" name="upgrade_value[first_order_money]" value="<?php echo $condition['first_order_money'];?>">
								<span>元</span>
							</label>
							<label class="radio-inline col-xs-12 col-sm-6" style="margin-left:50px;">
								<input type="checkbox" name="upgrade_type[first_order_count]" value="1" <?php if($type['first_order_count']){ ?> checked <?php } ?>>
								一级分销订单数量满
						...
						<div class="input-group row">
							<label class="radio-inline col-xs-12 col-sm-6">
								<input type="checkbox" name="upgrade_type[self_buy_money]" value="1" <?php if($type['self_buy_money']){ ?> checked <?php } ?>>
								自购订单金额满
							...
							<label class="radio-inline col-xs-12 col-sm-6" style="margin-left:50px;">
								<input type="checkbox" name="upgrade_type[self_buy_count]" value="1" <?php if($type['self_buy_count']){ ?> checked <?php } ?>>
								自购单数量满
						...
						<div class="input-group row">
							<label class="radio-inline col-xs-12 col-sm-6">
								<input type="checkbox" name="upgrade_type[settle_money]" value="1" <?php if($type['settle_money']){ ?> checked <?php } ?>>
								结算佣金总额满
