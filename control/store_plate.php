...
class store_plateControl extends BaseSellerControl {
    ...
    public function plate_addWt() {
        if (chksubmit()) {
            ...
            if ($result) {
                showDialog(L('wt_common_op_succ'), urlShop('store_plate', 'index'),'succ');
            } else {
                showDialog(L('wt_common_op_fail'), urlShop('store_plate', 'index'));
            }
        }
        // 是否能使用编辑器
