...
class sale_xianshiControl extends SystemControl{

    public function __construct(){
        parent::__construct();

        //读取语言包
        Language::read('sale_xianshi');

        //检查审核功能是否开启
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                array(
                    'url'=>'index.php?w=sale_xianshi&sale_allow=1',
    ...
    public function xianshi_listWt()
    {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_xianshi.list');
    }

    /**
     * 活动列表
     */
    public function xianshi_list_xmlWt()
    {
        $condition = array();
        ...
        foreach ($xianshi_list as $val) {
            $o  = '<a class="btn red confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('sale_xianshi', 'xianshi_del', array(
            ...
            if ($val['editable']) {
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('sale_xianshi', 'xianshi_cancel', array(
                    'xianshi_id' => $val['xianshi_id'],
                )) . '">取消活动</a></li>';
            }

            $o .= '<li><a class="confirm-on-click" href="' . urlAdminShop('sale_xianshi', 'xianshi_detail', array(
                'xianshi_id' => $val['xianshi_id'],
            )) . '">活动详细</a></li>';
     ...
     * 限时折扣活动取消
     **/
    public function xianshi_cancelWt() {
     ...
     * 活动详细信息
     **/
    public function xianshi_detailWt() {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_xianshi.detail');
    }

    /**
     * 套餐管理
     */
    public function xianshi_quotaWt()
    {
        $this->show_menu('xianshi_quota');
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_xianshi_quota.list');
    }

    /**
     * 套餐管理XML
     */
    public function xianshi_quota_xmlWt()
    {
        $condition = array();
     ...
     * 设置
     **/
    public function xianshi_settingWt() {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_xianshi.setting');
    }

    public function xianshi_setting_saveWt() {

        $sale_xianshi_price = intval($_POST['sale_xianshi_price']);
        if($sale_xianshi_price < 0) {
            $sale_xianshi_price = 20;
        }

        $model_setting = Model('setting');
        $update_array = array();
        $update_array['sale_xianshi_price'] = $sale_xianshi_price;

        $result = $model_setting->updateSetting($update_array);
        if ($result){
            $this->log('修改限时折扣价格为'.$sale_xianshi_price.'元');
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'xianshi_list'=>array('menu_type'=>'link','menu_name'=>Language::get('xianshi_list'),'menu_url'=>'index.php?w=sale_xianshi&t=xianshi_list'),
            'xianshi_detail'=>array('menu_type'=>'link','menu_name'=>Language::get('xianshi_detail'),'menu_url'=>'index.php?w=sale_xianshi&t=xianshi_detail'),
            'xianshi_quota'=>array('menu_type'=>'link','menu_name'=>Language::get('xianshi_quota'),'menu_url'=>'index.php?w=sale_xianshi&t=xianshi_quota'),
            'xianshi_setting'=>array('menu_type'=>'link','menu_name'=>Language::get('xianshi_setting'),'menu_url'=>'index.php?w=sale_xianshi&t=xianshi_setting'),
        );
