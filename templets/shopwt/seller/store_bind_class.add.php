...
<div class="wtsc-form-default">
  <form method="post" action="index.php?w=store_info&t=bind_class_save" target="_parent" name="store_bind_class_form" id="store_bind_class_form">
    <input type="hidden" name="form_submit" value="ok" />
    <input id="goods_class" name="goods_class" type="hidden" value="">
    <dl>
      <dt>选择分类<?php echo $lang['wt_colon'];?></dt>
      <dd id="gcategory">
          <select id="gcategory_class1" style="width: auto;">
            <option value="0">请选择</option>
            <?php if(!empty($output['gc_list']) && is_array($output['gc_list']) ) {?>
            <?php foreach ($output['gc_list'] as $gc) { if($gc['is_show']=='1'){continue;} ?>
...
<label id="error_message" style="display: none" class="error" for="sn_title"><i class="icon-exclamation-sign"></i>请选择分类</label>
    ...
    <div class="bottom">
      <label class="submit-border"><input type="button" id="btn_add_bind_class" class="submit" value="<?php echo $lang['wt_submit'];?>" /></label>
    </div>
  </form>
</div>
