...
class vr_billControl extends SystemControl{
    ...
    public function get_bill_xmlWt(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?w=vr_bill&t=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=vr_bill&t=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function get_bill_info_code_xmlWt(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0) {
            exit();
        }
        ...
        foreach ($code_list as $code_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?w=order_vr&t=show_order&order_id={$code_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            $list['pay_price'] = wtPriceFormat($code_info['pay_price']);
            $list['commis_rate'] = wtPriceFormat($code_info['pay_price']*$code_info['commis_rate']/100);
    ...
    public function bill_payWt(){
		...
		if(!empty($store_info)){
            ...
            if ($update){
				...
				$data['lg_desc'] = '虚拟结算时间'.date('Y-m-d',$bill_info['ob_start_date']).'至'.date('Y-m-d',$bill_info['ob_end_date']);
                ...
                showMessage('保存成功','index.php?w=vr_bill');
            }else{
                $this->log('账单付款,账单号：'.$ob_id,1);
    ...
    public function export_orderWt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?w=vr_bill&t=show_bill&ob_id='.$ob_id);
				Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                exit();
            }else{
