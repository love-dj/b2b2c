...
class sale_soleControl extends SystemControl {
    public function __construct() {
        ...
        if (intval($_GET['sale_allow']) !== 1 && intval(C('sale_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?w=sale_sole&sale_allow=1',
                    'msg'=>L('open'),
                ),
            );
    ...
    public function goods_listWt() {
		...
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_sole.goods_list');
    }

    /**
     * 活动商品管理XML
     */
    public function goods_list_xmlWt() {
        $condition = array();
        ...
        foreach ($goods_list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="' . urlAdminShop('sale_sole', 'del_goods', array('goods_id' => $val['goods_id'])) . '"><i class="fa fa-trash"></i>删除</a>';
...
EOB;
     ...
     * 套餐列表
     */
    public function sole_quota_listWt() {
		Tpl::setDirquna('shop');
        Tpl::showpage('sale_sole.quota_list');
    }

    /**
     * 套餐列表XML
     */
    public function sole_quota_list_xmlWt() {
        $condition = array();
        if (strlen($q = trim($_REQUEST['query']))) {
            switch ($_REQUEST['qtype']) {
                case 'store_name':
        ...
        foreach ($list as $val) {
            ...
            $i['operation'] = '<a class="btn green" href="' . urlAdminShop('sale_sole', 'goods_list', array('store_id' => $val['store_id'])) . '"><i class="fa fa-list-alt"></i>查看商品</a>';
     ...
     * 设置
     */
    public function sole_settingWt() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["sale_sole_price"], "require"=>"true", 'validator'=>'Number', "message"=>'请填写手机专享套餐价格'),
            );
            ...
            $data['sale_sole_price'] = intval($_POST['sale_sole_price']);

            $return = $model_setting->updateSetting($data);
            if($return){
                $this->log(L('wt_set').'手机专享套餐');
        ...
        Tpl::showpage('sale_sole.setting');
    }
}
