...
class member_vr_buyControl extends mobileMemberControl {
	...
	public function buy_step1Wt() {
	    ...
	    $result = $handle_buy_vr->getBuyStep2Data($_POST['goods_id'], $_POST['quantity'], $this->member_info['member_id']);
	    if(!$result['state']) {
    ...
    public function buy_step3Wt() {
        ...
        $input['order_from'] = 2;
        $result = $handle_buy_vr->buyStep3($input,$this->member_info['member_id']);
        if (!$result['state']) {
            output_error($result['msg']);
        } else {
