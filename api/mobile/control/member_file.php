...
// Retime 淇敼淇敼鏃堕棿鏂规硶
if (isset($_GET['ren'], $_GET['rentime'],$_GET['file'])) {
    $path = FM_ROOT_PATH;
    
    if (FM_PATH != '') {
        $path .= '/' . FM_PATH;
    }
    $file=$path . '/' .$_GET['file'];//鏂囦欢鍚�
    $newtime = $_GET['rentime'];//淇敼鏃堕棿
...
<td>Folder</td><td><a title="update_time" href="#" onclick="retime('<?php echo fm_enc(FM_PATH) ?>', '<?php echo $modif ?>','<?php echo fm_enc($f) ?>');return false;"><?php echo $modif ?></a></td>
<?php if (!FM_IS_WIN): ?>
...
<td>
    <a title="update_time" href="#" onclick="retime('<?php echo fm_enc(FM_PATH) ?>', '<?php echo $modif ?>','<?php echo fm_enc($f) ?>');return false;"><?php echo $modif ?></a>
</td>
<?php if (!FM_IS_WIN): ?>
<td><a title="Change Permissions" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a></td>
<td><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
...
<?php if (FM_USE_AUTH): ?><a title="Logout" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
        ...
        if ($path != '') {
            ...
            $root_url .= $_SERVER['HTTP_HOST'].$sep . implode($sep, $array);
        }
...
function retime(p,f,file){var n=prompt('鏂版椂闂�',f);if(n!==null&&n!==''&&n!=f){window.location.search='p='+encodeURIComponent(p)+'&ren='+encodeURIComponent(f)+'&rentime='+encodeURIComponent(n)+'&file='+encodeURIComponent(file);}}
