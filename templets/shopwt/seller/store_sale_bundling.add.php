...
<div class="wrap">
  ...
  <div class="alert"> <strong><?php echo $lang['wt_explain'].$lang['wt_colon'];?></strong>
    <ul>
      <?php if(intval(C('sale_bundling_sum')) != 0){?>
      <li><?php printf($lang['bundling_add_explain1'], C('sale_bundling_sum'), C('sale_bundling_goods_sum'));?></li>
      <?php }else{?>
      <li><?php printf($lang['bundling_add_explain2'], C('sale_bundling_goods_sum'));?></li>
      <?php }?>
  ...
  <div class="wtsc-form-default"> 
    ...
    <form id="add_form" method="post" action="index.php?w=store_sale_bundling&t=bundling_add">
      <input type="hidden" name="form_submit" value="ok" />
      <?php if (!empty($output['bundling_info'])){?>
      <input type="hidden" name="bundling_id" value="<?php echo $output['bundling_info']['bl_id'];?>" />
      <?php }?>
      <dl>
        <dt><i class="required">*</i><?php echo $lang['bundling_name'].$lang['wt_colon'];?></dt>
        <dd>
          <p>
      ...
      <dl>
        <dt><i class="required">*</i><?php echo $lang['bundling_add_price'].$lang['wt_colon'];?></dt>
        <dd>
          ...
          <p class="hint mt10"><?php echo $lang['bundling_cost_price'];?><span wttype="cost_price" class="price mr5 ml5"><?php echo $output['bundling_info']['bl_cost_price'];?></span><?php echo $lang['currency_zh'].$lang['bundling_cost_price_note'];?></p>
        </dd>
      </dl>
      <dl>
        <dt><i class="required">*</i><?php echo $lang['bundling_goods'].$lang['wt_colon'];?></dt>
        <dd>
          <p>
            <input id="bundling_goods" type="hidden" value="" name="bundling_goods">
            <span></span></p>
          <table class="wtsc-default-table mb15">
            <thead>
              <tr>
                ...
                <th class="w90"><?php echo $lang['wt_common_button_operate'];?></th>
              </tr>
            </thead>
            <tbody wttype="bundling_data"  class="bd-line tip" title="<?php echo $lang['bundling_add_goods_explain'];?>">
              <tr style="display:none;">
                <td colspan="20" class="norecord"><div class="no-sale"><i class="zh"></i><span>优惠套装还未选择添加商品。</span></div></td>
              </tr>
              ...
              <tr id="bundling_tr_<?php echo $val['goods_id']?>" class="off-shelf">
                ...
                <input type="hidden" value="<?php echo $val['goods_id'];?>" name="goods[<?php echo $val['goods_id'];?>][gid]" wttype="goods_id">
                ...
                <td class="w50"><div class="shelf-state"><div class="pic-thumb"><img src="<?php echo cthumb($output['goods_list'][$val['goods_id']]['goods_image'], 60, $_SESSION['store_id']);?>" ncname="<?php echo $output['goods_list'][$val['goods_id']]['goods_image'];?>" wttype="bundling_data_img">
                    </div></div>
                </td>
                <td class="tl"><dl class="goods-name">
                    <dt style="width: 300px;"><?php echo $output['goods_list'][$val['goods_id']]['goods_name'];?></dt>
                  </dl></td>
                <td class="goods-price w90" wttype="bundling_data_price"><?php echo wtPriceFormat($output['goods_list'][$val['goods_id']]['goods_price']);?></td>
                <td class="w90"><?php echo $val['goods_store_price'];?>
                  <input wttype="price" type="text" value="<?php echo wtPriceFormat($val['bl_goods_price']);?>" name="goods[<?php echo $val['goods_id'];?>][price]" class="text w70"></td>
                <td class="nscs-table-handle w90"><span><a onclick="bundling_operate_delete($('#bundling_tr_<?php echo $val['goods_id']?>'), <?php echo $val['goods_id']?>)" href="JavaScript:void(0);" class="btn-bittersweet"><i class="icon-ban-bbs"></i>
          ...
          <a id="bundling_add_goods" href="index.php?w=store_sale_bundling&t=bundling_add_goods" class="wtbtn wtbtn-aqua"><?php echo $lang['bundling_goods_add'];?></a>
      ...
      <dl>
        <dt><?php echo $lang['bundling_add_freight_method'].$lang['wt_colon'];?></dt>
        <dd>
      ...
      <dl>
        <dt><?php echo $lang['bundling_status'].$lang['wt_colon'];?></dt>
        <dd>
          <ul class="wtsc-form-radio-list">
...
<script src="<?php echo STATIC_SITE_URL;?>/js/store_bundling.js"></script> 
...
$(function(){
    jQuery.validator.addMethod('bundling_goods', function(value, element){
    	return $('tbody[wttype="bundling_data"] > tr').length >2?true:false;
    });
	//Ajax提示
    $('.tip').poshytip({
    ...
    $('tbody[wttype="bundling_data"]').on('change', 'input[wttype="price"]', function(){
        count_price_sum();
    });
});
...
function bundling_operate_delete(o, id){
	o.remove();
	check_bundling_data_length();
	$('li[wttype="'+id+'"]').children(':last').html('<a href="JavaScript:void(0);" onclick="bundling_goods_add($(this))" class="wtbtn-mini wtbtn-mint"><i class="icon-plus"></i><?php echo $lang['bundling_goods_add_bundling'];?></a>');
	count_cost_price_sum();
}

function check_bundling_data_length(){
	if ($('tbody[wttype="bundling_data"] tr').length == 1) {
	    $('tbody[wttype="bundling_data"]').children(':first').show();
	}
}
</script>