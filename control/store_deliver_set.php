...
class store_deliver_setControl extends BaseSellerControl {
     ...
     * 发货地址列表
     */
    public function daddress_listWt() {
     ...
     * 新增/编辑发货地址
     */
    public function daddress_addWt() {
        Language::read('member_member_index');
        ...
        if (chksubmit()) {
            ...
            showDialog($lang['wt_common_op_succ'],'reload','succ','CUR_DIALOG.close()');
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['store_daddress_wrong_argument'],'index.php?w=store_deliver_set&t=daddress_list','html','error');
            }
     ...
     * 删除发货地址
     */
    public function daddress_delWt() {
        ...
        if ($delete){
            showDialog(Language::get('store_daddress_del_succ'),'index.php?w=store_deliver_set&t=daddress_list','succ');
        }else {
            showDialog(Language::get('store_daddress_del_fail'),'','error');
        }
    }
     ...
     * 设置默认发货地址
     */
   public function daddress_default_setWt() {
       $address_id = intval($_GET['address_id']);
    ...
    public function expressWt() {
        ...
        if (chksubmit()){
            ...
            if ($result){
                showDialog(Language::get('wt_common_save_succ'),'reload','succ');
            }else{
     ...
     * 免运费额度设置
     */
    public function free_freightWt(){
     ...
     * 预计送达时间设置
     */
    public function free_timeWt(){
        $model_store = Model('store');
        if (chksubmit()) {
            $store_free_time = floatval(abs($_POST['store_free_time']));
            $model_store->editStore(array('store_free_time'=>$store_free_time),array('store_id'=>$_SESSION['store_id']));
            showDialog(L('wt_common_save_succ'),'','succ');
        }
        Tpl::output('store_free_time',$this->store_info['store_free_time']);
        self::profile_menu('daddress','free_time');
        Tpl::showpage('store_free_time.index');
    }

    /**
     * 默认配送区域设置
     */
    public function deliver_regionWt(){
        if (chksubmit()) {
     ...
     * 发货单打印设置
     */
    public function print_setWt(){
        $model = Model();
        $store_info = $model->table('store')->where(array('store_id'=>$_SESSION['store_id']))->find();
        if(empty($store_info)){
            showDialog(Language::get('store_storeinfo_error'),'index.php?w=store_printsetup','error');
        }
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch ($menu_type) {
            case 'daddress':
                $menu_array = array(
                array('menu_key'=>'daddress',   'menu_name'=>Language::get('store_deliver_daddress_list'),  'menu_url'=>'index.php?w=store_deliver_set&t=daddress_list'),
                array('menu_key'=>'express',        'menu_name'=>Language::get('store_deliver_default_express'),    'menu_url'=>'index.php?w=store_deliver_set&t=express'),
                array('menu_key'=>'free_freight',   'menu_name'=>'免运费额度',      'menu_url'=>'index.php?w=store_deliver_set&t=free_freight'),
				array('menu_key'=>'free_time',   'menu_name'=>'预计送达时间',      'menu_url'=>'index.php?w=store_deliver_set&t=free_time'),
                array('menu_key'=>'deliver_region',   'menu_name'=>'默认配送地区',      'menu_url'=>'index.php?w=store_deliver_set&t=deliver_region'),
                array('menu_key'=>'print_set', 'menu_name'=>'发货单打印设置', 'menu_url'=>'index.php?w=store_deliver_set&t=print_set')
                );
                break;
        }
