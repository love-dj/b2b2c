$(function(){
    ...
    var short_video = toconfigure('short_video');
    console.log(short_video);
    if (short_video ==1){
        $('#short_video').show();
    }else{
        $('#short_video').hide();
	...
	if(key){
        $.ajax({
            ...
            success:function(result){
                ...
                //验证三级分销是否开启
                var sjfx_isuse = parseInt(result.datas.distribution_isuse);
                if(sjfx_isuse != 1){
                    $("#sjfx").css("display","none");
                }
                //验证团队无限级是否开启
                var tdwxj_isuse = parseInt(result.datas.team_isuse);
                if(tdwxj_isuse != 1){
                    $("#tdwxj").css("display","none");
                }
                //验证区域代理是否开启
                var qydl_isuse = parseInt(result.datas.agent_isuse);
                if(qydl_isuse != 1){
                    $("#qydl").css("display","none");
                }				
				
                //验证两个消费返利模块是否开启
                var buy_return_isuse = parseInt(result.datas.buy_return_isuse);
                var full_return_isuse = parseInt(result.datas.full_return_isuse);
                if(buy_return_isuse != 1 && full_return_isuse != 1 ){
                    $("#return").css("display","none");
                }
				
				//验证股东分红是否开启
                var shareholder_isuse = parseInt(result.datas.shareholder_isuse);
                if(shareholder_isuse != 1){
