...
class store_deliverControl extends BaseSellerControl {
    ...
    public function indexWt() {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($order_info['order_type'] == 4) {//拼团订单
                ...
                $_info = $model_pingou->getOrderInfo(array('order_id'=> $order_id));
    ...
    public function sendWt(){
        ...
        if (chksubmit()){
            ...
            $result = $handle_order->changeOrderSend($order_info, 'seller', $_SESSION['seller_name'], $_POST);
            if (!$result['state']) {
     ...
     * @return boolean
     */
    public function buyer_address_editWt() {
        $order_id = intval($_GET['order_id']);
     ...
     * 收货地址保存
     */
    public function buyer_address_saveWt() {
        $model_order = Model('order');
        $data = array();
     ...
     * @return boolean
     */
    public function send_address_selectWt() {
     ...
     * 保存发货地址修改
     */
    public function send_address_saveWt() {
     ...
     * 延迟收货
     */
    public function delay_receiveWt(){
        $order_id = intval($_GET['order_id']);
     ...
     * 运单打印
     */
    public function waybill_printWt() {
        $order_id = intval($_GET['order_id']);
        if($order_id <= 0) {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(
                array('menu_key'=>'deliverno',          'menu_name'=>Language::get('wt_member_path_deliverno'), 'menu_url'=>'index.php?w=store_deliver&t=index&state=deliverno'),
                array('menu_key'=>'delivering',         'menu_name'=>Language::get('wt_member_path_delivering'),    'menu_url'=>'index.php?w=store_deliver&t=index&state=delivering'),
                array('menu_key'=>'delivered',      'menu_name'=>Language::get('wt_member_path_delivered'), 'menu_url'=>'index.php?w=store_deliver&t=index&state=delivered'),
                );
                break;
            case 'search':
                $menu_array = array(
                1=>array('menu_key'=>'nodeliver',           'menu_name'=>Language::get('wt_member_path_deliverno'), 'menu_url'=>'index.php?w=store_deliver&t=index&state=nodeliver'),
                2=>array('menu_key'=>'delivering',          'menu_name'=>Language::get('wt_member_path_delivering'),    'menu_url'=>'index.php?w=store_deliver&t=index&state=delivering'),
                3=>array('menu_key'=>'delivered',       'menu_name'=>Language::get('wt_member_path_delivered'), 'menu_url'=>'index.php?w=store_deliver&t=index&state=delivered'),
                4=>array('menu_key'=>'search',      'menu_name'=>Language::get('wt_member_path_deliver_info'),  'menu_url'=>'###'),
                );
                break;
        }
