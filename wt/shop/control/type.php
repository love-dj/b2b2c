...
class typeControl extends SystemControl {
    ...
    public function get_xmlWt() {
        ...
        foreach ((array)$type_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick='fg_del(". $value['type_id'] .")'><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?w=type&t=type_edit&t_id=".$value['type_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function type_addWt(){
        ...
        if (chksubmit()){
            ...
            $url = array(
                array(
                    'url'=>'index.php?w=type&t=type_add',
                    'msg'=>$lang['type_index_continue_to_dd']
                ),
                array(
                    'url'=>'index.php?w=type&t=type',
                    'msg'=>$lang['type_index_return_type_list']
                )
            );
            $this->log(L('wt_add,type_index_type_name').'['.$_POST['t_mane'].']',1);
        ...
        if(is_array($spec_list) && !empty($spec_list)){
            foreach($spec_list as $k=>$val){
                ...
                $s_list[$val['class_id']]['name'] = $val['class_name']==''?L('wt_default'):$val['class_name'];
            }
        }
        ksort($s_list);
        // 一级分类列表
    ...
    public function type_editWt(){
        ...
        if (chksubmit()){
            ...
            if ($return){
                ...
                $this->log(L('wt_edit,type_index_type_name').'['.$_POST['t_mane'].']',1);
                showMessage($lang['wt_common_save_succ'],$url);
            } else {
                $this->log(L('wt_edit,type_index_type_name').'['.$_POST['t_mane'].']',0);
                showMessage($lang['wt_common_save_fail']);
            }
        }
    ...
    public function type_delWt(){
        ...
        if(is_array($type_list) && !empty($type_list)){
            ...
            $this->log(L('wt_delete,type_index_type_name').'[ID:'.$id.']',1);
            exit(json_encode(array('state'=>true,'msg'=>L('type_index_del_succ'))));
        }else{
            $this->log(L('wt_delete,type_index_type_name').'[ID:'.$id.']',0);
            exit(json_encode(array('state'=>false,'msg'=>L('param_error'))));
        }
    }
}
