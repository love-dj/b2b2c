...
class homeControl extends MircroShopControl{
    ...
    public function __construct() {
        ...
        $visit_count = $model_what_member_info->updateMemberVisitCount($member_info['member_id']);
        ...
        Tpl::output('html_title',$member_info['member_name'].'-'.Language::get('wt_what').'-'.C('site_name'));
     ...
     * 喜欢列表
     */
    public function like_listWt() {
        $type = 'goods';
        if(isset($_GET['type'])) {
            $type = $_GET['type'];
        }
        $model_like = Model('what_like');
        $condition = array();
        ...
        case 'store':
            ...
            if(!empty($like_store_list)) {
                ...
                $store_list = $model_what_store->getListWithStoreInfo(array('what_store_id' => array('in' , $store_id)), null, 'what_sort asc');
            }
    ...
    public function add_followWt() {
        ...
        if(empty($member_list)){
            self::return_json(Language::get('wt_common_save_fail'),'false');
        }
        $self_info = array();
        $member_info = array();
        ...
        if($result){
            ...
            self::return_json(Language::get('wt_common_save_succ'),'true');
        }else{
            self::return_json(Language::get('wt_common_save_fail'),'false');
        }
    }
