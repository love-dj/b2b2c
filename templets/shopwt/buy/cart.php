...
<div class="wtc-main">
  ...
  <form action="<?php echo urlShop('buy','buy_step1');?>" method="POST" id="form_buy" name="form_buy">
    ...
    <table class="wtc-table-style" wt_type="table_cart">
      <thead>
        <tr>
          <th class="w50"><label>
      ...
      <tbody>
        <tr>
          <th colspan="20"> <!-- S 店铺名称 -->
            ...
            <div class="wtc-store-name">店铺：<a href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id), $output['store_list'][$store_id]['store_domain']);?>"><?php echo $cart_list[0]['store_name']; ?></a><span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span></div>
            
            <!-- E 店铺名称 --> 
            <!-- S 店铺满即送 -->
            
            <?php if (isset($output['voucher_template'][$store_id])) {?>
            <div class="wtc-store-sale" wttype="get_voucher"> <span>代金券</span>免费领取<i class="icon-angle-down"></i>
              <div class="combobox"><i></i>
                ...
                <div class="combobox-list"> <span class="par">¥<?php echo $voucher['voucher_t_price'];?></span> <span class="rule">
                  <p>全店通用，满<?php echo wtPriceFormat($voucher['voucher_t_limit']);?>元可折扣<?php echo $voucher['voucher_t_price'];?>元</p>
            ...
            <div class="wtc-store-sale ms" wt_group="<?php echo $cart_info['cart_id'];?>"><span>满即送</span><?php echo $output['mansong_rule_list'][$store_id][0];?><i class="icon-angle-down"></i>
            ...
            <div class="wtc-store-sale"><span>免运费</span><?php echo $output['free_freight_list'][$store_id];?></div>
            <?php } ?>
...
<?php } ?> /></td>
          ...
          <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="wtc-goods-content">
              ...
              <dd class="wtc-goods-gift"> <span>赠</span>
          ...
          <td><?php if ($cart_info['state']) {?>
            <em id="item<?php echo $cart_info['cart_id']; ?>_subtotal" wt_type="eachGoodsTotal" class="goods-subtotal"><?php echo $cart_info['goods_total']; ?></em>
            <?php }?></td>
        ...
        <tr class="shop-list <?php echo $cart_info['state'] ? '' : 'item_disabled';?> bundling-list" wt_group="<?php echo $cart_info['cart_id'];?>">
          <td class="tree td-border-left"></td>
          <td class="w100"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$goods_content['goods_id']));?>" target="_blank" class="wtc-goods-thumb"><img src="<?php echo cthumb($goods_content['goods_image'],60,$store_id);?>" alt="<?php echo $goods_content['goods_name']; ?>" /></a></td>
          ...
          <td><?php echo $cart_info['state'] ? '' : '无效';?>1件/套x<em wtType="blnum<?php echo $cart_info['bl_id'];?>"><?php echo $cart_info['goods_num']; ?></em></td>
          <td><em wtType="bltotal<?php echo $cart_info['bl_id'];?>" price="<?php echo $goods_content['bl_goods_price'];?>" class="goods-subtotal"><?php echo wtPriceFormat($goods_content['bl_goods_price']*$cart_info['goods_num']);?></em></td>
          <td class="tl td-border-right"><a href="javascript:void(0)" onclick="collect_goods('<?php echo $goods_content['goods_id']; ?>');">移入收藏夹</a></td>
        </tr>
        <?php } ?>
        <tr>
        ...
        <tr>
          <td colspan="20"><?php if ($is_chain) { ?>
            <div class="wtc-chain-tip"><strong>*</strong>该订单中包含支持“门店自提”服务的商品，如需到店自提，请选择提交<a data_store_id="<?php echo $store_id ?>" wt_type="chain" href="javascript:void(0)"><i class="icon-truck"></i>门店自提订单</a></div>
            <?php } ?></td>
        </tr>
        <?php } ?>
      </tbody>
      <tfoot>
        <tr>
          <td colspan="20"><div class="wtc-all-account"><?php echo $lang['cart_index_goods_sumary'];?><em id="cartTotal"><?php echo $output['cart_totals']; ?></em><?php echo $lang['currency_zh'];?></div>
            <a id="next_submit" href="javascript:void(0)" class="wtc-next-submit"><?php echo $lang['cart_index_ensure_info'];?></a></td>
        </tr>
      </tfoot>
    </table>
  </form>
  ...
  <tbody id="jjg-header-boxper">
    <tr class="jjg-xor-outer-false">
      <td colspan="20" class="td-border-left td-border-rihgt tl"><div class="jjg-xor-inner-false"><strong>【加价购】</strong>已购满 %m0% 元，再加 %p0% 元，即可换购商品。 </div>
        <div class="jjg-xor-inner-true"><strong>【加价购】</strong>已购满 <span data-jjg-header-mincost="%id%">%mincost%</span> 元，再加 <span data-jjg-header-plus="%id%">%plus%</span> 元，点击<a href="javascript:;" data-jjg-toggle="%id%" class="wtbtn-mini ml5">换购商品<i class="icon-caret-down ml5"></i></a></div>
...
$(function() {
	$('a[wt_type="chain"]').on('click',function(){
    ...
    // 领取代金券
    $('div[wttype="get_voucher"]').on('click', 'a[data-tid]', function(){
        var _tid = $(this).attr('data-tid');
        ajaxget('index.php?w=voucher&t=getvouchersave&jump=0&tid=' + _tid);
    });
	//
...
window.jjgCallback = (function(jjgDetails) {
    ...
    var jjgCost = function(id) {
        ...
        var c = 0;
        $items.filter(':checked').parents('tr.shop-list').find("em[wt_type='eachGoodsTotal']").each(function() {
    ...
    // 活动头部TPL
    var jjgHeaderTpl = $('#jjg-header-boxper').html();
