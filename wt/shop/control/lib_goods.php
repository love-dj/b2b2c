...
class lib_goodsControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                $operation .= '<a class="btn orange" href="index.php?w=lib_goods&t=goods_edit&goods_id='.$v['goods_id'].'"><i class="fa fa-pencil-square-o"></i>编辑</a>';
    ...
    public function goods_addWt(){
        ...
        if (chksubmit()) {
            ...
            $_array['goods_video'] = $_POST['video_path'];//shopwt

            $state = $model_goods->addGoods($_array);
            if ($state) {
                $this->log('新增商品，编号'.$state);
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=lib_goods');
            } else {
