...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?w=goods&t=get_xml&type=<?php echo $output['type'];?>',
        colModel : [
            ...
            {display: '商品视频', name : 'goods_video', width : 60, sortable : true, align: 'center'},//shopwt
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=goods&t=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=goods&t=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_lonkup(ids) {
    _uri = "index.php?w=goods&t=goods_lockup&id=" + ids;
    CUR_DIALOG = ajax_form('goods_lockup', '违规下架理由', _uri, 640);
}
//添加评论
function fg_pinglun(gids) {
    _uri = "index.php?w=goods&t=add&goods_id=" + gids;
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?w=goods&t=goods_del', {id:id}, function(data){
            if (data.state) {
...
function fg_verify(ids) {
    _uri = "index.php?w=goods&t=goods_verify&id=" + ids;
    CUR_DIALOG = ajax_form('goods_verify', '审核商品', _uri, 640);
}
// 加入商品库
function fg_goods_add(id) {
        $.getJSON('index.php?w=goods&t=goods_add', {id:id}, function(data){
            if (data.state) {
                showSucc(data.msg)
            } else {
                showError(data.msg)
...
function fg_see_video(ids) {
      _uri = "index.php?w=goods&t=see_video&id=" + ids;
