...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=consulting&t=get_consulting_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?w=consulting&t=get_consulting_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?w=consulting&t=get_consulting_xml',
        colModel : [
...
function fg_delete(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?w=consulting&t=delete",
        data: "consult_id="+id,
