...
class flea_regionControl extends SystemControl{
	public function __construct(){
		...
		if ($GLOBALS['setting_config']['flea_isuse'] != 1 ){
			showMessage(Language::get('flea_index_unable'),'index.php?w=dashboard&t=welcome');
			// showMessage(Language::get('admin_ztc_unavailable'),'index.php?w=dashboard&t=welcome');
		}
	}
	 ...
	 * @return 
	 */
	public function flea_region_importWt(){
		require_once(BASE_DATA_PATH.DS.'cache'.DS.'flea_cache.php');
		$lang	= Language::getLangContent();
		/**
		 * 实例化模型
		...
		if ($_POST['form_submit'] == 'ok'){
			if (!empty($_FILES['csv'])){
				...
				showMessage($lang['region_import_succ'],'index.php?w=flea_region&t=flea_region');
			}else {
	 ...
	 * @return 
	 */
	public function flea_import_default_areaWt() {
		$lang	= Language::getLangContent();
		
		$file = BASE_DATA_PATH.'/static/examples/flea_area.sql';
		if (!is_file($file)){
			showMessage($lang['region_import_csv_null']);
		}
		        ...
		        if (trim($buffer) != ''){
			        ...
			        if (substr(rtrim($buffer),-1) == ';'){
			        	...
			        	if (!empty($tmp_sql)){
			        		...
			        		$tmp_sql = str_replace("`shopwt_flea_area`","`".DBPRE."flea_area`",$tmp_sql);
			        		Db::query($tmp_sql);
			        		unset($tmp_sql);
			        	}
			        }
