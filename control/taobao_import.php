...
class taobao_importControl extends BaseSellerControl {
        ...
        if(!checkPlatformStore()){
            ...
            if (intval($this->store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $this->store_grade['sg_goods_limit']) {
                    showMessage(L('store_goods_index_goods_limit') . $this->store_grade['sg_goods_limit'] . L('store_goods_index_goods_limit1'), 'index.php?w=store_goods&t=goods_list', 'html', 'error');
                }
            }
        }
    }
		   ...
		   if (strtoupper(CHARSET) == 'GBK'){
			...
			if(intval($store_grade['sg_goods_limit']) != 0) {
				if($goods_num >= $store_grade['sg_goods_limit']) {
					showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'],'index.php?w=store_goods&t=goods_list','html','error');
				}
			...
			if(intval($store_info['store_end_time']) != 0) {
				if(time() >= $store_info['store_end_time']) {
					showMessage($lang['store_goods_index_time_limit'],'index.php?w=store_goods&t=goods_list','html','error');
				}
			}
			/**
			 * 循环添加数据
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					if($remain_num>0 and $k>=$remain_num){
						showMessage($lang['store_goods_index_goods_limit'].$store_grade['sg_goods_limit'].$lang['store_goods_index_goods_limit1'].$lang['store_goods_import_end'].(count($records)-$remain_num).$lang['store_goods_import_products_no_import'],'index.php?w=store_goods&t=taobao_import&step=4','html','error');
					}
					...
					$param['goods_marketprice']		= $record['goods_store_price']*(1+0.1);
					...
					$param['gc_id_2']					= intval($_POST['cls_2']);
					$param['gc_id_3']					= intval($_POST['cls_3']);
					$param['goods_sale_price']	    = $param['goods_price'];
					$param['goods_spec']				= 'N;';
					$param['store_name']	            =$store_info['store_name'];
					
			        $goods_id1=$model_store_goods->addGoods($param);
					//导入后直接生成二维码
					$PhpQRCode->set('date',WAP_SITE_URL . '/html/product_detail.html?goods_id='.$goods_id1);
	...
	private function get_goods_image($pic_string){
		...
		if(!empty($pic_array) && is_array($pic_array)){
			...
			file_put_contents(BASE_DATA_PATH.'/cache/imgarr.txt',$str,FILE_APPEND);
			return $array;
		}else{
			return false;
		}
