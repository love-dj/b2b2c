...
class order_bookHandle {
    ...
    public function buy($order_data = array(), $post_data = array()) {
        ...
        $goods_content = current($order_data['goods_list']);
        //如果选择只付定金，产生两条时段记录
        if ($post_data['book_pay_type'] == 'part') {
            $data = array();
            $data['book_order_id'] = key($order_data['order_list']);
            $data['book_step'] = 1;
            $data['book_amount'] = $goods_content['book_down_payment'] * $goods_content['goods_num'];
            ...
            $data['book_end_time'] = $goods_content['book_down_time'] + BOOK_AUTO_END_TIME * 3600;
            ...
            $data['book_end_time'] = TIMESTAMP + ORDER_AUTO_CANCEL_TIME * 3600;
            $data['book_deposit_amount'] = $goods_content['book_down_payment'] * $goods_content['goods_num'];
            $data['book_store_id'] = $order_info['store_id'];
            $insert = $model_order_book->addOrderBook($data);
            if (!$insert) {
                throw new Exception('订单保存失败[预定时段出现异常]');
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                if ($book_list[0]['book_end_time'] > TIMESTAMP) {
                    ...
                    if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                        ...
                        $book_list[0]['book_operate'] = '<a class="wtm-btn wtm-btn-orange fr mr15" href="index.php?w=buy&t=pay&pay_sn='.$pay_sn.'">付款</a>';
                    ...
                    $book_list[0]['book_amount_ext'] = '(含定金'.wtPriceFormat($book_list[0]['book_deposit_amount']).')';
                } else {
                    //已过期
                ...
                if (empty($book_list[1]['book_pay_time'])) {
                    ...
                    if ($book_list[1]['book_end_time'] > TIMESTAMP) {
                        ...
                        if ($order_info['order_state'] == ORDER_STATE_CANCEL) {
                            ...
                            $book_list[1]['book_operate'] = '<a class="wtm-btn wtm-btn-orange fr mr15" href="index.php?w=buy&t=pay&pay_sn='.$pay_sn.'">付款</a>';
    ...
    public function changeBookOrderReceivePay($order_info,$post) {
        ...
        if ($book_list[0]['book_step']) {
            ...
            if (empty($book_list[0]['book_pay_time'])) {
                ...
                Model('goods')->editGoods(array('book_buyers'=>array('exp','book_buyers+1')),array('goods_id'=>$order_goods_content['goods_id']));
            } else {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            $result = Handle('queue')->cancelOrderUpdateStorage($data);
            if (!$result['state']) {
