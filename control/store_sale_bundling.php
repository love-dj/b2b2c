...
class store_sale_bundlingControl extends BaseSellerControl {
    ...
    public function __construct() {
         ...
         * 读取语言包
         */
        Language::read('member_layout,member_store_sale_bundling');
        //检查是否开启
    ...
    public function indexWt() {
        $this->bundling_listWt();
    }

    /**
     * 发布的活动列表
     */
    public function bundling_listWt() {
        ...
        if (checkPlatformStore()) {
            ...
            if (!empty($bundling_quota)) {
                ...
                $bundling_surplus    = intval(C('sale_bundling_sum')) - intval($bundling_published);
        ...
        Tpl::showpage('store_sale_bundling.list');
    }

    /**
     * 套餐购买
     */
    public function bundling_quota_addWt() {
        if (chksubmit()) {
            $quantity = intval($_POST['bundling_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_bundling_price')); // 扣款数
            if ($quantity <= 0 || $quantity > 12) {
                showDialog(Language::get('bundling_quota_price_fail'), urlShop('store_sale_bundling', 'bundling_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_bundling = Model('p_bundling');
            ...
            if ($return) {
                ...
                $this->recordSellerLog('购买'.$quantity.'套优惠套装，单价'.intval(C('sale_bundling_price')).'元');
                showDialog(L('bundling_quota_price_succ'), urlShop('store_sale_bundling', 'bundling_list'), 'succ');
            } else {
                showDialog(L('bundling_quota_price_fail'), urlShop('store_sale_bundling', 'bundling_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('bundling_quota_add', 'bundling_quota_add');
        Tpl::showpage('store_sale_bundling.quota_add');
    }

    /**
     * 套餐续费
     */
    public function bundling_renewWt() {
        if (chksubmit()) {
            $model_bundling = Model('p_bundling');
            $quantity = intval($_POST['bundling_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('sale_bundling_price')); // 扣款数
            ...
            if ($return) {
                ...
                $this->recordSellerLog('续费'.$quantity.'套优惠套装，单价'.intval(C('sale_bundling_price')).'元');
                showDialog(L('bundling_quota_price_succ'), urlShop('store_sale_bundling', 'bundling_list'), 'succ');
            } else {
                showDialog(L('bundling_quota_price_fail'), urlShop('store_sale_bundling', 'bundling_quota_add'));
            }
     ...
     * 套餐活动添加
     */
    public function bundling_addWt() {
        /**
         * 实例化模型
         */
        $model_bundling = Model('p_bundling');
        ...
        // 验证套餐数量
        if (intval(C('sale_bundling_sum')) != 0 && !isset($_REQUEST['bundling_id'])) {
            $count = $model_bundling->getBundlingCount(array('store_id' => $_SESSION['store_id']));
            if (intval(C('sale_bundling_sum')) <= intval($count)) {
        ...
        if (chksubmit()) {
            ...
            if (!$return) {
                showDialog(L('wt_common_op_fail'), '', '', 'error');
            }
            ...
            if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
                foreach ($_POST['goods'] as $key => $val) {
                    // 验证是否为本店铺商品
                    $goods_content = $model_goods->getGoodsInfoByID($val['gid'], 'goods_id,goods_name,goods_image,store_id');
                    ...
                    $data['goods_image'] = $goods_content['goods_image'];
                    $data['bl_goods_price'] = wtPriceFormat($val['price']);
            ...
            showDialog(L('wt_common_op_succ'), urlShop('store_sale_bundling', 'bundling_list'), 'succ');
        }
        ...
        if (intval($_GET['bundling_id']) > 0) {
            ...
            if (empty($bundling_info['store_id'])) {
                showMessage(L( 'wrong_argument'), urlShop('store_sale_bundling', 'bundling_list'), '', 'error' );
            }
        ...
        Tpl::showpage('store_sale_bundling.add');
    }

    /**
     * 套餐活动添加商品
     */
    public function bundling_add_goodsWt() {
        /**
         * 实例化模型
         */
        $model_goods =Model('goods');
        ...
        Tpl::showpage('store_sale_bundling.add_goods', 'null_layout');
    }

    /**
     * 删除优惠套装活动
     */
    public function drop_bundlingWt(){
        /**
         * 参数验证
         */
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'bundling_quota_list':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_list'))
                );
                break;
            case 'bundling_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_quota_add', 'menu_name'=>Language::get('bundling_quota_add'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_quota_add'))
                );
                break;
            case 'bundling_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShop('store_sale_bundling', 'bundling_renew'))
                );
                break;
            case 'bundling_add':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_add', 'menu_name'=>Language::get('bundling_add'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_add'))
                );
                break;
            case 'bundling_edit':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_edit', 'menu_name'=>Language::get('bundling_edit'), 'menu_url'=>urlShop('store_sale_bundling', 'bundling_edit'))
                );
            break;
        }
