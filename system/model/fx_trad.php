...
class fx_tradModel extends Model
{
     ...
     * 获取佣金变更日志
     */
    public function getFenxiaoTradList($condition = array(), $field = '*', $page = 0, $order = 'lg_id desc', $limit = 0)
    {
        return $this->table('fx_trad_log')->field($field)->where($condition)->order($order)->page($page)->limit($limit)->select();
    }

    /**
     * 添加佣金变更日志
     */
    public function addFenxiaoTrad($data)
    {
        return $this->table('fx_trad_log')->insert($data);
    }

    /**
     * 获取佣金提现记录
     */
    public function getFenxiaoTradCashList($condition = array(), $field = '*', $page = 0, $order = 'tradc_id desc', $limit = 0)
    {
        return $this->table('fx_trad_cash')->field($field)->where($condition)->order($order)->page($page)->limit($limit)->select();
    }

    /**
     * 获取单条佣金提现记录
     */
    public function getFenxiaoTradCashInfo($condition = array(), $field = '*', $order = 'tradc_id desc')
    {
        return $this->table('fx_trad_cash')->field($field)->where($condition)->order($order)->find();
    }

    public function getFenxiaoCashCount($condition = array()){
        return $this->table('fx_trad_cash')->where($condition)->count();
    }

    /**
     * 添加佣金提现记录
     */
    public function addFenxiaoTradCash($data)
    {
        return $this->table('fx_trad_cash')->insert($data);
    }

    /**
     * 更新佣金提现记录
     */
    public function updateFenxiaoTradCash($data, $condition = array())
    {
        return $this->table('fx_trad_cash')->where($condition)->update($data);
    }
     ...
     * @return mixed
     */
    public function delFenxiaoTradCash($condition){
        return $this->table('fx_trad_cash')->where($condition)->delete();
    }
    ...
    public function changeDirtriTrad($change_type, $data = array())
    {
        ...
        $data_msg['trad_url'] = urlFenxiao('cash', 'cash_list');
        switch ($change_type) {
            case 'cash_apply':
        ...
        $insert = $this->table('fx_trad_log')->insert($data_log);
        if (!$insert) {
            throw new Exception('操作失败');
        }
     ...
     * 退出自动提现
     */
    public function autoFenxiaoTrad($member_info)
    {
        try {
            ...
            if($tradc_amount > 0){
                ...
                $insert = $this->addFenxiaoTradCash($data);
                if (!$insert) {
                    throw new Exception();
                }
