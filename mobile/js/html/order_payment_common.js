...
var password,rcb_pay,pd_pay,payment_code;
 ...
 function toPay(pay_sn,w,t) {
     $.ajax({
		 ...
		 var readytoPayPal = false;		 
		     ...
		     if (_payment_code == 'paypal' && readytoPayPal) {
                        $('#paypal').parents('label').show();
                        if (payment_code == '') {
                            payment_code = _payment_code;
                            $('#' + _payment_code).attr('checked', true).parents('label').addClass('checked');
                        }
 ...
 function goToPayment(pay_sn,t) {
     location.href = ApiUrl+'/index.php?w=member_payment&t='+t+'&key=' + key + '&pay_sn=' + pay_sn + '&password=' + password + '&rcb_pay=' + rcb_pay + '&pd_pay=' + pd_pay + '&payment_code=' + payment_code;
 }
