...
<?php } else { ?>
  ...
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_book', 'book_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-mint" href="javascript:void(0);" wttype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="wtbtn wtbtn wtbtn-aqua" href="<?php echo urlShop('store_sale_book', 'book_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['wt_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['book_quota']['bkq_endtime']);?></strong>
<?php }?>
  <ul>
...
<table class="wtsc-default-table">
  ...
  <tbody wttype="choose_goods_list">
    <tr wttype="tr_no_sale" style="display:none;">
      <td colspan="20" class="norecord"><div class="no-sale"><i class="zw"></i><span>预定商品列表暂无内容，请选择添加预定商品。</span></div></td>
    </tr>
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle">
        <span><a class="btn-grapefruit" href='javascript:void(0);' wttype="del_choosed" data-param="{gid:<?php echo $val['goods_id'];?>, type:<?php if ($val['is_book'] == 1) {?>'book'<?php } else if ($val['is_presell'] == 1) {?>'presell'<?php }?>}"><i class="icon-trash"></i><p><?php echo $lang['wt_del'];?></p></a></span></td>
    </tr>
    <?php } ?>
    <?php } ?>
  </tbody>
...
$(function(){
    ...
    $('input[wttype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlShop('store_sale_book', 'book_select_goods');?>';
    ...
    $('div[wttype="div_goods_select"]').on('click', 'a[wttype="a_choose_goods"]', function(){
        ...
        CUR_DIALOG = ajax_form('choose_goods', '预定商品规则设定', '<?php echo urlShop('store_sale_book', 'choosed_goods');?>&gid='+_gid+'&type='+_type, 640);
    });

    // 删除商品
    $('tbody[wttype="choose_goods_list"]').on('click','a[wttype="del_choosed"]', function(){
        $this = $(this);
        _url = '<?php echo urlShop('store_sale_book', 'del_choosed_goods');?>';
...
function choose_goods(data, type) {
    ...
    _tr = $('<tr class="bd-line"></tr>')
        ...
        .append('<td class="nscs-table-handle"><span><a class="btn-grapefruit" href="javascript:void(0);" data-param="{gid:'+ data.goods_id +', type:\'' + type + '\'}" wttype="del_choosed"><i class="icon-trash"></i><p>删除</p></a></span></td>')
