...
<div class="wrapper_search">
        ...
        <div class="wp_category_list">
          <div id="class_div_1" class="category_list">
            <ul>
              ...
              <?php foreach ($output['goods_class'] as $val) { if($val['is_show']=='1'){continue;}  ?>
  ...
  <div class="alert">
    <dl class="hover_tips_cont">
      ...
      <dt id="commoditydt" style="display: none;" class="current_sort"><?php echo $lang['store_goods_step1_current_choose_category'];?><?php echo $lang['wt_colon'];?></dt>
  ...
  <div class="wp_confirm">
    <form method="get">
      ...
      <input type="hidden" name="t" value="edit_goods" />
      <input type="hidden" name="commonid" value="<?php echo $output['commonid'];?>" />
      <input type="hidden" name="ref_url" value="<?php echo $_GET['ref_url'];?>" />
      <?php } else {?>
      <input type="hidden" name="w" value="store_goods_add" />
      <input type="hidden" name="t" value="add_step_two" />
      <?php }?>
      <input type="hidden" name="class_id" id="class_id" value="" />
      <input type="hidden" name="t_id" id="t_id" value="" />
      <div class="bottom tc">
      <label class="submit-border"><input disabled="disabled" wttype="buttonNextStep" value="<?php echo $lang['store_goods_add_next'];?>，填写商品信息" type="submit" class="submit"style=" width: 200px;" /></label>
      </div>
    </form>
  </div>
</div>
...
<script src="<?php echo STATIC_SITE_URL;?>/js/store_goods_add.step1.js"></script> 
<script>
SEARCHKEY = '<?php echo $lang['store_goods_step1_search_input_text'];?>';
STATIC_SITE_URL = '<?php echo STATIC_SITE_URL;?>';
<?php if(empty($output['goods_class']) ) {?>
$(function(){
      showDialog('找不到商品分类，点击确定去申请经营类目，审核通过后再来发布商品。','confirm', '', function(){
		  window.location.href='index.php?w=store_info&t=bind_class';
		});
})
      <?php }?>
</script>
