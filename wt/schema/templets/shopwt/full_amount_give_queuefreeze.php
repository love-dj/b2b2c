...
<div class="page" style="margin-bottom:30px; ">
  <div class="fixed-bar">
    <div class="item-title">
        <div class="subject" >
            <h3 style="line-height:50px;">满额赠送队列冻结</h3>
        </div>

        <div class="container">
            <form style="display: inline-block;" method="post" action="index.php?act=full_amount_give_queuefreeze">
                ...
                <a href="index.php?act=full_amount_give_queuefreeze&op=export_full_maountfreeze&member_keyword=<?php echo $_POST['member_keyword'] ;?>&interest_num=<?php echo $_POST['interest_num'] ;?>&start_time=<?php echo $_POST['start_time'] ;?>&end_time=<?php echo $_POST['end_time'] ;?>" style="margin-left: 46px;border: 1px solid;padding: 3px;cursor: pointer">导出EXCEL</a>
...
<div>
    <table>
        ...
        <tr class="td frozen">
            ...
            <td><?php echo $value['interest_dot_freeze'] ; ?></td>
            <td><?php
                echo ($value['interest_dot_freeze']*$value['unit_price']*$value['rate']/100).'元'.'<br>';
                echo $value['rate'].'%'.'<br/>';
                echo $value['interest_dot_freeze']*$value['unit_price'].'元'.'<br/>';
            ...
            <?php
            if($value['status']==2){
                echo '<td style="color:#fff;"><span style="border:1px solid;background-color: #2751b0ed;padding: 3px;font-size: 13px;">冻结中</span></td>';
...
<div style="margin-top: 10px;">
    <button type="button" id="btn"  >解除冻结</button>
</div>
...
<script type="text/javascript">
    ...
    $('#btn').click(function (){
        ...
        if(confirm('您确认要解冻这些选项吗？')){
            $.post("index.php?act=full_amount_give_queuefreeze&op=outforzen",{data:str},function(e){
                if(e.status==1){
    ...
    function fg_delete(id) {
        ...
        if(confirm('删除后将不能恢复，确认删除这 1 项吗？')){
            location.href = 'index.php?act=full_amount_give_queuefreeze&op=del_queuefreeze&id='+id;
