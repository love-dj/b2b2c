...
<div class="wrap">
  ...
  <form method="get" action="index.php" target="_self">
    <table class="wtm-search-table">
      <input type="hidden" name="w" value="member_order_vr" />
      <tr>
        <td>&nbsp;</td>
    ...
    <tbody>
      ...
      <tr>
        ...
        <!-- QQ -->
          ...
          <a href="javascript:void(0)" class="share-goods" wt_type="sharegoods" data-param='{"gid":"<?php echo $order_info['goods_id'];?>"}'><i class="icon-share"></i><?php echo $lang['member_order_snsshare'];?></a>
          <?php } ?>
          </th>
      </tr>
      <tr>
        <td class="bdl"></td>
        <td class="w70"><div class="wtm-goods-thumb"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$order_info['goods_id']));?>" target="_blank" onMouseOver="toolTip('<img src=<?php echo thumb($order_info, 240);?>>')" onMouseOut="toolTip()"/><img src="<?php echo thumb($order_info, 60);?>"/></a></div></td>
        <td class="tl"><dl class="goods-name">
          ...
          <?php } ?>
            <dd>
            <?php if ($order_info['order_sale_type'] == 1) { ?>
        ...
        <td><?php echo wtPriceFormat($order_info['goods_price']);?></td>
        <td><?php echo $order_info['goods_num'];?></td>
        <td> <?php if($order_info['if_refund']){ ?>
          <a href="index.php?w=member_vr_refund&t=add_refund&order_id=<?php echo $order_info['order_id']; ?>">退款</a>
          <?php } ?></td>
        <td class="bdl"><strong><?php echo $order_info['order_amount'];?></strong><p title="<?php echo $lang['member_order_pay_method'].$lang['wt_colon'];?><?php echo $order_info['payment_name']; ?>"><?php echo $order_info['payment_name']; ?></p></td>
        <td class="bdl"><p><?php echo $order_info['state_desc'];?></p>
          <p><a href="index.php?w=member_order_vr&t=show_order&order_id=<?php echo $order_info['order_id'];?>">订单详情</a></p></td>
        <td class="bdl bdr">

		  <?php if($order_info['if_pay']){ ?>
          <p><a class="wtbtn wtbtn-bittersweet" href="index.php?w=buy_vr&t=pay&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-shield"></i>订单支付</a></p>
          <?php } ?>
          ...
          <p><a class="wtbtn wtbtn-aqua" href="index.php?w=member_evaluate&t=add_vr&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>
