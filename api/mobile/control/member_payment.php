...
class member_paymentControl extends mobileMemberControl {
    ...
    public function __construct() {
        ...
        if($_GET['t'] != 'payment_list' && !$_POST['payment_code']) {
            $payment_code = 'alipay';

            if(in_array($_GET['t'], array('wx_app_pay', 'wx_app_pay3', 'wx_app_vr_pay', 'wx_app_vr_pay3'), true)) {
                $payment_code = 'wxpay';
            }
            else if (in_array($_GET['t'],array('alipay_native_pay','alipay_native_vr_pay'),true)) {
	...
	if (isset($_GET['payment_code'])) {
        ...
        $pay_info = $this->_get_order_vr_info($order_sn,$_GET);
        if(isset($pay_info['error'])) {
            exit($pay_info['error']);
        }
     ...
     * 虚拟订单支付
     */
    public function vr_payWt() {
        $pay_sn = $_GET['pay_sn'];

        $pay_info = $this->_get_order_vr_info($pay_sn);
        if(isset($pay_info['error'])) {
            output_error($pay_info['error']);
        }
    ...
    private function _api_pay($order_pay_info) {
        ...
        if ($this->payment_code == 'wxpay_jsapi') {
            ...
            $param['orderFee'] = (int) (wtPriceFormat(100 * $order_pay_info['api_pay_amount']));
	...
	if ($this->payment_code == 'paypal') {
            $payment_api = new $this->payment_code($this->payment_config,$order_pay_info);
            $payment_api->submit();
            exit;
        }
    ...
    private function _get_real_order_info($pay_sn,$rcb_pd_pay = array()) {
        ...
        //取订单信息
        $result = $handle_payment->getRealOrderInfo($pay_sn, $this->member_info['member_id']);
        if(!$result['state']) {
        ...
        if (!empty($result['data']['order_list'])) {
            foreach ($result['data']['order_list'] as $order_info) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) {
                    $pay_amount += $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount']- $order_info['points_money'];
                    $pay_amount = floatval(wtPriceFormat($pay_amount));
        ...
        if ($pay_amount == 0) {
            redirect(WAP_SITE_URL.'/html/member/order_list.html');
        }

        $result['data']['api_pay_amount'] = wtPriceFormat($pay_amount);
     ...
     * 获取虚拟订单支付信息
     */
    private function _get_order_vr_info($pay_sn,$rcb_pd_pay = array()) {
        $handle_payment = Handle('payment');

        //取得订单信息
        $result = $handle_payment->getVrOrderInfo($pay_sn, $this->member_info['member_id']);
        if(!$result['state']) {
        ...
        if ($result['data']['order_state'] == ORDER_STATE_NEW) {
            $pay_amount += $result['data']['order_amount'] - $result['data']['pd_amount'] - $result['data']['rcb_amount']- $result['data']['points_money'];
        }

        if ($pay_amount == 0) {
            redirect(WAP_SITE_URL.'/html/member/order_vr_list.html');
        }
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_payWt() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_pay3Wt() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 取得支付宝移动支付 订单信息 实物订单
     */
    public function alipay_native_payWt() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 取得支付宝移动支付 订单信息 虚拟订单
     */
    public function alipay_native_vr_payWt() {
        $pay_sn = $_POST['pay_sn'];
        if (!preg_match('/^\d+$/',$pay_sn)){
            output_error('支付单号错误');
        }
