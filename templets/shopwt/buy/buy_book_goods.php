...
<style>
.wtc-table-style tbody tr.item_disabled td { background: none repeat scroll 0 0 #F9F9F9; height: 30px; padding: 10px 0; text-align: center; }
</style>
<div class="wtc-receipt-info">
  ...
  <table class="wtc-table-style">
    ...
    <tbody>
      ...
      <tr>
        <td colspan="20"><div class="wtc-msg">买家留言：
          ...
          <div class="wtc-form-default"> </div>
          <div class="wtc-store-account">
            ...
            <dl wttype="book_pay_content_part" class="total">
              <dt>定金合计：</dt>
              <dd class="rule"><i>*</i>
                <input type="checkbox" value="1" name="agree_part" id="agree_part" class="vm mr5">
                同意支付定金 (不退)</span></dd>
              <dd class="sum"><em><?php echo wtPriceFormat($cart_info['book_down_payment']*$cart_info['goods_num']);?></em></dd>
            </dl>
            <dl wttype="book_pay_content_part">
              <dt>定时通知：</dt>
            ...
            <dl wttype="book_pay_content_full" style="display: none" class="total">
              <dt>全款合计：</dt>
              <dd class="rule"><i>*</i>
    ...
    <tfoot>
      <tr>
        <td colspan="20"><div class="wtc-all-account">本次需支付：<em id="orderBookTotal">...</em><?php echo $lang['currency_zh'];?></div>
          <a href="javascript:void(0)" id='submitOrder' class="wtc-next-submit ok"><?php echo $lang['cart_index_submit_order'];?></a></td>
      </tr>
    </tfoot>
  </table>
</div>
...
function calcOrder() {
    ...
    if ($('#book_pay_part').prop('checked')) {
    	$('#orderBookTotal').html('<?php echo wtPriceFormat($cart_info['book_down_payment'] * $cart_info['goods_num']);?>');
    } else {
...
$(function(){
	$('input[name="book_pay_type"]').on('change',function(){
		if ($(this).val() == 'full') {
			$('dl[wttype="book_pay_content_part"]').hide();
			$('dl[wttype="book_pay_content_full"]').show();
		} else {
			$('dl[wttype="book_pay_content_part"]').show();
			$('dl[wttype="book_pay_content_full"]').hide();
		}
		calcOrder();
	});
