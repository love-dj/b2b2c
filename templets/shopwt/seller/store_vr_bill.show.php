...
.bill_alert {
  ...
  <div class="bill_alert bill-alert-block mt10">
    ...
    <a class="wtbtn wtbtn-grapefruit mt5" onclick="ajax_get_confirm('一旦确认将无法恢复，系统将自动进入结算环节,<BR/>确认系统出账单计算无误吗?', 'index.php?w=store_vr_bill&t=confirm_bill&ob_id=<?php echo $_GET['ob_id'];?>');" href="javascript:void(0)">本期结算无误，我要确认</a>
    <?php } elseif ($output['bill_info']['ob_state'] == BILL_STATE_SUCCESS) {?>
    <a class="wtbtn mt5" target="_blank" href="index.php?w=store_vr_bill&t=bill_print&ob_id=<?php echo $_GET['ob_id'];?>">打印结算单</a>
    <?php } ?>
    </div>
    ...
    <ul>
      ...
      <li>本期应收：<?php echo wtPriceFormat($output['bill_info']['ob_result_totals']);?> = <?php echo wtPriceFormat($output['bill_info']['ob_order_totals']);?> (消费金额) - <?php echo wtPriceFormat($output['bill_info']['ob_commis_totals']);?> (佣金金额)</li>
