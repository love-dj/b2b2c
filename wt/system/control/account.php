...
class accountControl extends SystemControl{
    private $links = array(
        array('url'=>'w=account&t=qq','lang'=>'qqSettings'),
        array('url'=>'w=account&t=sina','lang'=>'sinaSettings'),
        array('url'=>'w=account&t=sms','text'=>'手机短信'),
        array('url'=>'w=account&t=wx','text'=>'微信登录'),
		//array('url'=>'w=account&t=uc','text'=>'UC互联')  //临时注释
    );
    ...
    public function qqWt(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result === true){
                    $this->log(L('wt_edit,qqSettings'),1);
                    showMessage(Language::get('wt_common_save_succ'));
                }else {
                    $this->log(L('wt_edit,qqSettings'),0);
    ...
    public function sinaWt(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $result = $model_setting->updateSetting($update_array);
                if ($result === true){
                    $this->log(L('wt_edit,sinaSettings'),1);
                    showMessage(Language::get('wt_common_save_succ'));
                }else {
                    $this->log(L('wt_edit,sinaSettings'),0);
                    showMessage(Language::get('wt_common_save_fail'));
                }
            }
        }
