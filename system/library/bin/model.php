        ...
        if(in_array(strtolower($method),array('table','order','where','on','limit','having','group','lock','master','distinct','index','attr','key'),true)) {
            ...
            if (strtolower($method) == 'table'){
            	if (strpos($args[0],',') !== false){
            		...
            		$this->table_name = $args[0];$this->fields = array();
            	}
            }
            return $this;
			...
			pagecmd('setEachNum',	$args[0]);
			$this->unoptions = false;
			pagecmd('setTotalNum',	$this->get_field('COUNT(*) AS wt_count'));
        	return $this;
        }elseif(in_array(strtolower($method),array('min','max','count','sum','avg'),true)){
            $field =  isset($args[0])?$args[0]:'*';
            return $this->get_field(strtoupper($method).'('.$field.') AS wt_'.$method);
        }elseif(strtolower($method)=='count1'){
            $field =  isset($args[0])?$args[0]:'*';
            $options['field'] = ('count('.$field.') AS wt_count');
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
                ...
                for($i=0;$i<$count;$i++) {
                    ...
                    if('exp'==strtolower($val[$i][0])) {
                        ...
                        $t = is_array($val[$i])?$this->comparison[strtolower($val[$i][0])]:'=';
                         if(preg_match('/IN/i',$t)){
                         	$whereStr .= '('.$key.' '.$t.' ('.$this->parseValue($data).')) '.$rule.' ';
                         }else{
                         	$whereStr .= '('.$key.' '.$t.' '.$this->parseValue($data).') '.$rule.' ';
                         }
