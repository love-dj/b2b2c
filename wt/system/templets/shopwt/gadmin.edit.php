...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?w=admin&t=gadmin" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['wt_limit_manage'];?> - <?php echo $lang['wt_edit'];?>权限组“<?php echo $output['ginfo']['gname'];?>”</h3>
        <h5><?php echo $lang['wt_limit_manage_subhead'];?></h5>
      </div>
    </div>
  </div>
  <!-- 操作说明 -->
  ...
  <form id="add_form" method="post" name="adminForm" style="margin-bottom: 50px;">
    ...
    <div class="wtap-form-all">
      ...
      <dl class="row">
        <dt class="tit">
          <span><input class="checkbox" type="checkbox" wttype="modulesAll">
          <?php echo $value['name'];?>模块功能</span></dt>
        <dd class="opt nobg nopd nobd nobs">
          <?php foreach ($value['child'] as $ke => $val) {?>
          <div class="wtap-account-container">
            <h4>
              <input class="checkbox" type="checkbox" wttype="groupAll">
      ...
      <?php } ?>
      <div class="fix-bot"><a href="JavaScript:void(0);" class="wtap-btn-big wtap-btn-green" onclick="document.adminForm.submit()"><?php echo $lang['wt_submit'];?></a></div>
    </div>
  </form>
</div>
<script>
	...
	$("#add_form").validate({
        ...
        rules : {
            gname : {
                required : true,
				remote	: {
                    url :'index.php?w=admin&t=ajax&branch=check_gadmin_name&gid=<?php echo $output['ginfo']['gid']?>',
                    type:'get',
                    data:{
                    	gname : function(){
                            return $('#gname').val();
        ...
        messages : {
            gname : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['wt_none_input'];?>',
                remote   : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['admin_add_admin_not_exists'];?>'
            }
        }
	});	
})
