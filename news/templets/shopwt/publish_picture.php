...
<script type="text/javascript" src="<?php echo STATIC_SITE_URL;?>/js/jquery.validation.min.js" charset="utf-8"></script>
...
$(document).ready(function(){
    ...
    //摘要计数
    $("[name='picture_image_abstract[]']").wt_text_count({max:140});
    ...
    $("#picture_image_upload").fileupload({
        dataType: 'json',
            url: "<?php echo NEWS_SITE_URL.DS.'index.php?w=publish&t=article_image_upload';?>",
            add: function(e,data) {
                ...
                if(result.status == "success") {
                    ...
                    var image_handle = '<a wttype="btn_set_picture_image" image_name="'+result.file_name+'"><?php echo $lang['news_cover'];?></a>';
                    image_handle += '<a wttype="btn_drop_picture_image" image_name="'+result.file_name+'"><?php echo $lang['wt_delete'];?></a>';
                    ...
                    var image_content = '<dd class="picture-abstract"><h4><?php echo $lang['news_image_abstract'];?></h4>';
                    ...
                    image_content += '<input class="btn-type-s" wttype="btn_add_image_goods" image_name="'+result.file_name+'" type="button" value="添加商品" />';
                    image_content += '</dd>';
                    $image_box.find("dl").append(image_content);

                    var image_goods_list = '<div class="goods-for-picture" wttype="image_goods_list"></div>';
                    $image_box.append(image_goods_list);

                    $image_box.find("textarea").wt_text_count({max:140});
    ...
    //设封面图
    $("[wttype='btn_set_picture_image']").live("click",function(){
    ...
    //图片删除
    $("[wttype='btn_drop_picture_image']").live("click",function(){
        var image_object = $(this).parents("li");
        var image_name = $(this).attr("image_name");
        $.getJSON("<?php echo NEWS_SITE_URL.DS.'index.php?w=publish&t=article_image_drop';?>", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
                    showError(result.error);
    ...
    //绑定商品
    $("[wttype='btn_add_image_goods']").live('click', function() {
        var add = '<div wttype="div_image_goods_add"><input class="text" style="width:235px;" wttype="image_goods_url" type="text" /><input class="btn-type-s" wttype="btn_goods_add_commit" type="button" image_name="'+$(this).attr("image_name")+'" value="<?php echo $lang['wt_confirm'];?>" /><input class="btn-type-s" wttype="btn_goods_add_cancel" type="button" value="<?php echo $lang['wt_cancel'];?>" /><div class="hint"><?php echo $lang['news_picture_image_tips'];?></div></div>'; 
        $(this).after(add);
        $(this).hide();
    });

    //绑定商品确认
    $("[wttype='btn_goods_add_commit']").live('click', function() {
        var btn = $(this);
        var goods_list = btn.parents("li").find("[wttype='image_goods_list']");
        if(goods_list.find("dl").length < 3) {
            var url = encodeURIComponent($(this).parent().find("[wttype='image_goods_url']").val());
            var image_name = $(this).attr("image_name");
            if(url != '') {
                $.getJSON("<?php echo NEWS_SITE_URL.DS.'index.php?w=api&t=goods_content_by_url';?>", { url: url}, function(data){
                    if(data.result == "true") {
                        ...
                        url_item += '<dd class="taobao-item-delete" wttype="btn_image_goods_delete"><?php echo $lang['news_delete'];?></dd>';
                        ...
                        btn.parent().parent().find("[wttype='btn_add_image_goods']").show();
                        btn.parent().remove();
                    } else {
                        alert(data.message);
                    }
    ...
    //绑定商品取消
    $("[wttype='btn_goods_add_cancel']").live('click', function() {
        $(this).parent().parent().find("[wttype='btn_add_image_goods']").show();
        $(this).parent().remove();
    });

    //绑定商品删除
    $("[wttype='btn_image_goods_delete']").live('click', function() {
        $(this).parent().remove();
    });

    //标签操作
    $("[wttype='news_tag']").live("click",function(){
        var current_css = $(this).attr("class");
        if(current_css == "btn-news-tag") {
            $(this).attr("class","btn-news-tag-selected");
        } else {
            $(this).attr("class","btn-news-tag");
        }
        var news_tag_selected = '';
        $(".btn-news-tag-selected").each(function(){
            news_tag_selected += $(this).attr("tag_id") + ",";
        });
        $("#picture_tag").val(news_tag_selected.substring(0, news_tag_selected.length-1));
    });

});
</script>
<div class="news-content mt10">
  <form id="add_form" method="post" action="index.php?w=publish&t=publish_picture_save">
    ...
    <div class="news-publish-content">
      <table class="news-publish-table news-publish-table-picture">
        <tbody>
          <tr>
            <th><?php echo $lang['news_article_class'];?><?php echo $lang['wt_colon'];?></th>
            ...
            <td rowspan="3" class="w200"><input id="picture_image" name="picture_image" type="hidden" value="" />
              <div class="edit-cover-pic">
                ...
                <?php if(!empty($output['picture_detail']['picture_image'])) { ?>
                 <img alt="" src="<?php echo getNEWSArticleImageUrl($output['picture_detail']['picture_attachment_path'],$output['picture_detail']['picture_image'], 'list');?>">
                 <?php } ?>
             </div>
             <p class="edit-cover-text"><?php echo $lang['news_cover'];?></p>
              </td>
          </tr>
          <tr>
            <th><?php echo $lang['news_title'];?><?php echo $lang['wt_colon'];?></th>
            <td><input id="picture_title" class="text w300" name="picture_title" type="text" value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_title'];?>" />
              <div class="hint"><?php echo $lang['news_title_explain'];?></div></td>
          </tr>
          <tr>
            <th><?php echo $lang['news_title_short'];?><?php echo $lang['wt_colon'];?></th>
            <td><input id="article_title_short" class="text w300" name="picture_title_short" type="text" value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_title_short'];?>" />
              <div class="hint"><?php echo $lang['news_title_short_explain'];?></div></td>
          </tr>
          <tr>
            <th><?php echo $lang['news_article_abstract'];?><?php echo $lang['wt_colon'];?></th>
            <td colspan="2"><div>
                ...
                <div class="hint"><?php echo $lang['news_article_abstract_explain'];?></div>
              </div></td>
          </tr>
          <tr>
            <th><?php echo $lang['news_image_upload'];?><?php echo $lang['wt_colon'];?></th>
            <td colspan="2">
            <div id="pic-upload-scrollbar"><ul class="picture-image-list">
                ...
                <li>
                  <dl class="picture-content">
                    <dt class="picture-img">
                      <div class="upload-thumb"><a href="Javascript: void(0);"><img wttype="picture_image_item" alt="" src="<?php echo getNEWSArticleImageUrl(empty($value['image_path'])?$output['picture_detail']['picture_attachment_path']:$value['image_path'], $value['image_name'], 'list');?>"></a></div>
                      <div class="handle"><a image_name="<?php echo $value['image_name'];?>" wttype="btn_set_picture_image"><?php echo $lang['news_cover'];?></a><a image_name="<?php echo $value['image_name'];?>" wttype="btn_drop_picture_image"><?php echo $lang['wt_delete'];?></a></div>
                    </dt>
                    <input type="hidden" value="<?php echo $value['image_name'].','.$value['image_path'];?>" name="picture_image_all[]">
                    <dd class="picture-abstract">
                      <h4><?php echo $lang['news_image_abstract'];?></h4>
                    ...
                    <dd class="picture-add-goods">
                      <input class="btn-type-s" wttype="btn_add_image_goods" image_name="<?php echo $value['image_name'];?>" type="button" value="<?php echo $lang['news_goods_add'];?>" />
                    </dd>
                  </dl>
                  <div class="goods-for-picture" wttype="image_goods_list">
                    ...
                    <dl class="taobao-item">
                      ...
                      <dd class="taobao-item-delete" wttype="btn_image_goods_delete" title="<?php echo $lang['wt_delete'];?>"><?php echo $lang['wt_delete'];?></dd>
              ...
              <div class="upload-btn"><a href="javascript:void(0);"><span>
                ...
                <div class="upload-button"><i></i><?php echo $lang['news_image_upload'];?></div>
                <input id="submit_button" style="display:none" type="button" value="&nbsp;" onClick="submit_form($(this))" />
                </a></div></td>
          </tr>
          <tr>
            <th><?php echo $lang['news_text_publisher'];?></th>
            <td colspan="2"><input id="picture_author" class="text w100" name="picture_author" type="text" value="<?php echo empty($output['picture_detail']['picture_author'])?$output['publisher_info']['name']:$output['picture_detail']['picture_author'];?>"/>
              <div class="hint"><?php echo $lang['news_publisher_explain'];?></div></td>
          </tr>
          <tr>
            <th><?php echo $lang['news_publish_time'];?></th>
            <td colspan="2"><input id="picture_publish_time" class="text w100" name="picture_publish_time" type="text" value="<?php echo empty($output['picture_detail'])?date('Y-m-d',time()):date('Y-m-d',$output['picture_detail']['picture_publish_time']);?>"/>
              <div class="hint"><?php echo $lang['news_publish_time_explain'];?></div></td>
          </tr>
          <tr>
            <th><?php echo $lang['news_tag'];?><?php echo $lang['wt_colon'];?></th>
            <td colspan="2"><?php if(!empty($output['tag_list']) && is_array($output['tag_list'])) {?>
              <ul>
                ...
                <li wttype="news_tag" class="<?php echo in_array($value['tag_id'],$tag_selected)?'btn-news-tag-selected':'btn-news-tag';?>" tag_id="<?php echo $value['tag_id'];?>"><?php echo $value['tag_name'];?></li>
                <?php } ?>
              </ul>
              <?php } ?>
          ...
          <tr>
            <th><?php echo $lang['news_keyword'];?><?php echo $lang['wt_colon'];?></th>
            <td colspan="2"><input id="picture_keyword" class="text w380" name="picture_keyword" type="text" value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_keyword'];?>"/><div class="hint"><?php echo $lang['news_keyword_expalin'];?></div></td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
            <td colspan="21"><a id="btn_draft" class="btn-draft" href="javascript:void(0);"><?php echo $lang['news_article_save_draft'];?></a> <a id="btn_publish" class="btn-publish" href="javascript:void(0);"><?php echo $lang['news_text_publish'];?></a></td>
          </tr>
        </tfoot>
      </table>
    </div>
