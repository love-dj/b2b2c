...
<table class="search-form">
  <tr>
    ...
    <td>
        ...
        <?php }?></span>
        <span wttype="gc2"></span>
        <span wttype="gc3"></span>
    </td>
    <td>&nbsp;</td>
  </tr>
</table>
<div wttype="class_spec" class="wtsc-goods-spec">
  <div wttype="spec_ul" class="spec-tabmenu"></div>
  <div wttype="spec_iframe" class="spec-iframe">
...
$(function() {
    ...
    $('select[wttype="gc"]').change(function(){
        ...
        $('div[wttype="spec_ul"]').html('');
        $('div[wttype="spec_iframe"]').html('');
        getClassSpec($(this));
    });
});
...
function getClassSpec($this) {
    ...
    $.getJSON('index.php?w=store_spec&t=ajax_class&id=' + id + '&deep=' + deep, function(data){
    	$('div[wttype="spec_iframe"]').empty();
        $('div[wttype="spec_ul"]').empty();
        if (data) {
            if (data.type == 'class') {
...
function specList(data, deep, gcid) {
    if (typeof(data) != 'undefined' && data != '') {
        var $_ul = $('<ul></ul>');
        $.each(data, function(i, n){
            $_ul.append('<li><a href="javascript:void(0);" wttype="editSpec" data-param="{spid:'+ n.sp_id +',gcid:' + gcid + '}">编辑' + n.sp_name + '规格</a></li>');
        });
        ...
        $('div[wttype="spec_ul"]').append($_ul);
    } else {
        $('div[wttype="spec_ul"]').append('<div class="warning-option"><i class="icon-warning-sign"></i><span>该分类不能添加规格</span></div>');
    }
    // 清理分类
    clearClassHtml(deep);
}
...
function editSpecValue(o) {
    ...
    $_iframe = $('<iframe id="iframepage" name="iframepage" frameBorder=0 scrolling=no width="100%" height="630px" src="<?php echo BASE_SITE_URL;?>/index.php?w=store_spec&t=add_spec&spid=' + data_str.spid + '&gcid=' + data_str.gcid + '" ></iframe>');
    $('div[wttype="spec_iframe"]').append($_iframe);
}
