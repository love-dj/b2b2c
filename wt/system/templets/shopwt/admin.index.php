...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?w=admin&t=get_admin_xml',
        colModel : [
            {display: '<?php echo $lang['wt_handle'];?>', name : 'operation', width : 150, sortable : false, align: 'center', className: 'handle'},
...
function fg_operation(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?w=admin&t=admin_add';
    }
}
function fg_operation_del(admin_id){
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        var _url = 'index.php?w=admin&t=admin_del&admin_id='+admin_id;
