...
class fx_goodsControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        $condition['is_fx'] = 1;
        $order = '';
        $param = array('goods_commonid', 'goods_name', 'goods_price', 'goods_state', 'goods_verify', 'goods_image', 'gc_name', 'store_name', 'is_own_shop', 'brand_name','fx_add_time','fx_commis_rate','sale_count','click_count');
        ...
        foreach ($goods_list as $value) {
            ...
            $operation .= "<a class='btn green' href='". urlShop('fx_goods', 'goods_detail', array('goods_id' => $value['goods_commonid'])) ."' target=\"_blank\"><i class='fa fa-list-alt'></i>查看</a>";
            $param['operation'] = $operation;
            $param['goods_commonid'] = $value['goods_commonid'];
            $param['goods_name'] = "<a href='". urlShop('fx_goods', 'goods_detail', array('goods_id' => $value['goods_commonid'])) ."' target=\"_blank\">".$value['goods_name']."</a>";
            ...
            $param['fx_add_time'] = date('Y-m-d', $value['fx_add_time']);
            $param['fx_commis_rate'] = $value['fx_commis_rate'].'%';
     ...
     * 分销详情
     */
    public function fx_infoWt(){}

    /**
     * 取消分销
     */
    public function del_fxWt(){
        ...
        $model_goods = Model('goods');
        $goods_content = $model_goods->getGoodsCommonInfoByID($goods_commonid);
        if(!empty($goods_content) && is_array($goods_content)){
            $model_fx_goods = Model('fx_goods');
            $stat = $model_fx_goods->delGoods(array('goods_commonid'=>$goods_commonid));
            if($stat){
                $data['state'] = true;
                exit(json_encode($data));
            }else{
    ...
    public function export_csvWt() {
        ...
        , 'gc_name', 'store_id', 'store_name', 'is_own_shop', 'brand_name','fx_add_time','fx_commis_rate','sale_count','click_count');
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=fx_goods&t=index');
				Tpl::setDirquna('fenxiao');
    ...
    private function createCsv($goods_list) {
	    ...
	    $csv->filename = $csv->charset('fx_goods_list',CHARSET).$_GET['curpage'] . '-'.date('Y-m-d');
	    $csv->export($data);
    }
}