(function($) {
	$.fn.wtUBB = function(options){
		...
		this.each(function() {
			...
			var $this = $(this);
			// 加粗
			$this.find('a[wttype="b"]').click(function(){
			...
			// 斜体
			$this.find('a[wttype="i"]').click(function(){
				UBBContent.inserEitherEndt({
					part_one : '[I]',
					part_two : '[/I]'
				});	
			...
			// 下划线
			$this.find('a[wttype="u"]').click(function(){
				UBBContent.inserEitherEndt({
					part_one : '[U]',
					part_two : '[/U]'
				});	
			...
			$this.find('a[wttype="chooseGoods"]').click(function(){
				// 已经插入的商品数量计算
				var count = 10 - $('.insert-goods > dl').length; // 0 已经插入的商品数量
				var _uri = BBS_SITE_URL+'/index.php?w=theme&t=choose_goods&c_id='+c_id+'&count='+count;
				CUR_DIALOG = ajax_form("choosegoods", '选择商品', _uri, 510);
			});
		
			$this.find('a[wttype="uploadImage"]').click(function(){
				var _uri = BBS_SITE_URL+'/index.php?w=theme&t=choose_image&c_id='+c_id;
				CUR_DIALOG = ajax_form("uploadimage", '选择图片', _uri, 480);
			});
			// insert affix
		    $this.find('a[wttype="affix_insert"]').click(function(){
				affixInsert($(this));
		    });
		    // delete affix
		    $this.find('a[wttype="affix_delete"]').click(function(){
				affixDelete($(this));
			});
		    // Administrator delete appendage affix
		    $this.find('a[wttype="maffix_delete"]').click(function(){
			...
			// 相册图片插入到话题
			$('a[wttype="imagealbum"]').die().live('click',function(){
				var data_str = $('.choose-image').find('a[wttype="chooseimage"][class="selected"]').attr('data-param'); eval('data_str = ' + data_str);
				if(typeof(data_str) == 'undefined') {return false;}
				insertImgUBB(data_str.img);
				DialogManager.close('uploadimage');
			});
			// 链接地址插入话题
			$('a[wttype="imageurl"]').die().live('click',function(){
				var img = $('input[wttype="imageurl"]').val();
				insertImgUBB(img);
				DialogManager.close('uploadimage');
			});

			// 插入到主题
			$('a[wttype="insertGoods"]').die().live('click',function(){
			...
			// Senior reply
			$this.find('a[wttype="highReply"]').click(function(){
				_uri = 'index.php?w=theme&t=reply&c_id='+c_id+'&t_id='+t_id;
		...
		function affixDelete(o){
			...
			$.getJSON(BBS_SITE_URL+'/index.php?w=theme&t=delimg&c_id='+c_id+'&id='+id, function(){
		...
		function affixDeleteManage(o){
			// 删除图片
			var id = o.attr('data-param');
			$.getJSON(BBS_SITE_URL+'/index.php?w=manage&t=delimg&c_id='+c_id+'&id='+id, function(){
				o.parents('li:first').remove();
				checkInsertAffix();
			});
		}
		...
		function updateAffixInsert(param){
			...
			.append('<div class="handle"><a href="javascript:void(0);" wttype="affix_insert" data-param="'+param.file_insert+'"><i class="c"></i>插入</a><a href="javascript:void(0);" wttype="affix_delete" data-param="'+param.file_id+'"><i class="d"></i>删除</a></div>')
			.find('a[wttype="affix_insert"]').click(function(){
				affixInsert($(this));
			}).end().find('a[wttype="affix_delete"]').click(function(){
				affixDelete($(this));
			});
		}
		// get unused affixes
		function getUnusedAffix(){
			...
			if(len == 0){
				$.getJSON('index.php?w=theme&t=unused_img&c_id='+c_id, function(data){	// 获取未使用附件
					if(data != null){
						$('div[wttype="affix"]').hide();
