...
<div class="wtsc-form-default">
  <div class="alert alert-block">
    <h4><?php echo $lang['wt_explain'].$lang['wt_colon'];?></h4>
    <ul>
  ...
  <form id="add_form" action="index.php?w=store_sale_mansong&t=mansong_save" method="post">
    <input id="level2_flag" type="hidden" value="false" />
    <input id="level3_flag" type="hidden" value="false" />
    <dl>
      <dt><i class="required">*</i><?php echo $lang['mansong_name'].$lang['wt_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['start_time'].$lang['wt_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt><i class="required">*</i><?php echo $lang['end_time'].$lang['wt_colon'];?></dt>
      <dd>
...
<?php } ?>
    ...
    <dl>
      <dt><i class="required">*</i>满即送规则<?php echo $lang['wt_colon'];?></dt>
      <dd>
        <input type="hidden" id="mansong_rule_count" name="rule_count">
        <ul id="mansong_rule_list" class="wtsc-mansong-rule-list">
        </ul>
        <a href="javascript:void(0);" id="btn_add_rule" class="wtbtn wtbtn-aqua"><i class="icon-plus-sign"></i>添加规则</a>
        <div id="div_add_rule" style="display:none;">
        <div class="wtsc-mansong-error"><span id="mansong_price_error" style="display:none;"><i class="icon-exclamation-sign"></i>规则金额不能为空且必须为数字</span><span id="mansong_discount_error" style="display:none;"><i class="icon-exclamation-sign"></i>满减金额必须小于规则金额</span></div>
        <div class="wtsc-mansong-rule">
        ...
        <span>送礼品&nbsp;<a href="javascript:void(0);" id="btn_show_search_goods" class="wtbtn"><i class="icon-gift"></i>选择礼品</a></span> <div id="mansong_goods_item" class="gift"></div>
        ...
        <div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
                    <table class="search-form">
                        <tr>
                            ...
                            <td class="w70 tc">
                                <a href="javascript:void(0);" id="btn_search_goods" class="wtbtn"/><i class="icon-search"></i><?php echo $lang['wt_search'];?></a>
                            </td>
                            <td class="w10"></td>
                            <td>
            ...
            <div class="mt10">
            <a href="javascript:void(0);" id="btn_save_rule" class="wtbtn wtbtn-aqua"><i class="icon-ok-bbs"></i>确定规则设置</a>
            <a href="javascript:void(0);" id="btn_cancel_add_rule" class="wtbtn wtbtn-bittersweet"><i class="icon-ban-bbs"></i>取消</a></div>
        </div>
        <span class="error-message"></span>
    ...
    <dl>
      <dt><?php echo $lang['text_remark'].$lang['wt_colon'];?></dt>
      <dd>
...
<a wttype="btn_del_mansong_rule" href="javascript:void(0);" class="wtbtn-mini wtbtn-grapefruit"><i class="icon-trash"></i>删除</a>
</li>
</script>
<script id="mansong_goods_template" type="text/html">
    <div wttype="mansong_goods" class="selected-mansong-goods">
    <a href="<%=goods_url%>" title="<%=goods_name%>" class="goods-thumb" target="_blank">
        <img src="<%=goods_image_url%>"/>
    </a>
    <input wttype="mansong_goods_id" type="hidden" value="<%=goods_id%>">
    </div><a wttype="btn_del_mansong_goods" href="javascript:void(0);" class="wtbtn-mini wtbtn-grapefruit"><i class="icon-trash"></i>删除已选择的礼品</a>
</script>
<script src="<?php echo STATIC_SITE_URL;?>/js/common.js"></script>
<script type="text/javascript" src="<?php echo STATIC_SITE_URL;?>/js/template.min.js"></script>
...
<?php } ?>
    ...
    $('#btn_save_rule').on('click', function() {
        ...
        mansong.goods = $('#mansong_goods_item').find('[wttype="mansong_goods"]').html();
        mansong.goods_id = Number($('#mansong_goods_item').find('[wttype="mansong_goods_id"]').val());
    ...
    // 删除已添加的规则
    $('#mansong_rule_list').on('click', '[wttype="btn_del_mansong_rule"]', function() {
        $(this).parents('[wttype="mansong_rule_item"]').remove();
        close_div_add_rule();
    });
    ...
    function close_div_add_rule() {
        var rule_count = $('#mansong_rule_list').find('[wttype="mansong_rule_item"]').length;
        if( rule_count >= 3) {
            $('#btn_add_rule').hide();
        } else {
            $('#btn_add_rule').show();
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlShop('store_sale_mansong', 'search_goods');?>";
    ...
    // 选择商品
    $('#div_goods_search_result').on('click', '[wttype="btn_add_mansong_goods"]', function() {
        var goods = {};
    ...
    // 删除以选的商品
    $('#mansong_goods_item').on('click', '[wttype="btn_del_mansong_goods"]', function() {
        $('#mansong_goods_item').html('');
    });
