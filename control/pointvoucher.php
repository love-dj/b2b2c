...
class pointvoucherControl extends BasePointShopControl {
     ...
     * 代金券列表
     */
    public function pointvoucherWt(){
        //查询会员及其附属信息
        parent::pointsMInfo();
        ...
        //分类导航
        $nav_link = array(
                0=>array('title'=>Language::get('homepage'),'link'=>BASE_SITE_URL),
     ...
     * 兑换代金券
     */
    public function voucherexchangeWt(){
        $vid = intval($_GET['vid']);
        if($vid <= 0){
        ...
        if($_SESSION['is_login'] != '1'){
            ...
            $js = "CUR_DIALOG = ajax_form('vexchange', '".L('home_voucher_exchangtitle')."', 'index.php?w=pointvoucher&t=voucherexchange&vid={$vid}', 550);";
            showDialog('','','js',$js);
            die;
        }
        $result = true;
     ...
     *
     */
    public function voucherexchange_saveWt(){
        if($_SESSION['is_login'] != '1'){
            $js = "login_dialog();";
            showDialog('','','js',$js);
        }
