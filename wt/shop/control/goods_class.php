...
class goods_classControl extends SystemControl{
    private $links = array(
        array('url'=>'w=goods_class&t=goods_class','lang'=>'wt_manage'),
        array('url'=>'w=goods_class&t=goods_class_import','lang'=>'goods_class_index_import'),
        array('url'=>'w=goods_class&t=tag','lang'=>'goods_class_index_tag')
    );
     ...
     * 商品分类添加
     */
    public function goods_class_addWt(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $insert_array['is_show']     = intval($_POST['is_show']);		//
                $result = $model_class->addGoodsClass($insert_array);
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?w=goods_class&t=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
                            'msg'=>$lang['goods_class_add_again'],
                        ),
                        array(
                            'url'=>'index.php?w=goods_class&t=goods_class',
                            'msg'=>$lang['goods_class_add_back_to_list'],
                        )
                    );
                    $this->log(L('wt_add,goods_class_index_class').'['.$_POST['gc_name'].']',1);
                    showMessage($lang['wt_common_save_succ'],$url);
                }else {
                    $this->log(L('wt_add,goods_class_index_class').'['.$_POST['gc_name'].']',0);
        ...
        if(is_array($type_list) && !empty($type_list)){
            foreach($type_list as $k=>$val){
                ...
                $t_list[$val['class_id']]['name'] = $val['class_name']==''?L('wt_default'):$val['class_name'];
            }
        }
        ksort($t_list);
     ...
     * 编辑
     */
    public function goods_class_editWt(){
        Tpl::output('show_type', $this->show_type);
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        ...
        if (chksubmit()){
            ...
            if (!$result){
                $this->log(L('wt_edit,goods_class_index_class').'['.$_POST['gc_name'].']',0);
            ...
            $url = array(
                array(
                    'url'=>'index.php?w=goods_class&t=goods_class_edit&gc_id='.intval($_POST['gc_id']),
            ...
            $this->log(L('wt_edit,goods_class_index_class').'['.$_POST['gc_name'].']',1);
        ...
        $this->links[] = array('url'=>'w=goods_class&t=goods_class_edit','lang'=>'wt_edit');
     ...
     * 分类导入
     */
    public function goods_class_importWt(){
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        //导入
        if (chksubmit()){
            ...
            if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
                ...
                showMessage($lang['wt_common_op_succ'],'index.php?w=goods_class&t=goods_class');
            }else {
     ...
     * 分类导出
     */
    public function goods_class_exportWt(){
            $model_class = Model('goods_class');
     ...
     * 删除分类
     */
    public function goods_class_delWt(){
        if ($_GET['id'] != ''){
            //删除分类
            Model('goods_class')->delGoodsClassByGcIdString($_GET['id']);
            $this->log(L('wt_delete,goods_class_index_class') . '[ID:' . $_GET['id'] . ']',1);
    ...
    public function tag_resetWt(){
        ...
        if(!$return){
            showMessage($lang['goods_class_reset_tag_fail'], 'index.php?w=goods_class&t=tag');
        }
        ...
        if(is_array($goods_class) and !empty($goods_class)) {
            ...
            if($return){
                $this->log(L('wt_reset').'tag',1);
                showMessage($lang['wt_common_op_succ'], 'index.php?w=goods_class&t=tag');
            }else{
                $this->log(L('wt_reset').'tag',0);
                showMessage($lang['wt_common_op_fail'], 'index.php?w=goods_class&t=tag');
            }
        }else{
            $this->log(L('wt_reset').'tag',0);
            showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?w=goods_class&t=tag');
        }
    }
    ...
    public function tag_updateWt(){
        ...
        if(is_array($tag_list) && !empty($tag_list)){
            foreach ($tag_list as $val){
                ...
                if(!$return){
                    $this->log(L('wt_update').'tag',0);
                    showMessage($lang['wt_common_op_fail'], 'index.php?w=goods_class&t=tag');
                }
            }
            $this->log(L('wt_update').'tag',1);
            showMessage($lang['wt_common_op_succ'], 'index.php?w=goods_class&t=tag');
        }else{
            $this->log(L('wt_update').'tag',0);
            showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?w=goods_class&t=tag');
        }

    }

    public function tag_editWt() {
        $model_class_tag = Model('goods_class_tag');
    ...
    public function nav_editWt() {
        ...
        if (chksubmit()) {
            ...
            $update['cn_show1_link'] = $_POST['cn_show1_link'];
            $update['cn_show2_link'] = $_POST['cn_show2_link'];
            ...
            if (!empty($_FILES['show1']['name'])) {//上传广告图片
                $upload = new UploadFile();
                @unlink(BASE_UPLOAD_PATH. '/' . ATTACH_GOODS_CLASS . '/' . $nav_info['cn_show1']);
                $upload->set('default_dir',ATTACH_GOODS_CLASS);
                $upload->upfile('show1');
                $update['cn_show1'] = $upload->file_name;
            }
            if (!empty($_FILES['show2']['name'])) {//上传广告图片
                $upload = new UploadFile();
                @unlink(BASE_UPLOAD_PATH. '/' . ATTACH_GOODS_CLASS . '/' . $nav_info['cn_show2']);
                $upload->set('default_dir',ATTACH_GOODS_CLASS);
                $upload->upfile('show2');
                $update['cn_show2'] = $upload->file_name;
            }
            if (empty($nav_info)) {
        ...
        $pic_name = BASE_UPLOAD_PATH . '/' . ATTACH_GOODS_CLASS . '/' . $nav_info['cn_show1'];
        if (file_exists($pic_name)) {
            $nav_info['cn_show1'] = UPLOAD_SITE_URL. '/' . ATTACH_GOODS_CLASS . '/' . $nav_info['cn_show1'];
        }
        $pic_name = BASE_UPLOAD_PATH . '/' . ATTACH_GOODS_CLASS . '/' . $nav_info['cn_show2'];
        if (file_exists($pic_name)) {
            $nav_info['cn_show2'] = UPLOAD_SITE_URL. '/' . ATTACH_GOODS_CLASS . '/' . $nav_info['cn_show2'];
        }
        ...
        if(is_array($brand_list) && !empty($brand_list)){
            foreach($brand_list as $k=>$val){
                ...
                $b_list[$val['class_id']]['name'] = $val['brand_class']==''?L('wt_default'):$val['brand_class'];
            }
        }
        ksort($b_list);
