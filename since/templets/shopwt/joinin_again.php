...
<style type="text/css">
body {
background: #39657a url(<?php echo DELIVERY_TEMPLATES_URL;
?>/images/login_bg.png) no-repeat center top;
}
</style>
<div class="wtd-joinin">
  <h1>物流自提自提点加盟-重新申请</h1>
  <form id="delivery_joinin" method="post" enctype="multipart/form-data" action="<?php echo DELIVERY_SITE_URL?>/index.php?w=joinin_again&t=edit_delivery">
    ...
    <dl class="r2">
      <dt>自提点信息提交：</dt>
      <dd>
        <label class="phrases">自提点名称</label>
        <input class="input-txt tip" type="text" name="daddressname" id="daddressname" value="<?php echo $output['dpoint_info']['dlyp_address_name']?>" title="自提点名称请使用您的实体店铺或经营场所、办公地点名称，例如“某某超市”、“某某小区物业”，便于收货人自提时使用；最多不超过15位字符。">
      </dd>
      <dd>
      ...
      <dd>
        ...
        <input class="input-txt tip w400" type="text" name="daddress" id="daddress" value="<?php echo $output['dpoint_info']['dlyp_address'];?>" title="自提点详细地址不需要重新填写省市区，必须大于5个字符，小于50个字符。">
        <span></span>
      </dd>
      <dd>
    ...
    $('#delivery_joinin').validate({
        ...
        rules : {
            dname : {
                ...
                rangelength : [3,15],
                lettersonly : true,
                remote   : 'index.php?w=joinin_again&t=check&did=<?php echo $output['dpoint_info']['dlyp_id'];?>'
            },
            dpasswd : {
                required : true,
                rangelength : [3,15]
