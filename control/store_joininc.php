...
class store_joinincControl extends BaseHomeControl {
    ...
    public function __construct() {
        ...
        if(!empty($seller_info)) {
            @header('location: index.php?w=seller_login');
        }

        if($_GET['t'] != 'check_seller_name_exist' && $_GET['t'] != 'checkname') {
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['t'], array('step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], BASE_SITE_URL.DS.'index.php?w=store_joininc&t=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['t'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], BASE_SITE_URL.DS.'index.php?w=store_joininc&t=pay');
                    }
                    break;
    ...
    public function step4Wt() {
        ...
        @header('location: index.php?w=store_joininc');
    ...
    public function pay_saveWt() {
        ...
        @header('location: index.php?w=store_joinin');
    }
