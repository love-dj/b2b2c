...
class messageControl extends SystemControl{
    private $links = array(
        array('url'=>'w=message&t=email','lang'=>'email_set'),
        array('url'=>'w=message&t=email_tpl','lang'=>'email_tpl')
    );
    ...
    public function emailWt(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('wt_edit,email_set'),1);
                showMessage(L('wt_common_save_succ'));
            }else {
                $this->log(L('wt_edit,email_set'),0);
     ...
     * 编辑邮件模板
     */
    public function email_tpl_editWt(){
        $model_templates = Model('mail_templates');
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $update_array['apicodeid'] = $_POST["apicodeid"];
                $result = $model_templates->editTpl($update_array,array('code'=>$_POST['code']));
                if ($result === true){
                    $this->log(L('wt_edit,email_tpl'),1);
                    showMessage(L('mailtemplates_edit_succ'),'index.php?w=message&t=email_tpl');
                }else {
                    $this->log(L('wt_edit,email_tpl'),0);
   ...
   /**
	 ...
	 * @return
	 */
	public function email_testingWt(){
		/**
		 * 读取语言包
		 */
