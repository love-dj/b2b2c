...
class member_mallconsultControl extends BaseMemberControl {
     ...
     * 平台咨询详细
     */
    public function mallconsult_infoWt() {
        $id = intval($_GET['id']);
        if ($id <= 0) {
     ...
     * 添加平台客服咨询
     */
    public function add_mallconsultWt() {
        // 咨询类型列表
     ...
     * 保存平台咨询
     */
    public function save_mallconsultWt() {
        if (!chksubmit()) {
     ...
     * 删除平台客服咨询
     */
    public function del_mallconsultWt() {
        $id = intval($_GET['id']);
        if ($id <= 0) {
        ...
        if ($result) {
            ...
            showDialog(L('wt_common_del_fail'));
        }
    }
