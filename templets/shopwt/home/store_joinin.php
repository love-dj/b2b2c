  ...
  <div class="user-box">
    ...
    <div class="user-joinin">
      ...
      <dl>
		...
		<dd> 若您还没有填写入驻申请资料<br/>
          请点击“<a href="<?php if(!($output['store_zizhi'])){?><?php echo urlShop('store_joinin', 'step0');?><?php }else { ?><?php echo urlShop('store_joininc', 'step0');?><?php } ?>" target="_blank">我要入驻</a>”进行入驻资料填写</dd>
        <dd>若您的店铺还未开通<br/>
          请通过“<a href="<?php if(!($output['store_zizhi'])){?><?php echo urlShop('store_joinin', 'index');?><?php }else { ?><?php echo urlShop('store_joininc', 'index');?><?php } ?>" target="_blank">查看入驻进度</a>”了解店铺开通的最新状况 </dd>
      </dl>
      <div class="bottom"><a href="<?php if(!($output['store_zizhi'])){?><?php echo urlShop('store_joinin', 'step0');?><?php }else { ?><?php echo urlShop('store_joininc', 'index');?><?php } ?>" target="_blank">查看入驻进度</a></div>
    </div>
     <?php }else { ?>
      ...
      <form id="login_form" action="<?php echo urlLogin('login')?>" method="post">
        ...
        <dl>
          ...
          <dd>
            ...
            <a href="JavaScript:void(0);" onclick="javascript:document.getElementById('codeimage').src='index.php?w=vercode&type=30x92&c=' + Math.random();" class="change" title="<?php echo $lang['login_index_change_checkcode'];?>">
            <img src="index.php?w=vercode" class="fl ml5" name="codeimage" id="codeimage" border="0"/></a>
            <label></label>
          </dd>
        </dl>
        <?php } ?>
        <dl>
          <dt></dt>
          <dd>
            <input type="hidden" value="<?php echo BASE_SITE_URL?>/index.php?w=show_joinin" name="ref_url">
            <input name="提交" type="submit" class="button" value="登&nbsp;&nbsp;录">
            </dd>
        </dl>
      </form>
      <div class="register">还没有帐号？ <a href="<?php echo MEMBER_SITE_URL;?>/index.php?w=login&t=register&ref_url=<?php echo BASE_SITE_URL;?>/index.php?w=show_joinin" target="_blank">免费注册</a></div>
    </div>
    <?php } ?>
  </div>
...
<div class="mains">
<div class="indextip">
    <h3>加入<?php echo C('site_name');?>，让所有消费者关注你</h3>
    <span class="content"><?php echo $output['show_txt'];?></span>
</div>
...
<?php } ?>
<script type="text/javascript" src="<?php echo STATIC_SITE_URL;?>/js/index_introduce.js" charset="utf-8"></script>
