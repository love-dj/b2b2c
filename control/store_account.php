...
class store_accountControl extends BaseSellerControl {
    ...
    public function account_saveWt() {
        ...
        if($result) {
            ...
            showDialog(Language::get('wt_common_op_succ'), urlShop('store_account', 'account_list'), 'succ');
        } else {
            $this->recordSellerLog('添加账号失败');
            showDialog(Language::get('wt_common_save_fail'), urlShop('store_account', 'account_list'), 'error');
        }
    }
