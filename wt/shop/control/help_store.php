...
class help_storeControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ($help_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href=\"javascript:void(0);\" onclick=\"fg_del('".$value['help_id']."')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?w=help_store&t=edit_help&help_id=".$value['help_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
     ...
     * 输出XML数据
     */
    public function get_type_xmlWt() {
        $model_help = Model('help');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
        ...
        foreach ($type_list as $value) {
            ...
            $operation .= "<a class='btn blue' href='index.php?w=help_store&t=edit_type&type_id=".$value['type_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function add_helpWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=help_store&t=help_store');
            } else {
    ...
    public function add_typeWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=help_store&t=help_type');
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
