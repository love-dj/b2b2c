...
class refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'w=refund','text'=>'待处理'),
            array('url'=>'w=refund&t=refund_all','text'=>'所有记录'),
            array('url'=>'w=refund&t=reason','text'=>'退款退货原因')
    );
     ...
     * 待处理列表
     */
    public function get_manage_xmlWt() {
        $model_refund = Model('refund_return');
        $condition = array();
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?w=refund&t=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function editWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
				...
				$wx_info = array();
				$wx_info['order_sn'] = $refund['order_sn'];
				$wx_info['msg'] = $refund['reason_info'];
				$wx_info['goods_name'] = $refund['goods_name'];
				$wx_info['refund_amount'] = $refund['refund_amount'];
				$param['param']['mp_array'] = Handle('wx_api')->getTemplateData($param['code'], $wx_info);
                QueueClient::push('sendMemberMsg', $param);

                $this->log('退款确认，退款编号'.$refund['refund_sn']);
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=refund&t=index');
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
    ...
    public function add_reasonWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),'index.php?w=refund&t=reason');
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
    ...
    public function del_reasonWt() {
        ...
        if ($state) {
            ...
            showMessage(Language::get('wt_common_del_succ'),'index.php?w=refund&t=reason');
        } else {
    ...
    private function createCsv($refund_list) {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            $list[$k]['refund_amount'] = wtPriceFormat($refund_info['refund_amount']);
            if(!empty($refund_info['pic_info'])) {
                $info = unserialize($refund_info['pic_info']);
                if (is_array($info) && !empty($info['buyer'])) {
                    foreach($info['buyer'] as $pic_name) {
