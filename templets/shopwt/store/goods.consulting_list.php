    ...
    <div class="wts-goods-main">
      ...
      <div class="wts-goods-content-content bd" id="wtGoodsRate">
        <div class="top" style="overflow: hidden;">
          <div class="wts-cosult-tips"> <i></i>
            <p><?php echo html_entity_decode(C('consult_prompt'));?></p>
          </div>
          <div class="wts-cosult-askbtn"><a href="#askQuestion" class="wtbtn wtbtn-grapefruit">我要提问</a></div>
        </div>
        ...
        <div class="wts-commend-main">
          <?php if(!empty($output['consult_list'])) { ?>
          <?php foreach($output['consult_list'] as $k=>$v){ ?>
          <div class="wts-cosult-list">
            <dl class="asker">
              <dt>咨询网友<?php echo $lang['wt_colon'];?></dt>
              <dd>
                <?php if($v['member_id']== '0') echo $lang['wt_guest']; else if($v['isanonymous'] == 1){?>
                <?php echo str_cut($v['member_name'],2).'***';?>
                <?php }else{?>
                <a href="index.php?w=member_snshome&mid=<?php echo $v['member_id'];?>" target="_blank" data-param="{'id':<?php echo $v['member_id'];?>}" wttype="mcard"><?php echo str_cut($v['member_name'],8);?></a>
                <?php }?>
            ...
            <dl class="ask-con">
              <dt><?php echo $lang['goods_index_consult_content'];?><?php echo $lang['wt_colon'];?></dt>
              <dd>
          ...
          <?php } else { ?>
          <div class="wts-norecord"><?php echo $lang['goods_index_no_reply'];?></div>
          <?php } ?>
        </div>
      </div>
      <?php if($output['consult_able']) { ?>
      <!-- S 咨询表单部分 -->
      <div class="wts-goods-title-bar" id="askQuestion">
        <h4>发表咨询</h4>
      </div>
      <form method="post" id="message" action="index.php?w=goods&t=save_consult">
        ...
        <div class="wts-consult-form">
          ...
          <dl>
            ...
            <dd>
              ...
              <label>
                <input type="radio" <?php if ($checked) {?>checked="checked"<?php }?> wttype="ctype<?php echo $val['ct_id'];?>" name="consult_type_id" class="radio" value="<?php echo $val['ct_id'];?>" />
                <?php echo $val['ct_name'];?></label>
              <?php $checked = false;}?>
            </dd>
          </dl>
          <?php $checked = true;foreach ($output['consult_type'] as $val) {?>
          <div class="wts-consult-type-intro" <?php if (!$checked) {?>style="display:none;"<?php }?> wttype="ctype<?php echo $val['ct_id'];?>"> <?php echo $val['ct_introduce'];?> </div>
          <?php $checked = false;}?>
          <?php }?>
          <div>
            <?php if($_SESSION['member_id']){ ?>
            <label><strong><?php echo $lang['goods_index_member_name'].$lang['wt_colon'];?></strong><?php echo $_SESSION['member_name'];?></label>
          ...
          <dl>
            ...
            <dd>
              ...
              <div class="code">
                <div class="arrow"></div>
                <div class="code-img"><a href="javascript:void(0)" onclick="javascript:document.getElementById('codeimage').src='index.php?w=vercode&type=30x92&c=' + Math.random();"><img src="index.php?w=vercode" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?w=vercode&type=30x92&c=' + Math.random()"/></a></div>
                <a href="JavaScript:void(0);" id="hide" class="close" title="关闭"><i></i></a> <a href="JavaScript:void(0);" onclick="javascript:document.getElementById('codeimage').src='index.php?w=vercode&type=30x92&c=' + Math.random();" class="change" title="<?php echo $lang['goods_index_change_checkcode'];?>"><i></i></a> </div>
              <?php } ?>
              <a href="JavaScript:void(0);" wttype="consult_submit" title="<?php echo $lang['goods_index_publish_consult'];?>" class="wtbtn wtbtn-grapefruit"><?php echo $lang['goods_index_publish_consult'];?></a></dd>
            <dd wttype="error_msg"></dd>
          </dl>
        </div>
      </form>
      <!-- E 咨询表单部分 -->
      ...
      <div class="wts-sidebar-container">
        ...
        <div class="content">
          <dl class="wts-comment-goods">
            ...
            <dd class="goods-price"><?php echo $lang['goods_index_goods_price'].$lang['wt_colon'];?><em class="saleP"><?php echo $lang['currency'].wtPriceFormat($output['goods']['goods_sale_price']);?></em></dd>
            <dd class="goods-raty"><?php echo $lang['goods_index_evaluation'].$lang['wt_colon'];?> <span class="raty" data-score="<?php echo $output['goods']['evaluation_good_star'];?>"></span> </dd>
          </dl>
        </div>
      </div>
...
$(function(){
    ...
    <?php if($output['consult_able']) { ?>
    $('a[wttype="consult_submit"]').click(function(){
    	$('#message').submit();
    });

    $('#message').validate({
    	errorPlacement: function(error, element){
    		$('dd[wttype="error_msg"]').append(error);
        },
    ...
    $('input[type="radio"]').click(function(){
        $('div[wttype^="ctype"]').hide();
        $('div[wttype="' + $(this).attr('wttype') + '"]').show();
    });
