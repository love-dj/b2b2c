...
class searchControl extends BasefenxiaoControl
{
...
print_r($checked_brand);
        ...
        // 当前位置导航
        $nav_link_list = $model_goods_class->getGoodsClassNavsearch_FenXiao(intval($_GET['cate_id']));
    ...
    public function fx_addWt()
    {
        ...
        if ($_SESSION['member_id'] > 0) {
            $member_info = Model('member')->getMemberInfoByID($_SESSION['member_id'], 'fx_state');
            if ($member_info['fx_state'] == 2) {
                ...
                if (intval($goods_commonid) > 0 && !empty($goods_common)) {
                    ...
                    $fx_goods = $model_dis->getFenxiaoGoodsInfo(array('member_id' => $_SESSION['member_id'], 'goods_commonid' => $goods_commonid,'fx_goods_state' => 1));
                ...
                $data['msg'] = '请先认证成为分销员';
                $data['url'] = urlFenxiao('fx_join', 'index');
            }
        } else {
            $data['msg'] = "请先登录";
