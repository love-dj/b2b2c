...
function update_flex(){
	$('.wtap-stat-general-single').load('index.php?w=stat_trade&t=get_plat_sale&'+$("#formSearch").serialize(),
		function(){
			$('.timer').each(count);
     	});
    $("#flexigrid").flexigrid({
        url: 'index.php?w=stat_trade&t=get_sale_xml&'+$("#formSearch").serialize(),
        colModel : [
...
function getStatdata(type){
	$('#'+type+'_div').load('index.php?w=stat_trade&t=sale_trend&type='+type+'&'+$("#formSearch").serialize());
}
function fg_operation(name, bDiv){
    var stat_url = 'index.php?w=stat_trade&t=sale';
