$(function(){
	...
	var predepoit_=0;
	//获取三级分销余额
	$.getJSON(ApiUrl + '/index.php?w=member_distribution&t=my_asset', {'key':key}, function(result){
		
		$("#allmoney").html(result.datas.trad_amount);
		predepoit_=result.datas.trad_amount;
	});	
	...
	$('#btn').click(function(){
		...
		if($.sValid()){
	          $.ajax({
				type:'post',
				url:ApiUrl+"/index.php?w=member_distribution&t=pd_cash_add",	
