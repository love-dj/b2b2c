    ...
    <div class="wtap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
          ...
          <span class="err"></span>
          <p class="notic">请输入已注册会员名称，系统会检测是否有该会员</p>
        </dd>
      </dl>
...
function checkmember(){
	...
	$.getJSON("index.php?w=member&t=checkmember", {'name':membername}, function(data){
	        if (data)
	        {
		        $("#tr_memberinfo").show();
...
$(function(){
    ...
    $('#points_form').validate({
        ...
        messages : {
			member_name: {
				required : '<i class="fa fa-exclamation-bbs"></i>请输入会员名'
			},
			member_id : {
				required : '<i class="fa fa-exclamation-bbs"></i>会员信息错误，请重新填写会员名'
            },
            pointsnum  : {
                required : '<i class="fa fa-exclamation-bbs"></i>请添加预存款',
                min : '<i class="fa fa-exclamation-bbs"></i>预存款必须大于0'
            }
        }
    });
});
