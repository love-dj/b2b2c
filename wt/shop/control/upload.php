...
class uploadControl extends SystemControl{
    private $links = array(
        array('url'=>'w=upload&t=param','lang'=>'upload_param'),
        array('url'=>'w=upload&t=default_thumb','lang'=>'default_thumb'),
        array('url'=>'w=upload&t=font','lang'=>'font_set')
    );
     ...
     * 默认图设置
     */
    public function default_thumbWt(){
        $model_setting = Model('setting');
        if (chksubmit()){
            //上传图片
            ...
            if ($result === true){
                ...
                $this->log(L('wt_edit,default_thumb'),1);
                showMessage(L('wt_common_save_succ'));
            }else {
                $this->log(L('wt_edit,default_thumb'),0);
                showMessage(L('wt_common_save_fail'));
            }
        }
