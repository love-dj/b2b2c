...
class favoritesModel extends Model{
    ...
    public function addFavorites($param) {
        ...
        if ($param['fav_type'] == 'goods') {
            ...
            $fields = 'goods_id,store_id,goods_name,goods_image,goods_price,goods_sale_price';
            $goods = $model_goods->getGoodsInfoByID($goods_id,$fields);
            $param['goods_name'] = $goods['goods_name'];
            $param['goods_image'] = $goods['goods_image'];
            $param['log_price'] = $goods['goods_sale_price'];//商品收藏时价格
            $param['log_msg'] = $goods['goods_sale_price'];//收藏备注，默认为收藏时价格，可修改
