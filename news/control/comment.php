...
class commentControl extends NEWSHomeControl{

    public function __construct() {
        parent::__construct();
        //是否关闭投稿功能
        if(intval(C('news_comment_flag')) !== 1) {
    ...
    public function comment_saveWt() {
        ...
        if(!empty($_SESSION['member_id'])) {
            ...
            if($result) {
                ...
                $data['member_link'] = SITEURL.DS.'index.php?w=member_snshome&mid='.$_SESSION['member_id'];
    ...
    public function comment_listWt() {
        ...
        if($comment_object_id > 0 && $comment_type > 0) {
            ...
            if($_GET['comment_all'] === 'all') {
                Tpl::output('show_page', $model_news_comment->showpage(2));
            }
            ...
            if(!empty($comment_quote_id)) {
                $comment_quote_list = $model_news_comment->getListWithUserInfo(array('comment_id'=>array('in', $comment_quote_id)));
            }
