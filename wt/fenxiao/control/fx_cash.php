...
class fx_cashControl extends SystemControl{
    ...
    public function cash_listWt(){
        ...
        Tpl::showpage('fx_cash.list');
    }

    /**
     * 删除提现记录
     */
    public function cash_delWt(){
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_trad = Model('fx_trad');
            $condition = array();
            $condition['tradc_id'] = $id;
            $condition['tradc_payment_state'] = 0;
            $info = $model_trad->getFenxiaoTradCashInfo($condition);
            if (!$info) {
                exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
            }
            try {
                $model_trad->beginTransaction();
                $result = $model_trad->delFenxiaoTradCash($condition);
                if (!$result) {
     ...
     * 更改提现为支付状态
     */
    public function cash_payWt(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage('参数错误','index.php?w=fx_cash&t=cash_list','','error');
        }
        $model_trad = Model('fx_trad');
        $condition = array();
        $condition['tradc_id'] = $id;
        ...
        if (!is_array($info) || count($info)<0){
            showMessage('记录不存在或已付款','index.php?w=fx_cash&t=cash_list','','error');
        }
        ...
        try {
            $model_trad->beginTransaction();
            $result = $model_trad->updateFenxiaoTradCash($update,$condition);
            if (!$result) {
            ...
            $model_trad->commit();
            $this->log($log_msg,1);
            showMessage('付款成功','index.php?w=fx_cash&t=cash_list');
        } catch (Exception $e) {
            $model_trad->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),'index.php?w=fx_cash&t=cash_list','html','error');
        }
    }
    ...
    public function cash_viewWt(){
		...
		Tpl::setDirquna('fenxiao');
        Tpl::showpage('fx_cash.view', 'null_layout');
    }
    ...
    public function export_cash_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            $count = $model_trad->getFenxiaoCashCount($condition);
            $array = array();
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=fx_cash&t=cash_list');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
                $data = $model_trad->getFenxiaoTradCashList($condition,'*','',$order,self::EXPORT_SIZE);
            ...
            $data = $model_trad->getFenxiaoTradCashList($condition,'*','',$order,"{$limit1},{$limit2}");
            $cashpaystate = array(0=>'未支付',1=>'已支付');
            foreach ($data as $k=>$v) {
                $data[$k]['tradc_payment_state'] = $cashpaystate[$v['tradc_payment_state']];
            }
    ...
    private function createCashExcel($data = array()){
        ...
        foreach ((array)$data as $k=>$v){
            ...
            $tmp[] = array('format'=>'Number','data'=>wtPriceFormat($v['tradc_amount']));
    ...
    public function get_xmlWt(){
        ...
        $page = $_POST['rp'];
        $cash_list = $model_trad->getFenxiaoTradCashList($condition,'*',$page,$order);
        $data = array();
        ...
        foreach ($cash_list as $value) {
            ...
            $param['operation'] .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('cash_info','查看提现编号“". $value['tradc_sn'] ."”的明细', 'index.php?w=fx_cash&t=cash_view&id=". $value['tradc_id'] ."', 640)\" ><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['tradc_amount'] = wtPriceFormat($value['tradc_amount']);
