...
class publishControl extends NEWSMemberControl{
    ...
    public function indexWt() {
        $this->publish_articleWt();
    }

    /**
     * 文章发布
     */
    public function publish_articleWt(){
        ...
        Tpl::showpage('publish_article','news_member_layout');
    }

    /**
     * 文章发布
     */
    public function publish_article_saveWt(){
        ...
        if(!empty($_POST['article_image_all'])) {
            ...
            foreach ($_POST['article_image_all'] as $value) {
                ...
                $article_image_url = getNEWSArticleImageUrl($file_path, $file_name, 'max');
        ...
        $model_tag_relation = Model('news_tag_relation');
        if(!empty($_POST['article_id'])) {
        ...
        if($article_id) {
            showMessage(Language::get('wt_common_save_succ'),self::NEWS_MEMBER_ARTICLE_URL);
        } else {
            showMessage(Language::get('wt_common_save_fail'),self::NEWS_MEMBER_ARTICLE_URL,'','error');
        }
    }
     ...
     * 图刊发布
     */
    public function publish_pictureWt(){
     ...
     * 图刊保存
     */
    public function publish_picture_saveWt() {
        ...
        if(!empty($_POST['picture_image_all'])) {
            ...
            for ($i = 0; $i < $image_count; $i++) {
                ...
                $picture_image_url = getNEWSArticleImageUrl($file_path, $file_name, 'max');
        ...
        if($picture_id) {
            showMessage(Language::get('wt_common_save_succ'),self::NEWS_MEMBER_PICTURE_URL);
        } else {
            showMessage(Language::get('wt_common_save_fail'),self::NEWS_MEMBER_PICTURE_URL,'','error');
        }
     ...
     * 文章图片上传
     */
    public function article_image_uploadWt() {
        $data = array();
        $data['status'] = 'success';
        if(!empty($this->publisher_name)) {
            if(!empty($_FILES['article_image_upload']['name'])) {
                $upload = new UploadFile();
                $upload->set('default_dir',ATTACH_NEWS.DS.'article'.DS.$this->attachment_path);
                ...
                $data['file_url'] = getNEWSArticleImageUrl($this->attachment_path, $upload->file_name, 'max');
     ...
     * 文章图片删除
     */
    public function article_image_dropWt() {
        $data = array();
        $data['status'] = 'success';
