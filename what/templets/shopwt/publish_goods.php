...
$(document).ready(function(){
    ...
    $(".btn_commend_dialog").click(function(){
        ...
        $("#div_commend_goods").what_form_show({width:480});
    });

    $("#div_commend_goods").what_form({title:'<?php echo $lang['what_text_commend'].$lang['what_text_goods'];?>'});
...
<div class="all-box-box">
  ...
  <h1><?php echo $lang['what_goods_buy'];?></h1>
  <?php } else { ?>
  <h1><?php echo $lang['what_goods_favorite'];?></h1>
  <?php } ?>
  ...
  <div class="publish-goods-list">
    <ul>
      ...
      <li class="<?php echo in_array($val['goods_id'],$output['commend_goods_array'])?'selected':'select' ?>">
        ...
        <dl>
          <dd class="fl"><?php echo $lang['currency'].wtPriceFormat($val['goods_price']);?></dd>
        </dl>
        <div class="recommand-btn">
          <?php if(in_array($val['goods_id'],$output['commend_goods_array'])) { ?>
          <a href="javascript:void(0)"><?php echo $lang['what_goods_commend_already'];?></a>
          <?php } else { ?>
          <a href="javascript:void(0)" class="btn_commend_dialog" goods_id="<?php echo $val['goods_id'];?>" goods_name="<?php echo $val['goods_name'];?>" goods_price="<?php echo $lang['currency'].$val['goods_price'];?>" goods_image="<?php echo thumb($val, 240);?>"><?php echo $lang['what_goods_commend_want'];?></a>
          <?php } ?>
        </div>
      </li>
      <?php } ?>
...
<form action="<?php echo WHAT_SITE_URL;?>/index.php?w=publish&t=goods_save" method="post" id="add_form" class="feededitor">
  ...
  <dl class="share">
    <dt><?php echo $lang['what_goods_publish_tile'];?></dt>
    <dd></dd>
...
<div class="no-content"> <i class="buy pngFix">&nbsp;</i>
  <?php if($output['goods_type'] == 'buy') { ?>
  <p><?php echo $lang['what_goods_buy_none'];?></p>
  <?php } else { ?>
  <p><?php echo $lang['what_goods_favorite_none'];?></p>
  <?php } ?>
</div>
<?php } ?>
