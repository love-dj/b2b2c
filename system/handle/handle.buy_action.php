...
class buy_actionHandle {
    ...
    public function getGoodsCartList($cart_list, \StdClass $jjgObj = null) {
        ...
        //抢购
        $this->getRobbuyCartList($cart_list);
    ...
    public function getGoodsOnlineInfo($goods_id,$quantity) {
        $goods_content = $this->_getGoodsOnlineInfo($goods_id,$quantity);

        //预定商品优先
        $this->getBookInfo($goods_content);

        if (!$goods_content['is_book']) {

            //手机专享
            $this->getMbSoleInfo($goods_content);

            //抢购
            $this->getRobbuyInfo($goods_content);

            //限时折扣
            $this->getXianshiInfo($goods_content,$goods_content['goods_num']);

        }

        //赠品
        $this->_getGoodsGiftList($goods_content);
    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
                $store_cart[$key]['goods_total'] = wtPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
            ...
            $store_goods_total[$store_id] = wtPriceFormat($tmp_amount);
        }
        ...
        if (array_key_exists('store_id', $goods_list)) {
            if (array_key_exists($goods_list['store_id'], $store_order_discount)) {
                ...
                if (!$goods_list['is_book']) {
                    $goods_list['goods_price'] = wtPriceFormat($goods_list['goods_price'] * $store_order_discount[$goods_list['store_id']]/100);
                    $goods_list['zkj'] = true;                     
                }
            }
        } else {
            ...
            foreach ($goods_list as $k => $goods_content) {
                if (array_key_exists($goods_content['store_id'], $store_order_discount)) {
                    $orderdiscount = $store_order_discount[$goods_content['store_id']];
                    if (is_array($goods_content['bl_goods_list']) && !empty($goods_content['bl_goods_list'])) {
                        foreach ($goods_content['bl_goods_list'] as $kk => $vv) {
                            $goods_list[$k]['bl_goods_list'][$kk]['bl_goods_price'] = wtPriceFormat($vv['bl_goods_price'] * $orderdiscount/100);
                            $goods_list[$k]['bl_goods_list'][$kk]['zkj'] = true;
                        }
                    }
                    $goods_list[$k]['goods_price'] = wtPriceFormat($goods_content['goods_price'] * $orderdiscount/100);
    ...
    public function getMansongRuleCartListByTotal($store_goods_total) {
        if (!C('sale_allow') || empty($store_goods_total) || !is_array($store_goods_total)) return array(array(),array());
        ...
        foreach ($store_goods_total as $store_id => $goods_total) {
            ...
            if (is_array($rule_info) && !empty($rule_info)) {
                ...
                $rule_info['discount'] = wtPriceFormat($rule_info['discount']);
    ...
    public function reCalcGoodsTotal($store_goods_total, $preferential_array = array(), $preferential_type) {
        ...
        switch ($preferential_type) {
            case 'mansong':
                if (!C('sale_allow')) return $store_goods_total;
            ...
            case 'rpt':
                foreach ($store_goods_total as $store_id => $goods_total) {
                    if (array_key_exists($store_id, $preferential_array)) {
                        $store_goods_total[$store_id] = wtPriceFormat($store_goods_total[$store_id]*$preferential_array[$store_id]/100);
                    }
                }
                break;
    ...
    public function getStoreAvailableRptList($member_id,$goods_total = 0,$order = '') {
        if (!C('coupon_allow')) return array();
        $condition = array();
        $condition['coupon_owner_id'] = $member_id;
        return Model('coupon')->getCurrentAvailableRpt($condition,$goods_total,$order);
    }
    ...
    public function reParseRptInfo($input_rpt_info,$order_total,$member_id) {
        ...
        $condition = array();
        $condition['coupon_owner_id'] = $member_id;
        $condition['coupon_t_id'] = $input_rpt_info['coupon_t_id'];
        $info = Model('coupon')->getCurrentAvailableRpt($condition,$order_total);
        if ($info) {
            return $info[$input_rpt_info['coupon_t_id']];
        } else {
            return array();
        }
    }
     ...
     * @param number $rpt_total 优惠券金额
     * @return array array(每个订单减去优惠券后的总金额,每个订单使用的优惠券值)
     */
    ...
    /**
     * 将店铺优惠券减去运费的余额追加到店铺总优惠里
     * @param unknown $store_sale_total
     * @param unknown $store_freight_total
     * @param unknown $store_rpt_total
     */
    public function reCalcStoreSaleTotal($store_sale_total,$store_freight_total,$store_rpt_total) {
        if (!is_array($store_rpt_total) || empty($store_rpt_total)) return $store_sale_total;
        foreach ($store_rpt_total as $store_id => $rpt_total) {
            $ptotal = $rpt_total - $store_freight_total[$store_id];
            if ($ptotal > 0) {
                $store_sale_total[$store_id] += $ptotal;
            }
        }
        return $store_sale_total;
    }
     ...
     */
    public function getXianshiInfo( & $goods_content, $quantity) {
        if (empty($quantity)) $quantity = 1;
        if (!C('sale_allow') || empty($goods_content['xianshi_info']) || !empty($goods_content['robbuy_info']) || $goods_content['ifsole']) return ;
        $goods_content['xianshi_info']['down_price'] = wtPriceFormat($goods_content['goods_price'] - $goods_content['xianshi_info']['xianshi_price']);
        if ($quantity >= $goods_content['xianshi_info']['lower_limit']) {
            $goods_content['goods_yprice'] = $goods_content['goods_price'];
            $goods_content['goods_price'] = $goods_content['xianshi_info']['xianshi_price'];
            $goods_content['sales_id'] = $goods_content['xianshi_info']['xianshi_id'];
            $goods_content['ifxianshi'] = true;
        }
    }
     ...
     * @return array
     */
    public function getStoreSaleTotal($store_goods_total, $store_freight_total, $store_final_goods_total) {
        if (!is_array($store_goods_total) || !is_array($store_freight_total) || !is_array($store_final_goods_total)) return array();
        $store_sale_total = array();
        foreach ($store_goods_total as $store_id => $goods_total) {
            $store_sale_total[$store_id] = $goods_total + $store_freight_total[$store_id] - $store_final_goods_total[$store_id];
        }
        return $store_sale_total;
    }
    ...
    public function getStoreFreightDescList($store_goods_total) {
        if (empty($store_goods_total) || !is_array($store_goods_total)) return array();
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        foreach ($buy_list as $key => $goods_content) {
            //免运费店铺的商品不需要计算，但如果设置了配送区域，则需要保存该模板ID
            if (in_array($goods_content['store_id'], $free_freight_sid_list)) {
                if (!$goods_content['transport_id']) {
                    unset($buy_list[$key]);
                    continue;                    
                }
            }
            ...
            if (intval($goods_content['bl_id'])) {
                unset($buy_list[$key]);
                $store_bl_goods_freight[$goods_content['store_id']][] = $goods_content['bl_id'];
                continue;
            }
            if (!intval($goods_content['transport_id']) &&  !in_array($goods_content['store_id'],$free_freight_sid_list)) {
                $freight_list[$goods_content['store_id']] += $goods_content['goods_freight'];
                unset($buy_list[$key]);
            }
        }
        //计算优惠套装商品运费
        ...
        foreach ($buy_list as $goods_content) {
            $transport_id = $goods_content['transport_id'];
            $goods_trans_v = $goods_content['goods_trans_v'];
            $goods_num = $goods_content['goods_num'];
            $store_id = $goods_content['store_id'];
            $_array = array();
            $_array['v'] = $goods_trans_v;
            $_array['n'] = $goods_num;
            $tpl_trans_list[$transport_id][] = $_array;
            if (empty($freight_list[$store_id])) $freight_list[$store_id] = array();
            if (!in_array($transport_id,$freight_list[$store_id])) $freight_list[$store_id][] = $transport_id;
        }
    ...
    public function appendPremiumsToCartList($store_cart_list, $store_premiums_list = array(), $store_mansong_rule_list = array(), $member_id) {
        ...
        $goods_sale = $this->_getEachGoodsSkToP($store_cart_list);
        ...
        if(is_array($store_premiums_list)) {
            foreach ($store_premiums_list as $store_id => $goods_list) {
                ...
                foreach ($goods_list as $goods_content) {
                    //如果没有库存了，则不再送赠品
                    if ($goods_storage_quantity[$goods_content['goods_id']] == 0) {
                    ...
                    $new_data['goods_id'] = $goods_content['goods_id'];
                    $new_data['goods_name'] = $goods_content['goods_name'];
                    $new_data['goods_price'] = 0;
                    $new_data['goods_image'] = $goods_content['goods_image'];
                    $new_data['bl_id'] = 0;
                    $new_data['state'] = true;
                    ...
                    //计算赠送数量，有就赠，赠完为止
                    if ($goods_storage_quantity[$goods_content['goods_id']] - $goods_content['goods_num'] >= 0) {
                        $goods_buy_quantity[$goods_content['goods_id']] += $goods_content['goods_num'];
                        $goods_storage_quantity[$goods_content['goods_id']] -= $goods_content['goods_num'];
                        $new_data['goods_num'] = $goods_content['goods_num'];
                    } else {
                        $new_data['goods_num'] = $goods_storage_quantity[$goods_content['goods_id']];
                        $goods_buy_quantity[$goods_content['goods_id']] += $goods_storage_quantity[$goods_content['goods_id']];
                        $goods_storage_quantity[$goods_content['goods_id']] = 0;
                    }
                    if (array_key_exists($goods_content['goods_id'],$zp_list)) {
                        $zp_list[$goods_content['goods_id']]['goods_num'] += $new_data['goods_num'];
                    } else {
                        $zp_list[$goods_content['goods_id']] = $new_data;
                    }
                }
                sort($zp_list);
        ...
        return array($store_cart_list,$goods_buy_quantity,$store_mansong_rule_list,$goods_sale);
    }
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        $available_rcb_amount = floatval(wtPriceFormat($buyer_info['available_rc_balance']));
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $order_amount = floatval($order_info['order_amount']);$order_amount = floatval($order_info['order_amount']) - floatval($order_info['rcb_amount']);
			 //这里如果使用了积分抵用,则订单总价应该重新计算
            if($order_info['points_money']>0){
                $order_amount -=floatval($order_info['points_money']);
            }
            $data_pd = array();
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                $available_rcb_amount = floatval(wtPriceFormat($available_rcb_amount));
                //记录订单日志(已付款)
                $data = array();
                ...
                if ($order_info['order_type'] == 2) {
                    $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = wtPriceFormat(floatval($order_info['original_rcb_amount']) + $order_amount);
                } else {
                ...
                if ($available_rcb_amount > 0) {
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = wtPriceFormat(floatval($order_info['original_rcb_amount']) + $available_rcb_amount);
                    } else {
    ...
    public function pdPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $order_amount = floatval(wtPriceFormat($order_amount));
            $data_pd = array();
            $data_pd['member_id'] = $member_id;
            $data_pd['member_name'] = $member_name;
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $available_pd_amount = floatval(wtPriceFormat($available_pd_amount));
                ...
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'predeposit';
                if ($order_info['order_type'] == 2) {
                    $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = wtPriceFormat(floatval($order_info['original_pd_amount']) + $order_amount);
                } else {
                ...
                if ($available_pd_amount > 0) {
                    ...
                    $data_order = array();
                    if ($order_info['order_type'] == 2) {
                        $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = wtPriceFormat(floatval($order_info['original_pd_amount']) + $available_pd_amount);
                    } else {
     ...
     * 抵用后直接扣除用户积分
     */
    public function pointsPlay($order_list, $pointNumber, $member_id){
        //没有输入积分就返回
        if(empty($pointNumber)||intval($pointNumber)<=0) return ;
        //系统没有开启功能就不使用
        if(C('points_money_isuse')==0) return;
        $buyer_member_info	= Model('member')->getMemberInfoByID($member_id);
        $member_name=$buyer_member_info['member_name'];
		$member_points=$buyer_member_info['member_points'];//得到会员积分
        //没有会员积分,或者输入的积分大于会员持有就返回
        if($member_points<=0||$pointNumber>$member_points) return;
        //将积分兑换成现金
        $points_parity = C('points_money_parity');
        $points_money = $pointNumber*$points_parity;
        //循环订单,用兑换现金支付
        $model_order = Model('order');
        $model_points = Model('points');
        foreach ($order_list as $order_info) {
            $order_amount = floatval($order_info['order_amount']);//取得订单总价格数据
            $data_order = array();
            //如果积分现金大于订单就马上支付
            if($points_money>=$order_amount){
                //将兑换现金写入订单中
                $data_order['points_money']=$order_amount;
                $data_order['points_number']=$order_amount/$points_parity;
                 //减去兑换值留给下一个订单
                ...
                if($points_money>0){
                    //不够完全支付的情况下
                    //写入最大兑换值到订单
                    $data_order['points_money']=$points_money;
                    $data_order['points_number']=$points_money/$points_parity;
                    //兑换值清零
                    $points_money = 0;
                }
            }
            ...
            $insert_arr['pl_points'] = -$data_order['points_number'];
            $insert_arr['order_sn'] = $order_info['order_sn'];
            $model_points->savePointsLog('pointstomoney',$insert_arr,true);
            $order_list[$order_info['order_id']]['points_money']=$data_order['points_money'];
            $order_list[$order_info['order_id']]['points_number']=$data_order['points_number'];
            if($points_money==0){break;}
        }
        return $order_list;
    }
    ...
    public function getMansongRuleList($store_id_array) {
        if (!C('sale_allow') || empty($store_id_array) || !is_array($store_id_array)) return array();
        $model_mansong = Model('p_mansong');
        $mansong_rule_list = array();
    ...
    public function createInvoiceData($invoice_info){
        ...
        if ($invoice_info['inv_state'] == 1) {
            $inv['类型'] = '普通发票 ';
			if($invoice_info['is_person'] == 1){
				$inv['抬头'] = $invoice_info['inv_title'];
				$inv['身份证号'] = $invoice_info['inv_useridnum'];
			}else{
				$inv['抬头'] = $invoice_info['inv_company'];
				$inv['纳税人识别号'] = $invoice_info['inv_code'];
				$inv['注册地址'] = empty($invoice_info['inv_reg_addr'])?'未填':$invoice_info['inv_reg_addr'];
				$inv['注册电话'] = empty($invoice_info['inv_reg_phone'])?'未填':$invoice_info['inv_reg_phone'];
				$inv['开户银行'] = empty($invoice_info['inv_reg_bname'])?'未填':$invoice_info['inv_reg_bname'];
				$inv['银行账户'] = empty($invoice_info['inv_reg_baccount'])?'未填':$invoice_info['inv_reg_baccount'];
			}
    ...
    public function extendInPay($order_list) {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            if ($order_info['order_state'] == ORDER_STATE_PAY) {
                ...
                //更新预定人数
                $order_goods_content = Model('order')->getOrderGoodsInfo(array('order_id'=>$order_info['order_id']),'goods_id','rec_id asc');
                $update = Model('goods')->editGoods(array('book_buyers'=>array('exp','book_buyers+1')),array('goods_id'=>$order_goods_content['goods_id']));
                if (!$update) {
        ...
        if ($order_list[0]['order_type'] == 4) {//拼团订单
            $model_pingou = Model('p_pingou');
            $model_pingou->payOrder($order_list[0]);
        }
        return callback(true);
    }
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array = array();
        $new_array['goods_num'] = $goods_content['is_fcode'] ? 1 : $quantity;
        $new_array['goods_id'] = $goods_id;
        $new_array['goods_commonid'] = $goods_content['goods_commonid'];
        $new_array['gc_id'] = $goods_content['gc_id'];
        $new_array['store_id'] = $goods_content['store_id'];
        $new_array['goods_name'] = $goods_content['goods_name'];
	    $new_array['is_bat'] = $goods_content['is_bat'];//v6.5 批发价
        $new_array['goods_price'] = $goods_content['goods_price'];
        $new_array['goods_costprice'] = $goods_content['goods_costprice'];
		//如果该商品有阶梯价格，按批发价格提取 v6.5
		if($new_array['is_bat']==1)
		{
            $goods_step_price = Model('goods')->getGoodStepPrice('common_id='.$new_array['goods_commonid'].' and step_l_num<='.$new_array['goods_num'].' and step_h_num>'.$new_array['goods_num']);
			if($goods_step_price)
			{
				$new_array['goods_price'] = $goods_step_price[0]['step_price'];
			}
		}
        $new_array['store_name'] = $goods_content['store_name'];
        $new_array['goods_image'] = $goods_content['goods_image'];
        $new_array['transport_id'] = $goods_content['transport_id'];
        $new_array['goods_freight'] = $goods_content['goods_freight'];
        $new_array['goods_trans_v'] = $goods_content['goods_trans_v'];
        $new_array['goods_vat'] = $goods_content['goods_vat'];
        $new_array['goods_storage'] = $goods_content['goods_storage'];
        $new_array['goods_storage_alarm'] = $goods_content['goods_storage_alarm'];
        $new_array['is_fcode'] = $goods_content['is_fcode'];
        $new_array['have_gift'] = $goods_content['have_gift'];
        $new_array['state'] = true;
        $new_array['storage_state'] = intval($goods_content['goods_storage']) < intval($quantity) ? false : true;
        $new_array['robbuy_info'] = $goods_content['robbuy_info'];
        $new_array['xianshi_info'] = $goods_content['xianshi_info'];
        $new_array['is_chain'] = $goods_content['is_chain'];
        $new_array['is_fx'] = $goods_content['is_fx'];

        //预定信息
        $new_array['is_book'] = $goods_content['is_book'];
        $new_array['book_down_payment'] = $goods_content['book_down_payment'];
        $new_array['book_final_payment'] = $goods_content['book_final_payment'];
        $new_array['book_down_time'] = $goods_content['book_down_time'];

        //商品独立分销奖金信息        
        $new_array['is_independent_bonus'] = $goods_content['is_independent_bonus'];
        $new_array['level_commission'] = $goods_content['level_commission'];
        ...
        if (array_key_exists('sole_info', $goods_content)) {
            $new_array['sole_info'] = $goods_content['sole_info'];
        }
        $new_array['contractlist'] = $goods_content['contractlist'];
        return $new_array;
    }
     ...
     * @param array $goods_content
     */
    public function getRobbuyInfo(& $goods_content = array()) {
        if (!C('robbuy_allow') || empty($goods_content['robbuy_info']) || $goods_content['ifsole']) return ;
        $robbuy_info = $goods_content['robbuy_info'];
        $goods_content['goods_price'] = $robbuy_info['robbuy_price'];
        if ($robbuy_info['upper_limit'] && $goods_content['goods_num'] > $robbuy_info['upper_limit']) {
            $goods_content['goods_num'] = $robbuy_info['upper_limit'];
        }
        $goods_content['upper_limit'] = $robbuy_info['upper_limit'];
        $goods_content['sales_id'] = $goods_content['robbuy_id'] = $robbuy_info['robbuy_id'];
        $goods_content['ifrobbuy'] = true;
		
		//v5.2 抢购 限制
		//$goods_model=Model('order');
		  $ordergoods=Model()->table('order_goods')->where(array('buyer_id'=>$_SESSION['member_id'],'goods_type'=>2,'sales_id'=>$robbuy_info['robbuy_id']))->sum('goods_num');
		  if(!empty($ordergoods)&&intval($ordergoods)>0)
		  {
		   $tnum=intval($robbuy_info['upper_limit'])-intval($ordergoods);//-intval($goods_content['goods_num']);
		   if($tnum<=0)
			$goods_content=null;
			//return;
		   else{
			if($goods_content['goods_num']>$tnum){
			 $goods_content['goods_num'] = $tnum;
			}
		   }
		  }
		//end
    ...
    /**
     * 手机专享价
     * @param unknown $goods_content
     */
    public function getMbSoleInfo(& $goods_content = array()) {
        if (!C('sale_allow') || empty($goods_content['sole_info'])) return ;
        $sole_info = $goods_content['sole_info'];
        $goods_content['goods_price'] = $sole_info['sole_price'];
        $goods_content['ifsole'] = true;
    }
     ...
     * @param unknown $goods_content
     */
    public function getBookInfo(& $goods_content = array()) {
        if (empty($goods_content['is_book'])) return ;
        if (!C('sale_allow') || $goods_content['book_down_time'] < TIMESTAMP) {
            $goods_content['is_book'] = 0;
            return ;
        }
        $goods_content['goods_price'] = $goods_content['book_down_payment']+$goods_content['book_final_payment'];
    }
     ...
     * @param array $goods_content
     */
    private function _getGoodsGiftList( & $goods_content) {
        if (!$goods_content['have_gift']) return ;
        $gift_list = Model('goods_gift')->getGoodsGiftListByGoodsId($goods_content['goods_id']);
        ...
        $goods_content['gift_list'] = $gift_list;
    }
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        $model_goods = Model('goods');
        $goods_online_list = $model_goods->getGoodsOnlineListAndSaleByIdArray($goods_id_array);
        //处理商品消费者保障服务信息
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
				...
				$cart_list[$key]['is_bat'] = $goods_online_info['is_bat']; //V6.5 批发价
                $cart_list[$key]['goods_price'] = $goods_online_info['goods_price'];
                $cart_list[$key]['goods_costprice'] = $goods_online_info['goods_costprice'];
				//如果该商品有阶梯价格，按批发价格提取 v6.5
				if($cart_list[$key]['is_bat']>=1)
				{
					$goods_step_price = Model('goods')->getGoodStepPrice('common_id='.$cart_info['goods_id'].' and step_l_num<='.$cart_list[$key]['goods_num'].' and step_h_num>'.$cart_list[$key]['goods_num']);
					if($goods_step_price)
					{
						$cart_list[$key]['goods_price'] = $goods_step_price[0]['step_price'];
					}
				}
                ...
                //商品独立分销信息
                $cart_list[$key]['is_independent_bonus'] = $goods_online_info['is_independent_bonus'];
                $cart_list[$key]['level_commission'] = $goods_online_info['level_commission'];
     ...
     */
    public function getRobbuyCartList(& $cart_list) {
        if (!C('robbuy_allow') || empty($cart_list)) return ;
        foreach ($cart_list as $key => $cart_info) {
            if (intval($cart_info['bl_id']) || empty($cart_info['robbuy_info']) || $cart_info['ifsole']) continue;
            $this->getRobbuyInfo($cart_info);
    ...
    public function getXianshiCartList(& $cart_list) {
        if (!C('sale_allow') || empty($cart_list)) return ;
        foreach ($cart_list as $key => $cart_info) {
            if (intval($cart_info['bl_id']) || empty($cart_info['xianshi_info']) || !empty($cart_info['robbuy_info']) || $cart_info['ifsole']) continue;
    ...
    private function _getBundlingCartList(& $cart_list) {
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            //取最新在售商品信息
            $goods_id_array = array();
            foreach ($cart_list[$key]['bl_goods_list'] as $goods_content) {
                $goods_id_array[] = $goods_content['goods_id'];
            }
            ...
            foreach ($goods_list as $goods_content) {
                $goods_online_list[$goods_content['goods_id']] = $goods_content;
            }
            unset($goods_list);
            ...
            foreach ($cart_list[$key]['bl_goods_list'] as $k => $goods_content) {
                if (array_key_exists($goods_content['goods_id'],$goods_online_list)) {
                    $goods_online_info = $goods_online_list[$goods_content['goods_id']];
                    //如果库存不足，标识false
                    if ($cart_info['goods_num'] > $goods_online_info['goods_storage']) {
                        $cart_list[$key]['storage_state'] = false;
                    }
                    ...
                    $cart_list[$key]['bl_goods_list'][$k]['is_fx'] = $goods_online_info['is_fx'];
                    $cart_list[$key]['bl_goods_list'][$k]['fx_commis_rate'] = $goods_online_info['fx_commis_rate'];
                    ...
                    //每个商品直降多少
                    $total_down_price += $cart_list[$key]['bl_goods_list'][$k]['down_price'] = wtPriceFormat($goods_online_info['goods_price'] - $goods_content['bl_goods_price']);
                } else {
                    //商品已经下架
                    $cart_list[$key]['state'] = false;
                    $cart_list[$key]['storage_state'] = false;
            ...
            $cart_list[$key]['down_price'] = wtPriceFormat($total_down_price);
        }
    }
    ...
    private function _getEachGoodsStorageQuantity($store_cart_list, $store_premiums_list = array()) {
        ...
        foreach ($store_cart_list as $store_cart) {
            foreach ($store_cart as $cart_info) {
                if (!intval($cart_info['bl_id'])) {
                    ...
                    //优惠套装
                    foreach ($cart_info['bl_goods_list'] as $goods_content) {
                        $goods_storage_quangity[$goods_content['goods_id']] = $goods_content['goods_storage'];
                    }
                }
            }
        }
        ...
        if (is_array($store_premiums_list)) {
            foreach ($store_premiums_list as $store_id => $goods_list) {
                foreach($goods_list as $goods_content) {
                    if (!isset($goods_storage_quangity[$goods_content['goods_id']])) {
                        $goods_storage_quangity[$goods_content['goods_id']] = $goods_content['goods_storage'];
                    }
                }
            }
        ...
        foreach ($store_cart_list as $store_cart) {
            foreach ($store_cart as $cart_info) {
                if (!intval($cart_info['bl_id'])) {
                    ...
                    $goods_sale[$cart_info['goods_id']] = $cart_info['goods_commonid'];
                } elseif (!empty($cart_info['bl_goods_list']) && is_array($cart_info['bl_goods_list'])) {
                    //优惠套装
                    foreach ($cart_info['bl_goods_list'] as $goods_content) {
                        $goods_buy_quangity[$goods_content['goods_id']] += $cart_info['goods_num'];
                        $goods_sale[$goods_content['goods_id']] = $goods_content['goods_commonid'];
                    }
                }
            }
        }
     ...
     * 取得每种商品sku=>spu
     * @param array $store_cart_list 购买列表
     * @return array sku=>spu
     */
    private function _getEachGoodsSkToP($store_cart_list) {
        if(empty($store_cart_list) || !is_array($store_cart_list)) return array();
        $goods_sale = array();
        foreach ($store_cart_list as $store_cart) {
            foreach ($store_cart as $cart_info) {
    ...
    private function _parseMansongRuleDesc($rule_info) {
        ...
        if (APP_ID == 'mobile') {
            ...
            if (!empty($rule_info['mansong_goods_name'])) {
                ...
                $goods_desc .= '<a class="wtc-store-gift" title="'.$rule_info['mansong_goods_name'].'" target="_blank" href="'.urlShop('goods','index',array('goods_id'=>$rule_info['goods_id'])).'"><img alt="'.$rule_info['mansong_goods_name'].'" src="'.cthumb($rule_info['goods_image'], 60).'"></a>';                    
            }
