...
<div class="page">
  ...
  <form id="post_form" method="post" action="index.php?w=vr_refund&t=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
    ...
    <div class="wtap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt"><?php echo wtPriceFormat($output['refund']['refund_amount']); ?> 
            <span id="pay_amount">
            <?php if ($output['detail_array']['pay_time'] > 0) { ?>
            (已完成在线退款金额 <?php echo wtPriceFormat($output['detail_array']['pay_amount']); ?>)
            <?php } ?>
            </span>
        </dd>
      </dl>
      ...
      <dl class="row">
        ...
        <dd class="opt"><?php echo wtPriceFormat($output['order']['order_amount']);?></dd>
      </dl>
      <?php if ($output['order']['refund_amount'] > 0) { ?>
      <dl class="row">
        <dt class="tit">已退款金额</dt>
        <dd class="opt"><?php echo wtPriceFormat($output['order']['refund_amount']);?></dd>
      </dl>
      <?php } ?>
      <?php if ($output['order']['pay_amount'] > 0) { ?>
      <dl class="row">
        <dt class="tit">在线支付金额</dt>
        <dd class="opt"><?php echo wtPriceFormat($output['order']['pay_amount']);?></dd>
      </dl>
          ...
          <dl class="row">
            <dt class="tit"></dt>
            <dd class="opt"><a href="JavaScript:void(0);" class="wtap-btn-big wtap-btn-green" id="wxpayBtn">确认微信退款</a></dd>
          </dl>
          <?php } ?>
          ...
          <dl class="row">
            <dt class="tit"></dt>
            <dd class="opt">
                <a href="<?php echo ADMIN_SITE_URL;?>/index.php?w=vr_refund&t=alipay&refund_id=<?php echo $output['refund']['refund_id']; ?>" target="_blank">支付宝退款</a>  
                <a href="JavaScript:void(0);" class="wtap-btn-big wtap-btn-green" id="alipayBtn">退款查询</a>
            </dd>
          </dl>
          <?php } ?>
          <?php } ?>
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="state1" class="cb-enable" title="<?php echo $lang['wt_yes'];?>"><?php echo $lang['wt_yes'];?></label>
            <label for="state0" class="cb-disable" title="<?php echo $lang['wt_no'];?>"><?php echo $lang['wt_no'];?></label>
...
<script type="text/javascript" src="<?php echo ADMIN_STATIC_URL;?>/js/refund.js"></script>
...
$(function(){
	...
	$("#wxpayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?w=vr_refund&t=wxpay&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
	$("#alipayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?w=vr_refund&t=get_detail&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
    $('#post_form').validate({
        ...
        messages : {
            admin_state : {
                required : '<i class="fa fa-exclamation-bbs"></i>请选择是否同意退款'
            },
            admin_message  : {
                required : '<i class="fa fa-exclamation-bbs"></i><?php echo $lang['refund_message_null'];?>'
            }
        }
    });
});
