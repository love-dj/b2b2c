...
class articleControl extends NEWSHomeControl{
     ...
     * 文章列表
     */
    public function article_detailWt() {
        ...
        if(empty($article_detail)) {
            showMessage(Language::get('article_not_exist'), NEWS_SITE_URL, '', 'error');
        }
     ...
     * 文章评论
     */
    public function article_comment_detailWt() {
        $article_id = intval($_GET['article_id']);
        if($article_id <= 0) {
            showMessage(Language::get('wrong_argument'),'','','error');
        }
     ...
     * 文章列表
     */
    public function article_searchWt() {
        $condition = array();
     ...
     * 根据标签搜索
     */
    public function article_tag_searchWt() {
        $article_list = array();
