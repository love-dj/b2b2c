...
class member_refundControl extends BaseMemberControl {
    ...
    public function add_refundWt(){
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?w=member_order&t=index','error');
        }
        $condition = array();
		...
		//使用积分抵用过的金额不能退，需要减去
        if($order['points_money']>0){
            $order_amount-=$order['points_money'];
        }
        ...
        if (chksubmit() && $goods_id > 0){
            ...
            $show_url = 'index.php?w=member_return&t=index';
            ...
            if ($refund_array['refund_type'] != '2') {
                ...
                $show_url = 'index.php?w=member_refund&t=index';
            }
            ...
            if ($state) {
                ...
                showDialog(Language::get('wt_common_save_succ'),$show_url,'succ');
            } else {
     ...
     *
     */
    public function add_refund_allWt(){
        ...
        //禁止退款金额
        $lock_amount = Handle('order_book')->getDepositAmount($order);
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(Language::get('wt_common_save_succ'),'index.php?w=member_refund&t=index','succ');
            } else {
                showDialog(Language::get('wt_common_save_fail'),'reload','error');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                $menu_array = array(
                array('menu_key'=>'buyer_refund','menu_name'=>Language::get('wt_member_path_buyer_refund'), 'menu_url'=>'index.php?w=member_refund'),
                array('menu_key'=>'buyer_return','menu_name'=>Language::get('wt_member_path_buyer_return'), 'menu_url'=>'index.php?w=member_return'),
                array('menu_key'=>'buyer_vr_refund','menu_name'=>'虚拟兑码退款',    'menu_url'=>'index.php?w=member_vr_refund'));
                break;
        }
