...
<div class="title"><i></i>
  <h3><a href="<?php echo urlFenxiao('search', 'index');?>"><?php echo $lang['wt_all_goods_class'];?></a></h3>
</div>
<div class="category">
  <ul class="menu">
    ...
    <li cat_id="<?php echo $val['gc_id'];?>" class="<?php echo $i%2==1 ? 'odd':'even';?>" <?php if($output['index_sign'] == 'index' && $i>11){?>style="display:none;"<?php }?>>
      ...
      <div class="class"><span class="arrow"></span>
        ...
        <?php } ?>
              <h4><a href="<?php echo urlFenxiao('search','index',array('cate_id'=> $val['gc_id']));?>"><?php echo $val['gc_name'];?></a></h4>
         </div>
      <div class="sub-class" cat_menu_id="<?php echo $val['gc_id'];?>">
        <div class="sub-class-content">
          <div class="recommend-class">
            ...
            <span><a href="<?php echo urlFenxiao('search','index',array('cate_id'=> $v['gc_id']));?>" title="<?php echo $v['gc_name']; ?>"><?php echo $v['gc_name'];?></a></span>
            <?php } ?>
            <?php } ?>
          </div>
		...
		<?php if($v['is_show']=='1'){ continue;}?>
          <dl>
            <dt>
             <h3><a href="<?php echo urlFenxiao('search','index',array('cate_id'=> $v['gc_id']));?>"><?php echo $v['gc_name'];?></a></h3>
            </dt>
            <dd class="goods-class">
              <?php if (!empty($v['class3']) && is_array($v['class3'])) { ?>
              <?php foreach ($v['class3'] as $k3 => $v3) { ?>
				<?php if($v3['is_show']=='1'){ continue;}?>
              <a href="<?php echo urlFenxiao('search','index',array('cate_id'=> $v3['gc_id']));?>"><?php echo $v3['gc_name'];?></a>
              <?php } ?>
              <?php } ?>
            </dd>
          </dl>
