...
class adminControl extends SystemControl{
    private $links = array(
        array('url'=>'w=admin&t=admin','lang'=>'limit_admin'),
        array('url'=>'w=admin&t=gadmin','lang'=>'limit_gadmin'),
    );
     ...
     * 管理员列表
     */
    public function get_admin_xmlWt(){
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['admin_is_super'] != 1) {
                    ...
                    $operation .= '<a class="btn blue" href="index.php?w=admin&t=admin_edit&admin_id='.$v['admin_id'].'"><i class="fa fa-pencil-square-o"></i>'.L('wt_edit').'</a>';
                }else {
                    $operation = '--';
                }
     ...
     * 管理员删除
     */
    public function admin_delWt(){
        if (!empty($_GET['admin_id'])){
            if ($_GET['admin_id'] == 1){
                exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
            }
            Model()->table('admin')->where(array('admin_id'=>intval($_GET['admin_id'])))->delete();
            $this->log(L('wt_delete,limit_admin').'[ID:'.intval($_GET['admin_id']).']',1);
    ...
    public function admin_addWt(){
        if (chksubmit()){
            ...
            if ($rs){
                $this->log(L('wt_add,limit_admin').'['.$_POST['admin_name'].']',1);
                showMessage(L('wt_common_save_succ'),'index.php?w=admin&t=admin');
            }else {
    ...
    public function admin_editWt(){
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('wt_edit,limit_admin').'[ID:'.intval($_GET['admin_id']).']',1);
                showMessage(Language::get('admin_edit_success'),'index.php?w=admin&t=admin');
            }else{
                showMessage(Language::get('admin_edit_fail'),'index.php?w=admin&t=admin');
            }
        }else{
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admininfo) || count($admininfo)<=0){
                showMessage(Language::get('admin_edit_admin_error'),'index.php?w=admin&t=admin');
            }
    ...
    public function gadminWt(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_POST['del_id'])){
                ...
                $this->log(L('wt_delete,limit_gadmin').'[ID:'.implode(',',$_POST['del_id']).']',1);
     ...
     * 添加权限组
     */
    public function gadmin_addWt(){
        if (chksubmit()){
            $model = Model('gadmin');
            $data['limits'] = encrypt(serialize($_POST['permission']),MD5_KEY.md5($_POST['gname']));
            $data['gname'] = $_POST['gname'];
            if ($model->insert($data)){
                $this->log(L('wt_add,limit_gadmin').'['.$_POST['gname'].']',1);
                showMessage(L('wt_common_save_succ'),'index.php?w=admin&t=gadmin');
            }else {
                showMessage(L('wt_common_save_fail'));
            }
        }
    ...
    public function gadmin_editWt(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                $this->log(L('wt_edit,limit_gadmin').'['.$_POST['gname'].']',1);
                showMessage(L('wt_common_save_succ'),'index.php?w=admin&t=gadmin');
            }else {
     ...
     * 组删除
     */
    public function gadmin_delWt(){
        if (is_numeric($_GET['gid'])){
            Model('gadmin')->where(array('gid'=>intval($_GET['gid'])))->delete();
            $this->log(L('wt_delete,limit_gadmin').'[ID'.intval($_GET['gid']).']',1);
            showMessage(Language::get('wt_common_op_succ'),'index.php?w=admin&t=gadmin');
        }else {
