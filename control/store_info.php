...
class store_infoControl extends BaseSellerControl {
    ...
    public function bind_class_saveWt() {
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核','index.php?w=store_info&t=bind_class','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['wt_common_save_fail']);
        }
    }
...
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
                ...
                showDialog(Language::get('wt_common_save_succ').'，需付款金额'.wtPriceFormat($data['re_pay_amount']).'元，请尽快完成付款，付款完成后请上传付款凭证','reload','succ','',5);
            }
        } else {
            showDialog(Language::get('wt_common_del_fail'));
        }
    ...
    //上传付款凭证
    public function reopen_uploadWt() {
        if (!chksubmit()) exit();
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['wt_member_path_bind_class'], 'menu_url'=>'index.php?w=store_info&t=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['wt_member_path_store_info'], 'menu_url'=>'index.php?w=store_info&t=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['wt_member_path_store_reopen'], 'menu_url'=>'index.php?w=store_info&t=reopen');
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['wt_member_path_bind_class'], 'menu_url'=>'index.php?w=store_bind_class&t=index'),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['wt_member_path_bind_class'], 'menu_url'=>'index.php?w=store_bind_class&t=index'),
                array('menu_key'=>'add', 'menu_name'=>$lang['wt_member_path_bind_class_add'], 'menu_url'=>'index.php?w=store_bind_class&t=add')
                );
                break;
        }
        if(!empty($array)) {
