...
class goodsControl extends SystemControl{
    private $links = array(
        array('url'=>'w=goods&t=goods','text'=>'所有商品'),
        array('url'=>'w=goods&t=lockup_list','text'=>'违规下架'),
        array('url'=>'w=goods&t=waitverify_list','text'=>'等待审核'),
        array('url'=>'w=goods&t=goods_set','text'=>'商品设置'),
    );
     ...
     * 等待审核商品管理
     */
    public function waitverify_listWt() {
    ...
    public function get_xmlWt() {
        ...
        foreach ($goods_list as $value) {
            ...
            $param['goods_price'] = wtPriceFormat($value['goods_price']);
            ...
            if(empty($value['goods_video'])){
                ...
                if(file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $value['store_id'] .'/' . 'goods_video' . '/' . $value['goods_video'])){
                    $param['goods_video'] = "<a href='javascript:void(0);' onclick='fg_see_video(".$value['goods_commonid'].")'><i class='fa fa-file-video-o'></i></a>";
                }else{
                    $param['goods_video'] = '';
                }
            }
            ...
            $param['goods_marketprice'] = wtPriceFormat($value['goods_marketprice']);
            $param['goods_costprice'] = wtPriceFormat($value['goods_costprice']);
            $param['goods_freight'] = $value['goods_freight'] == 0 ? '免运费' : wtPriceFormat($value['goods_freight']);
            $param['goods_storage'] = $storage_array[$value['goods_commonid']]['sum'];
            $param['is_virtual'] = $value['is_virtual'] ==  '1' ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $param['virtual_indate'] = $value['is_virtual'] == '1' && $value['virtual_indate'] > 0 ? date('Y-m-d', $value['virtual_indate']) : '--';
            $param['virtual_invalid_refund'] = $value['is_virtual'] ==  '1' ? ($value['virtual_invalid_refund'] == 1 ? '<span class="yes"><i class="fa fa-check-bbs"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>') : '--';
        ...
        if (chksubmit()){
            ...
            $result = Model('goods')->editGoodsById($data, $_POST['goodsid']);
                 ...
                 if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?w=goods&t=add&gid='.$_POST['goodsid'].'',
                            'msg'=>'继续',
                        ),
                        array(
                            'url'=>'index.php?w=goods',
                            'msg'=>'返回',
                        )
                    );
     ...
     * 违规下架
     */
    public function goods_lockupWt() {
        if (chksubmit()) {
            ...
            showDialog(L('wt_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();CUR_DIALOG.close()');
        }
     ...
     * 审核商品
     */
    public function goods_verifyWt(){
        if (chksubmit()) {
            $commonid = intval($_POST['commonid']);
            if ($commonid <= 0) {
                    showDialog(L('wt_common_op_fail'), 'reload');
     ...
     * ajax获取商品列表
     */
    public function get_goods_sku_listWt() {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            foreach ($goods_spec as $k => $v) {
                $spec_array[] = '<div class="goods_spec">' . $spec_name[$k] . L('wt_colon') . '<em title="' . $v . '">' . $v .'</em>' . '</div>';
            }
...
//         echo json_encode($goods_list);
    ...
    public function goods_setWt() {
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('wt_edit,wt_goods_set'),1);
                showMessage(L('wt_common_save_succ'));
            }else {
                $this->log(L('wt_edit,wt_goods_set'),0);
    ...
    public function export_csvWt() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?w=goods&t=index');
    ...
    public function see_videoWt(){
        ...
        Tpl::output('common_info' , $common_info );
        Tpl::showpage('goods_see_video' , 'null_layout');
    }
}
